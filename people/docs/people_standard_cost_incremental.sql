...
-- 适用场景：已执行旧版 people_schema.sql 的租户库，补齐岗位/职级标准成本、主任职与成本任职快照字段
-- 执行说明：
...
SET @ddl := (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE `people_assignments` ADD COLUMN `is_primary` TINYINT(1) NOT NULL DEFAULT 1 COMMENT ''是否主任职；有效区间排他由 People 领域写入校验'' AFTER `effective_to`',
    'SELECT 1'
  )
  FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'people_assignments'
...
SET @ddl := (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE `people_assignments` ADD KEY `idx_people_assignment_primary_effective` (`employee_uid`, `is_primary`, `approval_status`, `effective_from`, `effective_to`)',
    'SELECT 1'
  )
  FROM information_schema.STATISTICS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'people_assignments'
    AND INDEX_NAME = 'idx_people_assignment_primary_effective'
);
PREPARE stmt FROM @ddl;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @ddl := (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE `people_assignments` ADD CONSTRAINT `ck_people_assignment_primary` CHECK (`is_primary` IN (0, 1))',
    'SELECT 1'
  )
  FROM information_schema.TABLE_CONSTRAINTS
  WHERE CONSTRAINT_SCHEMA = DATABASE()
    AND TABLE_NAME = 'people_assignments'
    AND CONSTRAINT_NAME = 'ck_people_assignment_primary'
);
PREPARE stmt FROM @ddl;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @ddl := (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE `people_cost_snapshots` ADD COLUMN `assignment_code` VARCHAR(64) DEFAULT NULL COMMENT ''生成快照时命中的期间有效主任职编码'' AFTER `cost_basis`',
    'SELECT 1'
  )
  FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'people_cost_snapshots'
    AND COLUMN_NAME = 'assignment_code'
);
PREPARE stmt FROM @ddl;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @ddl := (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE `people_cost_snapshots` ADD COLUMN `dept_code_snapshot` VARCHAR(64) DEFAULT NULL COMMENT ''生成时部门编码快照'' AFTER `assignment_code`',
    'SELECT 1'
  )
  FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'people_cost_snapshots'
    AND COLUMN_NAME = 'dept_code_snapshot'
);
PREPARE stmt FROM @ddl;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @ddl := (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE `people_cost_snapshots` ADD COLUMN `dept_name_snapshot` VARCHAR(100) DEFAULT NULL COMMENT ''生成时部门名称快照'' AFTER `dept_code_snapshot`',
    'SELECT 1'
  )
  FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'people_cost_snapshots'
    AND COLUMN_NAME = 'dept_name_snapshot'
);
PREPARE stmt FROM @ddl;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @ddl := (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE `people_cost_snapshots` ADD COLUMN `position_code_snapshot` VARCHAR(64) DEFAULT NULL COMMENT ''生成时岗位编码快照'' AFTER `dept_name_snapshot`',
    'SELECT 1'
  )
  FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'people_cost_snapshots'
    AND COLUMN_NAME = 'position_code_snapshot'
);
PREPARE stmt FROM @ddl;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @ddl := (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE `people_cost_snapshots` ADD COLUMN `position_name_snapshot` VARCHAR(100) DEFAULT NULL COMMENT ''生成时岗位名称快照'' AFTER `position_code_snapshot`',
    'SELECT 1'
  )
  FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'people_cost_snapshots'
    AND COLUMN_NAME = 'position_name_snapshot'
);
PREPARE stmt FROM @ddl;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @ddl := (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE `people_cost_snapshots` ADD COLUMN `rank_code_snapshot` VARCHAR(32) DEFAULT NULL COMMENT ''生成时职级编码快照'' AFTER `position_name_snapshot`',
    'SELECT 1'
  )
  FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'people_cost_snapshots'
    AND COLUMN_NAME = 'rank_code_snapshot'
);
PREPARE stmt FROM @ddl;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @ddl := (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE `people_cost_snapshots` ADD COLUMN `rank_name_snapshot` VARCHAR(100) DEFAULT NULL COMMENT ''生成时职级名称快照'' AFTER `rank_code_snapshot`',
    'SELECT 1'
  )
  FROM information_schema.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'people_cost_snapshots'
    AND COLUMN_NAME = 'rank_name_snapshot'
);
PREPARE stmt FROM @ddl;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @ddl := (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE `people_cost_snapshots` ADD KEY `idx_people_cost_assignment` (`assignment_code`)',
    'SELECT 1'
  )
  FROM information_schema.STATISTICS
  WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'people_cost_snapshots'
    AND INDEX_NAME = 'idx_people_cost_assignment'
);
PREPARE stmt FROM @ddl;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
