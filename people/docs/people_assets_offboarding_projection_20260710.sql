-- People -> Assets authoritative offboarding projection.
-- Apply before enabling HZY_PEOPLE_ASSETS_OFFBOARDING_SYNC_ENABLED.
-- The outbox stores only a frozen lifecycle command; it does not copy Assets
-- recovery responsibility, asset lists, or People offboarding task state.
