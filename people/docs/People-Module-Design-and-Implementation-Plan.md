...
> 状态：Phase 3 设计与实现参考（代码/离线验证已有证据，真实环境切换待执行）
> 最后核对：2026-07-11
> 当前事实源：`docs/P3-P4-Release-Readiness-and-E2E-Acceptance.md`、`docs/release/P3-P4-2026-07.release.json`、`docs/MODULE_CONTRACTS.md`、People API/schema 与 data-runtime adapter
> 边界：本文的“已落地”仅指设计或代码实现；migration/seed、Cloudflare 发布、真实租户验收和生产切换须获单独授权并留档。
> 应用名称：People  
> 中文名称：人员  
> 建议 app code：`people`  
> 建议目录：`people/`  
...
- 生产切换期保留受控 `POST /api/admin/directory-sync/import` 兼容接口，用于从 Console Directory 初始化活跃用户到 People 员工事实和当前任职快照；员工页不再暴露该入口。
