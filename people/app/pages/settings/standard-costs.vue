...
const { data: response, error, refresh, status } = await useLazyFetch<ApiResponse<ListResponse<StandardCostRate>>>('/api/v1/standard-costs', {
  query,
  watch: [query]
})

const { data: parameterResponse, error: parameterError, refresh: refreshParameters } = await useLazyFetch<ApiResponse<FinanceCostParameters>>(peopleApiPath('/api/admin/cost-parameters/current'))
const { data: rankSettingResponse, error: rankSettingError, refresh: refreshRankSettings } = await useLazyFetch<ApiResponse<RankSeriesSettings>>(peopleApiPath('/api/admin/rank-settings/current'))
