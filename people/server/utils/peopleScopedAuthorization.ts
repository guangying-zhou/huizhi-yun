...
function actionMatches(granted: string, required: PermissionAction, policy?: ResourceActionPolicy) {
...
function permissionMatches(
  ...
  if (permission.appCode === appCode && permission.resourceCode === 'admin' && permission.action === 'admin') {
    return true
  }
  return permission.appCode === appCode
    && permission.resourceCode === resourceCode
    && actionMatches(permission.action, requiredAction, policy)
}
...
function employeeAccessQuery(result: EmployeeScopeResult, uid = ''): RuntimeQuery {
...
export async function resolvePeopleEmployeeAccessQuery(
  ...
  try {
    ...
    const scoped = await loadPeopleScopedGrants(event, normalizedUid, resourceCode, action)

    let sawPermission = false
    let allowSelf = false
    const allowedDeptCodes: string[] = []
    for (const grant of scoped.grants) {
      if (!grant.permissions.some(permission => permissionMatches(permission, resourceCode, action, scoped.actionPolicy))) continue
      sawPermission = true

      const scope = employeeScopeFromGrant(grant, currentDeptCodes)
      if (scope.access === 'all') {
        return employeeAccessQuery(scope, normalizedUid)
      }
