...
export interface ProjectConsoleDirectoryUserResult {
  uid?: string
...
export interface ProjectConsoleDirectoryUserInput {
  employeeUid: string
...
export async function projectConsoleDirectoryUserEmployment(event: H3Event, input: ProjectConsoleDirectoryUserInput) {
  const employeeUid = text(input.employeeUid)
  if (!employeeUid) {
    throw createError({ statusCode: 400, message: 'employeeUid is required.' })
  }
  ...
  const idempotencyKey = `people:employee:${employeeUid}:project-console-directory-employment:v1`
  const token = await requestServiceAccessToken({
    audience: 'console_directory',
    scope: 'console_directory:write',
    event
  ...
  const response = await $fetch<ConsoleEnvelope<ProjectConsoleDirectoryUserResult>>(
    appendPath(baseUrl, `/api/v1/console/service/directory/users/${encodeURIComponent(employeeUid)}/employment`),
    {
      method: 'POST',
      headers: {
        ...forwardedContextHeaders(event),
      ...
      body: compactBody({
        sourceApp: 'people',
        reason: text(input.reason) || 'people_employment_projection',
        operatorUid: text(input.operatorUid),
        loginName: text(input.loginName),
        displayName: text(input.displayName),
        realName: text(input.realName),
        deptCode: text(input.deptCode),
        positionCode: text(input.positionCode),
        positionName: text(input.positionName),
        employmentStatus: text(input.employmentStatus)
      }),
      timeout: 10000
    }
  )
  ...
  if (response.code !== undefined && response.code !== 0) {
    throw createError({ statusCode: 502, message: response.message || 'Console directory employment API returned an error.' })
  }
  if (!response.data) {
    throw createError({ statusCode: 502, message: 'Console directory employment API returned empty data.' })
  }
  return response.data
}
