...
export default defineEventHandler(async (event) => {
  ...
  const idempotencyKey = String(getHeader(event, 'idempotency-key') || `people-dingtalk-job-cancel-${randomUUID()}`).trim()
  return await mutateDingTalkPeopleSync(event, actorUid, jobId, 'cancel', idempotencyKey)
})
