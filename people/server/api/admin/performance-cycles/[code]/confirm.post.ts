...
export default defineEventHandler(async (event) => {
  ...
  const snapshot = await assertPeoplePermission(event, 'performance_cycles', 'approve')
  ...
  const runtime = await maybeCallTenantRuntime<ApiResponse<Record<string, unknown>>>(
    event,
    `/v1/people/service/performance-cycles/${encodeURIComponent(cycleCode)}:confirm`,
    {
      appCode,
      scope: 'people.write',
      method: 'POST',
  ...
  if (runtime.data.code !== undefined && runtime.data.code !== 0) {
    throw createError({ statusCode: 502, message: runtime.data.message || 'People performance cycle confirm failed' })
  }
