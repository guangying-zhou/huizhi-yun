...
function completeAimsTimeEntries(page: RuntimeList<AimsTimeEntry>, projectCode: string) {
  const items = page.items
  if (!Array.isArray(items) || !Number.isInteger(page.total) || page.total !== items.length
    || page.page !== 1 || page.pageSize !== items.length) {
    throw createError({ statusCode: 502, message: 'Aims time entries are not a complete snapshot.' })
  }
  for (const entry of items) {
    const employeeUid = text(entry.uid || entry.employee_uid || entry.employeeUid)
    const entryProjectCode = text(entry.projectCode || entry.project_code)
    const hours = Number(entry.hours)
    if (!employeeUid || entryProjectCode !== projectCode || !Number.isFinite(hours) || hours < 0) {
      throw createError({ statusCode: 502, message: 'Aims time entry cannot be safely included in the contribution snapshot.' })
    }
  }
  return items.filter(entry => text(entry.review_status || entry.reviewStatus) === 'approved')
}
...
async function resolveAimsProject(event: H3Event, projectCode: string) {
  ...
  if (matches.length !== 1) {
    throw createError({ statusCode: 502, message: `Aims project_code is not unique: ${projectCode}` })
  }
...
export default defineEventHandler(async (event) => {
  ...
  await assertPeoplePermission(event, 'performance_cycles', 'edit')
  ...
  const entries = completeAimsTimeEntries(timeEntries, projectCode)
  ...
  return {
    ...
    data: {
      ...
      removed: Number(peopleSync.removed || 0),
      peopleSync
    }
  }
})
