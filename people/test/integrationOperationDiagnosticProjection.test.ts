...
test('People browser diagnostic routes use the shared allow-list projection', () => {
  ...
  assert.match(admin, /interface RuntimeEnvelope<T>/)
  assert.match(admin, /maybeCallTenantRuntime<RuntimeEnvelope<T>>/)
  assert.match(admin, /return runtime\.data\.data as T/)
  assert.doesNotMatch(admin, /maybeCallTenantRuntime<T>/)
})
