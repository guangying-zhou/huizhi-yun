...
export default defineNuxtConfig({
  ...
  runtimeConfig: {
    hzy: {
      ...
      serviceClient: {
        clientId: process.env.HZY_PEOPLE_SERVICE_CLIENT_ID || process.env.HZY_SERVICE_CLIENT_ID || '',
        clientSecret: process.env.HZY_PEOPLE_SERVICE_CLIENT_SECRET || process.env.HZY_SERVICE_CLIENT_SECRET || '',
        tokenUrl: process.env.HZY_CONSOLE_TOKEN_URL || ''
      },
      notifications: {
        offboardingEnabled: process.env.HZY_PEOPLE_OFFBOARDING_NOTIFICATIONS_ENABLED || 'false',
        integrationOperationDeadLetterEnabled: process.env.HZY_PEOPLE_INTEGRATION_OPERATION_DEAD_LETTER_NOTIFICATIONS_ENABLED || 'false'
      },
      projections: {
        assetsOffboardingEnabled: process.env.HZY_PEOPLE_ASSETS_OFFBOARDING_SYNC_ENABLED || 'false',
        directoryLifecycleEnabled: process.env.HZY_PEOPLE_DIRECTORY_LIFECYCLE_SYNC_ENABLED || 'false'
      }
    },
    public: {
      appName: packageMeta.name || 'people',
      appVersion: packageMeta.version || '0.0.0',
      appDisplayName: process.env.NUXT_PUBLIC_APP_DISPLAY_NAME || '汇智云HR',
  ...
  nitro: {
    scheduledTasks: {
      '*/15 * * * *': ['notifications:offboarding-due', 'integrations:assets-offboarding', 'integrations:directory-lifecycle', 'integrations:dead-letter-notifications']
    },
    alias: {
