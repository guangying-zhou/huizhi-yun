...
生产切换期可由受控迁移流程调用兼容接口 `POST /api/admin/directory-sync/import`，从 Console Directory
导入当前企业活跃用户并初始化 People 员工事实与当前任职快照。员工列表不再提供该入口；正常运营期
的入转调离先在 People 形成事实，再通过 durable lifecycle operation 投影到 Console。
