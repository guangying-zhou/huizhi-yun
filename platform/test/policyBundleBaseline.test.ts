...
import {
  ...
  baselinePermissionKey,
  collectBaselinePermissions
} from '../server/utils/policyBundleBaseline.ts'
...
describe('policy bundle baseline permissions', () => {
  ...
  test('identifies locked baseline permission keys and excluded users', () => {
    const permission = {
      appCode: 'codocs',
    ...
    assert.equal(baselinePermissionKey(permission), 'codocs:documents:create:subject:self')
    assert.equal(baselinePermissionExcludesSubject(permission, 'u-excluded'), true)
    assert.equal(baselinePermissionExcludesSubject(permission, 'u-normal'), false)
  })
})
