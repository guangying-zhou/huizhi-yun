...
import {
  baselinePermissionExcludesSubject,
  collectConfiguredBaselinePermissionsWithQueries,
  type BaselinePermission
} from './policyBundleBaseline.ts'
...
function scopePredicateFromDimensionValue(
  dimensionValue: unknown,
  scopeValue: unknown,
  source: ScopePredicate['source']
): ScopePredicate | null {
  const dimension = stringValue(dimensionValue)
  const rawValue = stringValue(scopeValue)
...
function legacyScopePredicate(scope: RoleScopeRow): ScopePredicate | null {
  return scopePredicateFromDimensionValue(scope.scope_type, scope.scope_value, 'role_default')
}
...
function permissionFromBaseline(record: BaselinePermission): PermissionTriple {
  return {
...
function baselineScopePredicate(record: BaselinePermission): ScopePredicate[] {
  const scope = scopePredicateFromDimensionValue(record.scopeType, record.scopeValue, 'baseline')
  return scope ? [scope] : []
}

async function buildBaselineGrants(
  ...
  const records = await collectConfiguredBaselinePermissionsWithQueries(
    queries,
    appCode ? [appCode] : undefined
  )

  return records
    .filter(record => !baselinePermissionExcludesSubject(record, uid))
    .map((record) => {
      const permission = permissionFromBaseline(record)
      return {
        ...
        defaultScopes: [],
        assignmentScopes: [],
        scopes: baselineScopePredicate(record)
      }
    })
}
...
export async function buildDbAuthorizationGrantsWithQueries(
  ...
  const baselineGrants = await buildBaselineGrants(queries, appCode, uid, options)
