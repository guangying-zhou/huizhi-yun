...
function mergeConsoleLoginSettings(baseValue: unknown, overrideValue: unknown) {
  ...
  return {
    ...
    dingtalk: {
      ...recordValue(baseLogin.dingtalk),
      ...recordValue(overrideLogin.dingtalk)
    }
  }
}
...
export type ConsoleLoginMode = 'none' | 'oidc' | 'cas' | 'wecom' | 'dingtalk'
export type ConsoleLoginProvider = Exclude<ConsoleLoginMode, 'none'>
...
export const MAX_OIDC_DISPLAY_NAME_LENGTH = 10

export function normalizeOidcDisplayName(value: unknown) {
...
export function normalizeConsoleLoginProviders(value: unknown, mode: ConsoleLoginMode): ConsoleLoginProvider[] {
  if (mode === 'none') return []
  ...
  const configured = Array.isArray(value)
    ...
    : []
  const providers = configured.length ? configured : [mode]
  return Array.from(new Set([mode, ...providers]))
}
...
export function consoleLoginSettings(settings: Record<string, unknown>, environment?: unknown) {
  ...
  return {
    mode,
    enabledProviders: normalizeConsoleLoginProviders(login.enabledProviders, mode),
    oidc: {
    ...
    dingtalk: {
      clientId: String(dingtalk.clientId || dingtalk.appKey || '').trim(),
      corpId: String(dingtalk.corpId || '').trim()
    }
  }
}
