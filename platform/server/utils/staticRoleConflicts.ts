...
export const STATIC_ROLE_CONFLICT_RULES: StaticRoleConflictRule[] = [
  ...
  {
    ruleCode: 'codocs-review-submit-approval',
    ruleName: '文档发起与审阅审批分离',
    conflictType: 'segregation_of_duties',
    enforcement: 'warning',
    left: { appCode: 'codocs', resourceCode: 'reviews', action: 'submit' },
    right: { appCode: 'codocs', resourceCode: 'reviews', action: 'approve' },
    description: '文档审阅提交人与审阅审批长期兼任时，同一审阅实例应保留非发起人审批。'
  },
  {
    ...
    enforcement: 'warning',
    left: { appCode: 'codocs', resourceCode: 'reviews', action: 'submit' },
    right: { appCode: 'codocs', resourceCode: 'reviews', action: 'archive' },
    description: '文档发起人与发布归档长期兼任时，同一发文实例应保留复核或授权记录。'
  },
  {
    ruleCode: 'altoc-quotation-operator-approval',
    ruleName: '报价经办与报价审批分离',
    conflictType: 'segregation_of_duties',
    enforcement: 'warning',
    left: { appCode: 'altoc', resourceCode: 'quotation', action: 'edit' },
    right: { appCode: 'altoc', resourceCode: 'quotation', action: 'approve' },
    description: '报价维护和报价审批长期兼任时，同一报价实例应保留非经办人审批。'
  },
  {
    ruleCode: 'altoc-contract-operator-approval',
    ruleName: '合同经办与合同审批分离',
    conflictType: 'segregation_of_duties',
    enforcement: 'warning',
    left: { appCode: 'altoc', resourceCode: 'contract', action: 'edit' },
    right: { appCode: 'altoc', resourceCode: 'contract', action: 'approve' },
    description: '合同维护和合同审批长期兼任时，同一合同实例应保留复核或审批证据。'
  },
  {
    ruleCode: 'altoc-receivable-operator-confirmation',
    ruleName: '回款经办与回款确认分离',
    conflictType: 'segregation_of_duties',
    enforcement: 'warning',
    left: { appCode: 'altoc', resourceCode: 'receivable', action: 'edit' },
    right: { appCode: 'altoc', resourceCode: 'receivable', action: 'confirm' },
    description: '回款计划维护和确认长期兼任时，同一回款实例应保留非经办人确认。'
  },
  {
    ruleCode: 'aims-project-operator-approval',
    ruleName: '项目经办与项目审批分离',
    conflictType: 'segregation_of_duties',
    enforcement: 'warning',
    left: { appCode: 'aims', resourceCode: 'projects', action: 'edit' },
    right: { appCode: 'aims', resourceCode: 'projects', action: 'approve' },
    description: '项目维护和项目审批长期兼任时，同一项目审批实例应保留非经办人审批。'
  },
  {
    ruleCode: 'aims-work-item-operator-confirmation',
    ruleName: '工作项经办与确认分离',
    conflictType: 'segregation_of_duties',
    enforcement: 'warning',
    left: { appCode: 'aims', resourceCode: 'work_items', action: 'edit' },
    right: { appCode: 'aims', resourceCode: 'work_items', action: 'confirm' },
    description: '工作项维护和确认长期兼任时，同一工作项实例应保留复核或确认记录。'
  },
  {
    ruleCode: 'aims-timesheet-submit-approval',
    ruleName: '工时填报与工时审批分离',
    conflictType: 'segregation_of_duties',
    enforcement: 'warning',
    left: { appCode: 'aims', resourceCode: 'timesheet', action: 'submit' },
    right: { appCode: 'aims', resourceCode: 'timesheet', action: 'approve' },
    description: '工时填报和工时审批长期兼任时，同一工时实例应保留非填报人审批。'
  },
  {
    ruleCode: 'workflow-initiator-task-approval',
    ruleName: '流程发起与任务审批分离',
    conflictType: 'segregation_of_duties',
    enforcement: 'warning',
    left: { appCode: 'workflow', resourceCode: 'workflow_instances', action: 'resubmit' },
    right: { appCode: 'workflow', resourceCode: 'workflow_tasks', action: 'approve' },
    description: '流程发起/重提和审批长期兼任时，同一审批实例应保留非发起人审批。'
  }
]
...
export async function loadActiveRoleConflictRulesWithQueries(
  queries: StaticRoleConflictQueryAdapter,
  tenantCode: string,
  baseRules: StaticRoleConflictRule[] = STATIC_ROLE_CONFLICT_RULES
) {
  return mergeConflictRules(baseRules, await loadTenantRoleConflictRules(queries, tenantCode))
}
...
export async function evaluateSubjectRoleAssignmentConflictsWithQueries(
  ...
  const rules = input.rules
    || await loadActiveRoleConflictRulesWithQueries(queries, input.tenantCode)
