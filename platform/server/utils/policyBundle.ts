...
interface TenantPolicyRevisionRow extends RowDataPacket {
...
function hashPolicyBundleFactsForRevision(payload: Record<string, unknown>) {
  const facts = { ...payload }
...
async function resolveTenantPolicyRevision(
  tx: {
  ...
  await tx.execute<ResultSetHeader>(
    `INSERT INTO tenant_policy_revisions
      (tenant_code, policy_revision, policy_hash, policy_updated_at)
     VALUES (?, 0, NULL, NULL)
     ON DUPLICATE KEY UPDATE policy_revision = policy_revision`,
    [tenantCode]
  )

  const current = await tx.queryRow<TenantPolicyRevisionRow>(
  ...
  const currentRevision = normalizePolicyRevision(current?.policyRevision)
  if (currentRevision > 0 && current?.policyHash === policyHash) {
    return currentRevision
  }

  const nextRevision = currentRevision + 1
  await tx.execute<ResultSetHeader>(
    `UPDATE tenant_policy_revisions
     SET policy_revision = ?,
         policy_hash = ?,
         policy_updated_at = UTC_TIMESTAMP(),
         updated_at = UTC_TIMESTAMP()
     WHERE tenant_code = ?`,
    [nextRevision, policyHash, tenantCode]
  )
...
export async function generatePolicyBundle(input: {
  ...
  return withTransaction(async (tx) => {
    const policyRevision = await resolveTenantPolicyRevision(tx, tenantCode, policyHash)
    const payload = {
      ...draftPayload,
      policyRevision
    }
