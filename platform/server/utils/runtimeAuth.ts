...
import {
  isTenantRuntimeInstanceToken,
  verifyTenantRuntimeInstanceToken,
  type TenantRuntimeInstanceCredential
} from '~~/server/utils/tenantRuntimeInstanceToken'
...
export type RuntimeAccessContext = {
  ...
  credential: RuntimeCredentialSnapshot | TenantRuntimeInstanceCredential
}

async function verifyRuntimeCredential(options: {
  tenantCode: string
  token: string
  deploymentId?: number | null
}) {
  if (isTenantRuntimeInstanceToken(options.token)) {
    return await verifyTenantRuntimeInstanceToken(options)
  }
  return await verifyRuntimeToken({ tenantCode: options.tenantCode, token: options.token })
}
...
export async function requireRuntimeAccess(event: H3Event, path: string): Promise<RuntimeAccessContext> {
  ...
  if (deploymentCode) {
    ...
    const credential = await verifyRuntimeCredential({
