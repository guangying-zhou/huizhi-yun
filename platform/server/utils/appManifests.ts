...
import { enterpriseCompositionModules } from './enterpriseComposition'
...
export interface RegisterAppManifestResult {
  ...
  composition?: Array<{ appCode: string, manifestId: number }>
}
...
async function registerManifestInTransaction(tx: TransactionExecutor, input: RegisterAppManifestInput, snapshotOnly = false) {
  const {
    appCode,
    releaseVersion,
    manifestJson,
  ...
  if (snapshotOnly) return { manifestId: resolvedManifestId, releaseId: null, roleMaterialization }

  // 5. 找或建 release：按 (app_code, release_version) 唯一。已发布版本保持不可变；
  // 未发布版本每次导入都刷新来源，使相同 manifest 从新 tag/SHA 重试时仍可审计。
  ...
  if (existingRelease) {
      ...
      await tx.execute<ResultSetHeader>(
        `UPDATE platform_app_releases
           SET manifest_id = ?,
               ...
               updated_at = NOW()
           WHERE id = ?`,
        [resolvedManifestId, sourceTag ?? releaseVersion, sourceCommitSha, registration.insertId,
    ...
    const inserted = await tx.execute<ResultSetHeader>(
      `INSERT INTO platform_app_releases
          (app_code, release_version, source_tag, source_commit_sha, source_registration_id, manifest_id,
           bundle_uri, bundle_hash, bundle_size_bytes, status,
           released_by_account_id, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?, NOW(), NOW())`,
      [appCode, releaseVersion, sourceTag ?? releaseVersion, sourceCommitSha, registration.insertId, resolvedManifestId,
...
export async function registerAppManifest(input: RegisterAppManifestInput): Promise<RegisterAppManifestResult> {
  const modules = enterpriseCompositionModules(input.appCode, input.manifestJson)
  const { manifestId, releaseId, roleMaterialization, composition } = await withTransaction(async (tx) => {
    // Lock every participating application in a stable order before materializing
    // any catalog. Standalone imports use the same application lock.
    const codes = [input.appCode, ...modules.map(module => module.appCode)].sort()
    for (const appCode of codes) {
      const app = await tx.queryRow<RowDataPacket>(
        'SELECT id FROM platform_applications WHERE app_code = ? LIMIT 1 FOR UPDATE', [appCode])
      if (!app) throw createError({ statusCode: 404, message: `application not found: appCode=${appCode}` })
    }
    const composition: Array<{ appCode: string, manifestId: number }> = []
    for (const module of modules) {
      const result = await registerManifestInTransaction(tx, {
        ...input,
        appCode: module.appCode,
        manifestJson: module.manifest,
        sourceEndpoint: `${input.sourceEndpoint || 'enterprise-composition'}#module=${module.appCode}`,
        reviewComment: `Enterprise composition ${input.releaseVersion}: ${input.reviewComment || 'Approved via admin API'}`
      }, true)
      composition.push({ appCode: module.appCode, manifestId: result.manifestId })
    }
    const result = await registerManifestInTransaction(tx, input)
    return { ...result, composition }
  })
  if (releaseId === null) throw createError({ statusCode: 500, message: 'missing host release' })
