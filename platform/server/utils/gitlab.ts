...
import { buildReleaseRefCandidates, normalizeReleaseTagPrefix, releaseVersionFromTag } from '~~/server/utils/appReleaseRefs'
...
export async function resolveGitLabRefToCommitSha(repoUrl: string, ref: string, config: PlatformGitLabConfig = getPlatformGitLabConfig()) {
  ...
  const refCandidates = buildReleaseRefCandidates(ref)
...
export async function listGitLabReleases(
  repoUrl: string,
  config: PlatformGitLabConfig = getPlatformGitLabConfig(),
  releaseTagPrefix?: string | null
): Promise<GitLabReleaseSummary[]> {
  const projectPath = extractGitLabProjectPath(repoUrl)
  if (!projectPath) {
    throw createError({ statusCode: 400, statusMessage: 'Bad Request', message: `invalid GitLab repoUrl: ${repoUrl}` })
  }
  ...
  const [releases, tags] = await Promise.all([
    gitlabFetch<GitLabReleaseRaw[]>(
      `/api/v4/projects/${encodedProjectPath}/releases?per_page=100`,
      config
    ),
    gitlabFetch<GitLabTag[]>(
      `/api/v4/projects/${encodedProjectPath}/repository/tags?per_page=100`,
      config
    )
  ])

  const releaseItems = Array.isArray(releases) ? releases : []
  const tagItems = Array.isArray(tags) ? tags : []

  const normalizedPrefix = normalizeReleaseTagPrefix(releaseTagPrefix)
  const summaries = new Map<string, GitLabReleaseSummary>()

  for (const release of releaseItems) {
    const tagName = String(release.tag_name || '').trim()
    if (!tagName || (normalizedPrefix && !tagName.startsWith(normalizedPrefix))) {
      continue
    }

    summaries.set(tagName, {
      tagName,
      version: releaseVersionFromTag(tagName, normalizedPrefix),
  ...
  for (const tag of tagItems) {
    ...
    if (!tagName || summaries.has(tagName) || (normalizedPrefix && !tagName.startsWith(normalizedPrefix))) {
      continue
    }
    ...
    summaries.set(tagName, {
      ...
      version: releaseVersionFromTag(tagName, normalizedPrefix),
      name: tagName,
      description: tag.message ? String(tag.message) : null,
      releasedAt: null,
      createdAt: tag.commit?.created_at ? String(tag.commit.created_at) : null,
      commitSha: tag.commit?.id ? String(tag.commit.id) : null,
      upcoming: false
    })
  }
