...
function mapTenantRole(row: TenantRoleRow | null) {
  ...
  return {
    ...
    maxActiveAssignments: row.max_active_assignments,
    subjectTypeConstraint: row.subject_type_constraint,
    status: row.status
  }
}
...
async function loadSystemRole(executor: QueryExecutor, roleCode: string) {
  const role = await executor.queryRow<SystemRoleRow>(
    `SELECT id, role_code, role_name, role_type, description, is_required,
            max_active_assignments, subject_type_constraint, status
...
async function loadTenantRoleBySource(executor: QueryExecutor, tenantCode: string, systemRoleCode: string) {
  return executor.queryRow<TenantRoleRow>(
    `SELECT id, tenant_code, role_code, role_name, role_type, app_code, description,
            source, source_role_code, source_manifest_id, is_overridden, is_assignable,
            max_active_assignments, subject_type_constraint, status
     FROM tenant_roles
     WHERE tenant_code = ?
    ...
    const result = await executor.execute<ResultSetHeader>(
      `INSERT INTO tenant_roles
        (tenant_code, role_code, role_name, role_type, app_code, description,
         source, source_role_code, source_manifest_id, is_overridden, is_assignable,
         max_active_assignments, subject_type_constraint, status, created_at, updated_at)
      ...
      [
        ...
        systemRole.role_code,
        systemRole.max_active_assignments,
        systemRole.subject_type_constraint
      ]
    )
