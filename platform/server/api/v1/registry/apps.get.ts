...
import { buildAppHomeUrl, defaultAppBasePath } from '~~/server/utils/appUrls'
...
interface RegistryAppRow extends RowDataPacket {
  app_code: string
  app_name: string
...
export default defineEventHandler(async (event) => {
  ...
  const applications = await queryRows<RegistryAppRow[]>(
    `SELECT pa.app_code, pa.app_name, pa.description, pa.icon,
     ...
     LEFT JOIN platform_app_releases par ON par.id = s.target_release_id
     WHERE s.tenant_code = ?
       AND s.status = 'active'
       AND pa.status = ?
