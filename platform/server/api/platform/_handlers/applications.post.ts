...
import { normalizeManifestPath, normalizeReleaseTagPrefix } from '~~/server/utils/appReleaseRefs'
...
async function loadApplication(id: number) {
  return queryRow<ApplicationRow>(
    `SELECT pa.id, pa.app_code, pa.app_name, pa.description, pa.icon, pa.home_url, pa.callback_url, pa.logout_url, pa.repo_url,
            pa.manifest_path, pa.release_tag_prefix,
...
export default defineEventHandler(async (event) => {
  ...
  const appName = requireString(body.appName, 'appName')
  const description = normalizeNullableString(body.description)
  const icon = normalizeNullableString(body.icon)
  const homeUrl = normalizeNullableString(body.homeUrl)
  const callbackUrl = normalizeNullableString(body.callbackUrl)
  const logoutUrl = normalizeNullableString(body.logoutUrl)
  const repoUrl = normalizeNullableString(body.repoUrl)
  let manifestPath: string
  let releaseTagPrefix: string | null
  try {
    manifestPath = normalizeManifestPath(body.manifestPath)
    releaseTagPrefix = normalizeReleaseTagPrefix(body.releaseTagPrefix)
  } catch (error) {
    throw createError({ statusCode: 400, statusMessage: 'Bad Request', message: (error as Error).message })
  }
  const appType = requireAllowed(normalizeNullableString(body.appType) || 'internal', 'appType', ALLOWED_APP_TYPES)
  ...
  const authMode = requireAllowed(normalizeNullableString(body.authMode) || 'oidc', 'authMode', ALLOWED_AUTH_MODES)
  const status = requireAllowed(normalizeNullableString(body.status) || 'active', 'status', ALLOWED_STATUSES)
  const bundleEnabled = body.bundleEnabled === undefined ? true : Boolean(body.bundleEnabled)
  ...
  const created = await withTransaction(async (tx) => {
    ...
    if (existing) {
      throw createError({
        ...
        message: `application already exists: appCode=${appCode}`
      })
    }

    const insertResult = await tx.execute<ResultSetHeader>(
      `INSERT INTO platform_applications
        (app_code, app_name, description, icon, home_url, callback_url, logout_url, repo_url, manifest_path, release_tag_prefix,
         app_type, runtime_mode, auth_mode, bundle_enabled, sort_order, status, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
      [appCode, appName, description, icon, homeUrl, callbackUrl, logoutUrl, repoUrl, manifestPath, releaseTagPrefix,
        appType, runtimeMode, authMode, bundleEnabled ? 1 : 0, Math.trunc(sortOrder), status]
    )
  ...
  const application = await loadApplication(created)
  if (!application) {
    throw createError({
      statusCode: 500,
