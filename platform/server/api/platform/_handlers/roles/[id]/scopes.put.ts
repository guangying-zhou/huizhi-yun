  ...
  const prepared = scopes.map((item, index) => {
    ...
    if (!appCode) {
      throw createError({
        statusCode: 400,
        statusMessage: 'Bad Request',
        message: `scopes[${index}].appCode is required when role.appCode is empty`
      })
    }
    ...
    return { resourceCode, appCode, action, scopeType, scopeValue, status }
  })
    ...
    for (const scope of prepared) {
      const manifestAction = await tx.queryRow<ManifestActionRow>(
        `SELECT mra.id, mra.app_code, mra.resource_code, mra.action
         ...
         LIMIT 1`,
        [scope.appCode, scope.resourceCode, scope.action]
      )
      ...
      if (!manifestAction) {
        throw createError({
          statusCode: 404,
          statusMessage: 'Not Found',
          message: `manifest action not found: appCode=${scope.appCode}, resourceCode=${scope.resourceCode}, action=${scope.action}`
        })
      }
