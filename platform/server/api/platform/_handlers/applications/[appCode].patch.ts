...
function requireId(event: H3Event) {
  const raw = getRouterParam(event, 'appCode')
  const id = Number(raw)
...
export default defineEventHandler(async (event) => {
  ...
  if (body.manifestPath !== undefined) {
    try {
      updates.push('manifest_path = ?')
      params.push(normalizeManifestPath(body.manifestPath))
    } catch (error) {
      throw createError({ statusCode: 400, statusMessage: 'Bad Request', message: (error as Error).message })
    }
  }
  if (body.releaseTagPrefix !== undefined) {
    try {
      updates.push('release_tag_prefix = ?')
      params.push(normalizeReleaseTagPrefix(body.releaseTagPrefix))
    } catch (error) {
      throw createError({ statusCode: 400, statusMessage: 'Bad Request', message: (error as Error).message })
    }
  }
