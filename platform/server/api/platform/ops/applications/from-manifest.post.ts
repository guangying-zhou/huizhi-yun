...
export default defineEventHandler(async (event) => {
  ...
  const requestedVersion = requireString(body.version, 'version')
  ...
  try {
    ...
    manifestPath = normalizeManifestPath(body.manifestPath, config.defaultManifestPath)
  } catch (error) {
    throw createError({ statusCode: 400, statusMessage: 'Bad Request', message: (error as Error).message })
  }
  const ref = qualifyReleaseRef(normalizeNullableString(body.ref) || requestedVersion, releaseTagPrefix)
  const version = releaseVersionFromTag(requestedVersion, releaseTagPrefix)
  ...
  const applicationId = await withTransaction(async (tx) => {
    ...
    const insertResult = await tx.execute<ResultSetHeader>(
      ...
      [appCode, appName, description, icon, homeUrl, callbackUrl, logoutUrl, repoUrl, manifestPath, releaseTagPrefix,
        appType, runtimeMode, serviceRole, authMode, bundleEnabled ? 1 : 0, Math.trunc(sortOrder), status]
    )
