...
import {
  assertRoleAssignmentConstraints,
  bumpRoleHolderRevision
} from '~~/server/utils/roleAssignmentConstraints'
...
export default defineEventHandler(async (event) => {
  ...
  requireTenantOwnerForTenantAdmin(event, 'only tenant owner can assign subject roles')
  ...
  const assignment = await withTransaction(async (tx) => {
    const existingAssignment = await tx.queryRow<AssignmentRow>(
    ...
    await assertRoleAssignmentConstraints(tx, {
      tenantCode,
      roleId: role.id,
  ...
  return ok({
    id: assignment.id,
    roleHolderRevision: assignment.revision,
    tenantCode,
    subject: {
      id: subject.id,
      subjectType: subject.subject_type,
