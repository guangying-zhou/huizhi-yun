...
export default defineEventHandler(async (event) => {
  ...
  requireTenantOwnerForTenantAdmin(event, 'only tenant owner can update permission template roles')
    ...
    for (const role of prepared) {
      const targetRole = await tx.queryRow<RoleRow>(
        `SELECT id, tenant_code, role_code
         ...
         WHERE role_code = ?
           ...
           AND status = 'active'
         LIMIT 1`,
        [role.roleCode, template.tenant_code, TENANT_CONSOLE_APP_CODE]
      )
      ...
      if (!targetRole) {
        throw createError({
          statusCode: 404,
          statusMessage: 'Not Found',
          message: `tenant console role not found: roleCode=${role.roleCode}`
        })
      }
