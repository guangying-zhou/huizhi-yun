...
export default defineEventHandler(async (event) => {
  ...
  requireTenantOwnerForTenantAdmin(event, 'only tenant owner can update tenant role permissions')
  ...
  if (!role || role.tenant_code !== tenantCode) {
    throw createError({
      statusCode: 404,
  ...
  const prepared = permissions.map((item, index) => {
    ...
    return { resourceCode, appCode: TENANT_CONSOLE_APP_CODE, action }
  })
    ...
    for (const permission of prepared) {
      const manifestAction = await tx.queryRow<ManifestActionRow>(
        `SELECT mra.id, mra.app_code, mra.resource_code, mra.action
         ...
         LIMIT 1`,
        [TENANT_CONSOLE_APP_CODE, permission.resourceCode, permission.action]
      )
      ...
      if (!manifestAction) {
        throw createError({
          statusCode: 404,
          statusMessage: 'Not Found',
          message: `manifest action not found: appCode=${TENANT_CONSOLE_APP_CODE}, resourceCode=${permission.resourceCode}, action=${permission.action}`
        })
      }
