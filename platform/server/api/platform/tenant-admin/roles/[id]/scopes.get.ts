...
export default defineEventHandler(async (event) => {
  ...
  const tenantCode = stringValue(event.context.platformTenantCode)
  ...
  const items = await queryRows<ScopeRow[]>(
    `SELECT rs.app_code,
     ...
     WHERE rs.role_id = ?
       AND rs.tenant_code = ?
       AND rs.app_code = ?
     ORDER BY rs.resource_code ASC, rs.action ASC, rs.scope_type ASC, rs.scope_value ASC`,
    [id, role.tenant_code, TENANT_CONSOLE_APP_CODE]
  )
