...
function normalizeConsoleLoginInput(body: Record<string, unknown>, previousSettings: Record<string, unknown>) {
  ...
  const enabledProviders = normalizeConsoleLoginProviders(login.enabledProviders, mode)

  const nextOidcClientSecret = stringValue(oidc.clientSecret)
    || stringValue(previousOidc.clientSecret)
  const nextOidcDisplayName = Object.prototype.hasOwnProperty.call(oidc, 'displayName')
    ? stringValue(oidc.displayName) || DEFAULT_OIDC_DISPLAY_NAME
    : normalizeOidcDisplayName(previousOidc.displayName)
  if (Array.from(nextOidcDisplayName).length > MAX_OIDC_DISPLAY_NAME_LENGTH) {
    throw createError({
      statusCode: 400,
      statusMessage: 'Bad Request',
      message: `consoleLogin.oidc.displayName must not exceed ${MAX_OIDC_DISPLAY_NAME_LENGTH} characters`
    })
  }
  const next = {
    mode,
    enabledProviders,
    oidc: {
      displayName: nextOidcDisplayName,
    ...
    dingtalk: {
      clientId: stringValue(dingtalk.clientId || dingtalk.appKey),
      corpId: stringValue(dingtalk.corpId)
    }
  }
  ...
  if (enabledProviders.includes('oidc')) {
    ...
    if (!next.oidc.clientId) {
      throw createError({
        statusCode: 400,
        statusMessage: 'Bad Request',
        message: 'consoleLogin.oidc.clientId is required when OIDC login is enabled'
      })
    }
    if (!next.oidc.issuer && (!next.oidc.authorizationEndpoint || !next.oidc.tokenEndpoint)) {
      throw createError({
        statusCode: 400,
        statusMessage: 'Bad Request',
        message: 'consoleLogin.oidc requires issuer or both authorizationEndpoint and tokenEndpoint when OIDC login is enabled'
      })
    }
  }

  if (enabledProviders.includes('cas') && !next.cas.baseUrl) {
    throw createError({
      statusCode: 400,
      statusMessage: 'Bad Request',
      message: 'consoleLogin.cas.baseUrl is required when CAS login is enabled'
    })
  }

  if (enabledProviders.includes('wecom') && (!next.wecom.corpid || !next.wecom.agentid)) {
    throw createError({
      statusCode: 400,
      statusMessage: 'Bad Request',
      message: 'consoleLogin.wecom corpid and agentid are required when WeCom login is enabled; corpsecret belongs in Console Vault'
    })
  }

  if (enabledProviders.includes('dingtalk') && !next.dingtalk.clientId) {
    throw createError({
      statusCode: 400,
      statusMessage: 'Bad Request',
      message: 'consoleLogin.dingtalk.clientId is required when DingTalk login is enabled; appSecret belongs in Console Vault'
    })
  }
