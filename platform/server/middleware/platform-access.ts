...
const TENANT_RUNTIME_BOOTSTRAP_PATHS = new Set([
  ...
  '/api/v1/runtime/gateway-keyset', // independently authenticates enrolled control token; no business authority
...
export default defineEventHandler(async (event: EventWithAccessContext) => {
  ...
  if (path.startsWith(V1_PREFIX)) {
    if (TENANT_RUNTIME_BOOTSTRAP_PATHS.has(path)) {
      event.context.platformAccessScope = 'runtime-bootstrap'
      return
    }
