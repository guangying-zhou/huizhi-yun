...
状态：Draft v2
日期：2026-04-22
取代：`docs/archive/platform-v1/HZY-Platform-Schema-Draft-v1.md`（v1 在双域诊断后被定性为"单域错位"，本文档为重写而非增量）
前置：`docs/archive/platform-analysis/HZY-Platform-Domain-Separation-Redesign.md`（根因）、`docs/archive/platform-analysis/HZY-Platform-Schema-Critique-and-Fix.md`（字段级问题清单）
...
| description | |
| is_required | 租户创建必带 |
| max_active_assignments | 同一时点有效持有人上限；NULL 表示不限制 |
| subject_type_constraint | 可授予主体类型约束：user / department / job |
| status | |
...
### 7.12 Data Runtime 发布治理

`platform_runtime_releases` 保存 Platform 从制品仓库同步并使用当前 Ed25519 trust anchor
验签通过的 Runtime manifest 快照。制品本身仍以 R2 的不可变版本目录为事实源；Platform
不保存二进制，也不信任未经签名的 `latest` 文本指针。

`platform_runtime_release_channels` 保存 Runtime 渠道批准指针。当前使用 `stable` 渠道，
管理员通过 Platform Admin 动态批准版本，Agent heartbeat 与租户安装命令从该指针解析
`desired_version`，不再要求重新部署 Platform Worker。稳定渠道默认只能向更高版本移动；
降低版本必须由管理员显式确认，并以 `approval_kind=rollback` 持久化和写入平台审计日志。
渠道批准事务同时把目标版本物化到 release signing key 一致的 `tenant_runtime_instances`；
企业控制台读取时仍以 `stable` 指针作为有效目标，实例行由 heartbeat 最终校准，因此批准后
无需等待下一次心跳即可看到新目标，而 `current_version` 只有 Agent 实际升级并上报后才变化。

在首次动态批准前，`HZY_DATA_RUNTIME_APPROVED_VERSION` 仅作为迁移期 bootstrap 兜底；
一旦 `stable` 指针存在，数据库批准记录优先且不再读取该变量作为目标版本。

### 7.13 Tenant Runtime 注册与实例治理

`tenant_runtime_instances` 表示一个租户环境中的 tenant-runtime 逻辑实例。标准形态为
`(tenant_code, environment)` 一条记录，一个实例通过 `tenant_runtime_instance_apps`
绑定多个应用 deployment，避免把共享 Agent 任意归属到某一个业务应用 deployment。

`tenant_runtime_enrollments` 只保存一次性注册码的 SHA-256、尾号、有效期和兑换审计；
明文注册码只在租户管理员生成安装指令时返回一次。兑换成功后 Platform 分别签发
Agent 入站兼容 token 和 Agent→Platform control token，数据库只保存二者 hash。

`tenant_runtime_heartbeats` 仅保存 Runtime 版本、release signing key id、endpoint、
应用/Schema 粗粒度状态和数据库连通状态，不保存数据库地址、账号、密码或业务数据。
每次 Agent heartbeat 必须先按租户、环境和当前 active deployment 对已知 Runtime 应用执行
绑定补建/换绑，再应用本次 Schema 就绪状态；这样 Runtime 首次注册后新增或替换的业务应用
无需重新签发 enrollment，也不会因对缺失绑定执行空 `UPDATE` 而永久失去 Runtime 路由。
Gateway 只在实例为 `ready` 且对应应用绑定为 `schema_ready|active` 时下发 Runtime endpoint。
`ready` 表示 release signing key、数据库连接和 Runtime endpoint 均可用；当前版本落后于
`desired_version` 只记录 `runtime_version_incompatible` 更新漂移，不得把仍可提供业务数据的
Runtime 标记为不可用。应用 Schema 未就绪、数据库失败、签名 key 不匹配或 endpoint 缺失仍须失败关闭。
...
授权主体最小投影：用户 / 部门 / 委员会 / 岗位 / 项目统一表达。

| 字段 | 说明 |
|---|---|
| id / tenant_code | |
| subject_type | user / department / committee / job / project |
| subject_code | 租户内稳定编码；对 user 是 uid，对 department 是 dept_code，对 project 是 project_code |
| display_name | 对 department/committee/job/project 是脱敏业务标签；对 user 建议留空或等于 uid |
| external_ref | 上游系统引用 |
...
- `project` 只保存跨应用授权所需的脱敏最小投影，不在通用主体目录展示；项目成员授权优先使用动态关系，不继承部门/岗位主体角色。
...
| container_subject_id | FK `tenant_subjects.id`，通常为 department/committee/job/project |
...
约定：Console runtime 同步的多归属关系写入本表；active `member/manager/leader` 且容器为 `department/job` 的 membership 会参与 Platform 在线授权快照、DB grant adapter、Policy Bundle v1 导出和 Foundation bundle adapter，用于继承部门/职位主体上的角色与模板绑定。`committee/project` 只作为目录或动态关系事实，不作为运行时通用角色继承主体。
...
| max_active_assignments | 从平台企业角色物化的持有人上限，租户不得扩大 |
| subject_type_constraint | 从平台企业角色物化的主体类型约束，租户不得扩大 |
| created_at / updated_at | |
...
`project_director` 与 `qa` 固定为单一 user holder。通用授予写入必须在事务内锁定 `tenant_roles`，校验新旧任期是否重叠；更换当前持有人使用原子 replace 命令，不能先撤销再由另一个请求授予。

#### 8.2.3A `tenant_role_holder_revisions`

按 `(tenant_code, role_id)` 保存有效持有人变化的单调 `revision`。授予、撤销和原子替换均与业务变更同事务递增；Policy Bundle 以 `roleHolderRevisions` 投影给 Console/Workflow，用于识别项目总监等动态职责转移。
...
- `platform/docs/sql/HZY-Platform-SQL-DDL-Draft-v2.sql`（配套 DDL，已在本次同步产出）
- `HZY-Platform-Seed-Draft-v2.sql`（seed 需按新结构重写）
- `HZY-Platform-API-Draft-v2.md`（API 按 `/ops` `/tenant-admin` `/runtime` `/internal` 重分）
- `docs/Implementation-Backlog.md`：Epic 1 的 `[x]` 退回 `[~]`，新增 Epic 1B「平台域建设」
- `docs/Huizhi-yun-Platform-Target-Architecture.md`：附录 A 命名对照表需补 `platform_accounts / subscriptions / platform_signing_keys` 等新概念
