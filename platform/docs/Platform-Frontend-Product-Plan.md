...
- 角色页：
  ...
  - 权限矩阵
  - scope
  - 默认任务是“给哪些员工授予哪些企业角色”与“哪些高风险职责需要分离”
  - 权限解释、实例职责冲突解释属于高级诊断工具，只在排查具体用户或具体业务单据时显式展开，不作为默认配置页面内容展示
  - 职责冲突说明使用业务语言表达：防止同一人在同一张业务单内同时经办和审批、制单和确认；规则结果对管理员显示为“提醒/拦截”，避免暴露 `app/resource/action` 等实现术语
