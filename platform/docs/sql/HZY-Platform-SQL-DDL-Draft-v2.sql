...
-- 配套：platform/docs/HZY-Platform-Schema-Draft-v2.md
--
-- 设计骨架：
...
CREATE TABLE IF NOT EXISTS `platform_applications` (
  ...
  `manifest_path` VARCHAR(500) NOT NULL DEFAULT 'app.manifest.json' COMMENT 'Repository-relative manifest path; monorepo uses <app_code>/app.manifest.json',
  `release_tag_prefix` VARCHAR(128) NULL COMMENT 'Optional Git tag namespace such as aims/; NULL keeps legacy standalone-repo behavior',
...
CREATE TABLE IF NOT EXISTS `platform_app_releases` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `app_code` VARCHAR(64) NOT NULL,
  `release_version` VARCHAR(64) NOT NULL COMMENT '应用内版本，如 v1.2.3；不含 monorepo 组件前缀',
  `source_tag` VARCHAR(128) NOT NULL COMMENT '实际 GitLab tag ref；monorepo 示例 aims/v1.2.3',
  `source_commit_sha` VARCHAR(64) NULL,
  `source_registration_id` BIGINT UNSIGNED NULL COMMENT '产生本 release 来源快照的 manifest registration',
  ...
  CONSTRAINT `fk_platform_app_releases_registration`
    FOREIGN KEY (`source_registration_id`, `app_code`) REFERENCES `platform_app_manifest_registrations` (`id`, `app_code`),
...
CREATE TABLE IF NOT EXISTS `platform_system_roles` (
  ...
  `description` VARCHAR(500) NULL,
  `is_required` TINYINT(1) NOT NULL DEFAULT 0,
  `max_active_assignments` INT UNSIGNED NULL COMMENT '同一时点允许的有效持有人上限；NULL 表示不限制',
  `subject_type_constraint` VARCHAR(32) NULL COMMENT '允许的主体类型：user / department / job；NULL 表示沿用通用规则',
  `sort_order` INT NOT NULL DEFAULT 0,
  `status` VARCHAR(32) NOT NULL DEFAULT 'active',
  ...
  CONSTRAINT `chk_platform_system_roles_max_active`
    CHECK (`max_active_assignments` IS NULL OR `max_active_assignments` >= 1),
  CONSTRAINT `chk_platform_system_roles_subject_type`
...
-- ----- A.6 Data Runtime 发布治理 ---------------------------------------------
-- Runtime 包仍以 R2 上的签名 manifest 为制品事实；Platform 只保存已验签的发布快照和渠道批准指针。
...
CREATE TABLE IF NOT EXISTS `policy_bundles` (
  ...
  KEY `idx_policy_bundles_policy_revision` (`tenant_code`, `environment`, `policy_revision`),
...
-- 租户环境级 tenant-runtime 物理/逻辑实例。一个实例可承载多个应用 deployment。
...
CREATE TABLE IF NOT EXISTS `tenant_subjects` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `tenant_code` VARCHAR(64) NOT NULL,
  `subject_type` VARCHAR(32) NOT NULL COMMENT 'user / department / committee / job / project',
  `subject_code` VARCHAR(128) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `tenant_roles` (
  ...
  `is_assignable` TINYINT(1) NOT NULL DEFAULT 1,
  `max_active_assignments` INT UNSIGNED NULL COMMENT '从系统角色物化的有效持有人上限；租户不得扩大',
  `subject_type_constraint` VARCHAR(32) NULL COMMENT '从系统角色物化的主体类型约束；租户不得扩大',
  `status` VARCHAR(32) NOT NULL DEFAULT 'active',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  ...
  CONSTRAINT `chk_tenant_roles_max_active`
    CHECK (`max_active_assignments` IS NULL OR `max_active_assignments` >= 1),
  CONSTRAINT `chk_tenant_roles_subject_type`
    CHECK (`subject_type_constraint` IS NULL OR `subject_type_constraint` IN ('user', 'department', 'job')),
...
-- 版本历史
-- ============================================================
-- v2.0  初版三域模型（platform/docs/HZY-Platform-Schema-Draft-v2.md）
-- v2.1  运营补遗（platform/docs/HZY-Platform-Schema-Addendum-v2.1.md）：
...
-- v2.23 tenant_policy_revisions：
--       新增租户级单调策略版本状态表，并在 policy_bundles 快照 policy_revision / policy_hash。
-- v2.24 policy bundle v2 repair：
--       生产旧库补齐 tenant_policy_revisions、policy_bundles v2 元数据列与索引。
-- v2.25 default login baseline permissions：
--       默认登录用户 baseline 权限进入 Platform 表治理，并支持排除指定用户。
--
-- ============================================================
-- 下一步建议（仅注释，不执行）
-- ============================================================
...
CREATE TABLE IF NOT EXISTS service_command_receipt (
  receipt_id CHAR(36) PRIMARY KEY,operation_id CHAR(36) NOT NULL,operation_code VARCHAR(191) NOT NULL,tenant_code VARCHAR(64) NOT NULL,
  source_deployment_code VARCHAR(128) NOT NULL,deployment_code VARCHAR(128) NOT NULL,source_app VARCHAR(50) NOT NULL,target_app VARCHAR(50) NOT NULL,
  required_capability VARCHAR(191) NOT NULL,idempotency_key VARCHAR(191) NOT NULL,command_schema_version VARCHAR(30) NOT NULL,command_sha256 CHAR(64) NOT NULL,
  status VARCHAR(32) NOT NULL,target_biz_type VARCHAR(100) NULL,target_biz_code VARCHAR(191) NULL,response_http_status SMALLINT UNSIGNED NULL,
  response_summary_sha256 CHAR(64) NULL,received_at DATETIME(3) NOT NULL,last_received_at DATETIME(3) NOT NULL,completed_at DATETIME(3) NULL,
