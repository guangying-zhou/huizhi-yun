...
-- 配套：platform/docs/HZY-Platform-Schema-Addendum-v2.3.md
--
-- 业务约束（见 Addendum §2）：
...
--     · Advanced：Pro 全集 + {people, finance, insights} business
...
VALUES
  ...
  ('assets',    '资产管理',           '企业资产与资源管理，采购/分配/处置全流程',
                'business_app',       'customer-hosted', 'oidc', 1, 'active'),
  ('people',    '人员管理',           '员工事实、任职、职级、成本快照与绩效贡献',
                'business_app',       'customer-hosted', 'oidc', 1, 'active'),
  ('finance',   '经营财务',           '开票、回款、核销、项目成本与经营财务摘要',
                'business_app',       'customer-hosted', 'oidc', 1, 'active'),
...
SELECT p.`id`, app.`app_code`, app.`role_in_plan`, app.`sort_order`
  ...
  JOIN (
    ...
    -- Advanced：Pro 全集 + {people, finance, insights}
    ...
    SELECT 'advanced','people',   'business', 25 UNION ALL
    SELECT 'advanced','finance',  'business', 26 UNION ALL
    SELECT 'advanced','insights', 'business', 27
...
--   advanced | core=3 | business=8 | ...,people,finance,insights
