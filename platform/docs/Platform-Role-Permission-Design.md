...
本文是业务和技术口径说明；当前配套落地 SQL 见 `platform/docs/sql/HZY-Platform-SQL-Seed-v2.16-enterprise-roles.sql`。
...
| `finance` | 通过 `finance:expense_submitter` 发起本人报销、费用或付款申请 | 本人申请 |
...
| 人事审批 | `people:approver`，与 `people:manager` / `people:specialist` 等经办和管理角色拆分 |
...
| --- | --- |
| 概览 | `view` |
| 企业资料、目录用户、目录部门、项目注册表、目录源配置 | `view`、`edit`、`admin` |
| 目录同步 / subject export | `view`、`edit`、`export`、`admin` |
...
| `console:directory_operator` | 维护用户、部门和项目注册表，不管理目录源、同步任务或 subject export |
| `console:security_admin` | 管理集成、凭证库、服务凭证和安全相关参数 |
| `console:admin` | 管理企业控制台全部资源 |

`console:directory_operator` 用于 HR 默认岗位的基础目录维护，只覆盖用户、部门和项目注册表管理；外部目录源配置、目录同步任务管理和 subject export 由 `console:directory_manager` 作为附加职责授予，避免人事默认岗位天然具备目录源接入和主体导出能力。
...
| --- | --- |
| 工作台 | `view` |
| 审批任务 | `view`、`approve`、`reject`、`delegate`、`edit`、`admin` |
| 流程实例 | `view`、`cancel`、`resubmit`、`edit`、`admin` |
| 流程定义、表单定义、审批业务、路由规则 | `view`、`edit`、`admin` |
...
`workflow:admin` 不再默认包含 `workflow_tasks:approve|reject|delegate` 或 `workflow_instances:cancel|resubmit`；业务审批处理应通过 `workflow:approver` 授予，发起人撤回/重提应通过 `workflow:initiator` 或对象关系校验授予。
...
| `codocs:editor` | 创建、编辑被授权范围内的文档，不默认包含导出或组织资产维护 |
| `codocs:publisher` | 处理组织资产发布、审阅和归档 |
| `codocs:records_manager` | 治理企业档案、项目档案、部门档案和审阅归档，不包含应用全局配置 |
| `codocs:space_admin` | 管理文档空间、项目文档、部门文档、组织资产和资讯空间，不包含应用后台配置 |
| `codocs:admin` | 管理 Codocs 配置、模板、发文流程和全局文档能力，并显式包含项目文档导出 |
...
| 项目 | `view`、`create`、`edit`、`approve`、`close`、`admin` |
| 工作项 | `view`、`create`、`edit`、`delete`、`assign`、`confirm` |
| 看板 | `view`、`edit` |
| 工时 | `view`、`submit`、`approve`、`admin` |
| 报表 | `view`、`edit`、`admin`、`export` |
...
| `aims:member` | 执行任务、提交工时、参与看板流转；替代历史 `aims:dev` 展示语义 |
| `aims:dev` | 历史兼容角色，权限等同 `aims:member`，新 seed 不再默认使用 |
| `aims:pm` | 管理项目计划、迭代、人员、工时审核和工作项确认，不默认获得全局 `projects:admin` |
| `aims:pmo` | 管理项目组合、跨项目交付质量、项目状态、工时审核和研发报表，不包含项目审批、`projects:admin`、`timesheet:admin`、`reports:admin` 或应用后台配置 |
| `aims:project_approver` | 处理立项、项目变更、验收、重大里程碑等项目审批 |
| `aims:admin` | 管理 AIMS 系统管理员级资源，不默认承载项目审批、工作项确认、工时审批或报表导出 |
...
| --- | --- |
| 工作台 | `view` |
| 资产台账、产品资产、知识产权资产、数字资产、技术底座、环境视图、客户交付视图、供应商台账、采购单、入库激活、预警中心 | `view`、`edit`、`admin` |
| 资产操作 | `view`、`request`、`edit`、`approve`、`admin` |
...
| `assets:employee` | 仅查看 `asset:user` 精确关系下的本人资产、操作与预警；通过 `request` 发起本人领用、归还或释放，不含管理维护 |
| `assets:requester` | 发起采购，并在 `subject:self` 范围查看和发起本人领用、归还或释放申请；不含资产操作维护 |
| `assets:custodian` | 维护本人或被授权范围内的保管资产、资产操作和预警闭环 |
| `assets:owner` | 管理本部门或项目相关资产、采购申请、环境交付视图和预警闭环 |
| `assets:procurement` | 维护供应商、采购单和入库激活，不包含资产审批和资产台账管理 |
| `assets:inventory_manager` | 维护资产台账、产品资产、环境交付视图、入库记录和资产操作闭环，不包含资产审批 |
...
| 发票 | `view`、`edit`、`approve`、`admin` |
| 费用支出 | `view`、`edit`、`approve`、`confirm`、`admin` |
...
| `finance:ar_accountant` | 维护开票、正式发票和收款台账，不执行审批或收款确认 |
| `finance:ap_accountant` | 维护费用报销、项目支出和付款申请台账，不执行审批或付款确认 |
| `finance:cashier` | 确认到账收款和费用付款，不维护申请审批或银行账户资料 |
| `finance:accountant` | 维护发票、收款、核销和支出台账，不执行审批、付款确认或收款确认 |
| `finance:reconciliation_operator` | 处理发票、收款、合同和回款计划核销确认，不执行费用审批或付款确认 |
| `finance:invoice_approver` | 审批开票申请并确认开票申请进入已审批/已开票状态 |
| `finance:expense_approver` | 审批费用报销、项目支出和付款申请 |
| `finance:manager` | 管理财务台账、核销、项目核算、绩效金额财务口径和财务报表，不默认执行审批或付款确认 |
| `finance:report_viewer` | 查看和导出经营财务报表、项目核算摘要和绩效金额财务口径 |
| `finance:admin` | 管理 Finance 模块全部配置、权限和集成参数，并显式包含工作台/报表导出、开票/费用审批、收付款确认和核销确认等敏感动作 |

`finance:cashier`、`finance:reconciliation_operator`、`finance:invoice_approver` 和 `finance:expense_approver` 应作为附加职责或临时授权授予，避免默认岗位角色同时覆盖制单、审批、付款和核销。
...
| `insights:analyst` | 查看研发效能明细、部门视图、贡献者画像和异常监控 |
| `insights:report_exporter` | 导出研发效能看板和统计报表 |
| `insights:contributor_manager` | 维护贡献者画像、人员归并和贡献者映射 |
| `insights:ingestion_operator` | 查看仓库采集状态并手动触发采集或统计聚合任务 |
| `insights:monitoring_operator` | 查看异常监控并处理监控事件，不维护监控规则 |
| `insights:monitoring_admin` | 维护异常监控规则、事件级别和监控扫描配置 |
| `insights:repository_admin` | 管理代码仓库源、采集配置和仓库采集任务 |
| `insights:settings_admin` | 维护 Insights 全局参数、用户映射、部门同步和仓库源配置 |
| `insights:admin` | 管理仓库源、采集任务、监控规则和全局配置，并显式包含手动采集触发 |

`insights:report_exporter`、`insights:contributor_manager`、`insights:ingestion_operator`、`insights:monitoring_admin`、`insights:repository_admin` 和 `insights:settings_admin` 应作为附加职责授予，避免研发管理者默认具备采集触发、人员映射维护和系统配置能力。
...
| 企业角色 | 定位 | 默认应用角色 |
| --- | --- | --- |
| 系统管理员 | 企业平台最高管理员，具备全部管理权限 | `platform:authorization_admin`、`platform:authorization_simulator`、`console:admin`、`workflow:admin`、`codocs:admin`、`aims:admin`、`altoc:admin`、`assets:admin`、`finance:admin`、`insights:admin`、`people:admin` |
...
| 销售总监 | 销售经营负责人，管理客户、商机、报价和销售过程 | `altoc:sales`、`altoc:contract_manager`、`altoc:contract_approver`、`finance:viewer`、`codocs:viewer`、`workflow:approver` |
...
| 项目总监 | 多项目交付负责人，负责项目组合、交付质量、项目审批和资源协调 | `aims:pmo`、`aims:project_approver`、`assets:owner`、`finance:viewer`、`codocs:viewer`、`workflow:approver`、`insights:analyst`、`insights:report_exporter` |
| 项目经理 | 单项目管理负责人，管理项目计划、任务、成员、工时和项目资产 | `aims:pm`、`assets:owner`、`codocs:editor`、`workflow:approver`、`finance:viewer`、`insights:viewer` |
| 项目成员 | 研发、实施、交付成员，执行任务和协作文档 | `aims:member`、`codocs:editor`、`assets:employee`、`workflow:viewer` |
| 财务总监 | 财务负责人，管理财务台账、预算、费用、核算、审批和报表 | `finance:manager`、`finance:invoice_approver`、`finance:expense_approver`、`finance:report_viewer`、`altoc:viewer`、`assets:finance`、`workflow:approver`、`codocs:viewer` |
| 财务会计 | 日常财务处理人员，维护应收、应付、核销草稿和支出台账 | `finance:accountant`、`finance:ar_accountant`、`finance:ap_accountant`、`assets:finance`、`altoc:viewer`、`codocs:viewer` |
| 人力资源总监 | 人力资源体系负责人，承接组织查看、人事流程、People 管理和人事专业审批 | `console:directory_operator`、`workflow:approver`、`codocs:viewer`、`people:manager`、`people:approver` |
| 人事专员 | 日常人事操作人员，维护员工、组织、入转调离材料和人事流程 | `workflow:approver`、`codocs:editor`、`people:specialist` |
| 档案管理员 | 管理企业档案、合同档案、项目档案、人事档案和应用生成档案 | `codocs:records_manager`、`workflow:approver`；后续叠加各业务应用的档案查看和归档能力 |
| 部门经理 | 部门管理者，负责部门资产、部门费用审批、部门协同和部门范围内的业务管理 | `assets:owner`、`finance:expense_approver`、`workflow:approver`；部门文档协同依赖 baseline 的部门范围授权，文档发布审阅通过 `codocs:publisher` 作为附加职责授予 |
| 采购/资产管理员 | 采购、入库、资产台账和供应商维护人员 | `assets:procurement`、`assets:inventory_manager`、`assets:viewer`、`workflow:approver`、`finance:viewer` |
| 普通员工 | 企业基础成员的轻量岗位模板；系统必备能力由全员基础权限自动授予 | 默认不作为平台 seed 角色落库；如企业希望显式管理，可自定义角色并参考全员基础权限，按范围授予 `finance:expense_submitter`、`people:employee` |

说明：

- 系统管理员和企业超级管理员合并为一个“系统管理员”。
- 部分管理权限不再预置为独立平台角色，由企业通过自定义角色实现。
- Platform 授权治理已拆分为 `platform:authorization_admin` 与 `platform:authorization_simulator`：前者管理授权配置，后者只负责短期角色/用户模拟，避免普通授权管理员天然获得 impersonation 类高风险能力。
...
People 不再只依赖 `people:viewer` 与 `people:admin`。当前 manifest 已拆分以下应用角色，用于把员工自助、部门查看、人事经办、人力管理、专业审批、绩效、薪酬成本和字典维护分开：

| 应用角色 | 说明 |
| --- | --- |
| `people:employee` | 员工自助，查看本人档案、任职记录和绩效周期；必须通过 baseline 或授权范围限制到本人 |
| `people:department_manager` | 部门经理视角，查看授权部门内人员、任职和绩效信息；默认 seed 暂不直接映射，避免无范围授予时扩大为全局查看 |
| `people:manager` | 人力资源负责人，管理全量员工事实和任职记录，查看绩效与成本信息，不包含专业审批、绩效维护、薪酬成本和系统设置 |
| `people:specialist` | 人事专员，处理员工档案和入转调离等日常事务，不包含审批、薪酬成本和系统设置 |
| `people:approver` | 人事审批能力，如转正、调岗、调薪、离职等专业审批 |
| `people:performance_manager` | 绩效管理员，维护绩效周期和贡献快照，不包含绩效确认审批 |
| `people:compensation_admin` | 薪酬成本管理员，维护职级成本口径和成本快照，不包含成本调整审批 |
| `people:directory_admin` | People 字典管理员，维护员工事实、岗位字典、职级字典和 People 设置 |
| `people:admin` | 系统管理员级 People 全量权限，保留给平台最高权限或临时救援场景 |

企业角色默认映射按以下原则收敛：

- 人力资源总监由 `people:admin` 收敛为 `people:manager` + `people:approver`。
- 人事专员由 `people:admin` 收敛为 `people:specialist`。
- 部门经理和普通员工不通过无范围 seed 直接授予 `people:department_manager` / `people:employee`，应由 baseline、岗位职责目录或成员权限页携带 `subject:self` / `department:self` 等范围后授予。
- 薪酬成本、绩效管理、字典维护和 People 设置等职责以附加职责授予，不默认混入 HR 经办或人力负责人角色。
...
5. 调整平台预置企业角色 seed，以本文第 6 节为准；当前 SQL 文件为 `platform/docs/sql/HZY-Platform-SQL-Seed-v2.16-enterprise-roles.sql`。
...
10. 持续按岗位职责目录把 Finance 出纳、核销、审批和报表查看等附加职责从默认岗位角色中拆出。
...
| 全员基础权限 baseline | 已完成 | Platform policy bundle v2 输出 `baselineGrants`；Codocs 默认包含个人文档、部门文档、组织资料只读、资讯只读、审阅查看/提交，项目文档不进入全员默认权限；Assets 不进入 baseline，必须通过显式企业角色、应用角色或授权模板授予；Console 管理权限不进入 baseline；迁移期遗留 `console.viewer` / `tenant_console_view*` 不再作为 Console 管理入口授权；Console、Foundation 和各业务应用运行时鉴权会对有效员工自动叠加非 Console baseline。历史 bundle 的 `baselinePermissions` 仅保留兼容回退 |
| 平台预置企业角色 seed | 已完成代码和 SQL 文件 | 新增 `platform/docs/sql/HZY-Platform-SQL-Seed-v2.16-enterprise-roles.sql`，用于 upsert 平台企业角色和默认应用角色映射 |
| 应用列表可见性 | 已完成 | Console / Foundation 用户应用列表会根据企业角色权限和非 Console baseline 权限共同计算可见应用 |
| 运行时兼容 | 已完成 | 各应用 runtime authorization 已优先读取 v2 `baselineGrants` / `rolePermissionGrants`，并继续兼容历史 `baselinePermissions` / `rolePermissions` |
...
3. 执行 `platform/docs/sql/HZY-Platform-SQL-Seed-v2.16-enterprise-roles.sql`，写入平台预置企业角色和默认应用角色映射。
4. 重新生成或刷新租户 policy bundle，使 `baselineGrants`、企业角色和应用角色映射下发到运行时。

### 10.2 已知边界

- `baselineGrants` 已携带 `scopeDimension` / `scopePredicate`，但当前多数运行时授权工具仍按资源和动作聚合；本人、被分配、被分享等对象范围仍需要各业务 API 在查询和写入时做关系校验。历史 `baselinePermissions` 仍按兼容路径读取。
...
- Finance 已新增 `finance:expense_submitter`、`finance:ar_accountant`、`finance:ap_accountant`、`finance:cashier`、`finance:reconciliation_operator` 和 `finance:report_viewer`；出纳、核销和审批职责应通过岗位职责目录或成员权限页作为附加职责授予。

## 11. 后续补充事项

- `codocs:records_manager` 已用于表达档案管理员的档案治理能力；`codocs:publisher` 仍只表达文档发布审阅职责。
- 普通员工费用自助应通过带 `subject:self` 范围的 `finance:expense_submitter` 授予，不应进入无范围平台 seed。
