...
> **历史/目标设计参考**：最后核对日期：2026-07-11。本文件不是当前进度或发布资格的事实源；请以 [开发执行清单](../../docs/Development-Execution-Checklist-2026-07.md)、[综合运营路线图](../../docs/Huizhi-yun-Integrated-Operations-Roadmap.md) 和 [跨模块契约](../../docs/MODULE_CONTRACTS.md) 为准。保留原设计内容，供变更追溯参考。

> **实施状态（2026-06）**：Dashboard 端整套 UI 重设计已落地，呈现层规范、关键决策与已知偏差以 [`Platform-UIUX-Redesign-v2.md`](./Platform-UIUX-Redesign-v2.md) §11 为准。

状态：Draft v1.1  
日期：2026-04-22  
基线：`platform/docs/sql/HZY-Platform-SQL-DDL-Draft-v2.sql`（已合并 v2.1 增量）
...
5. `/dashboard/authorizations` 信息层级
- 默认页面聚焦租户管理员的主任务：分配企业角色、查看有效授权、维护职责冲突规则。
- “权限解释”和“实例职责冲突解释”是排障工具，不应作为默认页面主体展示；仅通过“高级诊断/排查具体问题”显式展开。
- 高级诊断必须用业务语言解释结果，例如“允许/不允许”“对象范围不匹配”“提醒/拦截”，避免把 `app/resource/action`、`authorizationMode` 等实现字段当作用户说明文案。
