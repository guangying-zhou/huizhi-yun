...
状态：Draft v2.4  
日期：2026-04-24  
基线：`platform/docs/sql/HZY-Platform-SQL-DDL-Draft-v2.sql` + `HZY-Platform-Schema-Addendum-v2.3.md`
...
- `platform_app_releases.release_version` 是应用内版本，如 `v0.0.1`；在独立仓中通常与 tag 相同，在 monorepo 中不包含组件前缀。
- `platform_app_releases.source_tag` 记录实际读取的完整 tag/ref；monorepo 使用 `<app_code>/vX.Y.Z`，例如 `aims/v0.0.1`。
- `platform_app_releases.source_commit_sha` 记录该 release/tag 解析出的 commit。
- `platform_applications.manifest_path / release_tag_prefix` 决定同一 monorepo 中该应用的 manifest 文件和 Tag 命名空间；前缀为空时保持旧独立仓兼容行为。
- `platform_app_releases.source_registration_id` 关联本次导入的 registration，后者的 `source_endpoint` 固定 repo URL、commit SHA 与 manifest path。
...
- GitLab release 导入入口：按应用配置过滤 release/tag，并从对应 commit 的 `manifest_path` 读取 manifest，生成/复用 manifest，并创建/更新 release。
