...
import { AUTHORIZATION_ACTION_GOLDEN_CASES } from './testing.ts'
...
test('actionSatisfies 默认层级真值表', () => {
  ...
  for (const sensitiveAction of ['confirm', 'export', 'close', 'deploy']) {
    assert.equal(actionSatisfies('edit', sensitiveAction), false)
  }
  ...
  for (const sensitiveAction of ['confirm', 'export', 'close', 'deploy']) {
    assert.equal(actionSatisfies('admin', sensitiveAction), false)
  }
...
test('authz-core 满足跨层共享动作黄金契约', () => {
  for (const item of AUTHORIZATION_ACTION_GOLDEN_CASES) {
    const actual = item.grantedActions.some(granted => actionSatisfies(granted, item.requiredAction, item.policy))
...
test('environment 范围使用谓词匹配对象环境字段', () => {
  const grants = [
    grant({
      grantId: 'prod-deploy',
      permission: { appCode: 'platform', resourceCode: 'deployments', action: 'deploy' },
      scopes: [{ dimension: 'environment', predicate: 'prod', source: 'assignment' }]
    })
  ]

  const allowedByEnvironment = evaluate({
    grants,
    required: { appCode: 'platform', resourceCode: 'deployments', action: 'deploy' },
    object: { environment: 'prod' }
  })
  assert.equal(allowedByEnvironment.allowed, true)

  const allowedByDeploymentEnvironment = evaluate({
    grants,
    required: { appCode: 'platform', resourceCode: 'deployments', action: 'deploy' },
    object: { deploymentEnvironment: 'prod' }
  })
  assert.equal(allowedByDeploymentEnvironment.allowed, true)
...
test('project 范围必须匹配项目编码，不能跨项目借用成员关系', () => {
  const grants = [
    grant({
      grantId: 'project-member',
      permission: { appCode: 'aims', resourceCode: 'projects', action: 'edit' },
      scopes: [{ dimension: 'project', predicate: 'member', value: 'PRJ-001', source: 'assignment' }]
    })
  ]
  ...
  const deniedOtherProject = evaluate({
    grants,
    required: { appCode: 'aims', resourceCode: 'projects', action: 'edit' },
    object: { actorUid: 'u1', projectCode: 'PRJ-002', projectMemberUids: ['u1'] }
  })
  assert.equal(deniedOtherProject.allowed, false)
  assert.equal(deniedOtherProject.reasonCode, 'scope_not_matched')
})

test('customer team 和 object assigned 使用对象事实匹配', () => {
  const customerDecision = evaluate({
    grants: [
      grant({
        grantId: 'customer-team',
        permission: { appCode: 'altoc', resourceCode: 'customers', action: 'view' },
        scopes: [{ dimension: 'customer', predicate: 'team', source: 'assignment' }]
      })
    ],
    required: { appCode: 'altoc', resourceCode: 'customers', action: 'view' },
    object: { actorUid: 'u1', customerTeamUids: ['u1', 'u2'] }
  })
  assert.equal(customerDecision.allowed, true)

  const objectDecision = evaluate({
    grants: [
      grant({
        grantId: 'object-assigned',
        permission: { appCode: 'workflow', resourceCode: 'workflow_tasks', action: 'approve' },
        scopes: [{ dimension: 'object', predicate: 'assigned', source: 'assignment' }]
      })
    ],
    required: { appCode: 'workflow', resourceCode: 'workflow_tasks', action: 'approve' },
    object: { actorUid: 'u2', assignedUids: ['u2'] }
  })
  assert.equal(objectDecision.allowed, true)
})
