 ...
 *  - project:code 仅匹配项目编码；project:member/owner 仍要求对应关系事实
...
function relationMatches(predicate: string, object?: ObjectContext) {
...
function predicateMatches(p: ScopePredicate, obj: ObjectContext | undefined): boolean {
  ...
  const actorUid = stringValue(obj.actorUid)

  if (dim === 'subject' && pred === 'self') {
    return !!actorUid && sameValue(actorUid, obj.ownerUid)
  }

  if (dim === 'department') {
    if (pred === 'self') {
      return valueMatchesOptionalScope(p.value, obj.departmentCode)
    }
    if (pred === 'tree') {
      const expected = stringValue(p.value)
      const departmentTree = new Set(stringList(obj.departmentTree))
      return !expected || departmentTree.has(expected) || sameValue(expected, obj.departmentCode)
    }
  }

  if (dim === 'project') {
    if (pred === 'code') {
      return sameValue(p.value, obj.projectCode)
    }
    if (pred === 'member') {
      const projectMatches = valueMatchesOptionalScope(p.value, obj.projectCode)
      if (!projectMatches) return false
      const memberUids = stringList(obj.projectMemberUids)
      return !!actorUid && memberUids.includes(actorUid)
    }
    if (pred === 'owner') {
      return valueMatchesOptionalScope(p.value, obj.projectCode)
        && !!actorUid
        && sameValue(actorUid, obj.projectOwnerUid ?? obj.ownerUid)
    }
  }

  if (dim === 'customer') {
    if (pred === 'owner') {
      return !!actorUid && sameValue(actorUid, obj.customerOwnerUid ?? obj.ownerUid)
    }
    if (pred === 'team') {
      return !!actorUid && stringList(obj.customerTeamUids).includes(actorUid)
    }
  }

  if (dim === 'object' && pred === 'assigned') {
    const assignedUids = stringList(obj.assignedUids)
    return !!actorUid && (
      assignedUids.includes(actorUid)
      || sameValue(actorUid, obj.assignedUid)
    )
  }

  if (dim === 'relation') {
    return relationMatches(pred, obj)
  }

  if (dim === 'environment') {
    const expected = stringValue(p.value ?? pred)
    return !!expected && (
      sameValue(obj.environment, expected)
      || sameValue(obj.deploymentEnvironment, expected)
    )
  }

  // 兜底：维度名直接匹配 objectContext 同名字段的值
  return p.value != null && sameValue(obj[dim], p.value)
}
