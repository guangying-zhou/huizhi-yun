...
export default defineNuxtConfig({
  ...
  runtimeConfig: {
    ...
    dataRuntimeRelease: {
      approvedVersion: process.env.HZY_DATA_RUNTIME_APPROVED_VERSION || '',
      packageBaseUrl: process.env.HZY_DATA_RUNTIME_PACKAGE_BASE_URL || 'https://downloads.huizhi.yun/packages/hzy-data-runtime',
      releasePublicKeyPem: process.env.HZY_DATA_RUNTIME_RELEASE_PUBLIC_KEY_PEM || '',
      enrollmentTtlSeconds: Number(process.env.HZY_DATA_RUNTIME_ENROLLMENT_TTL_SECONDS || '900'),
      jwtIssuer: process.env.HZY_DATA_RUNTIME_JWT_ISSUER || ''
    },
    public: {
      appCode: 'platform',
      appName: pkg.name,
