...
[platform-dev-db] next steps:
  ...
  2. Start platform-dev with a dev signing key, then import current app manifests.
  3. Execute platform/docs/sql/HZY-Platform-SQL-Seed-v2.16-enterprise-roles.sql after manifest import.
  4. Materialize system_admin for the target tenant and grant it to a test user, then generate a fresh ${TARGET_DEPLOYMENT_CODE} runtime token, license and test policy bundle.
     The root accept script validates policy-bundle.v2 roleAssignments/rolePermissionGrants and the platform:authorization_simulator grants:
     pnpm run accept:platform-policy-roles -- --base-url http://127.0.0.1:3011 --tenant-code <tenantCode> --subject-code <uid> --role-code system_admin --environment test --cookie-env PLATFORM_ACCEPT_COOKIE
  5. Run:
