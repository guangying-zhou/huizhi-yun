...
type GitLabReleasesResponse = ApiEnvelope<{
  items: GitLabReleaseItem[]
  source: {
    repoUrl: string
    manifestPath: string
...
const manifestPath = ref('app.manifest.json')
const resolvedRepoUrl = ref<string | null>(null)
const q = ref('')

const repoUrlText = computed(() => resolvedRepoUrl.value?.trim() || '')
const selectedRelease = computed(() => releases.value.find(item => item.tagName === selectedTag.value) || null)
const importVersion = computed(() => selectedRelease.value?.version || manualVersion.value.trim())
...
watch(isOpen, (open) => {
  if (open) {
    q.value = ''
    resolvedRepoUrl.value = null
    manifestPath.value = 'app.manifest.json'
    releases.value = []
    selectedTag.value = ''
    manualVersion.value = ''
    loadReleases()
...
async function loadReleases() {
  ...
  try {
    const response = await $fetch(
      ...
      { query: { appCode: props.appCode } }
    ) as GitLabReleasesResponse
    releases.value = response.data.items
    resolvedRepoUrl.value = response.data.source.repoUrl
    manifestPath.value = response.data.source.manifestPath || 'app.manifest.json'
...
<template>
  ...
  >
    <template #body>
      <div class="space-y-4">
        <UAlert
          v-if="!repoUrlText && !releasesPending"
          color="warning"
          variant="soft"
        ...
        <div class="grid gap-3 md:grid-cols-[1fr_220px]">
          <UFormField label="GitLab 仓库 URL">
            <UInput
              :model-value="repoUrlText || (releasesPending ? '正在读取应用配置…' : '未配置')"
              readonly
              class="w-full"
          ...
          <UFormField label="Manifest 路径">
            <UInput
              ...
              :disabled="releasesPending && !repoUrlText"
              class="w-full"
          ...
          >
            ...
            >
              <span class="release-option-main">
                ...
                <span class="muted">
                  {{ release.tagName === release.version ? (release.name || '未命名 Release') : release.tagName }}
                </span>
              </span>
