...
function copyRoleCodeFor(sourceRole: RoleItem) {
  const baseCode = `${sourceRole.roleCode}_custom`
  const existingCodes = new Set(roles.value.map(role => role.roleCode))
...
async function copySelectedRoleToCustom() {
  if (!selectedRole.value || !selectedRole.value.isSystem || !selectedRoleId.value) {
    notice.value = {
      type: 'error',
      message: '请先选择平台继承角色。'
    }
  ...
  try {
    const sourceRole = selectedRole.value
    const roleCode = copyRoleCodeFor(sourceRole)
    const response = await platformFetchJson<{ success: true, data: RoleCopyResponse }>(`${apiPrefix.value}/roles/${sourceRole.id}/copy`, {
      method: 'POST',
      body: {
        tenantCode: filters.tenantCode.trim(),
        roleCode,
        roleName: `${sourceRole.roleName} 自定义`,
        description: sourceRole.description || `Copied from ${sourceRole.roleCode}`,
        isAssignable: true,
        status: 'active'
      }
    })
    ...
    pagination.page = 1
    await loadRoles()
    fillForm(response.data.role)
    await Promise.all([
      loadRoleAppRoles(response.data.role.id),
      loadRolePermissions(response.data.role.id),
      loadRoleScopes(response.data.role.id)
    ])

    notice.value = {
      type: 'success',
      message: `已复制为自定义角色；复制 ${response.data.copied.appRoles} 个应用角色、${response.data.copied.permissions} 项额外权限、${response.data.copied.scopes} 条 scope。`
    }
  } catch (error) {
    notice.value = {
      type: 'error',
      message: error instanceof Error ? error.message : '复制自定义角色失败'
    }
  } finally {
            ...
            >
              ...
              <div class="flex flex-wrap gap-2">
                ...
                <UButton
                  ...
                  icon="i-lucide-copy"
                  :loading="copyPending"
                  @click="copySelectedRoleToCustom"
                >
                  复制为自定义角色
                </UButton>
              </div>
