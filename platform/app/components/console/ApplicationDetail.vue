...
function resetManifestForm(payload?: Record<string, unknown>) {
  ...
  gitlabImportForm.manifestPath = application.value?.manifestPath || 'app.manifest.json'
}
...
function fillForm(app: ApplicationItem) {
  ...
  form.manifestPath = app.manifestPath || 'app.manifest.json'
  form.releaseTagPrefix = app.releaseTagPrefix || ''
  gitlabImportForm.manifestPath = form.manifestPath
}
    ...
    const response = await fetchJson<{ success: true, data: ApplicationItem }>(
      `/api/platform/ops/applications/${application.value.id}`,
      {
        ...
        body: {
          ...
          manifestPath: form.manifestPath.trim() || 'app.manifest.json',
          releaseTagPrefix: form.releaseTagPrefix.trim() || null
        }
      }
    )
...
async function deleteApplication() {
  ...
  if (!(await confirm({
    title: '删除应用',
    message: `确定删除应用「${application.value.appName}」？删除后不可恢复。`,
    tone: 'danger'
  }))) return
...
async function loadReleasesForImport() {
  ...
  try {
    const response = await fetchJson<{ success: true, data: { items: GitLabReleaseItem[] } }>(
      ...
      { query: { appCode: application.value.appCode } }
    )
...
function pickRelease(release: GitLabReleaseItem) {
  gitlabImportForm.version = release.version
        ...
        >
          ...
          >
            <div class="grid gap-3 md:grid-cols-2">
              ...
              <label class="tenant-field">
                ...
                <UInput
                  ...
                  placeholder="aims/app.manifest.json"
                />
              </label>

              <label class="tenant-field">
                <span class="tenant-field__label">releaseTagPrefix</span>
                <UInput
                  v-model="form.releaseTagPrefix"
                  placeholder="aims/；旧独立仓留空"
                />
              </label>
            ...
            >
              ...
              <p class="mt-1">
                Manifest：{{ application.manifestPath }} · Tag 前缀：{{ application.releaseTagPrefix || '无（独立仓兼容）' }}
              </p>
              <p class="mt-1">
                ...
                <div class="space-y-4">
                  ...
                  >
                    <button
                      ...
                      type="button"
                      class="rounded-2xl border px-3 py-3 text-left transition"
                      :class="gitlabImportForm.ref === release.tagName ? 'border-sky-300 bg-sky-50' : 'border-slate-200 bg-white hover:border-sky-200'"
