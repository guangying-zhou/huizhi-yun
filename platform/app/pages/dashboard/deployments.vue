...
interface DeploymentSettings {
  ...
  consoleVault: {
    custody: 'platform_bootstrap' | 'tenant_runtime'
    fingerprint: string | null
    migratedAt: string | null
  }
  login: {
    mode: 'none' | 'oidc' | 'cas' | 'wecom' | 'dingtalk'
    enabledProviders: Array<'oidc' | 'cas' | 'wecom' | 'dingtalk'>
    oidc: {
      displayName: string
      providerCode: string
      issuer: string
...
const vaultMigrationPending = ref(false)
...
const form = reactive({
  ...
  loginMode: 'none' as 'none' | 'oidc' | 'cas' | 'wecom' | 'dingtalk',
  enabledLoginProviders: [] as Array<'oidc' | 'cas' | 'wecom' | 'dingtalk'>,
...
const loginProviderOptions: Array<{ code: LoginProvider, label: string, description: string }> = [
  { code: 'oidc', label: 'OIDC', description: '企业统一身份源或 Keycloak' },
  { code: 'wecom', label: '企业微信', description: '通过 Connector Runtime 完成身份交换' },
  { code: 'dingtalk', label: '钉钉', description: '通过 Connector Runtime 完成身份交换' },
  { code: 'cas', label: 'CAS', description: '兼容已有 CAS 单点登录' }
]

function isLoginProviderEnabled(provider: LoginProvider) {
  return form.enabledLoginProviders.includes(provider)
}

function setLoginProviderEnabled(provider: LoginProvider, enabled: boolean) {
  if (provider === form.loginMode) return
  form.enabledLoginProviders = enabled
    ? Array.from(new Set([...form.enabledLoginProviders, provider]))
    : form.enabledLoginProviders.filter(item => item !== provider)
}
...
const wecomCallbackDomain = computed(() => {
  if (!fullTenantUrl.value) return ''
  try {
    return new URL(fullTenantUrl.value).host
  } catch {
    return ''
  }
})
const wecomCallbackUrl = computed(() => fullTenantUrl.value
  ? `${fullTenantUrl.value}/api/auth/wecom-callback`
  : '')
...
function fillForm(value: DeploymentSettings) {
  ...
  form.enabledLoginProviders = value.login.enabledProviders?.length
    ? [...value.login.enabledProviders]
    : form.loginMode === 'none' ? [] : [form.loginMode]
  form.oidcDisplayName = value.login.oidc.displayName || DEFAULT_OIDC_DISPLAY_NAME
  ...
  form.dingtalkClientId = value.login.dingtalk.clientId || ''
  form.dingtalkCorpId = value.login.dingtalk.corpId || ''
}
...
async function saveSettings() {
  ...
  const oidcDisplayName = form.oidcDisplayName.trim() || DEFAULT_OIDC_DISPLAY_NAME
  if (Array.from(oidcDisplayName).length > MAX_OIDC_DISPLAY_NAME_LENGTH) {
    notice.value = {
      type: 'error',
      message: `OIDC 显示名称最多 ${MAX_OIDC_DISPLAY_NAME_LENGTH} 个汉字。`
    }
    return
  }

  form.oidcDisplayName = oidcDisplayName
  saving.value = true
  notice.value = null
  ...
  try {
    await $fetch('/api/platform/tenant-admin/deployment-settings', {
      ...
      body: {
        ...
        consoleLogin: {
          mode: form.loginMode,
          enabledProviders: form.loginMode === 'none'
            ? []
            : Array.from(new Set([form.loginMode, ...form.enabledLoginProviders])),
          oidc: {
          ...
          dingtalk: {
            clientId: form.dingtalkClientId.trim(),
            corpId: form.dingtalkCorpId.trim()
          }
        }
      }
    })
...
watch(() => form.loginMode, (mode) => {
  if (mode === 'none') {
    form.enabledLoginProviders = []
    return
  }
  if (!form.enabledLoginProviders.includes(mode)) {
    form.enabledLoginProviders = [mode, ...form.enabledLoginProviders]
  }
})
...
async function migrateConsoleVaultMasterKey() {
  if (!tenantCode.value || settings.value?.consoleVault.custody === 'tenant_runtime') return
  vaultMigrationPending.value = true
  notice.value = null
  try {
    await platformFetchJson('/api/platform/tenant-admin/deployment-settings/console-vault-migration', {
      method: 'POST',
      query: { tenantCode: tenantCode.value },
      body: { environment: selectedEnvironment.value }
    })
    notice.value = {
      type: 'success',
      message: 'Console Vault 主密钥已迁移到企业 Tenant Runtime，Platform 不再保留明文。'
    }
    await loadSettings({ clearNotice: false })
  } catch (error) {
    notice.value = {
      type: 'error',
      message: extractErrorMessage(error, 'Console Vault 主密钥迁移失败')
    }
  } finally {
    vaultMigrationPending.value = false
  }
}
    ...
    <template #body>
      ...
      >
        ...
        >
          ...
          <div
            v-if="settings?.dataRuntime.instance"
            class="grid gap-2 rounded-lg border border-default bg-muted px-4 py-3 text-sm text-muted md:grid-cols-2"
          >
            <p>Runtime：{{ settings.dataRuntime.instance.runtimeCode }}</p>
            <p>状态：{{ settings.dataRuntime.instance.status }}</p>
            <p>版本：{{ settings.dataRuntime.instance.currentVersion || '未上报' }} / {{ settings.dataRuntime.instance.desiredVersion }}</p>
            <p>最近心跳：{{ formatDate(settings.dataRuntime.instance.lastHeartbeatAt) }}</p>
            <div class="flex items-center gap-2 md:col-span-2">
              <UBadge
                :color="settings.consoleVault.custody === 'tenant_runtime' ? 'success' : 'warning'"
                variant="soft"
              >
                Vault 密钥：{{ settings.consoleVault.custody === 'tenant_runtime' ? '企业 Runtime 托管' : '待迁移' }}
              </UBadge>
              <UButton
                v-if="settings.consoleVault.custody !== 'tenant_runtime'"
                color="warning"
                variant="soft"
                size="xs"
                icon="i-lucide-key-round"
                :loading="vaultMigrationPending"
                :disabled="settings.dataRuntime.instance.currentVersion !== settings.dataRuntime.instance.desiredVersion"
                @click="migrateConsoleVaultMasterKey"
              >
                迁移 Vault 密钥
              </UButton>
            </div>
            <p
              v-if="settings.dataRuntime.instance.lastErrorCode"
              class="text-error md:col-span-2"
            >
              异常：{{ settings.dataRuntime.instance.lastErrorCode }}
            </p>
          </div>
          ...
          >
            ...
            <p>Runtime 标识：{{ installCommandMeta.runtimeCode }}</p>
            <p>目标版本：{{ installCommandMeta.desiredVersion }}</p>
            <p>启用应用：{{ installCommandMeta.enabledApps.length ? installCommandMeta.enabledApps.join(', ') : '无' }}</p>
            <p>一次性注册码尾号：{{ installCommandMeta.enrollmentCodeLast4 }}</p>
            <p>有效期：{{ Math.round(installCommandMeta.expiresInSeconds / 60) }} 分钟</p>
          </div>
          ...
          <div class="border-t border-default pt-5">
            <div class="console-panel__header">
              ...
              <UBadge
                ...
                variant="soft"
              >
                {{ form.loginMode === 'none' ? '未配置' : `${form.enabledLoginProviders.length} 个入口` }}
              </UBadge>
            </div>

            <div class="mt-4 grid gap-4 md:grid-cols-2">
              <label class="tenant-field">
                <span class="tenant-field__label">默认登录方式</span>
                <select
            ...
            <div
              v-if="form.loginMode !== 'none'"
              class="mt-4 grid gap-3 sm:grid-cols-2"
            >
              <div
                v-for="provider in loginProviderOptions"
                :key="provider.code"
                class="flex items-start gap-3 rounded-lg border border-default bg-muted px-3 py-3"
              >
                <UCheckbox
                  :model-value="isLoginProviderEnabled(provider.code)"
                  :disabled="provider.code === form.loginMode"
                  :aria-label="`启用${provider.label}登录`"
                  @update:model-value="value => setLoginProviderEnabled(provider.code, Boolean(value))"
                />
                <div class="min-w-0">
                  <div class="flex flex-wrap items-center gap-2">
                    <span class="text-sm font-medium text-highlighted">{{ provider.label }}</span>
                    <UBadge
                      v-if="provider.code === form.loginMode"
                      color="primary"
                      variant="soft"
                      size="sm"
                    >
            ...
            <div
              v-if="isLoginProviderEnabled('oidc')"
              class="mt-5 grid gap-4 rounded-lg border border-default p-4 md:grid-cols-2"
            >
              ...
              <label class="tenant-field md:col-span-2">
                <span class="tenant-field__label">显示名称</span>
                <UInput
                  v-model="form.oidcDisplayName"
                  :maxlength="MAX_OIDC_DISPLAY_NAME_LENGTH"
                  :placeholder="DEFAULT_OIDC_DISPLAY_NAME"
                  autocomplete="off"
                />
                <span class="text-xs text-muted">
                  用于 Console 登录页的 OIDC 登录按钮，最多 10 个汉字；留空恢复默认名称。
                </span>
              </label>
              <!-- <label class="tenant-field">
            ...
            <div
              v-if="isLoginProviderEnabled('cas')"
              class="mt-5 grid gap-4 rounded-lg border border-default p-4 md:grid-cols-2"
            >
              <div class="md:col-span-2">
                <h4 class="text-sm font-semibold text-highlighted">
            ...
            <div
              v-if="isLoginProviderEnabled('wecom')"
              class="mt-5 grid gap-4 rounded-lg border border-default p-4 md:grid-cols-2"
            >
              <div class="md:col-span-2">
                <h4 class="text-sm font-semibold text-highlighted">
              ...
              <div class="md:col-span-2 rounded-lg border border-default bg-muted p-3">
                <p class="text-sm font-semibold text-highlighted">
                  企业微信后台回调配置
                </p>
                <p class="mt-1 text-xs text-muted">
                  在同一自建应用的“企业微信授权登录 / Web 网页”填写授权回调域；不要包含协议、路径或通配符。“网页授权及 JS-SDK / 可信域名”使用同一个域名。
                </p>
                <div class="mt-3 grid gap-3 lg:grid-cols-2">
                  <div class="rounded-md border border-default bg-default p-3">
                    ...
                    <div class="mt-2 flex items-center gap-2">
                      <code class="min-w-0 flex-1 break-all text-sm text-highlighted">{{ wecomCallbackDomain || '填写企业子域名后生成' }}</code>
                      <UButton
                        type="button"
                        color="neutral"
                        variant="ghost"
                        size="xs"
                        icon="i-lucide-copy"
                        :disabled="!wecomCallbackDomain"
                        @click="copyArtifact(wecomCallbackDomain, 'wecom-callback-domain')"
                      >
                        {{ copyFeedback?.key === 'wecom-callback-domain' ? copyFeedback.message : '复制' }}
                      </UButton>
                    </div>
                  </div>
                  <div class="rounded-md border border-default bg-default p-3">
                    ...
                    <div class="mt-2 flex items-center gap-2">
                      <code class="min-w-0 flex-1 break-all text-sm text-highlighted">{{ wecomCallbackUrl || '填写企业子域名后生成' }}</code>
                      <UButton
                        type="button"
                        color="neutral"
                        variant="ghost"
                        size="xs"
                        icon="i-lucide-copy"
                        :disabled="!wecomCallbackUrl"
                        @click="copyArtifact(wecomCallbackUrl, 'wecom-callback-url')"
                      >
                        {{ copyFeedback?.key === 'wecom-callback-url' ? copyFeedback.message : '复制' }}
                      </UButton>
                    </div>
                  </div>
                </div>
              ...
              <UAlert
                ...
                title="Corp Secret 由 Console Vault 管理"
                description="Platform 只保存登录模式、Corp ID 和 Agent ID；企业微信密钥请在企业控制台的集成中心配置到 wecom.default。"
              />
            </div>

            <div
              v-if="isLoginProviderEnabled('dingtalk')"
              class="mt-5 grid gap-4 rounded-lg border border-default p-4 md:grid-cols-2"
            >
              <div class="md:col-span-2">
                <h4 class="text-sm font-semibold text-highlighted">
              ...
              <label class="tenant-field">
                <span class="tenant-field__label">登录 OAuth Client ID / AppKey（非 UnifiedAppId）</span>
                <UInput
                  v-model="form.dingtalkClientId"
                  autocomplete="off"
                />
              </label>
              <label class="tenant-field">
                <span class="tenant-field__label">Corp ID（可选校验）</span>
                <UInput
              ...
              <UAlert
                ...
                variant="soft"
                icon="i-lucide-shield-check"
                title="App Secret 由 Console Vault 管理"
                description="Platform 只保存登录模式、OAuth Client ID 和 Corp ID；请使用钉钉网页登录能力的 Client ID（企业内部应用通常为 AppKey），不要使用 UnifiedAppId。AppSecret 请在企业控制台集成中心配置到 dingtalk.default。"
              />
            </div>
          </div>
