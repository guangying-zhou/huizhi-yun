...
const PLATFORM_AUTHORIZATION_SIMULATION_ACTIONS = ['simulate-role', 'simulate-user']
...
function hasActiveRolePermissionGrant(payload, action) {
  const grants = Array.isArray(payload?.rolePermissionGrants) ? payload.rolePermissionGrants : []
  return grants.some(grant =>
    stringValue(grant?.roleCode) === 'system_admin'
    && stringValue(grant?.appCode) === 'platform'
    && stringValue(grant?.resourceCode) === 'authorization'
    && stringValue(grant?.action) === action
    && stringValue(grant?.appRoleCode) === PLATFORM_AUTHORIZATION_SIMULATOR_APP_ROLE
    && stringValue(grant?.status || 'active') === 'active'
  )
}

function hasAdminSourcedSimulationGrant(payload) {
  const grants = Array.isArray(payload?.rolePermissionGrants) ? payload.rolePermissionGrants : []
  return grants.some(grant =>
    stringValue(grant?.roleCode) === 'system_admin'
    && stringValue(grant?.appCode) === 'platform'
    && stringValue(grant?.resourceCode) === 'authorization'
    && PLATFORM_AUTHORIZATION_SIMULATION_ACTIONS.includes(stringValue(grant?.action))
    && stringValue(grant?.appRoleCode) === PLATFORM_AUTHORIZATION_ADMIN_APP_ROLE
    && stringValue(grant?.status || 'active') === 'active'
  )
}

async function checkPlatformAuthorizationSimulationCapability(connection, tenantCode, bundlePayload) {
  const failures = []
  for (const table of [
    ...
    if (!await tableExists(connection, table)) {
      failures.push(`required table is missing for role simulation capability check: ${table}`)
    }
  }
  if (failures.length) return failures

  const manifestActions = await queryRows(
    connection,
    `SELECT action, status
     FROM platform_app_manifest_resource_actions
     WHERE app_code = 'platform'
       AND resource_code = 'authorization'
       AND action IN (?, ?)`,
    PLATFORM_AUTHORIZATION_SIMULATION_ACTIONS
  )
  const activeManifestActions = new Set(manifestActions
    .filter(row => stringValue(row.status) === 'active')
    .map(row => stringValue(row.action)))
  for (const action of PLATFORM_AUTHORIZATION_SIMULATION_ACTIONS) {
    if (!activeManifestActions.has(action)) {
      failures.push(`missing active Platform manifest action platform:authorization:${action}`)
    }
  }

  const simulatorRole = await queryOne(
    connection,
    `SELECT role_code AS roleCode, app_code AS appCode, status
     FROM platform_app_roles
     WHERE role_code = ?
     LIMIT 1`,
    [PLATFORM_AUTHORIZATION_SIMULATOR_APP_ROLE]
  )
  if (!simulatorRole) {
    failures.push(`missing app role ${PLATFORM_AUTHORIZATION_SIMULATOR_APP_ROLE}; import platform manifest first`)
  } else {
    if (stringValue(simulatorRole.appCode) !== 'platform') {
      failures.push(`expected ${PLATFORM_AUTHORIZATION_SIMULATOR_APP_ROLE} app_code platform, got ${stringValue(simulatorRole.appCode) || '<empty>'}`)
    }
    if (stringValue(simulatorRole.status) !== 'active') {
      failures.push(`expected ${PLATFORM_AUTHORIZATION_SIMULATOR_APP_ROLE} status active, got ${stringValue(simulatorRole.status) || '<empty>'}`)
    }
  }

  const simulatorPermissions = await queryRows(
    connection,
    `SELECT arp.action
     FROM platform_app_role_permissions arp
     INNER JOIN platform_app_roles ar
       ON ar.id = arp.app_role_id
     WHERE ar.role_code = ?
       AND ar.status = 'active'
       AND arp.app_code = 'platform'
       AND arp.resource_code = 'authorization'
       AND arp.action IN (?, ?)`,
    [PLATFORM_AUTHORIZATION_SIMULATOR_APP_ROLE, ...PLATFORM_AUTHORIZATION_SIMULATION_ACTIONS]
  )
  const simulatorPermissionActions = new Set(simulatorPermissions.map(row => stringValue(row.action)))
  for (const action of PLATFORM_AUTHORIZATION_SIMULATION_ACTIONS) {
    if (!simulatorPermissionActions.has(action)) {
      failures.push(`missing ${PLATFORM_AUTHORIZATION_SIMULATOR_APP_ROLE} permission platform:authorization:${action}`)
    }
  }

  const adminSimulationPermission = await queryOne(
    connection,
    `SELECT COUNT(*) AS total
     ...
     WHERE ar.role_code = ?
       ...
       AND arp.resource_code = 'authorization'
       AND arp.action IN (?, ?)`,
    [PLATFORM_AUTHORIZATION_ADMIN_APP_ROLE, ...PLATFORM_AUTHORIZATION_SIMULATION_ACTIONS]
  )
  const adminSimulationPermissionCount = Number(adminSimulationPermission?.total || 0)
  if (adminSimulationPermissionCount > 0) {
    failures.push(`${PLATFORM_AUTHORIZATION_ADMIN_APP_ROLE} must not contain role/user simulation permissions, found ${adminSimulationPermissionCount}`)
  }

  const systemAdminMaps = await queryRows(
    connection,
    `SELECT sarm.app_role_code AS appRoleCode
     ...
     WHERE sr.role_code = 'system_admin'
       AND sarm.app_role_code IN (?, ?)`,
    [PLATFORM_AUTHORIZATION_ADMIN_APP_ROLE, PLATFORM_AUTHORIZATION_SIMULATOR_APP_ROLE]
  )
  const systemAdminAppRoles = new Set(systemAdminMaps.map(row => stringValue(row.appRoleCode)))
  for (const appRoleCode of [PLATFORM_AUTHORIZATION_ADMIN_APP_ROLE, PLATFORM_AUTHORIZATION_SIMULATOR_APP_ROLE]) {
    if (!systemAdminAppRoles.has(appRoleCode)) {
      failures.push(`missing system_admin platform system-role mapping to ${appRoleCode}; run v2.16 enterprise-role seed`)
    }
  }

  const tenantSystemAdminMaps = await queryRows(
    connection,
    `SELECT tram.app_role_code AS appRoleCode
     ...
     WHERE tram.tenant_code = ?
       AND tr.role_code = 'system_admin'
       AND tram.app_role_code IN (?, ?)`,
    [tenantCode, PLATFORM_AUTHORIZATION_ADMIN_APP_ROLE, PLATFORM_AUTHORIZATION_SIMULATOR_APP_ROLE]
  )
  const tenantSystemAdminAppRoles = new Set(tenantSystemAdminMaps.map(row => stringValue(row.appRoleCode)))
  for (const appRoleCode of [PLATFORM_AUTHORIZATION_ADMIN_APP_ROLE, PLATFORM_AUTHORIZATION_SIMULATOR_APP_ROLE]) {
    if (!tenantSystemAdminAppRoles.has(appRoleCode)) {
      failures.push(`missing tenant ${tenantCode} system_admin mapping to ${appRoleCode}; materialize tenant roles after seed`)
    }
  }

  if (!Array.isArray(bundlePayload?.rolePermissionGrants)) {
    failures.push('expected policy bundle v2 rolePermissionGrants array for role simulation capability check')
  } else {
    for (const action of PLATFORM_AUTHORIZATION_SIMULATION_ACTIONS) {
      if (!hasActiveRolePermissionGrant(bundlePayload, action)) {
        failures.push(`active policy bundle is missing system_admin rolePermissionGrant from ${PLATFORM_AUTHORIZATION_SIMULATOR_APP_ROLE} for platform:authorization:${action}`)
      }
    }
    if (hasAdminSourcedSimulationGrant(bundlePayload)) {
      failures.push(`active policy bundle must not source role/user simulation grants from ${PLATFORM_AUTHORIZATION_ADMIN_APP_ROLE}`)
    }
  }
...
async function checkReadyArtifacts(connection, args) {
  ...
  const bundle = await queryOne(
    ...
    `SELECT pb.id, pb.bundle_version AS bundleVersion, pb.bundle_hash AS bundleHash,
            pb.policy_revision AS policyRevision, pb.policy_hash AS policyHash,
  ...
  if (!bundle) {
    ...
    if (stringValue(bundle.schemaVersion) !== 'policy-bundle.v2') {
      failures.push(`expected policy bundle schemaVersion policy-bundle.v2, got ${stringValue(bundle.schemaVersion) || '<empty>'}`)
    }
    if (Number(bundle.policyRevision || 0) <= 0) {
      failures.push(`expected positive policy_revision on active policy bundle for ${expectedTestDeployment}`)
    }
    ...
    if (!bundlePayload) {
      ...
      if (stringValue(bundlePayload.schemaVersion) !== 'policy-bundle.v2') {
        failures.push(`expected policy bundle payload schemaVersion policy-bundle.v2, got ${stringValue(bundlePayload.schemaVersion) || '<empty>'}`)
      }
      if (Number(bundlePayload.policyRevision || 0) !== Number(bundle.policyRevision || 0)) {
        failures.push(`expected payload policyRevision ${Number(bundle.policyRevision || 0)}, got ${Number(bundlePayload.policyRevision || 0)}`)
      }
      ...
      failures.push(...await checkPlatformAuthorizationSimulationCapability(connection, tenantCode, bundlePayload))
    }
  }
