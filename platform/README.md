...
`wiztek` Platform 默认发布到 Cloudflare Worker，并通过 `https://huizhi.yun` / `https://www.huizhi.yun` 提供控制面。`pnpm run deploy:cloudflare` 会依次执行 lint、typecheck、测试、Cloudflare 构建和 dry-run，全部通过后直接发布，不再需要 override 环境变量。

在仓库根目录执行：

```bash
pnpm --filter platform deploy:cloudflare
```
...
只有明确要求固定国内 IP 的企业微信等场景才采用 `.env.prod` / `.env.dev` 配合 PM2 + Nginx 的可选自托管方式；这不是 wiztek 的默认发布路径。
...
Worker 名称和路由以 `platform/wrangler.jsonc` 为准；当前生产 Worker 为 `hzy-platform`，默认绑定 `huizhi.yun` 和 `www.huizhi.yun`。真实 `platform/.env.cloudflare` 仅保留在部署机器本地并由 `.gitignore` 忽略；仓库只跟踪 `platform/.env.cloudflare.example`。根目录 `pnpm run validate:platform-cloudflare-default` 会确认默认域名、Hyperdrive、部署前置检查和 env 跟踪规则。

生产环境需要提供 Platform 根签名私钥。该私钥用于签发 license、policy bundle 与 revocation；如果缺失，保存租户应用订阅、生成测试环境 bundle 或重新签发 license 时会返回 503。

### 生成 Platform 根签名私钥

Platform 根签名私钥使用 Ed25519 PKCS#8 PEM。`openssl genpkey -algorithm Ed25519` 生成的密钥格式应为 PKCS#8，而不是 OpenSSH 私钥格式。
...
# Console Vault 主密钥由 Platform 后端一次性迁移到 Tenant Runtime，
# 不再写入 Console Worker secret 或浏览器可见的 .env 工件。
...
可选 PM2/Nginx profile 的完整上线验收仍可使用根目录统一入口；默认 Cloudflare Platform 发布使用 `deploy:cloudflare` 自带的 preflight、build 和 dry-run 门禁：
...
- 先查看 Cloudflare System Status。如果 Dashboard / API 正在故障、降级或影响创建/修改服务，等待恢复后重新执行 `pnpm --filter platform deploy:cloudflare`。
