...
export interface CollaborationDocumentClaims {
  docId: number
  docUuid: string
  docType: string
  ossPath: string
...
const getCollaborationAuthSecret = () => {
  ...
  const secret = String(config.collaborationAuthSecret || '').trim()
...
export const signCollaborationToken = (payload: CollaborationAuthPayload): string => {
  const encodedPayload = encode(JSON.stringify(payload))
  const signature = createSignature(encodedPayload, getCollaborationAuthSecret())

  return `${encodedPayload}.${signature}`
}

export const verifyCollaborationToken = (token: string): CollaborationAuthPayload | null => {
  ...
  try {
    const payload = JSON.parse(decode(encodedPayload)) as Partial<CollaborationAuthPayload>
