...
export async function resolveCodocsCollaborationContext(
  ...
  const trustedDeptCode = String(options.deptCode || '').trim()
  const query: Record<string, unknown> = {
    actorUid: options.actorUid,
    actorName: options.actorName
  }
  if (trustedDeptCode) {
    query.trusted_department_read_dept_code = trustedDeptCode
    query.trustedDepartmentReadDeptCode = trustedDeptCode
  }
