...
import {
  ...
  fetchDirectoryUserDepartmentsByService
...
export async function fetchUserDepartments(event: H3Event, uid: string): Promise<UserDepartmentsResult> {
  ...
  const deptResponse = (await fetchDirectoryDepartmentsByService(event)).data
  if (!deptResponse) {
    return { departments: [], primaryDeptCode: null }
  }
  ...
  try {
    const userDeptResponse = (await fetchDirectoryUserDepartmentsByService(event, uid)).data as DirectoryUserDepartmentsData
    if (userDeptResponse) {
