...
export type RuntimeOSSClientOptions = OSSClientOptions & {
  event?: H3Event | null
}
...
function runtimeIntegrationCode() {
  const config = useRuntimeConfig() as unknown as { oss?: Record<string, unknown> }
  return stringValue(config.oss?.integrationCode) || 'oss.default'
}

function runtimeClientOptions(options: RuntimeOSSClientOptions = {}) {
  const { event: _event, ...clientOptions } = options
...
export const downloadImageBuffer = async (
  ossPath: string,
  options: RuntimeOSSClientOptions = {}
): Promise<Buffer | null> => {
  const client = options.event
    ? await createRuntimeImagesOSSClient(options)
    : await createImageClient(options)
  try {
