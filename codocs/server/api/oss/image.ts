...
import { normalizeImageObjectPath } from '../../utils/ossImagePath'

export default defineEventHandler(async (event) => {
  const query = getQuery(event)
  const imagePath = normalizeImageObjectPath(query.path)

  // console.log('[OSS Image] Requested path:', imagePath)

  try {
    const content = await downloadImageBuffer(imagePath, { event })
    if (!content) {
      throw createError({
        statusCode: 404,
