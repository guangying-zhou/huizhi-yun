...
export default defineEventHandler(async (event) => {
  ...
  await requirePermission(event, 'info', 'edit', '缺少资讯推荐权限')
  const body = await readBody<RecommendBody>(event)

  if (!body?.articleTitle || body.articleId === undefined || body.articleId === null || body.articleId === '') {
    throw createError({ statusCode: 400, message: '缺少文章标识或标题' })
  }
  ...
  const requestKey = requireCodocsNotificationRequestKey(event, 'Article recommendation')
  ...
  if (body.toUsers && body.toUsers.length > 0) {
    try {
      await sendNotification(buildCodocsNotification({
        ...
        url,
        btntxt: '阅读文章',
        eventType: 'codocs.info.recommended_to_users',
        category: 'info_recommendation',
        severity: 'info',
        bizType: 'info_article',
        bizId: body.articleId,
        idempotencyKey: codocsNotificationIdempotencyKey('info-recommend-users', requestKey, body.articleId),
        metadata: {
          notificationKind: 'manual_recommendation',
          requestKeyHash: codocsNotificationIdempotencyKey('request', requestKey),
          articleId: body.articleId,
          actorUid,
          recipientCount: body.toUsers.length
        }
      }))
    ...
    try {
      ...
      if (!deptUserUids.length) {
        ...
        await sendNotification(buildCodocsNotification({
          ...
          url,
          btntxt: '阅读文章',
          eventType: 'codocs.info.recommended_to_departments',
          category: 'info_recommendation',
          severity: 'info',
          bizType: 'info_article',
          bizId: body.articleId,
          idempotencyKey: codocsNotificationIdempotencyKey('info-recommend-departments', requestKey, body.articleId),
          metadata: {
            notificationKind: 'manual_recommendation',
            requestKeyHash: codocsNotificationIdempotencyKey('request', requestKey),
            articleId: body.articleId,
            actorUid,
            departmentCodes: body.toDepts,
            recipientCount: deptUserUids.length
          }
        }))
