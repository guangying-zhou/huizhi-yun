...
export default defineEventHandler(async (event) => {
  if (!isLegacyAuthEnabled(event)) {
    throw createError({
      statusCode: 410,
      message: 'Codocs direct WeCom login is disabled. Use Console OIDC login.'
    })
  }

  const query = getQuery(event)
  const { corpid, agentid } = await getWecomOAuthIntegrationConfig()
  const targetApp = typeof query.target_app === 'string' && query.target_app.trim() ? query.target_app.trim() : 'codocs'
