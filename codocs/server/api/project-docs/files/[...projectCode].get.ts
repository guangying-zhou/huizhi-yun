...
export default defineEventHandler(async (event) => {
  ...
  // Folder/document project scope requires a signed source-domain assertion;
  // global projects:view and a browser project code are not sufficient.
  throw createError({
    statusCode: 503,
  ...
  const projectCandidates: DirectoryProject[] = projectResponse.data?.flat?.length
  ...
  const isGroup = Number(resolvedProject.isGroup) === 1
  const isGitLabProject = Boolean(resolvedProject.repoUrl && (!isGroup || resolvedProject.docsSyncedAt))
  if (isGitLabProject) {
    const repoUrl = String(resolvedProject.repoUrl || '')
    const repoPath = extractGitProjectPath(repoUrl)
    if (!repoPath) throw createError({ statusCode: 400, message: '项目未配置仓库地址或仓库地址不正确' })
