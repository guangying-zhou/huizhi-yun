...
export default defineEventHandler(async (event) => {
  ...
  const { departments, primaryDeptCode } = await fetchUserDepartments(event, uid)
  ...
  const shareResult = await callCodocsTenantRuntime<{ id?: number | string }>(event, '/v1/codocs/dept-shares', {
    method: 'POST',
    scope: 'codocs.write',
    body: {
      document_id: doc.id,
  ...
  try {
    ...
    if (managerUids.length > 0 && shareResult.id) {
      ...
      await sendNotification(buildCodocsNotification({
        ...
        btntxt: '查看详情',
        eventType: 'codocs.department_share.transfer_requested',
        category: 'document_share',
        severity: 'warning',
        bizType: 'department_share',
        bizId: shareResult.id,
        idempotencyKey: codocsNotificationIdempotencyKey('department-share-transfer-requested', shareResult.id),
        metadata: {
    ...
    } else if (managerUids.length > 0) {
      console.warn('[DeptShare] Skip notification: runtime did not return stable share id')
    }
  } catch (e) {
