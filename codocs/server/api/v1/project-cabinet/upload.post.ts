...
export default defineEventHandler(async (event) => {
  await requireAimsProjectCabinetServiceAuth(event, AIMS_PROJECT_CABINET_UPLOAD_SERVICE_AUTH)
  ...
  const ownerUid = multipart.find(x => x.name === 'owner_uid')?.data.toString().trim()
  const projectCode = multipart.find(x => x.name === 'project_code')?.data.toString().trim()
  const deptCode = multipart.find(x => x.name === 'dept_code')?.data.toString().trim()
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (metadata.project_code !== projectCode) {
        throw createError({
          statusCode: 503,
          message: 'Codocs 项目文件柜元数据未启用 project_code，请先执行 cabinet_files 迁移'
        })
      }
