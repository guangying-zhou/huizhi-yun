...
async function notifyShareSender(share: DeptShareRow, handlerUid: string, action: 'accept' | 'reject') {
  ...
  try {
    ...
    await sendNotification(buildCodocsNotification({
      ...
      btntxt: '查看文档',
      eventType: `codocs.department_share.${action === 'accept' ? 'accepted' : 'rejected'}`,
      category: 'document_share',
      severity: action === 'accept' ? 'success' : 'warning',
      bizType: 'department_share',
      bizId: share.id,
      idempotencyKey: codocsNotificationIdempotencyKey('department-share-handled', share.id, action),
      metadata: {
        notificationKind: 'business_event',
        shareId: share.id,
        documentUuid: share.document_uuid || null,
        departmentCode: share.dept_code,
        handlerUid,
        action
      }
    }))
...
export default defineEventHandler(async (event) => {
  ...
  await requireDepartmentManagerAccess(event, uid, share.dept_code, '仅部门经理可接收或拒绝移交文档')

  const nextStatus = action === 'accept' ? 'accepted' : 'rejected'
  await callCodocsTenantRuntime(event, `/v1/codocs/dept-shares/${encodeURIComponent(id)}`, {
    method: 'PATCH',
    scope: 'codocs.write',
    query: { [TRUSTED_DEPARTMENT_SHARE_MANAGER_QUERY_KEY]: share.dept_code },
    body: {
      status: nextStatus,
      accepted_by: uid,
      handled_by: uid,
