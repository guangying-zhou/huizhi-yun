 ...
 * body: { dept_code, dept_name, year, week }
 *
...
export default defineEventHandler(async (event) => {
  ...
  const { dept_code, dept_name, year, week } = body

  if (!dept_code || !dept_name || !year || !week) {
    throw createError({
      statusCode: 400,
      message: '缺少参数: dept_code, dept_name, year, week'
    })
  }
  ...
  const fillerName = getRequestDisplayName(event)
