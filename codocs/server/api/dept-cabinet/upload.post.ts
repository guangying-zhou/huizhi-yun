...
import { createRuntimeOSSClient } from '../../utils/oss'
...
export default defineEventHandler(async (event) => {
  ...
  await requireDepartmentManagerAccess(event, actorUid, deptCode, '仅部门经理可上传部门文件柜文件')
  ...
  for (const file of files) {
    ...
    try {
      ...
      const ossPath = `codocs/departments/${safePathSegment(deptCode)}/cabinet/${uuid}.${ext}`
      ...
      await createCabinetFileMetadata(event, 'department', {
        ...
        dept_code: deptCode,
        folder_id: folderId
      }, { departmentManagerCode: deptCode })
