...
const compiled = ts.transpileModule(readFileSync(new URL('../server/api/v1/service/product-documents/[uuid]/metadata.post.ts', import.meta.url), 'utf8'), { compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022 } }).outputText
function harness(options: { scopes?: string[], badSignature?: boolean, tenant?: string, app?: string, bodyPatch?: object, hash?: string, runtimeFailure?: boolean, realSignature?: boolean, tamper?: string } = {}) {
  ...
  const envelope = { operationId: 'operation-1', targetApp: 'codocs', operationCode: 'aims.codocs.product-document.read.v1', requiredCapability: 'codocs:product-document:read', idempotencyKey: 'key-1', commandSchemaVersion: 'aims.codocs.product-document.read.v1', commandSha256: options.hash ?? 'hash', command }
  runInNewContext(compiled, { exports, defineEventHandler: (fn: unknown) => fn, require: (name: string) => {
    if (name === 'h3') return { createError, setHeader: () => {}, getHeader: (_: unknown, key: string) => headers[key], getQuery: () => ({}), getRouterParam: () => uuid, getRequestURL: () => new URL('https://codocs.example/api/v1/service/product-documents/' + uuid + '/metadata'), readBody: async () => {
      calls.push('body')
    ...
    if (name.endsWith('/codocsRuntime')) return { callCodocsTenantRuntime: async (_: unknown, path: string, args: any) => {
      ...
      return { uuid, title: '产品说明', doc_type: 'product', updated_at: '2026-09-08', oss_path: 'must-not-leak', owner_uid: 'owner' }
    } }
  ...
  return { calls, signed, runtime, run: async () => {
    if (options.realSignature) {
      ...
      Object.assign(headers, await buildServiceCommandRuntimeHeaders({
        token: 'test-token', method: 'POST', requestTarget: `/api/v1/service/product-documents/${uuid}/metadata`, requestId: 'request-1',
...
test('product metadata handler binds distinct deployments and returns a field whitelist', async () => {
  ...
  assert.equal(h.runtime[0].path, `/v1/codocs/service/product-documents/${uuid}/metadata`)
  assert.deepEqual(Object.keys(result.data).sort(), ['doc_type', 'title', 'updated_at', 'uuid'])
})
test('product metadata handler rejects before body or runtime at each trust boundary', async () => {
...
test('product metadata handler verifies real HMAC and rejects substituted signed context', async () => {
