...
function harness(options: { scopes?: string[], badSignature?: boolean, tenant?: string, app?: string, bodyPatch?: object, hash?: string, runtimeFailure?: boolean, realSignature?: boolean, tamper?: string } = {}) {
  ...
  const envelope = { operationId: 'operation-1', targetApp: 'codocs', operationCode: 'assets.codocs.product-document.read.v1', requiredCapability: 'codocs:product-document:read', idempotencyKey: 'key-1', commandSchemaVersion: 'assets.codocs.product-document.read.v1', commandSha256: options.hash ?? 'hash', command }
  runInNewContext(compiled, { exports, defineEventHandler: (fn: unknown) => fn, require: (name: string) => {
    if (name === 'h3') return { createError, setHeader: () => {}, getHeader: (_: unknown, key: string) => headers[key], getQuery: () => ({}), getRouterParam: () => uuid, getRequestURL: () => new URL('https://codocs.example/api/v1/service/assets-product-documents/' + uuid + '/metadata'), readBody: async () => {
      calls.push('body')
...
test('product metadata handler binds distinct deployments and returns a field whitelist', async () => {
  ...
  assert.equal(h.signed[0].sourceDeploymentCode, 'custom-assets')
  assert.equal(h.signed[0].targetDeploymentCode, 'custom-codocs')
  assert.equal(h.signed[0].tenantCode, 'TENANT')
  assert.equal(h.signed[0].sourceClientId, 'assets.runtime')
  ...
  assert.equal(h.runtime[0].path, `/v1/codocs/service/assets-product-documents/${uuid}/metadata`)
...
test('product metadata handler rejects before body or runtime at each trust boundary', async () => {
  for (const options of [{ scopes: ['codocs:*'] }, { tenant: 'OTHER' }, { app: 'assets' }]) {
