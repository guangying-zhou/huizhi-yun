...
const ISSUE_PROJECT_CODE = computed(() => String(route.query.project_code || '').trim() || 'huizhi-yun')

// --- 筛选状态 ---
const filterStatus = ref<string>('all')
const filterType = ref<string>('all')
const filterPriority = ref<string>('all')
const { search: searchQuery, debounced: debouncedSearchQuery } = useDebouncedSearch()
...
async function fetchIssues() {
  ...
  try {
    const res = await $fetch<IssueListResponse>('/api/issues', {
      query: {
        project_code: ISSUE_PROJECT_CODE.value,
...
watch([filterStatus, filterType, filterPriority, debouncedSearchQuery], () => {
  currentPage.value = 1
  fetchIssues()
})
...
async function viewIssue(issue: Issue) {
  try {
    const res = await $fetch<IssueDetailResponse>(`/api/issues/${issue.id}`, {
      query: { project_code: issue.project_code || ISSUE_PROJECT_CODE.value }
    })
...
async function updateStatus(issue: Issue, newStatus: string) {
  try {
    await $fetch(`/api/issues/${issue.id}`, {
      method: 'PATCH',
      body: { project_code: issue.project_code || ISSUE_PROJECT_CODE.value, status: newStatus }
    })
...
async function doResolve() {
  ...
  try {
    await $fetch(`/api/issues/${resolveTarget.value.id}`, {
      method: 'PATCH',
      body: {
        project_code: resolveTarget.value.project_code || ISSUE_PROJECT_CODE.value,
        status: 'resolved',
        resolution: resolveResolution.value || null
      }
    })
...
async function doDelete() {
  ...
  try {
    await $fetch(`/api/issues/${deleteTarget.value.id}`, {
      method: 'DELETE',
      query: { project_code: deleteTarget.value.project_code || ISSUE_PROJECT_CODE.value }
    })
...
async function addComment() {
  ...
  try {
    await $fetch(`/api/issues/${selectedIssue.value.id}/comments`, {
      method: 'POST',
      body: {
        project_code: selectedIssue.value.project_code || ISSUE_PROJECT_CODE.value,
    ...
    const res = await $fetch<IssueDetailResponse>(`/api/issues/${selectedIssue.value.id}`, {
      query: { project_code: selectedIssue.value.project_code || ISSUE_PROJECT_CODE.value }
    })
    selectedIssue.value = res.data
  } catch {
