  ...
  <UDashboardPanel grow>
    ...
    <div class="flex-1 overflow-auto p-2 sm:p-6">
      ...
      <div v-else-if="items.length === 0" class="flex flex-col items-center justify-center py-20 text-center">
        <UIcon name="i-lucide-book-open" class="w-16 h-16 text-muted mb-4" />
      ...
      <!-- Article Cards Grid -->
      <div v-else class="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-5">
        <div
          v-for="item in items"
          :key="item.id"
          class="group rounded-xl border border-default bg-default overflow-hidden hover:shadow-lg hover:border-primary/30 transition-all duration-200 cursor-pointer"
          @click="openDetail(item)"
        >
          <div v-if="item.cover_image" class="w-full aspect-5/2 relative overflow-hidden bg-gray-100 dark:bg-gray-800">
            <img
          ...
          <div
            v-else
            class="w-full aspect-5/2 bg-linear-to-br from-primary/10 to-primary/5 flex items-center justify-center"
          >
            <UIcon name="i-lucide-file-text" class="w-10 h-10 text-primary/30" />
          </div>
        ...
        <!-- Pagination -->
        <div v-if="totalPages > 1" class="col-span-full flex justify-center pt-6">
