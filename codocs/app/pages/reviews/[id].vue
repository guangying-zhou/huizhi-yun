<template>
  <UDashboardPanel grow>
              ...
              <div class="space-y-2">
                ...
                <template v-if="isReviewer && review.status === 'in_progress'">
                  <UButton
                    ...
                    :loading="isConflictLoading('review', review.id)"
                    @click="openReviewConflictExplanation('approve')"
                  >
                    冲突解释
                  </UButton>
                  <UButton
                ...
                <template v-if="isInitiator">
                  ...
                  <UButton
                    ...
                    icon="i-lucide-shield-alert"
                    :loading="isConflictLoading('review', review.id)"
                    @click="openReviewConflictExplanation('archive')"
                  >
                    冲突解释
                  </UButton>
    ...
    <CodocsInstanceConflictExplanationModal
...
const {
  ...
  isConflictLoading
} = useCodocsInstanceConflictExplanation()
...
function openReviewConflictExplanation(action: 'approve' | 'archive') {
  if (!review.value) return
  openConflictExplanation({
    targetType: 'review',
    id: review.value.id,
    action,
    label: `${review.value.document_title || '审阅记录'} / ${action === 'archive' ? '确认发布' : '审阅通过'}`
  })
}
