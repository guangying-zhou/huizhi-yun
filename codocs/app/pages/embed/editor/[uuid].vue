...
const isPublishedDocument = ref(false)
...
const toggleViewMode = () => {
  if (isPublishedDocument.value) return
...
const loadDocument = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      isPublishedDocument.value = docReadonly.value && (
        Boolean(res.data.publish_info)
        || ['company', 'knowledge', 'product'].includes(docType.value)
        || String(res.data.oss_path || '').startsWith('codocs/company/')
      )
      if (isPublishedDocument.value) viewMode.value = 'richtext'
    }
  } catch (err: unknown) {
      ...
      >
        <div class="mx-auto min-h-full w-full max-w-4xl px-4 py-6 sm:px-8 sm:py-8">
          <EditorDocLazyPreview
            ...
            :disable-selection="isPublishedDocument"
          />
        </div>
      </div>
