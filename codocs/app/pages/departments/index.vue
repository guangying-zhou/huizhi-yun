...
const loadDocumentPreview = async (doc: DocRecord | FolderRecord) => {
  ...
  try {
    const response = await $fetch<{ success: boolean, data: DocRecord | null }>(`/api/documents/${(doc as DocRecord).uuid}`, {
      query: {
        dept_code: deptCode.value,
        include_deleted: showRecycleBin.value ? '1' : undefined
      }
    })
...
const showOpenFolderConfirm = ref(false)
const openFolderTarget = ref<{ folderId: number, isOpen: boolean, name: string, deptCode: string } | null>(null)
const isUpdatingFolderOpen = ref(false)
...
const toggleFolderOpen = (folderId: number, isOpen: boolean, name: string) => {
  if (!ensureDepartmentManager('仅部门负责人可设置开放目录')) return

  openFolderTarget.value = { folderId, isOpen, name, deptCode: deptCode.value }
  showOpenFolderConfirm.value = true
}

const cancelToggleFolderOpen = () => {
  showOpenFolderConfirm.value = false
  openFolderTarget.value = null
}

const executeToggleFolderOpen = async () => {
  if (!openFolderTarget.value) return

  const target = openFolderTarget.value
  isUpdatingFolderOpen.value = true
  try {
    await $fetch(`/api/folders/${target.folderId}/open`, {
      method: 'PATCH',
      body: { is_open: !target.isOpen, dept_code: target.deptCode }
    })
    toast.add({
      title: target.isOpen ? '已取消开放目录' : '已设为开放目录',
      description: `「${target.name}」${target.isOpen ? '仅部门内可见' : '登录用户均可查看'}`,
      color: 'success'
    })
    await refreshFolders()
    cancelToggleFolderOpen()
  } catch (error: unknown) {
    const err = error as { data?: { message?: string }, message?: string }
    toast.add({
      title: '设置失败',
      description: err.data?.message || err.message || '无法更新目录开放状态',
      color: 'error'
    })
  } finally {
    isUpdatingFolderOpen.value = false
  }
}
...
const navigateToEdit = (uuid: string) => {
  ...
  navigateTo({
    path: `/documents/${uuid}`,
    query: deptCode.value ? { dept_code: deptCode.value } : undefined
  })
}
...
<template>
  <UDashboardPanel grow>
          ...
          <ClientOnly>
            <div v-if="!showRecycleBin">
              ...
              <FileTreeItem
                ...
                :can-manage-open="isDepartmentManager"
    ...
    <!-- Open Folder Confirmation Modal -->
    <UModal v-model:open="showOpenFolderConfirm">
      <template #content>
        <UCard>
          <template #header>
            <div class="flex items-center gap-2">
              <UIcon
                :name="openFolderTarget?.isOpen ? 'i-lucide-lock' : 'i-lucide-globe'"
                class="w-5 h-5 text-primary"
              />
              <h3 class="text-lg font-semibold">
                {{ openFolderTarget?.isOpen ? '取消开放目录' : '设为开放目录' }}
              </h3>
            </div>
          </template>

          <p class="text-muted">
            确定要{{ openFolderTarget?.isOpen ? '取消开放' : '开放' }}目录
            <strong class="text-default">"{{ openFolderTarget?.name }}"</strong> 吗？
          </p>
          <p v-if="!openFolderTarget?.isOpen" class="text-sm text-warning mt-2">
            开放后，该目录及其子目录中的文档将对所有登录用户只读可见。
          </p>
          <p v-else class="text-sm text-muted mt-2">
            取消后，登录用户将不能通过“部门开放文档”查看该目录下的内容，部门原有权限不变。
          </p>

          <template #footer>
            <div class="flex justify-end gap-2">
              <UButton color="neutral" variant="outline" @click="cancelToggleFolderOpen">
                取消
              </UButton>
              <UButton
                :color="openFolderTarget?.isOpen ? 'warning' : 'primary'"
                :loading="isUpdatingFolderOpen"
                @click="executeToggleFolderOpen"
              >
                {{ openFolderTarget?.isOpen ? '取消开放' : '设为开放目录' }}
              </UButton>
            </div>
          </template>
        </UCard>
