...
export interface UseCollaborationOptions {
  ...
  /** 部门文档查看上下文 */
  deptCode?: MaybeRefOrGetter<string | null | undefined>
  /** WebSocket 服务器地址 */
...
export function useCollaboration(options: UseCollaborationOptions) {
  ...
  const resolveToken = async (currentDocumentName: string) => {
    ...
    const currentDeptCode = stringValue(toValue(deptCode))
    if (currentDeptCode) {
      query.dept_code = currentDeptCode
    }

    const response = await $fetch<{ success: boolean, data?: { token?: string } }>('/api/collaboration/token', { query })
