...
type SankeyMeta = Record<string, unknown>

interface CustomNode extends SankeyNode<SankeyMeta, SankeyMeta> {
  id: string
  name: string
  category: string
  // D3 adds these
...
interface CustomLink extends SankeyLink<CustomNode, SankeyMeta> {
...
const renderChart = () => {
  ...
  // Clear previous
  d3.select(container.value as unknown as Element).selectAll('*').remove()
  ...
  const svg = d3.select(container.value as unknown as Element)
    .append('svg')
