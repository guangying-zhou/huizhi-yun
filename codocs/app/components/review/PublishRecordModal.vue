<template>
  <UModal v-model:open="isOpen" :title="record?.quick_publish_record ? '直接发布记录' : '发布审批与执行过程'" :ui="{ content: 'sm:max-w-xl' }">
    <template #body>
      <div class="p-4">
        ...
        <div v-else class="space-y-4">
          ...
          <!-- 审批流程 -->
          <p v-if="record.quick_publish_record" class="text-sm text-muted">
            管理员直接发布，未经过审批，未发送企业微信通知。
          </p>
          <div v-else>
