...
const itemIcon = computed(() => {
  if (!isFolder.value) return 'i-lucide-file-text'
  return props.showOpenIndicator && isOpenFolder.value ? 'i-lucide-folder-open' : 'i-lucide-folder'
})
...
const handleToggleOpen = () => {
  if (!isFolder.value || !props.canManageOpen) return
  emit('toggleOpen', props.item.data.id as number, isOpenFolder.value, props.item.name)
}
</script>
...
<template>
  <div>
    ...
    >
      ...
      <UIcon
        v-if="showOpenIndicator && isFolder && isOpenFolder"
        name="i-lucide-globe"
      ...
      <div
        v-if="!isEditing && (canDelete || (isFolder && canManageOpen))"
        class="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0"
      >
        <button
          v-if="isFolder && canManageOpen"
          class="p-0.5 rounded hover:bg-muted text-muted hover:text-primary"
          :title="isOpenFolder ? '收回为部门内可见' : '设为开放目录'"
          @click.stop="handleToggleOpen"
        >
          <UIcon :name="toggleOpenIcon" class="w-3.5 h-3.5" />
        </button>
        <button
          v-if="canDelete"
    ...
    <div v-if="isFolder && isExpanded && hasChildren">
      <FileTreeItem
        ...
        :can-manage-open="canManageOpen"
        :show-open-indicator="showOpenIndicator"
