...
import type { Mail } from '~/types/index-back'

const props = defineProps<{
  mails: Mail[]
}>()

const mailsRefs = ref<Array<Element | undefined>>([])

const setMailRef = (mailId: number) => (element: Element | ComponentPublicInstance | null) => {
  if (element instanceof Element) {
    mailsRefs.value[mailId] = element
  } else {
    mailsRefs.value[mailId] = undefined
  }
}
...
<template>
  <div class="overflow-y-auto divide-y divide-default">
    <div v-for="(mail, index) in mails" :key="index" :ref="setMailRef(mail.id)">
      <div
