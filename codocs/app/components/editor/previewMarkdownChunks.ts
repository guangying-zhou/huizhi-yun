...
const splitIntoMarkdownBlocks = (full: string) => {
  ...
  while (cursor < lines.length) {
    ...
    // split/join removes the newline at each slice boundary; retain it so
    // blank lines keep paragraphs and adjacent tables as separate blocks.
...
export const buildPreviewChunks = (full: string) => {
  // Table layout metadata addresses tables by their index in the whole document.
  // Keep it with the body so the initial render restores merges and column widths.
  if (/<!--\s*HZY_TABLE_MERGE:/.test(full)) return [normalizeMarkdown(full)]
