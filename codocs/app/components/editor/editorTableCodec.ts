...
const collectLogicalTableColumnLayout = (tableNode: ProsemirrorNode) => {
  const map = TableMap.get(tableNode)
  const colwidths: (number | null)[] = Array.from({ length: map.width }, () => null)
  const alignments: string[] = Array.from({ length: map.width }, () => 'left')
  const seen = new Set<number>()
  ...
  for (const relativeCellPos of map.map) {
    ...
    const cell = tableNode.nodeAt(relativeCellPos)
    if (!cell) continue
    const rect = map.findCell(relativeCellPos)
    ...
    for (let column = rect.left; column < rect.right; column++) {
      const width = cellWidths[column - rect.left]
...
export const collectTableLayoutMetaFromDoc = (view: EditorView): TableLayoutMeta[] => {
  ...
  view.state.doc.descendants((node) => {
    ...
    const { colwidths, alignments } = collectLogicalTableColumnLayout(node)
...
const applyColumnWidthsToTable = ({
  ...
  for (const relativeCellPos of map.map) {
    ...
    const cell = node.nodeAt(relativeCellPos)
    if (!cell) continue
    const rect = map.findCell(relativeCellPos)
    if (rect.right > widths.length) continue

    const targetWidths = widths.slice(rect.left, rect.right)
    const targetAttr = targetWidths.some(width => width != null) ? targetWidths : null
    const currentAttr = Array.isArray(cell.attrs.colwidth) && cell.attrs.colwidth.length > 0
      ? cell.attrs.colwidth
      : null
    const differs = currentAttr === null
      ? targetAttr !== null
      : targetAttr === null
        || currentAttr.length !== targetAttr.length
        || currentAttr.some((width: number | null, index: number) => width !== targetAttr[index])

    if (differs) {
      tr.setNodeMarkup(pos + 1 + relativeCellPos, null, { ...cell.attrs, colwidth: targetAttr })
      changed = true
    }
  }
...
export const rescaleTableColwidthsToContainer = (view: EditorView) => {
  ...
  view.state.doc.descendants((node, pos) => {
    if (node.type.name !== 'table') return
    const { colwidths } = collectLogicalTableColumnLayout(node)

    const MIN_W = 40
    const hasExplicit = colwidths.some(width => width != null && width > 0)
...
export const applyColwidthFromMarkdown = ({
  ...
  view.state.doc.descendants((node, pos) => {
    ...
    const logicalColumnCount = TableMap.get(node).width
    // 以显式 colwidth 元数据优先；dashes 次之；再其次对既有部分 colwidth 做等比缩放。
    // 这里不再按"内容长度"回退，避免协同编辑时因数据抖动退化为等宽。
    if (explicitColwidths.length === logicalColumnCount && explicitColwidths.some(width => width !== null && width > 0)) {
      const normalizedWidths = normalizeColumnWidthsToTarget(explicitColwidths)
      const dashWidths = dashCounts.length === logicalColumnCount
    ...
    if (dashCounts.length === 0) {
      const { colwidths: existingColwidths } = collectLogicalTableColumnLayout(node)
      const existingFirstWidths = existingColwidths.slice(0, -1).map(width => width && width > 0 ? width : 0)
      const hasAnyColwidth = existingColwidths.some(width => width != null && width > 0)
