...
import EditorTableQuickActions from './EditorTableQuickActions.vue'
import {
...
import { type EditorTableQuickAction, useEditorTableUi } from './useEditorTableUi'
import { useEditorTableQuickActions } from './useEditorTableQuickActions'
...
function selectionTouchesPublishedContent() {
  if (!props.readonly || !props.disableSelection || !editorRef.value) return false
  const active = document.activeElement
  // The link dialog and other page inputs must remain selectable/copyable.
  if (active instanceof HTMLElement && !editorRef.value.contains(active)
    && (active.matches('input, textarea') || active.isContentEditable)) return false
  const selection = window.getSelection()
  if (!selection || selection.isCollapsed) return false
  for (let index = 0; index < selection.rangeCount; index++) {
    if (selection.getRangeAt(index).intersectsNode(editorRef.value)) return true
  }
  return false
}

function clearPublishedSelection() {
  if (selectionTouchesPublishedContent()) window.getSelection()?.removeAllRanges()
}

function preventPublishedCopy(event: ClipboardEvent) {
  // Ctrl/Cmd+A can originate on body and include CodeMirror's editable DOM.
  const insidePublishedEditor = props.readonly && props.disableSelection
    && event.target instanceof Node && editorRef.value?.contains(event.target)
  if (insidePublishedEditor || selectionTouchesPublishedContent()) {
...
onMounted(() => {
  document.addEventListener('selectionchange', clearPublishedSelection)
  document.addEventListener('copy', preventPublishedCopy, true)
})
onBeforeUnmount(() => {
  document.removeEventListener('selectionchange', clearPublishedSelection)
  document.removeEventListener('copy', preventPublishedCopy, true)
})
watch(() => props.readonly && props.disableSelection, async (restricted) => {
  if (restricted) {
    await nextTick()
...
const sourceTextareaRef = shallowRef<HTMLTextAreaElement | null>(null)
...
const mermaidSvgCache = createMermaidSvgCache({
  ...
  getVariant: () => JSON.stringify({
    ...
    mermaidRuntime: MERMAID_RUNTIME_VARIANT,
...
const {
  ...
  getSelection: () => {
    ...
    return {
      ...
      from: selection.from,
      to: selection.to,
      isText: selection instanceof TextSelection
    }
  },
  ...
  getSelectionCoords: (selection) => {
    const view = getEditorViewSafe()
    return view?.coordsAtPos(selection.to) || { top: 0, left: 0 }
  },
  getAiButtonRect: () => {
    const toolbar = editorRef.value?.querySelector('.milkdown-toolbar')
    const allItems = toolbar?.querySelectorAll('.toolbar-item')
    const aiButton = allItems?.[allItems.length - 1]
    return aiButton?.getBoundingClientRect() || null
  },
  replaceMarkdown: ({ from, to, markdown }) => {
    const view = getEditorViewSafe()
    if (!view || !crepe) return false
  ...
  getCurrentUserId: () => String(authUser.value || '').trim(),
  writeClipboard: async ({ uid, content }) => {
    await $fetch('/api/account/clipboard', {
      method: 'POST',
      body: { uid, content, contentType: 'markdown', sourceApp: 'codocs' }
    })
  },
  readClipboard: async (uid) => {
    const response = await $fetch<{ code: number, data: { content: string, contentType: string } | null }>('/api/account/clipboard', {
      params: { uid }
    })
...
const {
  ...
  onPluginViewUpdate: (view) => {
    if (aiMenuVisible.value && view.state.selection.empty) {
      closeAiMenu()
    }
  }
})
...
const {
  ...
  runTableQuickAction
} = useEditorTableQuickActions({
  readonly: computed(() => props.readonly),
  isTableColumnResizing,
  isEditorDestroying,
  isEditorCreated,
  getEditorView: getEditorViewSafe,
  getActiveCell: getActiveTableQuickActionCell,
  runLater: runEditorTimeout,
  rescaleToContainer: rescaleTableColwidthsToContainer,
  updatePosition: updateTableQuickActionsPosition,
  warn: title => toast.add({ title, color: 'warning' })
})

const handleTableQuickAction = (action: EditorTableQuickAction) => {
  switch (action) {
    case 'row-before':
      runTableQuickAction(addRowBefore)
      return
    case 'row-delete':
      runTableQuickAction(deleteRow)
      return
    case 'row-after':
      runTableQuickAction(addRowAfter)
      return
    case 'column-before':
      runTableQuickAction(addColumnBefore)
      return
    case 'column-delete':
      runTableQuickAction(deleteColumn)
      return
    case 'column-after':
      runTableQuickAction(addColumnAfter)
      return
    case 'header-toggle':
      runTableQuickAction(toggleHeaderRow)
      return
    case 'table-delete':
      runTableQuickAction(deleteTable)
      return
    case 'merge':
      runTableQuickAction(mergeCellsCombineContent, {
        preserveSelection: true,
        failHint: '请先框选多个单元格'
      })
      return
    case 'split':
      runTableQuickAction(splitCell, {
        preserveSelection: true,
        failHint: '当前单元格不可拆分'
      })
  }
...
const initEditor = async () => {
  ...
  crepe = new Crepe({
    ...
    // Render only the body; retain modelValue for table layout restoration below.
    defaultValue: isCollaborationActive.value ? '' : parseTableMergeMetaFromMarkdown(props.modelValue || '').cleanMarkdown,
    features: {
...
<template>
  <div class="crepe-container" :class="{ 'with-sidebar': showSidebar, 'crepe-selection-disabled': readonly && disableSelection }" :style="{ height: containerHeight }">
    <div class="crepe-wrapper group relative">
      ...
      <div
        ...
        @selectstart.capture="readonly && disableSelection && $event.preventDefault()"
      ...
      <EditorTableQuickActions
        :state="tableQuickActions"
        :readonly="readonly"
        :view-mode="viewMode"
        :can-merge="canRunTableQuickAction(mergeCellsCombineContent, { preserveSelection: true })"
        :can-split="canRunTableQuickAction(splitCell, { preserveSelection: true })"
        @action="handleTableQuickAction"
      />
...
/* Published content can scroll and follow links without selecting its text. */
.crepe-container.crepe-selection-disabled .crepe-editor.crepe-readonly {
  pointer-events: auto;
}
...
.crepe-container.crepe-selection-disabled .cm-selectionBackground,
.crepe-container.crepe-selection-disabled .selectedCell::after {
  display: none !important;
}
