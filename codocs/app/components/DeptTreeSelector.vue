...
const isExpanded = ref(false) // 默认折叠，避免混淆
const isSelected = computed(() => {
  const selected = props.selectedDeptCode === props.node.deptCode
  // 调试：如果选中，打印信息
  if (selected && import.meta.dev) {
    console.log('[DeptTreeSelector] Selected:', props.node.name, props.node.deptCode)
  }
  return selected
})
...
// 如果当前节点被选中，也展开（以便看到自己）
watch(isSelected, (selected) => {
  if (selected && hasChildren.value) {
    isExpanded.value = true
  }
}, { immediate: true })
