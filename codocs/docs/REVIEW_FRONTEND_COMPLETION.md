...
> **历史文档（已废弃）**：本文记录 2026-03 的旧版 Codocs 本地审批实现。`/admin/publish`、`/admin/templates`、`/api/reviews/templates` 和 `review_flow_templates` 已移除；当前发文流程统一由 Workflow 配置和驱动，请以 `CODOCS_API_SPEC.md` 与 `Codocs-Workflow-Publish-Approval-Plan.md` 为准。
...
- `downloadDocument(ossPath, docType, { documentUuid })` - 通过后端 API 下载普通文档内容，后端按 UUID 读取 tenant-runtime 元数据并校验 OSS 路径
- `downloadDocument(ossPath, 'git-project', { projectCode })` - 通过项目编码校验 Git 项目文档 OSS 前缀后下载
- 用于审阅详情页面加载文档内容
...
- `POST /api/documents/download-content` - 用于 OSS 文档内容下载（供 `utils/oss-client.ts` 使用）；普通文档需具备 `documents:export` 并传 `document_uuid`，Git 项目文档需具备 `projects:export` 并传 `project_code`
...
2. **补充后端 API**
   - 实现 `/api/documents/download-content` 接口；普通文档要求 `documents:export + document_uuid`，Git 项目文档要求 `projects:export + project_code`
   - 确保支持不同 bucket 的文档下载，同时禁止仅凭 OSS path 读取正文
