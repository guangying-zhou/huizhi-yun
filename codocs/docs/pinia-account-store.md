...
> `account` 是保留的前端 store 与 BFF 路径名称。当前目录读取经本应用 `/api/account/**` BFF 和 Foundation Console Directory adapter 完成；客户端不持有 Account API key，也不直接调用 legacy Account。
...
本地开发如需覆盖 Console 地址，只配置运行时地址；生产由 Console runtime 解析。不要配置静态 Account API 凭据：
