...
状态：历史 / 目标设计参考（需继续实施）
原始日期：2026-05-24
定位：Codocs 从自有服务器运行形态迁向 Cloudflare 托管形态的历史规划与后续目标路线；不替代当前 Worker、tenant-runtime 或部署运行手册。

> **文档状态与证据边界**
>
> - **状态 / 参考范围：** 历史 / 目标设计参考；描述 Codocs、Collab 与对象存储的目标迁移路线，不替代当前运行配置或操作手册。
> - **最后核验：** 2026-07-11。
> - **当前权威来源：** [`codocs/deploy/cloudflare/README.md`](../deploy/cloudflare/README.md)、[`codocs/CLAUDE.md`](../CLAUDE.md)、[`docs/Tenant-Runtime-API-Contract-v1.md`](../../docs/Tenant-Runtime-API-Contract-v1.md)、[`deploy/cloudflare/tenant-gateway/README.md`](../../deploy/cloudflare/tenant-gateway/README.md)、[`docs/Huizhi-Yun-Cloudflare部署指南.md`](../../docs/Huizhi-Yun-Cloudflare部署指南.md)、[`docs/MODULE_CONTRACTS.md`](../../docs/MODULE_CONTRACTS.md) 与当前 `.env.cloudflare.example` / `wrangler` 配置。
...
- `collab/docs/Collab-Runtime-Architecture.md`
...
Codocs Worker
  -> Console OIDC / JWKS
  -> Tenant Gateway 注入的 tenant-runtime / data-runtime Agent -> customer hzy_codocs
...
业务应用当前的 Cloudflare 标准形态是：

```text
Nuxt Worker + Console OIDC + Tenant Gateway + managed-cloud Agent
```

`managed-cloud-direct-db` 只允许作为明确记录的历史例外，不能作为 Codocs 目标形态；Codocs Worker 固定 `managed-cloud-agent`，不得配置 `DB_*`、Hyperdrive 或直连 MySQL。
...
| 依赖 | Cloudflare 形态 |
| --- | --- |
| 元数据 / MySQL | Tenant Gateway 注入的 tenant-runtime / data-runtime Agent -> `hzy_codocs`；Worker 不配置 DB/Hyperdrive |
...
- 保留 `/codocs/` 当前路由。
- 离线核对 Codocs Worker 的 `managed-cloud-agent` 生成配置不含 `DB_*`、Hyperdrive 或直连 MySQL。
- 将 Console OIDC callback/logout、Agent enrollment 与 `oss.default` 的真实可用性列为后续获批环境验收，而非由规划文档宣告完成。

退出条件：

- `finance`、`altoc` Cloudflare 模式稳定。
- 已获批准的环境验收确认 Console OIDC、Gateway 注入的 runtime Agent 和对象存储集成可用。
- Codocs 未被本轮变更影响。
...
- 按当前 `docs/MODULE_CONTRACTS.md`、`docs/Tenant-Runtime-API-Contract-v1.md` 与 `codocs/deploy/cloudflare/README.md` 保持 Console OIDC、Gateway 和 tenant-runtime 合同。
...
- 获批 live acceptance 中，`https://<tenant>/codocs/` 能加载应用壳并完成 OIDC 登录回调。
- 获批 live acceptance 中，文档列表、详情、创建、删除、权限检查均经 tenant-runtime 可用，且不回退直连 MySQL。
- 未启用协作时，普通文档读写可用。
...
- 仓库包含 Collab Durable Object POC、collaboration token 校验、Yjs sync、awareness 广播和最小 snapshot 代码，以及 Tenant Gateway 的 `/codocs/ws` WebSocket 路由配置。
- 是否已部署 `hzy-collab-codocs-poc`、`/collab/health` 是否可用、以及 `wss://<tenant>/codocs/ws` 是否可完成认证握手，均只能由获批 live acceptance 证明；当前不将其标为完成事实。
...
- 在明确环境授权后，做双浏览器真实编辑、断线重连、刷新恢复和 readonly 写入拒绝测试。
- 由 Console/Platform runtime config 控制 `hocuspocus` 与 `cloudflare-durable-object` 灰度切换。

退出条件：

- 获批环境中，两个浏览器同时编辑测试文档可同步，刷新/空闲后可从 snapshot 恢复。
- 获批环境中，readonly token 被拒绝且受控 Worker tail/log 可定位 token、room、flush 错误。
...
- Platform Dashboard 支持：
  - Codocs runtime Agent enrollment / 健康状态
  - 对象存储连接测试
  - Collab provider 选择
...
- 租户新开通时自动创建：
  ...
  - 受管理的 tenant-runtime / data-runtime Agent enrollment（而非业务 Worker 直连数据库 binding）
...
### 6.1 MySQL 与 tenant-runtime

Codocs 业务数据库仍为客户侧 `hzy_codocs`，但所有元数据数据库读写由 tenant-runtime / data-runtime Agent 承担；Codocs Worker 不配置 Hyperdrive、`DB_*` 或直连 MySQL。
...
- TLS。
- schema 版本检查。
- Agent 安装/enrollment、schema/status/health 验收及任何大批量迁移均须单独获得环境授权，且不得在 Worker request 中执行。
...
以下命令、双浏览器协作和 Worker tail 都是会接触真实环境的验收步骤。离线代码检查不能替代它们，且必须先获得明确的目标环境、租户和执行授权。
...
- 创建文档经签名 runtime 请求写入 `hzy_codocs`，Worker 不回退直连数据库。
- Markdown 内容写入对象存储。
...
| runtime Agent 延迟或不可达 | 文档列表慢或失败 | Agent 健康/部署绑定验收、明确超时错误、分页与只读缓存；不回退 DB/Hyperdrive |
...
6. Codocs 按 `MODULE_CONTRACTS`、Tenant-Runtime API Contract 和 Cloudflare 部署骨架核对 Console OIDC、Gateway 与 Agent 合同。
7. 获得明确环境授权后，做一次端到端灰度：登录 -> 文档列表 -> 打开测试文档 -> 双人协作 -> 保存 -> 刷新恢复。
