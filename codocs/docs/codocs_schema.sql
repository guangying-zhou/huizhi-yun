...
CREATE TABLE `folders` (
    ...
    `is_open` TINYINT NOT NULL DEFAULT 0 COMMENT '是否开放目录: 0-部门内可见 1-登录用户可见',
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',

    INDEX `idx_folders_owner` (`owner_uid`),
    INDEX `idx_folders_department` (`dept_code`),
    INDEX `idx_folders_department_open` (`folder_type`, `is_open`, `dept_code`),
...
CREATE TABLE `document_relations` (
    ...
    `related_uid` VARCHAR(64) NOT NULL COMMENT '关联主体；用户UID或稳定service principal',
...
CREATE TABLE `document_versions` (
    ...
    `content_size` INT UNSIGNED DEFAULT 0 COMMENT '内容大小(字节)',
    `content_sha256` CHAR(64) NULL COMMENT '确定版本 Markdown 内容 SHA-256；历史版本补算前可为空',
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
...
DROP TABLE IF EXISTS `document_review_grants`;
CREATE TABLE `document_review_grants` (
    ...
    CONSTRAINT `fk_document_review_grant_document` FOREIGN KEY (`document_id`) REFERENCES `documents` (`id`) ON DELETE RESTRICT,
    CONSTRAINT `fk_document_review_grant_version` FOREIGN KEY (`document_version_id`) REFERENCES `document_versions` (`id`) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Aims 确定文档版本只读评审授权';
...
CREATE TABLE IF NOT EXISTS `document_publish_requests` (
  ...
  `archive_oss_path` VARCHAR(500) NULL COMMENT '发布后文档OSS路径',
  `execution_status` VARCHAR(30) NULL COMMENT '发布后执行状态：pending_seal/pending_send/pending_receive/received',
  `published_document_uuid` VARCHAR(36) NULL COMMENT '发布版本文档UUID',
  `sealed_at` DATETIME NULL COMMENT '确认盖章时间',
  `sent_at` DATETIME NULL COMMENT '确认发送时间',
  `received_at` DATETIME NULL COMMENT '确认接收时间',
  ...
  INDEX `idx_publish_execution_queue` (`review_type`, `execution_status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='文档发布申请表';

-- 3. 审阅操作记录表（仅保留历史只读兼容，新发文不再写入）
...
CREATE TABLE IF NOT EXISTS `document_send_records` (
  ...
  `channel` VARCHAR(20) NOT NULL COMMENT '发送途径：email/wecom/wechat_qq/web_upload/sf_express/other_courier/other_method/usb',
...
CREATE TABLE `cabinet_files` (
    ...
    `project_code` VARCHAR(100) NULL COMMENT '所属项目编码(NULL表示非项目文件柜)',
    ...
    INDEX `idx_cabinet_project` (`project_code`),
