...
> **历史文档（已废弃）**：本文的本地流程模板、数据库与 Cookie 请求示例不构成当前上线步骤。`/admin/publish`、`/api/reviews/templates` 和 `review_flow_templates` 已移除，发文流程统一在 Workflow 管理。当前认证、目录与服务调用以 `CODOCS_API_SPEC.md`、`MODULE_CONTRACTS.md` 和 Console runtime 为准；不得从本文复制或配置静态 Account API key、企业微信 secret、数据库口令或其他真实凭据。
...
```bash
# 1. 使用经批准的运维身份连接目标数据库（不要在命令或文档中写入口令）
mysql -h <tenant-db-host> -u <operator> -p <database>
...
```env
# 1. 企业微信等外部集成凭据由 Console integration-config + credential-vault 管理，
#    不写入 Codocs 本地 .env。

# 2. 本地开发可按运行时配置提供 Console URL；不配置 Account API key/secret。
