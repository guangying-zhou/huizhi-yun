...
> **历史文档（已废弃）**：其中本地流程模板、数据库操作与旧 Cookie 测试示例不构成当前部署流程。`/admin/publish`、`/api/reviews/templates` 和 `review_flow_templates` 已移除，发文流程统一在 Workflow 管理。当前认证、目录与服务调用以 `CODOCS_API_SPEC.md`、`MODULE_CONTRACTS.md` 和 Console runtime 为准；不得从本文复制或配置静态 Account API key、企业微信 secret 或其他真实凭据。

## 当前 Workflow 发文执行链路部署

1. 在租户 `hzy_codocs` 数据库执行 `docs/migrations/20260717_workflow_publish_execution_contract.sql`。
2. 发布包含 Codocs BFF 与 data-runtime Codocs adapter 的同一版本，确认租户已启用 `HZY_CODOCS_AGENT_ENABLED`。
3. 使用 Workflow 已审批通过的发布申请依次验收归档；对外发文按配置继续验收盖章、发送、接收。四个 BFF 入口不得再返回遗留合同 503。
4. `reviews:archive` 授予发布发起人，`reviews:admin` 授予盖章执行管理员；指定发送人只需 `reviews:view` 即可确认接收，但 runtime 仍会核对发送记录中的 UID。

下文仅保留历史背景，不可作为当前操作步骤。
...
```env
# 当前集成凭据由 Console integration-config + credential-vault 管理；
# Codocs 不在本地 .env 保存企业微信或 Account API 凭据。
# 本地仅在显式开发环境按运行时配置提供 Console URL。
...
- 检查 Console Directory 与当前用户会话是否正常
- 检查 Foundation/Console runtime 能否取得当前请求所需的授权上下文
