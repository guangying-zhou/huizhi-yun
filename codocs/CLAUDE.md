...
当前目标（2026-09-21）：Codocs 保留文档领域职责，普通页面及自有完整文档工作区以 Enterprise 原生组合为目标；独立前端/iframe 只为未迁消费者提供兼容，Collab 等必要后台能力可独立运行。Host 已注册原文档页候选，本机可显示正文与侧栏；版本 403、协作通道与写入恢复尚待验收，不能据此停止旧部署。下段 2026-09-19 记录为当时范围和实施状态，不代表当前尚未注册页面。

2026-09-19：项目文档链当前未完成浏览器全链验收，现阶段暂停后续迁移。Codocs 整合方向为除后置演示文稿外的 `mydocs` 页面树及其可达现有操作整体先接入 Enterprise（含文件柜、最近使用、共享/移交/发文、收藏、回收站）；演示文稿页及专用编辑/预览/导出/API 按用户最新决定后续迁移，本轮只记录；既有通用 slide 兼容和原数据保留，文件柜 PPT/PPTX 能力不后置。工作日志/个人周报主入口迁至工作台→我的工作→工作汇报，`mydocs` 仅保留指向同一页面的兼容入口，不做两套实现。编辑器与 Collab 继续复用现有实现；团队汇报后续部门批次，项目周报后续项目管理，旧 Codocs 项目文档页面后置暂不迁移且数据保留。数据库与 OSS 暂留现有实现；本条仅记录计划，尚未注册实现。此前固定 Aims 来源的签名 `project-document-access/execute`、目标 manifest 的 read/manage/create 及文件柜来源/目标 deployment 分开校验等既有实现保持不变；见根 MODULE_CONTRACTS 与测试部署记录。
...
- 预览启动正文缓存已按 module cache scope + user/tenant 隔离；会话变化同步清理且旧 composable 实例永久失效，迟到请求不能向新会话写入缓存。Host 部署隔离由 enterprise-cache-scope 提供，不另猜部署参数；新实例可继续使用原 get/set/consume/clear API。此缓存仅是预览优化，不代替 Runtime 文档 ACL。

- 个人柜转文档 Host 候选已补（更新下列早期“转换未完成”状态）：只读计划、请求级 OSS 条件存储、重新授权、Runtime Serializable 事务创建文档及 owner relation 并关联源文件。失败保留幂等 key；重放不覆盖后续编辑或改变后来的转存关联。目录选择已接真实分页；真实环境与 Host 页面组合仍未完成。

- 个人柜 Host multipart 上传候选已接 plan→条件 OSS PUT→commit；精确 create，提交前重新授权，Runtime 目录/UUID 行锁与唯一键保证同请求重放不重复、不复活删除文件。客户端逐文件、失败保留字节/顺序/会话绑定 key；单文件仍 100 MiB。转文档及环境授权/真实联调未完成。

- 个人柜 Host DELETE 已接固定 `personal-cabinet:delete`：documents:delete 权限、当前 owner/非部门项目 SQL 锁与幂等回执同事务；仅软删除，保留 OSS 和已转换文档。转换写链仍未闭环。Codocs `test` 使用测试专用 TS 路径解析 hook，保持生产导入规则不变。
...
- Milkdown 编辑器依赖重，按文档路由/打开动作加载，继续留在 Codocs 领域组件中，不放入 Foundation 层；Enterprise 的自有完整文档工作区以原生组合为目标。iframe 只作旧页面/明确隔离能力的受控过渡，不是长期默认方式。Host 协作 token/WS、保存恢复和撤权未验收前，不把共享编辑当作已完成。
