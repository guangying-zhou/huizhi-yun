       ...
       └─ Foundation server util:
          ...
          └─ POST {webdev service base}/api/webdev/issues/intake   （带 service JWT）
                └─ WebDev 校验 Console JWKS / aud=webdev / token_use=service / scope=webdev:issue:write
                   ...
                   └─ dataRuntimeFetch → POST /v1/webdev/service/issues/intake   （受限 service bridge，写 hzy_webdev.webdev_issues）
...
[WebDev 控制台 issues.vue]
  └─ GET/PATCH /api/webdev/issues[/...]   （Console OIDC 用户会话，webdev-auth 中间件）
       └─ dataRuntimeFetch → /v1/webdev/issues[...]   （request-target HMAC actor delegation + Data Runtime 鉴权 + tenant 强制过滤）
```
...
托管 tenant-runtime 部署中，`dataRuntimeFetch` 统一复用 Foundation
`maybeCallTenantRuntime()`，以 `webdev.runtime` 身份向 Console 申请短期
`token_use=service` JWT。读请求使用 `webdev.read` 语义 scope，写请求使用
`webdev.write`，由 Foundation 按实际 audience 规范化为
`data-runtime:webdev:read|write` 或 `tenant-runtime:webdev:read|write`；
`service-client-policy` 将令牌绑定到目标租户的 `<tenant>-webdev` deployment。

`HZY_WEBDEV_DATA_RUNTIME_TOKEN` 仅保留给显式非 tenant-runtime 的本地/PoC
兼容部署。托管部署不会在短期令牌签发或运行时鉴权失败后退回 static token。
...
已加入 `internal/apps/webdev/adapter.go` 的 `HandleRuntime` switch。鉴权目标态使用 Console service token，scope 采用 `webdev:read` / `webdev:write`；迁移期兼容现有 Data Runtime legacy scope `webdev.read` / `webdev.write`。
...
- `server.go` 在鉴权后重建 `current_user`、运行时 tenant/deployment/source context；普通 `/v1/webdev/**` 请求必须携带绑定完整 request target 的 HMAC actor delegation。`createdBy`、manual Issue `reporterUid/reporterName`、Issue event `actor` 与 body/query 的 tenant 均由该受信上下文覆盖。
- 仅 intake/mine 与其固定 auto-claim 内部步骤是显式 actorless service bridge：`POST /v1/webdev/service/issues/intake`、`GET /v1/webdev/service/issues/mine`、`GET /v1/webdev/service/issues/settings`、`GET/PATCH /v1/webdev/service/issues/{id}`、`POST /v1/webdev/service/issues/{id}/claim`、`POST /v1/webdev/service/jobs`。它们只能由已验证 WebDev service endpoint 将上游 service-token 的来源/租户/会话派生投影后调用，不得把 WebDev runtime service client subject 当作用户 actor，也不得扩展为通用 `/v1/webdev/service/**` allowlist。
...
| POST | `/v1/webdev/issues` | Console 用户手动创建；签名 actor 写入 reporter/actor，tenant/server fields 服务端写入 | `webdev.issues.create` |
...
| POST | `/v1/webdev/service/issues/intake` | 仅已验证 WebDev service intake 的跨应用上报投影 | `webdev.service_issues.intake` |
| GET | `/v1/webdev/service/issues/mine` | 仅已验证 WebDev service bridge 的当前上报者列表投影 | `webdev.service_issues.mine` |
| `GET/PATCH/POST` | `/v1/webdev/service/issues/{id}`、`/claim`、`/settings`、`POST /v1/webdev/service/jobs` | intake 触发的固定自动领取读/锁/回填/持久化步骤；不接受任意 service 资源 | `webdev.service_issues.*` / `webdev.service_jobs.create` |
