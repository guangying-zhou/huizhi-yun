...
>
> - **最后事实核验：** 2026-07-11。
> - **当前事实源：** [`webdev/CLAUDE.md`](../CLAUDE.md)、[`webdev/README.md`](../README.md)、[`dev-agent/README.md`](../../dev-agent/README.md)、[`dev-agent/config.example.json`](../dev-agent/config.example.json)、[`data-runtime/CLAUDE.md`](../../data-runtime/CLAUDE.md)、[`docs/MODULE_CONTRACTS.md`](../../docs/MODULE_CONTRACTS.md) 与 [`docs/Development-Execution-Checklist-2026-07.md`](../../docs/Development-Execution-Checklist-2026-07.md)。
> - **执行边界：** 本 PoC 手册的本地启动、Cloudflare Tunnel、DNS、Worker secret、Cloudflare 部署、Dev Agent 命令和 data-runtime 配置均可能改变外部状态；离线核对不等同于已上线或获得执行许可，须按目标环境逐项、明确授权。
...
托管 tenant-runtime 模式先在租户 Console 数据库执行并核验
`Console-SQL-Seed-v2.0-webdev-runtime-identity.sql`；WebDev 随后以
`webdev.runtime` 换取短期 Data Runtime JWT，不再配置长期
`HZY_WEBDEV_DATA_RUNTIME_TOKEN`。该变量只用于显式本地/PoC static-token 模式。
...
显式本地/PoC static-token 模式可用以下变量把 job snapshot 和 event 以
best-effort 方式写入 Data Runtime；托管 tenant-runtime 模式只配置 URL，令牌由
Console 短期签发：
