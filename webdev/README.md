...
- **历史** (`/history`) — 持久化任务与日志事件（Data Runtime），并提供任务级部署职责冲突解释入口。
...
# Explicit local/PoC runtime compatibility only. Managed tenant-runtime
# deployments obtain a short-lived webdev.runtime token from Console.
HZY_WEBDEV_DATA_RUNTIME_URL=http://127.0.0.1:18080
HZY_WEBDEV_DATA_RUNTIME_TOKEN=<local-data-runtime-token>
...
`hzy-data-runtime` `/v1/webdev/*` APIs using a short-lived Console token issued
to `webdev.runtime`. The static Data Runtime token is only a local/PoC fallback.
WebDev history can explain deployment segregation-of-duties risk through
`POST /api/webdev/authorization/instance-conflict-explain`: it reads the
persisted job from Data Runtime, treats `createdBy` as the requester/executor,
and delegates the execute/deploy conflict explanation to Console/Foundation.
...
See `webdev/docs/WebDev-PoC-Runbook.md` for Tunnel and Cloudflare operations.
...
Install and verify `console/docs/sql/Console-SQL-Seed-v2.0-webdev-runtime-identity.sql`
for the tenant before enabling the managed runtime route. A managed Worker does
not need `HZY_WEBDEV_DATA_RUNTIME_TOKEN`.
