...
async function loadAutoClaimRule(event: Parameters<typeof dataRuntimeFetch>[0]): Promise<AutoClaimRule> {
  try {
    const settings = unwrap<{ autoClaimEnabled?: boolean, severityMin?: string, kinds?: string[], apps?: string[] }>(
      await dataRuntimeFetch(event, '/v1/webdev/service/issues/settings')
    )
    return {
...
export default defineEventHandler(async (event): Promise<unknown> => {
  ...
  const created = unwrap<{ id?: string, severity?: string, kind?: string }>(
    await dataRuntimeFetch(event, '/v1/webdev/service/issues/intake', {
      method: 'POST',
      body: {
        appCode,
        tenant,
  ...
  return unwrap(await dataRuntimeFetch(event, `/v1/webdev/service/issues/${encodeURIComponent(issueId)}`))
})
