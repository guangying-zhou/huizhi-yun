...
function isServiceBridge(event: H3Event) {
  const auth = event.context.consoleAuth as { authenticated?: boolean, tokenUse?: string, subjectType?: string } | undefined
  return Boolean(auth?.authenticated && auth.tokenUse === 'service' && auth.subjectType === 'service')
}

function issueRuntimePath(event: H3Event, id = '') {
  const prefix = isServiceBridge(event) ? '/v1/webdev/service/issues' : '/v1/webdev/issues'
...
export async function claimIssueAndCreateJob(
  ...
  const issue = unwrap<IssueRecord>(
    await dataRuntimeFetch(event, issueRuntimePath(event, id))
  )

  const claim = unwrap<ClaimRecord>(
    await dataRuntimeFetch(event, `${issueRuntimePath(event, id)}/claim`, {
      method: 'POST',
