async (page) => {
  ...
  for (const width of [1440,390]) {
    ...
    for(const node of [paragraph,editor.locator('.cm-line').first(),editor.locator('th').first()]){
      await node.dblclick();
      await page.waitForFunction(()=>!window.getSelection()?.toString(),{},{timeout:2000});
      await page.keyboard.press('ControlOrMeta+A');
      await page.waitForFunction(()=>!window.getSelection()?.toString(),{},{timeout:2000});
      const selection=await page.evaluate(()=>window.getSelection()?.toString()||'');
      if(/测试拖选|const greeting|第一列标题/.test(selection))throw new Error(width+' select all included content '+selection.slice(0,100));
