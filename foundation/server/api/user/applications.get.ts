...
import { CONSOLE_WORKER_USER_AGENT } from '../../utils/consoleServiceBinding'
import {
...
import { localGatewayConsoleUrl } from '../../utils/localGatewayConsoleUrl'
import { fetchConsoleServiceJson } from '../../utils/serviceOidc'
import { shouldUseConsoleUserApplications } from '../../utils/userApplicationSelection'
import { userApplicationsTimeoutMs } from '../../utils/userApplicationsTimeout'
...
function consolePortalApplications(event: H3Event): AppItem[] {
  const pub = runtimePublicConfig(event)
  const localConsoleUrl = localGatewayConsoleUrl({
    enabled: process.env.HZY_LOCAL_TEST_GATEWAY_ENABLED,
    nodeEnv: process.env.NODE_ENV,
    environment: process.env.HZY_PLATFORM_ENVIRONMENT,
    consoleUrl: process.env.HZY_CONSOLE_URL || pub.consoleUrl
  })
  const homeUrl = localConsoleUrl
    || configuredDeploymentPublicUrl(event)
...
function normalizeAppItem(event: H3Event, value: unknown, deploymentPublicUrl?: string | null): AppItem | null {
  ...
  const homeUrl = nullableString(record.homeUrl)
...
async function fetchConsoleUserApplications(
  ...
  if (!url) return { apps: [], authorizationEvaluated: false }

  const headers = authForwardHeaders(event)
  if (!headers.cookie && !headers.authorization) {
    return { apps: [], authorizationEvaluated: false }
  }
  headers['user-agent'] = CONSOLE_WORKER_USER_AGENT

  const response = await fetchConsoleServiceJson<ConsoleUserApplicationsEnvelope>(event, url, {
    headers,
    timeout: userApplicationsTimeoutMs(useRuntimeConfig(event).consoleUserApplicationsTimeoutMs)
  })
...
function mergeAuthorizedCurrentApplication(apps: AppItem[], currentApp: AppItem) {
  if (!apps.some(app => app.appCode === currentApp.appCode)) {
    return apps
  }

  return mergeApplication(apps, currentApp, {
    preferFallbackHomeUrl: true,
    preferFallbackIcon: true
  })
}
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (shouldUseConsoleUserApplications(consoleUserApplications.apps, {
      authorizationEvaluated: consoleUserApplications.authorizationEvaluated
    })) {
      const consoleUserApps = consoleUserApplications.apps
      const canAccessConsole = consoleUserApps.some(app => app.appCode === 'console')
      return {
        code: 0,
        data: sortApplications(mergeAuthorizedCurrentApplication(
          mergeApplications(
            consoleUserApps,
