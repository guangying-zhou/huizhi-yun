...
export async function notifyFeedbackRecipients(event: H3Event, payload: {
  ...
  let raw = ''
  try {
    raw = stringValue(await getRuntimeSetting<string>('feedback.notify.wecomUsers', '', {
      ttlMs: 60000,
      event
    }))
  } catch {
  ...
  const issueId = stringValue((created as Record<string, unknown> | undefined)?.id)
  ...
  await sendNotification({
    ...
    btntxt: '查看反馈',
    eventType: 'webdev.feedback.created',
    category: 'feedback',
    severity: kind === 'bug' && severity === 'high' ? 'error' : 'info',
    bizType: 'webdev_issue',
    bizId: issueId,
    idempotencyKey: `webdev-feedback:${issueId}:created`,
    metadata: {
      kind,
      severity: severity || undefined,
      displayNo: displayNo || undefined
    }
  })
