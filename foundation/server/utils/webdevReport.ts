...
// 跨应用调用使用共享应用 origin，绕过 Tenant Gateway 的二次转发；这里透传网关注入的
...
function downstreamWebDevHeaders(event: H3Event) {
  return {
    ...tenantContextHeaders(event),
    ...trustedServiceRequestHeaders(event, WEBDEV_APP_CODE)
  }
}
...
function webdevBaseUrl(event: H3Event) {
  // Console 等业务 Worker 正在由 Tenant Gateway 的 Service Binding 调用时，不能再
  // fetch 当前租户 host 的 /webdev 路径，否则会形成 Gateway -> Console -> Gateway
  // 的循环等待并最终返回 522。服务路由目录只在可信网关上下文中可用，因此直接使用
  // WebDev 的 tenant-neutral origin，并在下游请求头中重写目标应用上下文。
  const base = resolveServiceAppBaseUrl(event, WEBDEV_APP_CODE, { directTarget: true })
  if (!base) {
...
async function callWebDevIssueApi<T>(event: H3Event, path: string, scope: string, options: WebDevFetchOptions) {
  const baseUrl = webdevBaseUrl(event)
  const token = await issueServiceToken(event, scope)
  try {
    return await fetchExternal<T>(`${baseUrl}${path}`, {
      ...options,
      headers: {
        ...downstreamWebDevHeaders(event),
