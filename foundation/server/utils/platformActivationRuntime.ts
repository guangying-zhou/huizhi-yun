...
async function fetchConsolePlatformConfig(config: PlatformActivationConfig, licenseToken: LicenseToken) {
  ...
  const response = await fetchExternal<ConsolePlatformConfigEnvelope>(
...
export async function fetchAndVerifyPlatformPolicyBundle(config: PlatformActivationConfig): Promise<PlatformCachedPolicyBundle> {
  const response = await fetchExternal<PlatformBundleEnvelope>(
...
export async function postPlatformRuntimeHeartbeat(config: PlatformActivationConfig, bundle: PlatformCachedPolicyBundle | null) {
  return fetchExternal<HeartbeatEnvelope>(
...
export async function loadPlatformActivationStatus() {
  ...
  if (!config.enabled) {
    const status = emptyPlatformActivationStatus()
    status.lastCheckedAt = new Date().toISOString()
    status.lastError = config.missing.length
  ...
  if (config.missing.length) {
    const status = emptyPlatformActivationStatus()
    status.mode = 'failed'
    status.envValid = false
    status.tenantCode = config.tenantCode || null
    status.deploymentCode = config.deploymentCode || null
    status.lastCheckedAt = new Date().toISOString()
    status.lastError = `platform activation env missing: ${config.missing.join(', ')}`
    return status
  }
