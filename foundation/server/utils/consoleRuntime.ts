...
const pendingRuntimeLoadsKey = Symbol('hzy.consoleRuntime.pendingLoads')

type ConsoleRuntimePendingEventContext = H3Event['context'] & {
  [pendingRuntimeLoadsKey]?: Map<string, Promise<ConsoleRuntimeConfig>>
}
...
function tenantGatewayRequestHeaders(event: H3Event | null | undefined, config: Record<string, unknown>) {
  if (!event || !resolveTrustedTenantGatewayContext(event, config)) return undefined
...
function consoleRuntimeCacheKey(seed: ConsoleRuntimeSeedConfig, event: H3Event | null | undefined, config: Record<string, unknown>) {
  const context = event ? resolveTrustedTenantGatewayContext(event, config) : null
  return [
    seed.consoleApiUrl,
    seed.appCode,
    context?.tenant || 'unbound',
    context?.deployment || 'unbound',
    context?.environment || 'unbound',
    context?.appCode || seed.appCode || 'unbound'
  ].join('|')
}

export function getCachedConsoleRuntimeConfig(
  event?: H3Event | null,
  configInput?: RuntimeConfigShape | Record<string, unknown>
) {
  const config = (configInput || useRuntimeConfig(event || undefined)) as unknown as RuntimeConfigShape
  const record = config as Record<string, unknown>
  const seed = resolveConsoleRuntimeSeedConfig(config, event)
  const entry = runtimeCache.get(consoleRuntimeCacheKey(seed, event, record))
...
export function resolveConsoleRuntimeBaseUrl(
  ...
  return normalizeBaseUrl(
    resolveServerConsoleBaseUrl(
      getCachedConsoleRuntimeConfig(event, config)?.console.baseUrl || '',
...
export function resolveConsoleRuntimeTokenUrl(
  ...
  const cached = getCachedConsoleRuntimeConfig(event, config)
  const seed = resolveConsoleRuntimeSeedConfig(config, event)
...
/**
 * 托管云 Worker 必须经 `HZY_CONSOLE_SERVICE` Service Binding 访问 Console。
 *
 * 走公网会重新进入公共边缘：生产 zone `huizhi.yun` 的 `CN_CA_JP` WAF 规则 block 掉
 * `ip.src.country ∉ {CA, CN, JP}` 的请求，而 cron 触发的 Worker 子请求没有访客上下文、
 * 被归属为 `country=US`，因此稳定 403（浏览器触发的子请求继承访客国家反而能通过）。
 * 该规则的 bypass 例外只覆盖 data-runtime 主机，不覆盖 Console。
 *
 * 没有 Service Binding 的部署（自托管 / 本地）仍走 HTTP，但必须带上 WAF bypass
 * 规则识别的 Worker UA，否则同样会被地理规则拦掉。
 */
async function fetchConsoleRuntimeConfig(
  url: string,
  gatewayHeaders: Record<string, string> | undefined,
  timeoutMs: number,
  event?: H3Event | null
): Promise<ConsoleRuntimeEnvelope> {
  const headers: Record<string, string> = {
    ...(gatewayHeaders || {}),
    'accept': 'application/json',
    'user-agent': CONSOLE_WORKER_USER_AGENT
  }
  ...
  if (binding) {
    const signal = typeof AbortSignal?.timeout === 'function'
      ...
      : undefined
    const bound = await binding.fetch(normalizeConsoleServiceBindingUrl(url), { headers, signal })
    const payload = await bound.json().catch(() => ({})) as ConsoleRuntimeEnvelope
    if (!bound.ok) {
      throw createError({
        statusCode: bound.status,
        message: payload?.message || bound.statusText || 'Console runtime API request failed'
      })
    }
    return payload
  }

  const runtimeFetch = $fetch as unknown as (
    url: string,
    options: { headers?: Record<string, string>, timeout: number }
  ) => Promise<ConsoleRuntimeEnvelope>
  return await runtimeFetch(url, { headers, timeout: timeoutMs })
}
...
export async function getConsoleRuntimeConfig(options: {
  ...
  const config = useRuntimeConfig(options.event || undefined) as unknown as RuntimeConfigShape
  const record = config as Record<string, unknown>
  const seed = resolveConsoleRuntimeSeedConfig(config, options.event)
  const cacheKey = consoleRuntimeCacheKey(seed, options.event, record)
  ...
  const pendingKey = `${cacheKey}|${options.allowFallback === false ? 'strict' : 'fallback'}`
  // Cloudflare Workers must not await an I/O promise created by another
  // request. Keep single-flight deduplication request-local; resolved runtime
  // values may still be shared safely through runtimeCache.
  const pendingRuntimeLoads = options.event
    ? ((options.event.context as ConsoleRuntimePendingEventContext)[pendingRuntimeLoadsKey] ||= new Map())
    : null
  const existingLoad = pendingRuntimeLoads?.get(pendingKey)
  if (existingLoad) return await existingLoad

  const pendingLoad = (async () => {
    try {
      const configUrl = `${seed.consoleApiUrl}/api/v1/console/runtime/apps/${encodeURIComponent(seed.appCode)}/config`
      const response = await fetchConsoleRuntimeConfig(
        configUrl,
        tenantGatewayRequestHeaders(options.event, record),
        seed.timeoutMs,
        options.event
      )
  ...
  pendingRuntimeLoads?.set(pendingKey, pendingLoad)
  try {
    return await pendingLoad
  } finally {
    if (pendingRuntimeLoads?.get(pendingKey) === pendingLoad) {
      pendingRuntimeLoads.delete(pendingKey)
    }
  }
}
