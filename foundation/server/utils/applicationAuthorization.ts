...
interface BundleGrantSource {
  roleCode: string
  source: 'direct' | 'template' | 'override' | 'simulation'
  grantId: string
  assignmentId?: string
}
...
function assignmentScopeRecords(payload: Record<string, unknown>) {
  return preferredRecords(payload, 'assignmentScopes', 'subjectRoleScopes')
}

function roleDefaultScopeRecords(payload: Record<string, unknown>) {
  return preferredRecords(payload, 'roleDefaultScopes', 'roleScopes')
}
...
export function baselineGrantAppliesToSubject(record: BundleRecord, uid: string) {
  const normalizedUid = stringValue(uid)
  if (!normalizedUid) return false
  return !stringArray(record.excludedSubjectCodes).includes(normalizedUid)
}
...
export function buildPolicyBundleActionPolicy(
  payload: Record<string, unknown> | null | undefined,
  appCode?: string | null,
  resourceCode?: string | null
): ResourceActionPolicy | undefined {
  const targetAppCode = stringValue(appCode)
  const targetResourceCode = stringValue(resourceCode)
  const implications: ResourceActionPolicy['implications'] = {}

  for (const record of records(payload?.actionImplications)) {
    if (targetAppCode && !optionalFieldMatches(record.appCode, targetAppCode)) continue
    if (targetResourceCode && !optionalFieldMatches(record.resourceCode, targetResourceCode)) continue

    const action = stringValue(record.action)
    const implies = Array.isArray(record.implies)
      ? record.implies.map(item => stringValue(item)).filter(Boolean)
      : []
    if (!action || !implies.length) continue

    implications[action] = [...new Set([...(implications[action] || []), ...implies])].sort()
  }

  return Object.keys(implications).length ? { implications } : undefined
}
...
function addSimulatedEnterpriseRole(input: {
  authorizationMode: AuthorizationMode
  requestedRoleCode: string
  roleByCode: Map<string, BundleRecord>
  roleSources: Map<string, Set<string>>
  roleGrantSources?: Map<string, BundleGrantSource[]>
}) {
  if (input.authorizationMode !== 'role_simulation' || !input.requestedRoleCode) return

  const simulatedRole = input.roleByCode.get(input.requestedRoleCode)
  if (!simulatedRole || !isEnterpriseRoleRecord(simulatedRole) || !isActive(simulatedRole)) return

  addRole(input.roleSources, input.requestedRoleCode, 'simulation')

  if (input.roleGrantSources && !(input.roleGrantSources.get(input.requestedRoleCode)?.length)) {
    addGrantSource(input.roleGrantSources, {
      roleCode: input.requestedRoleCode,
      source: 'simulation',
      grantId: `simulation:${input.requestedRoleCode}`
    })
  }
}
...
function defaultPredicateForDimension(dimension: string, scopeValue: string) {
  ...
  // Legacy project scopes without an explicit predicate remain membership
  // scopes. A project-code-only grant must opt into predicate=code.
...
function collectRoleDefaultScopes(
  ...
  for (const scope of roleDefaultScopeRecords(payload)) {
...
function collectAssignmentScopeRecords(
  ...
  return assignmentScopeRecords(payload)
    .filter((scope) => {
...
function collectRolePermissions(
  ...
  for (const record of rolePermissionGrantRecords(payload)) {
...
export function buildAllowedAppCodesFromPolicyBundle(input: PolicyBundleAllowedAppCodesInput): PolicyBundleAllowedAppCodesResult {
  ...
  const requestedRoleCode = stringValue(input.requestedRoleCode)
  addSimulatedEnterpriseRole({
    authorizationMode,
    requestedRoleCode,
    roleByCode,
    roleSources
  ...
  if (hasActiveUserSubject && input.includeBaseline !== false) {
    for (const permission of baselineGrantRecords(payload)) {
      if (!baselineGrantAppliesToSubject(permission, input.uid)) continue
      const appCode = stringValue(permission.appCode)
...
export function buildScopedAuthorizationGrantsFromPolicyBundle(
  ...
  if (hasActiveUserSubject && input.includeBaseline !== false) {
    for (const record of baselineGrantRecords(payload)) {
      if (!isActive(record)) continue
      if (!baselineGrantAppliesToSubject(record, input.uid)) continue
...
export function evaluatePolicyBundleScopedAuthorization(
  ...
  const actionPolicy = buildPolicyBundleActionPolicy(input.payload, input.required.appCode, input.required.resourceCode)
  const decision = evaluateFoundationScopedAuthorization({
    grants: grantResult.grants,
    required: input.required,
    object: input.object,
    policyOf: actionPolicy ? () => actionPolicy : undefined
  })
