...
export function deriveWecomCallbackUrl(event: H3Event, query: URLSearchParams) {
  ...
  const callback = resolveCurrentAppUrl(event, '/api/auth/wecom-callback')
  return encoded ? `${callback}?${encoded}` : callback
}

export function deriveDingTalkCallbackUrl(event: H3Event) {
  return resolveCurrentAppUrl(event, '/api/auth/dingtalk-callback')
}
