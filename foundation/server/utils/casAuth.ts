...
import { getConsoleOidcConfig } from './consoleOidc'
import { fetchExternal } from './externalFetch'

function requireExplicitLegacyAuthBridge(event: H3Event) {
  // A missing Console issuer must not silently re-enable CAS. The bridge is
  // available only when the deployment explicitly selects its legacy mode.
  if (getConsoleOidcConfig(event).legacyFallback) return

  throw createError({
    statusCode: 410,
    message: 'Legacy CAS authentication is disabled; use Console OIDC'
  })
}
...
async function fillOptionalRoleCookie(event: H3Event, email: string, cookieOptions: ReturnType<typeof getAuthCookieOptions>) {
  ...
  try {
    const sysUser = await fetchExternal<{ umask?: number }>('/api/system/users/by-email', {
      params: { email }
    })
...
export async function handleCasLogin(event: H3Event) {
  requireExplicitLegacyAuthBridge(event)
