...
type AccountApiFetch = <T>(
  url: string,
  options: { headers: { Authorization: string }, timeout: number, params?: Record<string, unknown> }
) => Promise<T>
...
export async function fetchAccountApi<T = unknown>(path: string, options?: { params?: Record<string, unknown> }): Promise<T> {
  ...
  const fetchAccount = $fetch as unknown as AccountApiFetch
  const response = await fetchAccount<T>(`${apiBaseUrl}${path}`, {
