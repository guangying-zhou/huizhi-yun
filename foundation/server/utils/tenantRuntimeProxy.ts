...
export async function maybeProxyCurrentApiToTenantRuntime<T = unknown>(
  ...
  const rawBody = method === 'GET' ? undefined : await readRequestBodyCompat(event)
  const bodyRecord = method === 'GET' ? undefined : objectBody(rawBody)
  const body = method === 'GET'
    ? undefined
    : includeCurrentUser
      ? withCurrentUser(bodyRecord || {}, currentUser, currentDeptCodes)
      : rawBody
  if (bodyRecord) {
    context.body = includeCurrentUser ? (body as Record<string, unknown>) : bodyRecord
  }
