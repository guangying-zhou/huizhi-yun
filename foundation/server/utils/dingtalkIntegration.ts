...
export async function getDingTalkAccessToken(integrationCode = 'dingtalk.default') {
  ...
  const response = await fetchExternal<DingTalkTokenResponse>(`${config.baseUrl}/v1.0/oauth2/accessToken`, {
    method: 'POST',
    body: {
      appKey: config.appId,
...
export async function fetchDingTalkReports(input: FetchDingTalkReportsInput): Promise<DingTalkReport[]> {
  ...
  while (hasMore) {
    const response = await fetchExternal<DingTalkReportListResponse>(`${config.oapiBaseUrl}/topapi/report/list`, {
      method: 'POST',
