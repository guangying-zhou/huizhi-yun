...
export async function fetchRuntimeSettings(keys: string[], event?: H3Event) {
  ...
  const baseUrl = normalizeBaseUrl(runtime?.console.baseUrl || getConsoleRuntimeSettingsBaseUrl(config))
  if (!baseUrl) {
  ...
  const url = new URL(`${baseUrl}/api/v1/console/settings/values`)
  url.searchParams.set('keys', normalizedKeys.join(','))
  const response = await fetchConsoleServiceJson<ConsoleApiResponse<{ items: RuntimeSettingValue[] }>>(
    event,
    url,
    {
      headers: {
...
export async function getRuntimeSetting<T = unknown>(
  key: string,
  fallback?: T,
  options: { ttlMs?: number, forceRefresh?: boolean, event?: H3Event } = {}
): Promise<T | undefined> {
  ...
  try {
    const settings = await fetchRuntimeSettings([settingKey], options.event)
