...
export function isDataRuntimeEnabled(event: H3Event, appCode: string, config = useRuntimeConfig() as unknown as Record<string, unknown>) {
  return isTenantRuntimeProfile(resolveDeploymentProfile(config)) && Boolean(dataRuntimeEndpoint(event, appCode, config))
}
...
export async function maybeCallDataRuntime<T>(
  ...
  if (!isTenantRuntimeProfile(resolveDeploymentProfile(config)) || !endpoint) {
