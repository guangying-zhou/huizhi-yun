...
type ConsoleServerSession = {
  authenticated?: boolean
  refreshable?: boolean
  uid?: string | null
}

type OidcRefreshCoordinatorOptions = {
  requestKey: string
  getTokenKey: () => string
  hasActiveToken: () => boolean
  syncCookies: () => void
  performRefresh: () => Promise<void>
  withCrossTabLock?: (name: string, task: () => Promise<void>) => Promise<void>
}
...
const oidcRefreshRequests = new Map<string, Promise<void>>()

export function runConsoleOidcRefreshSingleFlight(options: OidcRefreshCoordinatorOptions) {
  const requestKey = String(options.requestKey || 'app').trim() || 'app'
  const existingRequest = oidcRefreshRequests.get(requestKey)
  if (existingRequest) {
    return existingRequest
  }

  const startingTokenKey = options.getTokenKey()
  const performCoordinatedRefresh = async () => {
    options.syncCookies()
    const currentTokenKey = options.getTokenKey()
    if (
      currentTokenKey
      && currentTokenKey !== startingTokenKey
      && options.hasActiveToken()
    ) {
      return
    }

    await options.performRefresh()
    options.syncCookies()
  }

  const lockName = `hzy:oidc-refresh:${requestKey}`
  const request = (
    options.withCrossTabLock
      ? options.withCrossTabLock(lockName, performCoordinatedRefresh)
      : performCoordinatedRefresh()
  ).finally(() => {
    if (oidcRefreshRequests.get(requestKey) === request) {
      oidcRefreshRequests.delete(requestKey)
    }
  })

  oidcRefreshRequests.set(requestKey, request)
  return request
}
...
function getBrowserLockManager() {
  if (!import.meta.client) return null
  return (navigator as unknown as { locks?: BrowserLockManager }).locks || null
}
...
export function useConsoleOidcAuth() {
  ...
  async function refresh() {
    ...
    await runConsoleOidcRefreshSingleFlight({
      requestKey: cookieNames.accessToken,
      getTokenKey: currentTokenKey,
