...
import { authorizationResourcesAllow } from '../../shared/utils/authorizationActions'
...
export function usePlatformPermission() {
  ...
  function hasPermission(resource: string, action: PermissionAction = 'view'): boolean {
    const authorization = getAuthorization() as LegacyAuthorizationSnapshot | null
    if (!authorization) return false
    return authorizationResourcesAllow(
      authorization.resources,
      resource,
      action,
      authorization.actionPolicies?.[resource]
    )
  }
