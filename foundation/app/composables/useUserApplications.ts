...
function getAuthIdentityFingerprint(auth: ReturnType<typeof useAuth> | null) {
  if (!auth) return ''
...
function parseCachedApplication(value: unknown): UserApplicationItem | null {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null
  const record = value as Partial<UserApplicationItem>
  const appCode = String(record.appCode || '').trim()
  const appName = String(record.appName || '').trim()
  const appType = String(record.appType || '').trim()
  if (!appCode || !appName || !appType) return null
  ...
  return normalizeApplication({
    ...
    homeUrl: typeof record.homeUrl === 'string' ? record.homeUrl : null,
    basePath: typeof record.basePath === 'string' ? record.basePath : null,
...
function clearSessionCache() {
  ...
  clearUserApplicationsSessionCache(sessionStorage)
}
...
function hydrateFromSessionCache(
  ...
  if (!import.meta.client || !auth?.authenticated.value || loaded.value || apps.value.length) return false

  const cached = readUserApplicationsSessionCache(
    sessionStorage,
    authFingerprint,
    parseCachedApplication,
    Date.now(),
    { allowStale: true }
  )
  if (!cached?.length || businessAppCount(cached) === 0) return false

  apps.value = sortApplications(cached)
  loaded.value = true
...
function persistSessionCache(
  ...
  if (
    ...
    || !authFingerprint
    || businessAppCount(nextApps) === 0
  ) {
    return
  }

  writeUserApplicationsSessionCache(sessionStorage, authFingerprint, nextApps)
}
...
function ensureAuthWatcher() {
  ...
  watch(
    () => ({
      auth: getAuthFingerprint(auth),
      identity: getAuthIdentityFingerprint(auth)
    }),
    (fingerprint, previousFingerprint) => {
      if (fingerprint.auth === previousFingerprint.auth) return

      if (fingerprint.identity === previousFingerprint.identity) {
        if (auth.authenticated.value && lastLoadFailed.value) {
          void loadApplications(true)
        }
        return
      }
...
async function loadApplications(force = false) {
  ...
  const hydratedFromSessionCache = hydrateFromSessionCache(auth, authFingerprint)
  const loadGeneration = cacheGeneration
  const canReuseLoadedState = loaded.value
    && !hydratedFromSessionCache
    && !force
  ...
  if (loading.value) {
    if (force) reloadAfterCurrentLoad = true
    return apps.value
  }
  ...
  try {
    ...
    if (shouldKeepExistingApps(nextApps, auth)) {
      ...
      apps.value = nextApps
      persistSessionCache(nextApps, auth, authFingerprint)
    }
    loaded.value = true
    lastLoadFailed.value = false
    lastAuthFingerprint.value = authFingerprint
    ...
    const authorizationRejected = statusCode === 401 || statusCode === 403
    loaded.value = true
...
export function useUserApplications() {
  return {
    ...
    clearApps: clearApplicationsCache
  }
}
