...
async function fetchAuthorizationSnapshot(activeRoleCodeOverride?: string | null) {
  ...
  const requestedActiveRoleCode = String(activeRoleCodeOverride || '').trim()
  const preferredActiveRoleCode = String(activeRoleCode.value || '').trim()
  ...
  if (response.code === 0 && response.data) {
    ...
    const active = availableRoles.some(role => role.roleCode === requestedActiveRoleCode)
      ? requestedActiveRoleCode
      : availableRoles.some(role => role.roleCode === preferredActiveRoleCode)
        ? preferredActiveRoleCode
        : availableRoles.some(role => role.roleCode === responseActive)
          ? responseActive
          : availableRoles[0]?.roleCode || ''
    if (active !== preferredActiveRoleCode) {
