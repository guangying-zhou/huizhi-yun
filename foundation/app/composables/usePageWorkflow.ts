...
export function usePageWorkflowState() {
  ...
  const isInitializing = computed(() => {
    ...
    if (!cfg || actions.value.length > 0) return false
    if (cfg.initializing?.value) return true
    const bizId = String(cfg.bizId.value || '').trim()
    return !bizId || bizId === '0' || bizId === 'NaN'
  })

  /** 是否有页面流程 */
  const hasPageWorkflow = computed(() => actions.value.length > 0 || isInitializing.value)

  const loadingTitle = computed(() => config.value?.loadingTitle || '审批事项')
...
export function usePageWorkflow(options: {
  ...
  loadingTitle?: string
  /** 页面业务与权限上下文是否仍在初始化 */
  initializing?: Ref<boolean> | ComputedRef<boolean>
  ...
  const config: PageWorkflowConfig = {
    ...
    initializing: options.initializing ? computed(() => unref(options.initializing!)) : undefined,
