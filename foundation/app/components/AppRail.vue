...
function normalizeRailItem(item: AppRailItem, defaultExternal: boolean) {
  ...
  return {
    ...
    homeUrl,
    basePath: (override && 'basePath' in override ? override.basePath : item.basePath) || null,
...
// 避免 hydration mismatch。用户点击发生在挂载之后，那时已是深链。
function directAppEntryUrl(app: AppRailDisplayItem) {
  if (!clientReady.value) return app.homeUrl
  if (isCurrentApp(app.appCode)) return app.homeUrl
  if (isApplicationShellUrl(app.homeUrl, window.location.origin)) return app.homeUrl
  if (!app.restoreLastRoute) return app.homeUrl
  return resolveAppEntryUrl(app.homeUrl, app.appCode, app.basePath)
}

function appEntryUrl(app: AppRailDisplayItem) {
  if (!clientReady.value) return app.homeUrl
...
function prefetchApp(app: AppRailDisplayItem) {
