...
function handleDirectoryProfileUpdated(event: Event) {
  const detail = (event as CustomEvent<{ avatar?: string | null }>).detail
  const avatar = String(detail?.avatar || '').trim()
  if (!avatar) return
  fetchedAccountUser.value = {
    uid: fetchedAccountUser.value?.uid || String(authUser.value || ''),
    email: fetchedAccountUser.value?.email || String(authEmail.value || ''),
    realName: fetchedAccountUser.value?.realName || String(authRealname.value || '') || null,
    avatar
  }
}

onMounted(() => window.addEventListener('hzy:directory-profile-updated', handleDirectoryProfileUpdated))
onBeforeUnmount(() => window.removeEventListener('hzy:directory-profile-updated', handleDirectoryProfileUpdated))
...
function normalizeAvatarPath(value: string | null | undefined) {
  ...
  return avatar.replace(/^\/+/, '').replace(/^avatars\//, '') || null
}
...
const avatarUrl = computed(() => {
  return resolveAvatarSrc(
    ...
    || normalizeAvatarPath(accountFallbackUser.value?.avatar)
