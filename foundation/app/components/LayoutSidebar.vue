...
const props = withDefaults(defineProps<{
  ...
  appTitle?: string
  hideNavbar?: boolean
  hideNavbarWhenEmbedded?: boolean
  embeddedNavbarMobileOnly?: boolean
  autoHideEmptyNavbarWhenEmbedded?: boolean
  hidePageTitle?: boolean
  seamlessTopHeader?: boolean
  primaryLinks?: Array<Record<string, unknown>>
  utilityLinks?: Array<Record<string, unknown>>
  refreshHandler?: () => void
}>(), {
  appNavigation: 'rail',
  autoHideEmptyNavbarWhenEmbedded: true
})
...
// SSR 时读不到 localStorage，先用 Nuxt UI 的布局 Cookie 保证首屏宽度与内容一致。
// 客户端启动后再由用户显式偏好或响应式规则统一接管。
if (import.meta.server && typeof sidebarLayoutState.value?.collapsed === 'boolean') {
  isSidebarCollapsed.value = sidebarLayoutState.value.collapsed
}
...
watch(isSidebarCollapsed, (collapsed) => {
  // Nuxt UI 用 Cookie 控制侧栏宽度，菜单内容则跟随用户偏好状态。
  // 必须同步更新，否则首次加载会出现“内容已折叠、容器仍展开”。
  if (sidebarLayoutState.value?.collapsed !== collapsed) {
    sidebarLayoutState.value = {
      ...sidebarLayoutState.value,
      collapsed
    }
  }
...
function handleSidebarCollapsedUpdate(collapsed: boolean) {
  // 拖拽到折叠阈值或双击恢复属于用户显式操作，应与按钮切换一样记住偏好。
...
const pageNavbarRef = ref<ComponentPublicInstance | null>(null)
const embeddedNavbarHasBusinessContent = ref(false)
let pageNavbarObserver: MutationObserver | null = null

function resolvePageNavbarElement() {
  const target = pageNavbarRef.value
  const element = target?.$el
  return element instanceof HTMLElement ? element : null
}

function navbarHasBusinessContent(element: HTMLElement) {
...
function updateEmbeddedNavbarContent() {
  if (!applicationShellEmbedded.value || !props.autoHideEmptyNavbarWhenEmbedded) {
    embeddedNavbarHasBusinessContent.value = true
    return
  }

  const element = resolvePageNavbarElement()
  embeddedNavbarHasBusinessContent.value = element
    ? navbarHasBusinessContent(element)
    : false
}

async function observePageNavbar() {
  pageNavbarObserver?.disconnect()
  pageNavbarObserver = null
  await nextTick()

  const element = resolvePageNavbarElement()
  if (!element) {
    updateEmbeddedNavbarContent()
    return
  }

  updateEmbeddedNavbarContent()
  pageNavbarObserver = new MutationObserver(updateEmbeddedNavbarContent)
  pageNavbarObserver.observe(element, {
    childList: true,
    subtree: true,
    characterData: true
  })
...
const showAppRail = computed(() => (
  !applicationShellEmbedded.value
  && props.appNavigation === 'rail'
  && !isSidebarDrawerMode.value
))
const showAppLauncher = computed(() => {
  if (applicationShellEmbedded.value || props.appNavigation === 'none') return false
  return props.appNavigation === 'popover' || isSidebarDrawerMode.value
})
...
const showStaticSidebarBorder = computed(() => (
  !isSidebarDrawerMode.value && !open.value && !isSidebarPreviewActive.value
))
...
const showPageNavbar = computed(() => (
  !applicationShellEmbedded.value
  || !props.autoHideEmptyNavbarWhenEmbedded
  || isApprovalMode.value
  || embeddedNavbarHasBusinessContent.value
))

onMounted(() => {
  void observePageNavbar()
})

watch(
  [applicationShellEmbedded, () => route.fullPath, renderPageNavbar],
  () => void observePageNavbar()
)
...
const isPageWorkflowInitializing = computed(() => pageWorkflow?.isInitializing.value ?? false)
const pageWorkflowLoadingTitle = computed(() => pageWorkflow?.loadingTitle.value ?? '审批事项')
...
// 审批中心是稳定能力入口，不能依赖远端待办查询成功后才显示；查询只更新角标。
const workflowEnabled = ![false, 'false'].includes(useRuntimeConfig().public.workflowEnabled as boolean | string)
const showApprovalEntry = ref(Boolean(workflowEnabled && currentAppCode && currentAppCode !== 'workflow'))
const pendingCount = ref(0)
const approvalAuth = useAuth()
const APPROVAL_PENDING_CACHE_KEY = 'hzy:workflow-pending-count:v1'
const APPROVAL_PENDING_CACHE_TTL_MS = 60_000
let pendingCountRequest: Promise<void> | null = null

function approvalAuthFingerprint() {
  return buildShellCacheFingerprint({
    authenticated: approvalAuth.authenticated.value,
    user: approvalAuth.user.value,
    tenant: approvalAuth.tenant.value,
    policyVersion: approvalAuth.policyVersion.value
  })
}
...
async function loadPendingCount(force = false) {
  if (!workflowEnabled) return
  const fingerprint = approvalAuthFingerprint()
  if (!force && fingerprint) {
    const cached = readShellSessionCache(
      sessionStorage,
      APPROVAL_PENDING_CACHE_KEY,
      fingerprint,
      APPROVAL_PENDING_CACHE_TTL_MS,
      parsePendingCount
    )
    if (cached) {
      pendingCount.value = cached.value
      return
    }
  }

  if (pendingCountRequest) return await pendingCountRequest

  pendingCountRequest = (async () => {
    try {
      ...
      // Workflow 暂不可达时仍保留审批中心入口，由列表页呈现可重试错误。
    } finally {
      pendingCountRequest = null
    }
  })()

  return await pendingCountRequest
}

// workflow 模块不自动注入（它有自己的导航）
if (workflowEnabled && currentAppCode && currentAppCode !== 'workflow' && import.meta.client) {
  onMounted(() => void loadPendingCount())
}
...
<template>
  ...
  <UDashboardSidebar
    ...
    @update:collapsed="handleSidebarCollapsedUpdate"
  ...
  >
    <template #default>
      <div
        ...
        :class="[
          sidebarShellClass,
          showStaticSidebarBorder && !seamlessTopHeader ? 'border-r border-default' : ''
        ]"
      ...
      >
        ...
        <div
          ...
          :class="showStaticSidebarBorder && seamlessTopHeader ? 'border-r border-default' : ''"
        >
          <!-- 主菜单（可滚动，避免挤掉底部工具菜单） -->
        ...
        <!-- Sidebar Footer: 折叠按钮 -->
        <div
          class="relative h-12 shrink-0 border-t border-default py-1"
          :class="showStaticSidebarBorder && seamlessTopHeader ? 'border-r border-default' : ''"
        >
          <button
            type="button"
  ...
  <div class="flex min-w-0 min-h-0 flex-1 flex-col bg-default h-svh overflow-hidden">
    <!-- 顶部导航栏：宽屏由 AppRail 提供应用入口，窄屏使用 AppLauncher -->
    <UDashboardNavbar
      v-if="renderPageNavbar"
      v-show="showPageNavbar"
      ref="pageNavbarRef"
      class="border-b border-default bg-default"
      :class="[
        applicationShellEmbedded && embeddedNavbarMobileOnly && !isApprovalMode ? 'md:hidden' : '',
        applicationShellEmbedded
          && autoHideEmptyNavbarWhenEmbedded
          && !embeddedNavbarMobileOnly
          && !isApprovalMode
          ? 'hzy-embedded-auto-hide-navbar'
          : ''
      ]"
    ...
    >
      <template #left>
        ...
        <!-- <slot v-else name="navbar-left">
          <h1 v-if="pageTitle && !hidePageTitle" class="truncate text-base font-semibold">
            {{ pageTitle }}
          </h1>
        </slot> -->
      </template>
      ...
      <template #right>
        ...
        <NotificationBell v-if="!applicationShellEmbedded" />
        <AppLauncher v-if="showAppLauncher" />
        <UserMenu v-if="!applicationShellEmbedded" header />
      </template>
    </UDashboardNavbar>
        ...
        <template v-else-if="hasPageWorkflow">
          ...
          >
            <div class="shrink-0 border-b border-default px-4 py-3">
              <div class="pb-4 text-md font-semibold text-highlighted">
                {{ pageWorkflowLoadingTitle }}
              </div>
              <div class="flex items-center gap-2">
                <UIcon name="i-lucide-git-pull-request" class="size-4 text-primary" />
                <span class="text-sm font-medium">审批流程</span>
              </div>
            </div>
            <div class="flex flex-1 items-center justify-center gap-2 text-sm text-dimmed">
  ...
  <NotificationsSlideover v-if="!applicationShellEmbedded" />

  <IssueReporter v-if="!applicationShellEmbedded" />
</template>
...
/*
 * Shell iframe 会保活最近访问的应用。页面间切换时，Teleport 动作可能在
 * MutationObserver 与路由更新之间短暂交错，因此最终可见性必须由当前 DOM
 * 而不是上一页的响应式状态决定。
 */
:global(.hzy-embedded-auto-hide-navbar) {
  display: none !important;
}

:global(.hzy-embedded-auto-hide-navbar:has([data-slot="left"] > :not(button):not(:empty))),
:global(.hzy-embedded-auto-hide-navbar:has([data-slot="right"] > :not([data-page-refresh]):not(:empty))) {
