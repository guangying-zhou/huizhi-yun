...
const props = withDefaults(defineProps<{
  ...
  floating?: boolean
  // Console Shell 可把反馈交给当前业务应用自己的 BFF，来源身份仍由目标 Worker 签发。
  targetBasePath?: string | null
  targetPageUrl?: string | null
  targetRoutePattern?: string | null
}>(), {
  floating: true,
  targetBasePath: null,
  targetPageUrl: null,
  targetRoutePattern: null
})

const toast = useToast()
const { collectContext, submit, fetchMine, currentRoutePattern } = useIssueReporter({
  targetBasePath: () => props.targetBasePath,
  targetPageUrl: () => props.targetPageUrl,
  targetRoutePattern: () => props.targetRoutePattern
})
const auth = useAuth()
