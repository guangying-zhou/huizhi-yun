        ...
        if (bizRes.code === 0 && bizRes.data) {
          ...
          if (
            (
              ...
              || (data.status === 'rejected' && !data.capabilities?.can_resubmit)
            )
            ...
            instance.value = {
              ...
              action_name: data.action_name || props.launchPayload.actionName || null,
              biz_id: data.biz_id,
            ...
            const { user } = useAuth()
            const currentUid = user.value
            if (currentUid && tasks.value.length) {
              const myPendingTask = tasks.value.find(
                t => t.assignee_uid === currentUid && t.status === 'pending'
              )
              if (myPendingTask) {
                task.value = myPendingTask
...
// 只在审批查询的稳定业务身份发生变化时重新加载。
// launchPayload 还包含 bizTitle、formData 等展示/提交字段；页面并行加载仓库、
// 里程碑和立项书时会重建该对象，但这些变化不应重复查询同一个流程实例。
const workflowLoadKey = computed(() => JSON.stringify([
  props.taskId || null,
  props.instanceId || null,
  props.launchPayload?.appCode || '',
  props.launchPayload?.resourceCode || '',
  props.launchPayload?.actionCode || '',
  props.launchPayload?.bizId || ''
]))
watch(workflowLoadKey, loadData)
    ...
    <div v-else class="flex-1 overflow-y-auto">
      ...
      <div v-if="capabilities" class="px-4 pb-4">
        ...
        <div v-if="capabilities.can_resubmit" class="border-t border-default pt-4 mt-3">
          ...
          <div
            v-if="completenessIssues && completenessIssues.length > 0"
            class="space-y-2 mb-3"
          >
            <div class="text-xs font-medium text-dimmed">
              重新提交前请完成
            </div>
            <div
              v-for="issue in completenessIssues"
