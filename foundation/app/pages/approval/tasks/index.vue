...
function getApplication(appCode: string) {
  return apps.value.find(app => app.appCode === appCode) || null
}

function getApplicationHomeUrl(appCode: string) {
  return getApplication(appCode)?.homeUrl || null
}
...
function currentApplicationRoute(target: string, targetAppCode: string) {
  if (!import.meta.client || targetAppCode !== runtimeAppCode) return null

  const application = getApplication(targetAppCode)
  if (!application?.homeUrl) return null

  const targetUrl = new URL(target, window.location.origin)
  const appHome = new URL(application.homeUrl, window.location.origin)
  const configuredBasePath = String(application.basePath || '').trim()
  const routeBasePath = configuredBasePath.startsWith('/') ? configuredBasePath : appHome.pathname
  const appBasePath = routeBasePath.endsWith('/') ? routeBasePath : `${routeBasePath}/`
  const appBasePathWithoutSlash = appBasePath.replace(/\/$/, '') || '/'
  if (
    targetUrl.origin !== appHome.origin
    || (targetUrl.pathname !== appBasePathWithoutSlash && !targetUrl.pathname.startsWith(appBasePath))
  ) {
    return null
  }

  const relativePath = stripAppBasePath(targetUrl.pathname, routeBasePath)
  return `${relativePath}${targetUrl.search}${targetUrl.hash}`
}

async function navigateToWorkflowTarget(target: string, targetAppCode: string) {
  if (isAbsoluteUrl(target)) {
    if (import.meta.client && applicationShellEmbedded.value) {
      const localTarget = currentApplicationRoute(target, targetAppCode)
      if (localTarget) {
        await navigateTo(localTarget)
        return
      }

      const shellEntry = applicationShellEntryUrl(targetAppCode, target, window.location.origin)
      window.parent.location.assign(
        isApplicationShellUrl(shellEntry, window.location.origin) ? shellEntry : target
      )
      return
    }
...
async function navigateToApproval(item: WorkflowTaskItem) {
  ...
  await navigateToWorkflowTarget(await getApprovalTarget(item, 'task'), item.app_code)
}

async function navigateToApprovalDone(item: WorkflowTaskItem) {
  enterApprovalMode({ instanceId: item.instance_id })
  await navigateToWorkflowTarget(await getApprovalTarget(item, 'instance'), item.app_code)
}

async function navigateToInitiated(item: WorkflowInitiatedItem) {
  enterApprovalMode({ instanceId: item.instance_id })
  await navigateToWorkflowTarget(await getInitiatedTarget(item), item.app_code)
}
</script>
    ...
    <div class="flex-1 min-h-0 overflow-y-auto px-4 pb-8 pt-5 sm:px-6 lg:px-8">
      <div class="mx-auto flex w-full max-w-[1600px] flex-col gap-6">
        ...
        <UAlert
          ...
          :actions="[{ label: '重试', color: 'error', variant: 'outline', onClick: loadBoard }]"
        />
