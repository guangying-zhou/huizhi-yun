...
export default defineNuxtConfig({
  ...
  runtimeConfig: {
    consoleUserApplicationsTimeoutMs: 3000,
    public: {
      workflowEnabled: process.env.NUXT_PUBLIC_WORKFLOW_ENABLED !== 'false',
  ...
  // Foundation applications are SPAs backed by Nitro endpoints. Serve icons
  // from the application Worker instead of depending on the public Iconify API.
  icon: {
    provider: 'server',
    fallbackToApi: false,
    serverBundle: {
