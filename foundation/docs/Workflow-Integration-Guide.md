...
> **当前运行契约（2026-07-11 核对）**：本指南的页面/动作设计仍有效。业务用户认证由 Foundation 接入 **Console OIDC**；目录与审批人组织信息来自 **Console Directory**；Foundation workflow proxy 先验证用户会话，再以当前应用的短期 Console `token_use=service` JWT（精确 `aud=workflow`、capability）调用 Workflow。`HZY_ACCOUNT_API_*`、`Bearer {apiKey}:{apiSecret}`、业务应用直连 Account 及静态跨应用密钥均不是当前主路径。`hzy.workflowApiUrl` 仅可作为显式本地/兼容覆盖，正常由 Console runtime 解析 Workflow 地址。
...
Foundation 通过 `resolveWorkflowApiUrl()` 从 Console runtime 解析 Workflow 服务地址；本地开发若必须覆盖，才显式设置 `hzy.workflowApiUrl`。业务模块不配置 `HZY_ACCOUNT_API_*`，也不自行拼装静态 Authorization header；Foundation 会按调用应用身份申请并校验短期 Console service token。
...
export default defineNitroPlugin(() => {
  setTimeout(async () => {
    try {
      ...
      console.log(`[SyncApprovalActions] Synced: ${result.created} new, ${result.updated} updated, ${result.deprecated} deprecated`)
    } catch {
...
### Q: 审批人看不到待办任务
检查 Console Directory 中部门的 `managerId`/`leaderId` 是否正确设置。审批人由流程引擎根据部门配置自动解析。
...
### Q: 分管领导为空导致流程卡住
流程引擎会自动向上级部门递归查找负责人（最多 5 层）。如果整个链路都没有设置负责人，该节点将无法创建任务。请在 Console Directory 中完善部门管理者配置。
...
- **驳回重提**：继续使用当前动作的 `canSubmit` 和 `completenessIssues`；不完整时显示缺失项并禁用“重新提交”
