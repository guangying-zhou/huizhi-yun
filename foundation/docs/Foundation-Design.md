...
> **当前事实边界（2026-07-11 核对）**：本文保留 Layer 初始设计与迁移复盘，不是当前运行契约。默认认证是 Console OIDC；目录经 Console Directory adapter；服务端跨应用调用由 Foundation 申请短期 Console `token_use=service` JWT；企业级 Git/AI/WeCom/OSS 凭据经 Console integration-config + credential-vault adapter 消费；已迁移业务数据走 tenant-runtime/data-runtime。文中 Account URL、CAS、静态 key/secret、业务应用直连 Platform 或本地 DB 的说明仅属历史或显式 legacy bridge。以根 `CLAUDE.md`、`docs/MODULE_CONTRACTS.md`、`foundation/CLAUDE.md` 与当前源码为准；不据此执行外部操作。
...
| 头像 404（模块无 OSS） | 各模块缺少 `/api/oss/avatar` | 头像统一由 Console OSS 代理提供，`resolveAvatarSrc` 拼 Console URL |

### 2.5 头像统一方案

头像图片统一由 Console 的 `/api/oss/avatar?path=xxx` 接口提供，文件存储在 Console
`oss.default` 集成所指向的 OSS Bucket 中；业务应用不读取 OSS 凭证。

`resolveAvatarSrc()` 逻辑：
1. 完整 URL / data / blob → 直接返回
2. OSS 相对路径 → 拼 `{consoleUrl}/api/oss/avatar?path=xxx`
3. `consoleUrl` 从 `runtimeConfig.public.consoleUrl` 读取；Console 自身使用本地代理
4. 迁移期仅在 `consoleUrl` 缺失时兼容 `runtimeConfig.public.accountUrl`

各模块 `nuxt.config.ts` 需配置：
```typescript
public: {
  consoleUrl: process.env.HZY_CONSOLE_URL || 'http://localhost:3000'
}
```
...
企业 Shell 嵌入模式下，LayoutSidebar 默认检测导航栏的实际业务内容：空导航栏、仅包含 Nuxt UI 注入的左侧侧栏开关或标记为 `data-page-refresh` 的本地刷新按钮时不占用页面高度；页面标题、创建/导入/切换等业务操作出现时自动恢复导航栏。为兼容 Shell 的 iframe 保活和跨页 Teleport 切换，最终显隐还由基于当前 DOM 的 `:has()` 规则实时兜底，避免沿用上一页的标题栏状态。刷新当前嵌入应用由 Console Shell 顶栏统一提供。独立模式（`?standalone=1`）继续保留应用自身的完整顶栏。

标准列表页把创建、视图切换等操作放在筛选工具栏右侧时，可通过 `hideNavbar` 显式移除页面导航栏；审批模式会覆盖该设置并继续显示审批提示。
...
- `rail`：桌面端左侧显示 AppRail，窄屏回退 AppLauncher；同源业务应用优先进入 Console 企业 Shell，跨源应用跳转到 `homeUrl`。
- `popover`：仅保留顶栏 AppLauncher，适合窄屏、嵌入页或暂不启用统一壳层的模块。
- `none`：不显示应用层入口，适合公开页、登录页、iframe 嵌入页或特殊全屏页面。

AppRail 数据源与 AppLauncher 保持一致，统一读取当前应用本地 `/api/user/applications`。该接口以 Console 当前用户应用列表作为可见性事实源；本地开发模式才使用离线目录。应用列表按 Platform 下发的 `sortOrder` 升序展示。当前应用高亮由 `runtimeConfig.public.appCode` 判断。应用入口图标使用 manifest / platform application 的 `icon` 字段，优先渲染 `i-lucide-*` 等 Iconify 图标名；AppRail 会在图标下方展示去掉 `汇智云` 前缀后的短名称。Console 企业 Shell 通过 `appOverrides` 把同源业务应用导航改为内部 Shell 路由，并用 `intent` 事件预热 iframe；嵌入应用通过受校验的同源消息把运行时 `appLogo` 提供给 Shell 品牌位，应用侧栏完整移除重复品牌行，使业务菜单直接上移到侧栏顶部。业务应用内的 AppRail/AppLauncher 则在导航意图阶段预取目标业务文档和 Shell 文档。
...
| 权限控制 | aims | 菜单权限过滤 |
| 在线状态心跳 | 全部 Foundation 应用 | 每 2 分钟上报；业务应用经共享 `/api/heartbeat` BFF 固定 appCode 后写 Console tenant-runtime，Console 直写 `/api/v1/heartbeat` |
...
- 所有同源业务应用统一使用嵌入协议，跨源应用保持直接导航
- 确认无回归后再纳入 codocs
- account 最后迁移（需特殊处理）

### 7.4 域名与 iframe 前提

当前托管租户通过 Tenant Gateway 把 Console 与业务应用暴露在**同一 origin 的不同 base path**。因此：
- Console 企业 Shell 只嵌入同源、当前用户已授权且 target 位于应用 home path 内的业务页面。
- Shell 最多保留两个同源业务 iframe；跨源部署和新窗口入口继续直接导航。
- 同源业务 URL 顶层刷新默认恢复 Shell；只有显式 `standalone=1` 才保留独立应用模式，Shell 新窗口入口负责追加该标记。
- 子应用仍由自身 OIDC、路由权限和服务端对象授权保护，Shell 不是授权边界。
- 若未来采用跨 origin 部署，需要重新评估 Cookie、CSP `frame-ancestors`、postMessage origin 和登录跳转，不得自动启用 Shell。
