...
状态：Draft  
日期：2026-04-21  
定位：目标设计，作为 `docs/Huizhi-yun-Platform-Target-Architecture.md`、`docs/Identity-Plane-Design.md` 与 `docs/Control-Plane-API-Contract.md` 的配套文档
...
type VerifiedPolicyBundle = {
  ...
  bundle: {
    ...
    compatSchemaVersions: string[]
    policyRevision: number
    applications: unknown[]
    resources: unknown[]
    roles: unknown[]
    roleAssignments: unknown[]
    rolePermissionGrants: unknown[]
    assignmentScopes: unknown[]
    roleDefaultScopes: unknown[]
    baselineGrants: unknown[]
...
新生成的签名 bundle 使用 `policy-bundle.v2` 作为主 schema。SDK 消费端应优先读取
`roleAssignments / rolePermissionGrants / assignmentScopes / roleDefaultScopes / baselineGrants`；
仅在处理历史 bundle 时回退旧 `subjectRoles / rolePermissions / roleScopes / baselinePermissions`。
