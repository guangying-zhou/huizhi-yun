...
- 可以保留：仅在显式 `HZY_AUTH_MODE=legacy` 或 `HZY_LEGACY_AUTH_BRIDGE=true` 时的 CAS 登录、回调、旧 Account API 依赖、兼容登出行为
- 默认 Console OIDC 模式下，`/api/auth/cas-login` 与 `/api/auth/cas-callback` 必须在读取 query/ticket、构造回调 URL、请求 CAS、查询 Account、写 legacy Cookie 或上报登录审计之前返回 `410`；缺少 Console OIDC issuer 不能隐式重新开启 CAS。
