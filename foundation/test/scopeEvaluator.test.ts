...
test('scope evaluator：支持运行时 action policy 蕴含', () => {
  const decision = evaluateFoundationScopedAuthorization({
    grants: [
      grant({
        permissions: [{ appCode: 'aims', resourceCode: 'project', action: 'review' }]
      })
    ],
    required: { appCode: 'aims', resourceCode: 'project', action: 'view' },
    policyOf: () => ({ implications: { review: ['view'] } })
  })

  assert.equal(decision.allowed, true)
  assert.equal(decision.matchedGrantId, 'grant-1')
})

test('scope evaluator：未提供 action policy 时保持保守默认', () => {
  const decision = evaluateFoundationScopedAuthorization({
    grants: [
      grant({
        permissions: [{ appCode: 'aims', resourceCode: 'project', action: 'review' }]
...
test('scope evaluator：支持客户、指派对象、关系别名和部署环境范围', () => {
  assert.equal(
    foundationScopeSetMatches([
      { dimension: 'customer', predicate: 'owner', source: 'assignment' }
    ], {
      actorUid: 'u1',
      customerOwnerUid: 'u1'
    }),
  ...
  assert.equal(
    foundationScopeSetMatches([
      { dimension: 'customer', predicate: 'team', source: 'assignment' }
    ], {
      actorUid: 'u1',
      customerTeamUids: ['u2']
    }),
  ...
  assert.equal(
    foundationScopeSetMatches([
      { dimension: 'object', predicate: 'assigned', source: 'assignment' }
    ], {
      actorUid: 'u1',
      assignedUids: ['u2', 'u1']
    }),
  ...
  assert.equal(
    foundationScopeSetMatches([
      { dimension: 'relation', predicate: 'participant', source: 'relation' }
    ], {
      matchedRelations: ['relation:participant']
    }),
    true
  )
  assert.equal(
