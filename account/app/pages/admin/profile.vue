...
// Decode umask to role names
function getRolesFromUmask(umask: number): string[] {
  const roles: string[] = []
  if (umask & 16) roles.push('超级管理员')
  if (umask & 8) roles.push('分管领导')
  if (umask & 4) roles.push('HR')
  if (umask & 2) roles.push('部门经理')
  if (umask & 1) roles.push('普通用户')
  return roles
}
...
// Computed roles from umask
const roles = computed(() => profile.value ? getRolesFromUmask(profile.value.umask) : [])
