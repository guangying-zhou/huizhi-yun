...
| `WECOM_CORPID` / `CORPSECRET` / `AGENTID` | legacy 企业微信登录 OAuth 配置，不用于业务通知 |
...
Align 当前没有业务通知直发 API。轻量通知属于 Console employee portal；企业微信
secret 仅由 legacy 登录 OAuth 回调使用。由于本模块第一阶段暂缓，本次不扩大为
Console credential-vault 迁移；恢复 Align 开发前必须先完成认证集成凭据收口。
