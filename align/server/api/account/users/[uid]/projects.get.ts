...
export default defineEventHandler(async (event) => {
  ...
  try {
    const response = await $fetch<ApiResponse<unknown>>(
      `${apiBaseUrl}/api/v1/users/${encodeURIComponent(uid)}/projects`,
      {
    ...
    console.error(`[AccountApi] Failed to fetch git_projects for user ${uid}:`, accountError.message || error)
    throw createError({
      statusCode: accountError.statusCode || 500,
      message: accountError.message || 'Failed to fetch user git_projects from Account API'
    })
  }
})
