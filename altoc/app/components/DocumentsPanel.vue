...
async function removeLink(doc: DocumentLink) {
  if (!(await confirm({
    title: '取消关联',
    message: '确定取消该文档关联？不会删除 Codocs 中的文档。',
    confirmLabel: '取消关联',
    tone: 'warning'
  }))) return
  try {
...
const previewLinkId = ref<number | undefined>()
const externalPreviewOpen = ref(false)
const externalPreviewUrl = ref('')
const externalPreviewTitle = ref('')

function externalDocumentViewUrl(doc: DocumentLink) {
  const params = new URLSearchParams()
  params.set('url', String(doc.external_url || ''))
  params.set('entity_type', props.entityType)
  params.set('entity_id', String(props.entityId))
  params.set('link_id', String(doc.id))
  return `/api/v1/documents/external-view?${params.toString()}`
}
...
function openExternalDocument(doc: DocumentLink) {
  openExternalUrl(externalDocumentViewUrl(doc))
}

function openDocument(doc: DocumentLink) {
  const externalUrl = String(doc?.external_url || '').trim()
  if (externalUrl) {
    externalPreviewUrl.value = externalDocumentViewUrl(doc)
      ...
      >
        ...
        <div class="flex items-center gap-1 shrink-0">
          <UButton
            ...
            size="xs"
            title="新窗口打开"
            @click.stop="openExternalDocument(doc)"
          />
          <UButton
