...
interface ActivateDeliveryResponse {
  data?: {
    deliveryOrchestrated?: boolean
    deliveryPending?: boolean
...
usePageTitle(computed(() => contract.value?.name || '合同详情'))
...
async function startActivation() {
  ...
  try {
    ...
    const response = await $fetch<ActivateDeliveryResponse>(`/api/v1/service/contracts/${encodeURIComponent(contract.value.code)}/activate-delivery`, {
      method: 'POST',
      headers: {
        'idempotency-key': `altoc:contract:${contract.value.code}:activate-delivery:${requestKey}`
      }
    })
    if (response?.data?.deliveryOrchestrated) {
      toast.add({ title: '履约启动已执行', color: 'success' })
    } else {
      // 合同状态与回款计划已提交，但 Aims 交付项目/里程碑尚未建成。
      // 这一段是「自动编排」的核心，失败必须让用户看见。
      const pending = Number(response?.data?.pendingOperationCount || 0)
      toast.add({
        title: '合同已生效，但交付编排未完成',
        description: pending > 0
          ? `${pending} 个交付命令尚未送达 Aims，请在下方履约启动作业中查看失败步骤并重试。`
          : '交付命令尚未送达 Aims，请在下方履约启动作业中查看失败步骤并重试。',
        color: 'warning'
      })
    }
...
async function submitContractManagement(action: ContractManagementAction) {
  ...
  const confirmTitle = action === 'force_complete'
    ? '强制完成合同'
    : action === 'terminate'
      ? '终止合同'
      : '作废合同'
...
function invoiceFilePreviewUrl(row: Record<string, unknown>) {
  ...
  params.set('contract_id', String(id.value))
  const invoiceCode = String(row.code || row.invoice_code || row.invoiceCode || '').trim()
  const name = invoiceFileName(row)
  const mimeType = invoiceFileMimeType(row)
  if (invoiceCode) params.set('invoice_code', invoiceCode)
