...
const { search: keyword, debounced: debouncedKeyword, flush: flushKeyword, reset: resetKeyword } = useDebouncedSearch({
  initial: savedFilterState.value.keyword || '',
  onChange: () => {
    page.value = 1
  }
})
...
const columns: TableColumn<ContractListRow>[] = [
  ...
  { id: 'conflict_actions', header: '风险', enableSorting: false },
      ...
      <div class="altoc-list-page min-w-0">
        <div class="altoc-list-toolbar flex flex-wrap items-center gap-2 px-4 py-3 border-b border-default">
          ...
          <UButton
            ...
            @click="resetFilters"
          />
          <div class="ml-auto flex w-full items-center justify-end sm:w-auto">
            <UButton
              label="新建合同"
              icon="i-lucide-plus"
              color="primary"
          ...
          >
            ...
            <template #conflict_actions-cell="{ row }">
              <UButton
                ...
                color="warning"
                size="xs"
                :loading="isConflictLoading('contract', row.original.id || row.original.code)"
                @click.stop="openConflictExplanation({
                  targetType: 'contract',
                  id: row.original.id || row.original.code,
                  action: 'approve',
                  label: `合同 ${row.original.code || row.original.name || row.original.id}`
                })"
              >
                冲突
              </UButton>
