...
const arQuadrants = computed(() => receivables.value?.ar_quadrants)
const arCollectionRate = computed(() => {
  const total = Number(arQuadrants.value?.plan_total || 0)
  if (total <= 0) return null
  return Math.round((Number(arQuadrants.value?.received || 0) / total) * 100)
})
const hasOverdue = computed(() => Number(arQuadrants.value?.overdue || 0) > 0)
const hasUninvoiced = computed(() => Number(arQuadrants.value?.uninvoiced || 0) > 0)
        ...
        <UCard>
          <template #header>
            <div class="flex flex-wrap items-center justify-between gap-2">
              <span class="font-semibold text-sm">应收全景</span>
              <span class="text-xs text-muted">计划总额 {{ formatFullMoney(arQuadrants?.plan_total) }}</span>
            </div>
          </template>
          <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="space-y-1">
              ...
              <div class="font-mono font-semibold text-lg text-primary break-all">
                {{ formatFullMoney(arQuadrants?.receivable) }}
              </div>
              <div class="text-xs text-muted">
                尚未收回
              </div>
            ...
            <div class="space-y-1">
              ...
              <div class="font-mono font-semibold text-lg text-success break-all">
                {{ formatFullMoney(arQuadrants?.received) }}
              </div>
              <div class="text-xs text-muted">
                回款率 {{ formatPercent(arCollectionRate) }}
              </div>
            </div>
            <div class="space-y-1">
              <div class="text-xs text-muted">
              ...
              <div class="font-mono font-semibold text-lg break-all" :class="hasOverdue ? 'text-error' : 'text-muted'">
                {{ formatFullMoney(arQuadrants?.overdue) }}
              </div>
              <NuxtLink v-if="hasOverdue" to="/payments?status=overdue" class="text-xs text-primary hover:underline">
                去催收
              </NuxtLink>
            ...
            <div class="space-y-1">
              ...
              <div class="font-mono font-semibold text-lg break-all" :class="hasUninvoiced ? 'text-warning' : 'text-muted'">
                {{ formatFullMoney(arQuadrants?.uninvoiced) }}
              </div>
              <NuxtLink v-if="hasUninvoiced" to="/payments?status=to_invoice" class="text-xs text-primary hover:underline">
                去开票
              </NuxtLink>
              <div v-else class="text-xs text-muted">
                无待开票
          ...
          <p class="mt-4 text-xs text-muted">
            应收余额 + 已实收 = 计划总额；逾期与未开票均为应收余额的子集，可能重叠，不能相加。
          </p>
        </UCard>
