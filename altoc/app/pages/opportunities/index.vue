    ...
    <template #body>
      <div class="altoc-list-page">
        ...
        <template v-if="viewMode === 'kanban'">
          <p class="px-4 pt-3 text-sm text-muted" role="status">
            看板仅展示当前已加载的 {{ items.length }} 条商机（筛选范围共 {{ total }} 条）。各阶段数量和金额仅统计已加载卡片；查看其余商机请切换列表。
          </p>
              ...
              >
                ...
                <div class="flex items-center justify-between mb-3">
                  <div>
                    ...
                    <span class="text-xs text-muted ml-1">（已加载 {{ col.items.length }} 条）</span>
                  </div>
                  <span class="text-xs font-mono text-muted">已加载金额 {{ formatMoney(col.totalAmount) }}</span>
                </div>
