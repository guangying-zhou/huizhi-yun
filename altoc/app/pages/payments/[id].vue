...
interface ReceivablePlanDetail {
  ...
  collection_responsible_uid?: string | null
...
interface InvoiceRequestResponse {
  data?: {
    // 开票申请编号由 Finance 侧的 receipt 返回，只在投递成功后才有值。
    // prepared.invoiceRequest 是 runtime 内存里拼的对象，不含 code。
    invoiceRequestCode?: string | null
    delivered?: boolean
...
usePageTitle(computed(() => plan.value?.plan_name || '回款详情'))
...
const collectionResponsibleUid = ref('')
const canEditReceivable = computed(() => permissionsLoaded.value && hasPermission('receivable', 'edit'))
...
function openCollectionResponsible() {
  collectionResponsibleUid.value = plan.value?.collection_responsible_uid || ''
  collectionModalOpen.value = true
}

async function saveCollectionResponsible() {
  collectionSaving.value = true
  try {
    await $fetch(`/api/v1/payments/${id.value}`, {
      method: 'PUT',
      body: { collection_responsible_uid: collectionResponsibleUid.value.trim() || null }
    })
    toast.add({ title: collectionResponsibleUid.value.trim() ? '催收责任人已更新' : '已清空催收责任人；该计划不会发送临期通知', color: 'success' })
    collectionModalOpen.value = false
    await refresh()
  } catch (error: unknown) {
    toast.add({ title: errorMessage(error, '催收责任人更新失败'), color: 'error' })
  } finally {
...
async function requestInvoice() {
  ...
  try {
    ...
    const invoiceRequestCode = response?.data?.invoiceRequestCode
    if (response?.data?.delivered) {
      ...
      // 投递未完成时申请并没有到达 Finance，不能提示成功。
      toast.add({
        title: '开票申请尚未送达 Finance',
        description: '申请已在本地登记但未成功投递，请联系管理员检查跨应用授权后重试。',
        color: 'warning'
      })
    }
    await refresh()
...
<template>
  <UDashboardPanel id="payment-detail">
    <template #body>
      <Teleport to="#altoc-layout-header-actions">
        ...
        <template v-if="plan">
          <UButton
            v-if="canEditReceivable"
            label="催收责任人"
            icon="i-lucide-user-round-cog"
            variant="soft"
            color="neutral"
            @click="openCollectionResponsible"
          />
          <UButton
      ...
      <div v-else-if="plan" class="p-4 space-y-4">
        ...
        <UCard>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-y-3 gap-x-8 text-sm">
            ...
            <div class="flex">
              <span class="text-muted w-28 shrink-0">催收责任人</span>
              <UserName v-if="plan.collection_responsible_uid" :uid="plan.collection_responsible_uid" />
              <span v-else class="text-warning">未分配（不通知）</span>
            </div>
            <div class="flex">
              <span class="text-muted w-28 shrink-0">逾期天数</span><span :class="receivableOverdueDays(plan) > 0 ? 'text-error' : ''">{{ receivableOverdueDays(plan) }} 天</span>
            </div>
          </div>
        </UCard>
  ...
  <!-- 催收责任人弹窗 -->
  <UModal v-model:open="collectionModalOpen">
    <template #content>
      <UCard>
        <template #header>
          <span class="font-semibold">维护催收责任人</span>
        </template>
        <div class="space-y-3">
          <UserPicker v-model="collectionResponsibleUid" placeholder="选择唯一催收责任人" />
          <p class="text-xs text-muted">
            仅通知该直接责任人；未分配时不使用合同负责人、部门或管理员兜底。
          </p>
        </div>
        <template #footer>
          <div class="flex justify-end gap-2">
            ...
            />
            <UButton label="保存" :loading="collectionSaving" @click="saveCollectionResponsible" />
          </div>
        </template>
      </UCard>
    </template>
