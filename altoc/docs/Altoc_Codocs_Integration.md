...
| POST | `/api/v1/documents` | 创建新的 Codocs 文档并关联；传入已有 `document_uuid` 当前返回 `503 scoped_document_attach_contract_required` |
...
- 预览内容：`codocs:documents:read`（仅限已通过 Altoc 实体关联与范围校验的读取路径）

`document_uuid` 不是可由 Altoc 实体权限推导出的 Codocs 文档授权。已有 UUID 关联使用精确 `attach` signed service-command：Altoc 先重验实体 `edit` 范围和实体存在，Codocs 再对同一 actor 重验 owner/write-share ACL；read share、relation 与只读文档不能关联。预览使用独立 `content` command，先重验实体 `view` 与精确持久化 `document_link`，再重新执行 Codocs read ACL。两条命令均绑定 source/target、tenant/deployment、actor、实体 type+ID、UUID、action、route/envelope/request ID 的 60 秒 HMAC，不返回 OSS path 或签名 URL，也不恢复 `codocs:documents:read` 的宽摘要/正文接口。关联重放按 `(entity_type, entity_id, document_uuid)` 返回既有 link；Console v1.53 grant/verify 是未执行的待批准输入。

当前主路径不再使用应用级 `license.lic` bootstrap；接口不透传用户 Cookie，也不依赖旧 Account CAS 会话。服务身份由 Foundation 通过 Console runtime 获取短期 token；仅经批准的离线兼容部署才可另行配置兼容凭据。
