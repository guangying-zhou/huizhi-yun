...
CREATE TABLE contract_line_profit_summary (
    ...
    frozen_at                   DATETIME DEFAULT NULL COMMENT '财务冻结时间；冻结后禁止重算和分摊改写',
    frozen_by                   VARCHAR(50) DEFAULT NULL COMMENT '财务冻结人',
    freeze_key                  VARCHAR(160) DEFAULT NULL COMMENT '冻结幂等键或审批引用',
    ...
    INDEX idx_clps_frozen (contract_line_code, period_start, period_end, frozen_at)
...
DROP TABLE IF EXISTS product_feedback_status_projection;
DROP TABLE IF EXISTS service_ticket_product_feedback;
...
CREATE TABLE service_ticket (
    ...
    aims_dispatch_status    VARCHAR(20) NOT NULL DEFAULT 'idle' COMMENT 'Aims派发：idle/pending/succeeded',
    aims_dispatch_operation_key VARCHAR(191) DEFAULT NULL COMMENT 'Aims派发可靠operation key',
    aims_delivery_generation BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '已应用Aims结果事件代次',
    aims_delivery_status    VARCHAR(30) DEFAULT NULL COMMENT '已应用Aims结果状态',
    codocs_document_uuid    VARCHAR(100) DEFAULT NULL COMMENT '运维知识文档UUID',
    ops_knowledge_pending_uuid VARCHAR(100) DEFAULT NULL COMMENT '运维知识编排预留UUID；完成前阻止换绑',
    ops_knowledge_status    VARCHAR(20) NOT NULL DEFAULT 'idle' COMMENT '运维知识编排状态：idle/pending/linked',
    ops_knowledge_idempotency_key VARCHAR(191) DEFAULT NULL COMMENT '运维知识编排幂等键',
    source_app              VARCHAR(50) DEFAULT NULL COMMENT '来源应用',
    source_biz_type         VARCHAR(50) DEFAULT NULL COMMENT '来源业务类型',
    source_biz_id           VARCHAR(100) DEFAULT NULL COMMENT '来源业务ID',
    created_by              VARCHAR(50) DEFAULT NULL COMMENT '创建人',
    ...
    INDEX idx_ops_knowledge_pending (ops_knowledge_pending_uuid, ops_knowledge_status),
    INDEX idx_priority (priority),
    INDEX idx_aims_work_item_key (aims_work_item_key),
    INDEX idx_aims_dispatch (aims_dispatch_status, aims_dispatch_operation_key),
    INDEX idx_deleted_at (deleted_at),
...
-- 2.18 回款计划表
-- ------------------------------------------------------------
DROP TABLE IF EXISTS altoc_receivable_notification_checkpoint;
...
CREATE TABLE receivable_plan (
    ...
    risk_level              VARCHAR(20) DEFAULT NULL COMMENT '风险等级：high/medium/low',
    owner_user_id           VARCHAR(50) DEFAULT NULL COMMENT '负责人',
    collection_responsible_uid VARCHAR(50) DEFAULT NULL COMMENT '应收催收唯一直接责任人UID（不从合同负责人推导）',
    remark                  VARCHAR(500) DEFAULT NULL COMMENT '备注',
    created_by              VARCHAR(50) DEFAULT NULL COMMENT '创建人',
    updated_by              VARCHAR(50) DEFAULT NULL COMMENT '更新人',
    created_at              DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    ...
    INDEX idx_owner_user_id (owner_user_id),
    INDEX idx_rp_collection_due (status, planned_payment_date, id),
    INDEX idx_rp_collection_responsible (collection_responsible_uid, status, planned_payment_date),
    INDEX idx_deleted_at (deleted_at),
    CONSTRAINT ck_rp_collection_responsible CHECK (
      collection_responsible_uid IS NULL OR (
        collection_responsible_uid = TRIM(collection_responsible_uid)
        AND collection_responsible_uid <> ''
        AND LOWER(collection_responsible_uid) <> '@all'
        AND collection_responsible_uid NOT REGEXP '[[:cntrl:]]'
      )
    ),
...
CREATE TABLE altoc_receivable_notification_checkpoint (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    event_stream ENUM('receivable_plan_due') NOT NULL,
    source_type ENUM('receivable_plan') NOT NULL,
    ...
    PRIMARY KEY (id),
    UNIQUE KEY uk_arn_event (event_version),
    UNIQUE KEY uk_arn_idempotency (idempotency_key),
    ...
    CONSTRAINT ck_arn_state CHECK ((state='open' AND close_reason IS NULL AND closed_at IS NULL)
      OR (state='closed' AND close_reason IS NOT NULL AND closed_at IS NOT NULL)),
    CONSTRAINT ck_arn_ack CHECK ((notification_id IS NULL AND notified_recipient_uid IS NULL AND acknowledged_at IS NULL)
      OR (notification_id IS NOT NULL AND notified_recipient_uid IS NOT NULL AND acknowledged_at IS NOT NULL)),
    CONSTRAINT ck_arn_lifecycle CHECK ((lifecycle_next_version IS NULL AND lifecycle_closed_at IS NULL)
...
-- ------------------------------------------------------------
-- 3.3 跨应用命令操作表（调用方 Outbox）
-- ------------------------------------------------------------
DROP TABLE IF EXISTS integration_operation_dead_letter_actionable;
DROP TABLE IF EXISTS integration_operation_attempt;
DROP TABLE IF EXISTS service_command_receipt;
DROP TABLE IF EXISTS integration_operation;
...
CREATE TABLE integration_operation_dead_letter_actionable (
...
-- ------------------------------------------------------------
-- 3.4 跨应用命令尝试日志（append-only）
-- ------------------------------------------------------------
CREATE TABLE integration_operation_attempt (
...
-- 3.5 目标服务命令回执（Inbox）
-- ------------------------------------------------------------
CREATE TABLE service_command_receipt (
...
CREATE TABLE audit_log (
    ...
    entity_id       BIGINT NOT NULL COMMENT '实体ID',
    action          VARCHAR(64) NOT NULL COMMENT '操作类型：create/update/delete/status_change/approve/reject 等语义动作名',
...
-- Assets 状态可靠投影单调水位完整增量 DDL：
-- docs/migrations/042_assets_delivery_status_projection.sql
