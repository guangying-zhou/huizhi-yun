...
export async function handleProductFeedbackStatusService(event: H3Event) {
  ...
  const binding = await requireProductFeedbackStatusAuth(event)
  ...
  if (!body || typeof body !== 'object' || Array.isArray(body) || Object.keys(body).length !== 1 || !envelope || typeof envelope !== 'object' || Array.isArray(envelope) || !command || typeof command !== 'object' || Array.isArray(command) || Object.keys(command).length !== 6 || !text(command.productCode, 64) || /[/\\]/.test(command.productCode) || !text(command.ticketCode, 30) || /[/\\]/.test(command.ticketCode) || !uuid(command.requestBizId) || !uuid(command.canonicalRequestBizId) || !['submitted', 'evaluating', 'accepted', 'deferred', 'rejected', 'merged'].includes(command.decisionStatus) || (command.decisionStatus === 'merged') !== (command.requestBizId !== command.canonicalRequestBizId) || !Number.isSafeInteger(command.sourceRevision) || command.sourceRevision <= 0 || envelope.targetApp !== 'altoc' || envelope.operationCode !== 'aims.altoc.product-feedback.update-status.v1' || envelope.requiredCapability !== 'altoc:product-feedback:update-status' || envelope.commandSchemaVersion !== 'product-feedback-status.v1' || !uuid(envelope.operationId) || !text(envelope.idempotencyKey, 191) || envelope.commandSha256 !== await hashServiceCommandPayload(command)) throw createError({ statusCode: 403, message: '反馈签名命令无效' })
  ...
  const response = await maybeCallTenantRuntime<{ code: number, data: Record<string, unknown> }>(event, '/v1/altoc/internal/product-feedback:status', {
    appCode: 'altoc', method: 'POST', scope: 'altoc.write altoc:product-feedback:update-status',
