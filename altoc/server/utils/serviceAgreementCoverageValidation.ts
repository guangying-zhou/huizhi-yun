...
async function callAssetsReferencesResolve(
  ...
  try {
    response = await serviceAppFetch<RuntimeEnvelope<AssetsReferenceResolution>>(event, 'assets', appendPath(resolveAssetsBaseUrl(event), '/api/v1/service/references:resolve'), {
      method: 'POST',
      headers: {
