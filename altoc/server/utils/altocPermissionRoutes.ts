...
const exportableResources = new Set(['dashboard'])

const unsupportedSensitiveResource = { resource: 'unsupported_export', action: 'admin' as const }
...
export function resolveAltocApiPermission(
  ...
  if (normalizedPath.startsWith('dashboard') || /^altoc\/analytics\/(contract|customer)\/[^/]+$/.test(normalizedPath) || /^analytics\/(contract|customer)\/[^/]+$/.test(normalizedPath)) {
    return {
      resource: 'dashboard',
      action: normalizedPath.startsWith('dashboard') && isDashboardExportPath(normalizedPath) ? 'export' : 'view'
    }
  }
  ...
  if (normalizedPath === 'authorization/instance-conflict-explain') return { resource: 'receivable', action: 'view' }
  ...
  if (/^service\/contract-lines\/[^/]+\/cost-allocations$/.test(normalizedPath)) {
    return { resource: 'contract', action }
  }
  if (/^service\/contract-lines\/[^/]+\/profit-summary:freeze$/.test(normalizedPath)) {
    return { resource: 'contract', action: 'edit' }
  }
  if (/^service\/contracts\/[^/]+\/profit-summary:recalculate$/.test(normalizedPath)) {
    return { resource: 'contract', action: 'edit' }
  }
  if (/^service\/service-agreements\/[^/]+\/cost-summary(:recalculate)?$/.test(normalizedPath)) {
    return { resource: 'contract', action }
  }
  ...
  if (isExportLikePath(normalizedPath)) {
    return exportLikePermission(normalizedPath) || unsupportedSensitiveResource
  }
  ...
  if (/^customers\/[^/]+$/.test(normalizedPath) && (normalizedMethod === 'PUT' || normalizedMethod === 'PATCH') && normalizedCommand === 'approved') {
    return { resource: 'customer', action: 'approve' }
  }
...
function isDashboardExportPath(path: string) {
  return /^dashboard(\/|$)/.test(path)
    && isExportLikePath(path)
}

function isExportLikePath(path: string) {
  return /(^|\/)(export|download|csv|xlsx)(\/|$)/.test(path)
}

function exportLikePermission(path: string): { resource: string, action: 'export' } | null {
  const resource = resourceFromPath(path)
  if (!resource || !exportableResources.has(resource)) return null
...
function resourceFromPath(path: string) {
  ...
  if (path.startsWith('quotes')) return 'quotation'
  if (path.startsWith('tenders')) return 'quotation'
  if (path.startsWith('contracts')) return 'contract'
  if (path.startsWith('payments')) return 'receivable'
  if (path.startsWith('maintenance-contracts')) return 'maintenance_contract'
  if (path.startsWith('service-entitlements')) return 'service_entitlement'
  if (path.startsWith('service-tickets')) return 'service_ticket'
  if (path.startsWith('renewal-opportunities')) return 'renewal_opportunity'
  return ''
}
