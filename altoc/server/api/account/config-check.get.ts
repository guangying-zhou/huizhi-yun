...
export default defineEventHandler(async (event) => {
  await requireAltocSessionUid(event)
  ...
  if (!directory.consoleApiUrl) {
    issues.push('Console Directory 地址未配置，也无法从统一网关默认值推导')
  }
  if (!directory.consoleClientId || !directory.consoleClientSecret) {
    warnings.push('未配置 Console service client，将使用迁移期 license bootstrap 或无服务凭证模式访问 Console Directory')
  }
