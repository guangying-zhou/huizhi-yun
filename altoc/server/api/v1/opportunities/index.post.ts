...
import { notifyOpportunityAssignedItem } from '~~/server/utils/runtimeNotifications'
import type { OpportunityAssignedNotice } from '~~/server/utils/runtimeNotificationBuilders'
...
export default defineEventHandler(async (event) => {
  ...
  const eventVersion = result.notification_event_version
    || result.opportunity?.notification_event_version
    || result.opportunity?.notificationEventVersion
    || getHeader(event, 'idempotency-key')
    || (result.opportunity?.id || result.id ? `created:${result.opportunity?.id || result.id}` : null)
  const notified = await notifyOpportunityAssignedItem(
    result.opportunity,
    creatorUid,
    eventVersion
  )
