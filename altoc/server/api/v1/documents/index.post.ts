...
import { authorizeCodocsAltocEntityDocumentAttach, createCodocsDocument, type AltocEntityType } from '~~/server/utils/codocsApi'
...
export default defineEventHandler(async (event) => {
  ...
  let documentUuid = text(body.document_uuid)
  let documentTitle = body.title.trim()
  ...
  if (!documentUuid) {
    ...
    // Entity edit is only the Altoc half of the decision. The exact existing
    // UUID is bound to an Altoc-signed command and Codocs rechecks owner or
    // explicit write-share ACL before this link can be written.
    const authorized = await authorizeCodocsAltocEntityDocumentAttach({
      event,
      actorUid: uid,
    ...
    documentTitle = text(authorized.title) || documentTitle
  }
