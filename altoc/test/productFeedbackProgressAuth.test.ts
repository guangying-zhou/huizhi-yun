...
const code = ts.transpileModule(readFileSync(new URL('../server/utils/productFeedbackProgressAuth.ts', import.meta.url), 'utf8'), { compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022 } }).outputText
...
test('status source requires exact service scope and distinct trusted deployment identities', async () => {
  for (const mode of ['valid', 'wide', 'client', 'tenant', 'target', 'missing', 'user']) {
    ...
    const promise = exports.requireProductFeedbackProgressAuth!({})
    if (mode === 'valid') {
