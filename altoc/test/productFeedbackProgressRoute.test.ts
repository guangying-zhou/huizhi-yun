...
test('status route invokes dedicated authenticated handler before generic forwarding', async () => {
  ...
  runInNewContext(compiled, { exports, defineEventHandler: (handler: unknown) => handler, require: (name: string) => {
    if (name === 'h3') return { createError, getRequestURL: () => ({ pathname: '/api/v1/service/product-feedback/progress' }) }
    if (name.endsWith('/productFeedbackProgressService')) return { handleProductFeedbackProgressService: async () => {
      calls++
      return { code: 0 }
    } }
