...
const compiled = ts.transpileModule(readFileSync(new URL('../server/utils/productFeedbackProgressService.ts', import.meta.url), 'utf8'), { compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022 } }).outputText
async function run(mode = '') {
  const exports: any = {}, calls: string[] = []
  const id = '00000000-0000-4000-8000-000000000001'
  const command = { productCode: 'P', ticketCode: 'ST-1', requestBizId: id, canonicalRequestBizId: id, decisionStatus: 'accepted', sourceRevision: 3, canonicalDecisionStatus: 'accepted', versions: [] }
  const envelope = { operationId: id, targetApp: 'altoc', operationCode: 'aims.altoc.product-feedback.update-progress.v1', requiredCapability: 'altoc:product-feedback:update-progress', commandSchemaVersion: 'product-feedback-progress.v1', idempotencyKey: 'original-key', commandSha256: await hashServiceCommandPayload(command), command }
  if (mode === 'tamper') command.sourceRevision = 4
  runInNewContext(compiled, { exports, require: (name: string) => {
    if (name === './productFeedbackProgressInput') return { validProductFeedbackProgress }
    if (name === './productFeedbackRuntimeError') return { productFeedbackRuntimeError }
    if (name === 'h3') return { createError, setHeader: () => {}, getQuery: () => ({}), getHeader: () => 'Bearer token', getRequestURL: () => ({ pathname: '/api/v1/service/product-feedback/progress' }), readBody: async () => ({ serviceCommand: envelope }) }
    if (name === './productFeedbackProgressAuth') return { requireProductFeedbackProgressAuth: async () => ({ tenant: 'TENANT', sourceDeployment: 'AIMS', targetDeployment: 'ALTOC' }) }
    if (name.endsWith('/tenantRuntimeClient')) return { hashServiceCommandPayload, verifyServiceCommandRuntimeHeaders: async () => {
      ...
      assert.equal(path, '/v1/altoc/internal/product-feedback:progress')
  ...
  try {
    return { calls, result: await exports.handleProductFeedbackProgressService({ method: 'POST' }) }
  } catch (error) {
    return { calls, error }
  }
}
