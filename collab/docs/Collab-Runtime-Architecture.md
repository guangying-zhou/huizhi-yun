...
当前默认 provider 是 `hocuspocus`，但 Hocuspocus 只作为内部 provider，不再作为平台模块边界暴露。后续增加 Cloudflare Durable Objects 或 Y-Sweet 时只需要新增 provider 实现。Codocs 迁移到 Cloudflare 的协同服务、对象存储与灰度路线见 `codocs/docs/Codocs-Cloudflare-Migration-Plan.md`。
