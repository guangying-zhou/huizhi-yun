...
>
> - **最后事实核验：** 2026-07-14。
> - **当前事实源：** [`Notification-Runtime-Deployment.md`](Notification-Runtime-Deployment.md)、[`notification-runtime/CLAUDE.md`](../CLAUDE.md)、[`notification-runtime/README.md`](../README.md)、Console `server/utils/notificationRuntimeInstall.ts` 与 [`docs/MODULE_CONTRACTS.md`](../../docs/MODULE_CONTRACTS.md)。
> - **执行边界：** 本手册涉及客户服务器、DNS/SSL/Nginx、数据库 migration、Console service client/grant、企业微信可信 IP/测试消息及生产通知；不得将文档或离线验证视为授权，须按目标 tenant/deployment 单独、明确授权。
...
默认单实例方案使用本机 SQLite，不需要先创建数据库。安装器会自动创建 `/opt/hzy/notification-runtime/data/delivery.db` 和 schema；该路径必须是本机磁盘，不能使用 NFS/SMB/共享卷。多实例或高可用方案才使用独立 MySQL 库，并由受控 migration 账号依次执行发布包内 001/002 schema。两种模式都不使用内存 fallback。
...
- `/opt/hzy/notification-runtime/data/delivery.db`
...
## 12.1 Delivery ledger 查询与人工对账

为运维读取和对账分别签发精确 scope，不复用业务应用的 send token：
...
读取当前租户/部署的 `partial_unknown`（`limit` 1–100，按响应 `nextCursor` 翻页）：

```bash
curl -H "Authorization: Bearer $OPS_READ_TOKEN" \
  'https://wecom-api.example.com/v1/deliveries?status=partial_unknown&limit=50'
```

仅当企业微信管理后台、供应商支持或内部事故工单已明确最终结果时才对账：

```bash
curl -X POST \
  -H "Authorization: Bearer $OPS_RECONCILE_TOKEN" \
  -H 'Content-Type: application/json' \
  'https://wecom-api.example.com/v1/deliveries/42/reconcile' \
  -d '{
...
- reason/reference 只写最小证据，不写消息正文、收件人、URL、token 或 provider body。
- 接口只接受当前 `partial_unknown`；processing/succeeded/failed 返回冲突，不能重置。
- succeeded 对账后原 send 只 replay；failed 对账后由原业务调用以原 key/hash 重放 send。
- known failed 已能由原调用重放，不存在额外 reset/retry 管理接口。
- reconciliation audit 只追加；runtime DB 账号不得拥有该表 UPDATE/DELETE 权限。
...
- 点击新发送的企业微信测试消息后进入租户工作台 `/notifications`，不得跳转到 Console 管理页 `/notification-runtime`。
...
- `/runtime/health` 返回 `deliveryStore=ready`；schema 缺失时不得继续发送验收。
...
- 使用同一 `idempotencyKey` 重放时不再次调用企业微信；同键异消息返回 409。
- `partial_unknown` 不自动重发，需先核对企业微信与 ledger 状态。
