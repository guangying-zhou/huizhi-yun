...
install -d -m 0755 "$INSTALL_DIR/schema"
...
existing_store="$(existing_env_value "$env_file" HZY_NOTIFICATION_RUNTIME_STORE)"
if [[ -z "$existing_store" && -n "$(existing_env_value "$env_file" HZY_NOTIFICATION_RUNTIME_DB_HOST)" && -n "$(existing_env_value "$env_file" HZY_NOTIFICATION_RUNTIME_DB_USER)" ]]; then
  # Existing installations predate the explicit store selector. Preserve their
  # MySQL ledger unless the caller intentionally selects SQLite.
  existing_store="mysql"
fi
prompt_value HZY_NOTIFICATION_RUNTIME_STORE "Notification delivery store (sqlite/mysql)" "${existing_store:-sqlite}"
HZY_NOTIFICATION_RUNTIME_STORE="$(printf '%s' "$HZY_NOTIFICATION_RUNTIME_STORE" | tr '[:upper:]' '[:lower:]')"

case "$HZY_NOTIFICATION_RUNTIME_STORE" in
  sqlite)
    prompt_value HZY_NOTIFICATION_RUNTIME_SQLITE_PATH "Notification Runtime SQLite path" "$INSTALL_DIR/data/delivery.db"
    ;;
  mysql)
    existing_db_host="$(existing_env_value "$env_file" HZY_NOTIFICATION_RUNTIME_DB_HOST)"
    existing_db_port="$(existing_env_value "$env_file" HZY_NOTIFICATION_RUNTIME_DB_PORT)"
    existing_db_name="$(existing_env_value "$env_file" HZY_NOTIFICATION_RUNTIME_DB_NAME)"
    existing_db_user="$(existing_env_value "$env_file" HZY_NOTIFICATION_RUNTIME_DB_USER)"
    prompt_value HZY_NOTIFICATION_RUNTIME_DB_HOST "Notification Runtime MySQL host" "${existing_db_host:-127.0.0.1}"
    prompt_value HZY_NOTIFICATION_RUNTIME_DB_PORT "Notification Runtime MySQL port" "${existing_db_port:-3306}"
    prompt_value HZY_NOTIFICATION_RUNTIME_DB_NAME "Notification Runtime MySQL database" "${existing_db_name:-hzy_notification_runtime}"
    prompt_value HZY_NOTIFICATION_RUNTIME_DB_USER "Notification Runtime MySQL user" "${existing_db_user:-hzy_notification_runtime}"
    prompt_secret HZY_NOTIFICATION_RUNTIME_DB_PASSWORD "Notification Runtime MySQL password"
    ;;
  *)
    echo "HZY_NOTIFICATION_RUNTIME_STORE must be sqlite or mysql" >&2
    exit 1
    ;;
esac
...
HZY_NOTIFICATION_RUNTIME_INSTALL_DIR=$INSTALL_DIR
...
HZY_NOTIFICATION_RUNTIME_STORE=$HZY_NOTIFICATION_RUNTIME_STORE
HZY_NOTIFICATION_RUNTIME_DELIVERY_LEASE_MS=120000
HZY_NOTIFICATION_RUNTIME_PACKAGE_BASE_URL=$PACKAGE_BASE_URL
HZY_NOTIFICATION_RUNTIME_SERVICE_NAME=$SERVICE_NAME
EOF

if [[ "$HZY_NOTIFICATION_RUNTIME_STORE" == "sqlite" ]]; then
  cat >> "$env_file" <<EOF
HZY_NOTIFICATION_RUNTIME_SQLITE_PATH=$HZY_NOTIFICATION_RUNTIME_SQLITE_PATH
EOF
else
  cat >> "$env_file" <<EOF
HZY_NOTIFICATION_RUNTIME_DB_HOST=$HZY_NOTIFICATION_RUNTIME_DB_HOST
HZY_NOTIFICATION_RUNTIME_DB_PORT=$HZY_NOTIFICATION_RUNTIME_DB_PORT
HZY_NOTIFICATION_RUNTIME_DB_NAME=$HZY_NOTIFICATION_RUNTIME_DB_NAME
HZY_NOTIFICATION_RUNTIME_DB_USER=$HZY_NOTIFICATION_RUNTIME_DB_USER
HZY_NOTIFICATION_RUNTIME_DB_PASSWORD=$HZY_NOTIFICATION_RUNTIME_DB_PASSWORD
HZY_NOTIFICATION_RUNTIME_DB_CONNECTION_LIMIT=5
EOF
fi

if ! HZY_NOTIFICATION_RUNTIME_STORE="$HZY_NOTIFICATION_RUNTIME_STORE" \
  HZY_NOTIFICATION_RUNTIME_SQLITE_PATH="${HZY_NOTIFICATION_RUNTIME_SQLITE_PATH:-}" \
  HZY_NOTIFICATION_RUNTIME_DB_HOST="${HZY_NOTIFICATION_RUNTIME_DB_HOST:-}" \
  HZY_NOTIFICATION_RUNTIME_DB_PORT="${HZY_NOTIFICATION_RUNTIME_DB_PORT:-}" \
  HZY_NOTIFICATION_RUNTIME_DB_NAME="${HZY_NOTIFICATION_RUNTIME_DB_NAME:-}" \
  HZY_NOTIFICATION_RUNTIME_DB_USER="${HZY_NOTIFICATION_RUNTIME_DB_USER:-}" \
  HZY_NOTIFICATION_RUNTIME_DB_PASSWORD="${HZY_NOTIFICATION_RUNTIME_DB_PASSWORD:-}" \
  "$tmp_dir/hzy-notification-runtime" -check-store; then
  echo "Delivery ledger is not ready; the existing binary was preserved and the service was not enabled or restarted." >&2
  if [[ "$HZY_NOTIFICATION_RUNTIME_STORE" == "mysql" ]]; then
    echo "Apply $INSTALL_DIR/schema/001_notification_delivery_ledger.sql and 002_notification_delivery_reconciliation.sql in order with a controlled migration account, then rerun the installer." >&2
  else
    echo "Check that $HZY_NOTIFICATION_RUNTIME_SQLITE_PATH is on a local writable filesystem, then rerun the installer." >&2
  fi
  exit 1
fi
