...
# Publish immutable archives and mutable convenience files first. latest.json
# is the release pointer and must become visible only after every referenced
# object is readable in R2.
while IFS= read -r -d '' file; do
  upload_file "$file"
done < <(find "$PACKAGE_DIR" -type f ! -path "$PACKAGE_DIR/latest.json" -print0)

upload_file "$PACKAGE_DIR/latest.json"
