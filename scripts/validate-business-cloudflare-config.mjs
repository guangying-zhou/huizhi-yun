...
function runRenderer(module, outputPath) {
  const env = {
    ...
    ...(module.dir === 'aims' ? { HZY_AIMS_CPU_MS: '300000' } : {})
  }
...
function assertCpuLimit(config, module) {
  const cpuMs = Number(config.limits?.cpu_ms || 0)
  if (module.dir === 'aims') {
    if (cpuMs !== 300000) {
      fail(`${module.dir}: HZY_AIMS_CPU_MS must render to limits.cpu_ms=300000`)
    }
    return
  }

  if (cpuMs) {
    fail(`${module.dir}: limits.cpu_ms must be opt-in, got ${cpuMs}`)
  }
}

function assertConsoleServiceBinding(config, module) {
  const binding = (config.services || []).find(item => item.binding === 'HZY_CONSOLE_SERVICE')
  if (!binding) {
    fail(`${module.dir}: HZY_CONSOLE_SERVICE binding is required to avoid tenant-gateway token self-fetch timeouts`)
  }
  if (binding.service !== 'hzy-console-prod') {
    fail(`${module.dir}: HZY_CONSOLE_SERVICE must target hzy-console-prod, got ${binding.service || '<empty>'}`)
  }
}

function assertCrossAppServiceBindings(config, module) {
  if (!['aims', 'codocs'].includes(module.dir)) return
  const expected = module.dir === 'codocs' ? { HZY_WORKFLOW_SERVICE: 'hzy-workflow' } : {
    HZY_ALTOC_SERVICE: 'hzy-altoc',
    HZY_ASSETS_SERVICE: 'hzy-assets',
    HZY_PEOPLE_SERVICE: 'hzy-people',
    HZY_CODOCS_SERVICE: 'hzy-codocs'
  }
  ...
  for (const [binding, worker] of Object.entries(expected)) {
    if (services.get(binding) !== worker) {
      fail(`${module.dir}: ${binding} must target ${worker}, got ${services.get(binding) || '<missing>'}`)
    }
  }
}
...
try {
  for (const module of MODULES) {
    ...
    assertCpuLimit(config, module)
    assertConsoleServiceBinding(config, module)
    assertCrossAppServiceBindings(config, module)
  }
