...
function validateConsoleProdTest(envs, reporter, options = {}) {
  ...
  for (const [config, label] of [[prod, 'console-prod'], [test, 'console-test']]) {
    if (consoleCacheBackend(config) === 'db') {
      reporter.error(`${label} must not use DB runtime cache; use file for PM2 or memory for Cloudflare`)
    }
    for (const key of ['DB_HOST', 'DB_PORT', 'DB_USER', 'DB_PASSWORD', 'DB_NAME', 'HZY_CONSOLE_HYPERDRIVE_ID', 'HZY_CONSOLE_VAULT_MASTER_KEY', 'CONSOLE_AUTH_SIGNING_PRIVATE_JWK']) {
      reporter.requireBlank(config, key, `${label} Tenant Runtime-owned configuration`)
    }
  }
...
function printSummary(envs, reporter) {
  ...
  console.info(`  - console-prod: data=tenant-runtime, platform=${consolePlatformUrl(envs.consoleProd) || '<empty>'}, deployment=${consoleDeploymentCode(envs.consoleProd) || '<empty>'}, pm2=${consolePm2Name(envs.consoleProd)}, listen=${consoleHost(envs.consoleProd)}:${consolePort(envs.consoleProd)}, cache=${consoleCacheBackend(envs.consoleProd)}:${consoleCacheDir(envs.consoleProd) || consoleCacheScope(envs.consoleProd) || '<memory>'}, collab=${effectiveConsoleCollabMode(envs.consoleProd)}, authClients=${consoleAuthClientMaterializeMode(envs.consoleProd)}, authKeyAutogen=${consoleAuthSigningKeyAutogenerate(envs.consoleProd)}, authKeyRotate=${consoleAuthSigningKeyRotateUnusable(envs.consoleProd)}`)
  console.info(`  - console-test: data=tenant-runtime, platform=${consolePlatformUrl(envs.consoleTest) || '<empty>'}, deployment=${consoleDeploymentCode(envs.consoleTest) || '<empty>'}, pm2=${consolePm2Name(envs.consoleTest)}, listen=${consoleHost(envs.consoleTest)}:${consolePort(envs.consoleTest)}, cache=${consoleCacheBackend(envs.consoleTest)}:${consoleCacheDir(envs.consoleTest) || consoleCacheScope(envs.consoleTest) || '<memory>'}, collab=${effectiveConsoleCollabMode(envs.consoleTest)}, authClients=${consoleAuthClientMaterializeMode(envs.consoleTest)}, authKeyAutogen=${consoleAuthSigningKeyAutogenerate(envs.consoleTest)}, authKeyRotate=${consoleAuthSigningKeyRotateUnusable(envs.consoleTest)}`)
