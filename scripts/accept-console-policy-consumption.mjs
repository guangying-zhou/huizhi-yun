 ...
 * UI 行为核对见 console/docs/Console-Policy-Authorization-Acceptance-Runbook.md。
 */
...
function snapshotAllows(data, resource, action) {
  return authorizationResourcesAllow(
    data.resources || {},
    resource,
    action,
    data.actionPolicies?.[resource]
  )
}
...
async function main() {
  ...
  check(
    snapshotAllows(data, args.resource, args.action),
  ...
  if (deniedCookie) {
    ...
    if (denied.status === 200 && denied.json?.code === 0) {
      ...
      check(
        !snapshotAllows(deniedData, args.resource, args.action),
  ...
  info('提示：菜单过滤与路由守卫的 UI 核对步骤见 console/docs/Console-Policy-Authorization-Acceptance-Runbook.md')
}
