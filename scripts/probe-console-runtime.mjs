...
function validateDiagnostics(target, data, expected) {
  ...
  if (expected.cacheBackend === 'memory') {
...
async function probeTarget(target, baseUrl, token, expected) {
  ...
  console.info(`[console-runtime] ${target}: url=${normalizeUrl(baseUrl)}, runMode=${data.runtime?.runMode}, runtime=${data.runtime?.runtimeEnabled ? 'enabled' : 'disabled'}, tenantRuntime=${data.database?.connected ? 'connected' : 'unavailable'}, publicUrl=${data.process?.deploymentPublicUrl || '<empty>'}, platform=${data.platform?.baseUrl || '<disabled>'}, deployment=${data.platform?.deploymentCode || '<none>'}, cache=${data.cache?.backend}:${data.cache?.cacheDir || data.cache?.scope || '<memory>'}, collab=${data.collab?.mode || '<unknown>'}/${data.collab?.status || '<unknown>'}, authClients=${data.auth?.clientMaterializeMode || '<unknown>'}, authKey=${data.auth?.signingKey?.kid || '<none>'}/${data.auth?.signingKey?.privateKeyUsable ? 'usable' : 'unusable'}, authKeyAutogen=${data.auth?.signingKeyAutogenerate}, authKeyRotate=${data.auth?.signingKeyRotateUnusable}, bundle=${data.bundle?.bundleVersion || '<none>'}`)
