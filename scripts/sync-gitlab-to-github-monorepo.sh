...
looks_like_source_workspace() {
  local path="$1"
  [[ -d "$path/.git" && -f "$path/pnpm-workspace.yaml" && -d "$path/aims" && -d "$path/console" ]]
}
...
echo "Building public snapshot from indexed monorepo files at $SOURCE_WORKSPACE"
copy_index_snapshot "$SOURCE_WORKSPACE" "$staging_dir" "monorepo"
