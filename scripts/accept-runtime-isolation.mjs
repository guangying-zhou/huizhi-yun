...
function assertStrictCoverage(args) {
  ...
  if (hasAnyValue(args.consoleDb)) missing.push('remove all --console-db-* options; Console acceptance must not connect to the tenant database')
  if (
    args.publicRouting
...
async function runStaticValidation(args) {
  ...
  await runCommand('Console zero-DB cutover validation', 'node', [
...
async function runPlatformCloudflareDefaultValidation() {
  await runCommand('static Platform Cloudflare default validation', 'node', ['scripts/validate-platform-cloudflare-default.mjs'])
}
...
async function runConsoleRuntimeCacheVerify(args) {
  if (hasAnyValue(args.consoleDb)) {
    throw new Error('Console DB verification options are retired; use Runtime health/schema probes and zero-DB cutover verification')
  }
}
...
async function main() {
  ...
  await runPlatformCloudflareDefaultValidation()
