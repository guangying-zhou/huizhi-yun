...
onMounted(async () => {
  ...
  const redirectUrl = redirectTarget.value.startsWith('http://') || redirectTarget.value.startsWith('https://')
    ? redirectTarget.value
    : `${window.location.origin}${redirectTarget.value}`
  const params = new URLSearchParams({ redirect: redirectUrl })
  window.location.assign(`/api/auth/oidc-login?${params.toString()}`)
})
</script>
