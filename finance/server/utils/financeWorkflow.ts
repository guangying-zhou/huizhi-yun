...
export async function createWorkflowApprovalInstance(
  ...
  const actorUid = cleanString(getRequestUid(event))
  if (!actorUid) throw new Error('Verified Finance actor is required for Workflow submission')
  ...
  return await requestWithServiceAccessToken({
    audience: 'workflow', scope: 'workflow:proxy', event,
    async request(token) {
      const headers = {
        'authorization': `Bearer ${token}`,
        'content-type': 'application/json',
        'x-hzy-actor-uid': actorUid,
        // Must match the Finance source identity in the Console service token.
        'x-hzy-request-app-code': 'finance'
      }
      const prepare = await $fetch<WorkflowPrepareResponse>(`${baseUrl}/api/v1/instances/prepare`, {
        method: 'POST', headers,
        body: {
          app_code: 'finance', resource_code: actionConfig.resourceCode, action_code: actionConfig.actionCode,
          biz_id: cleanString(row.code), biz_title: bizTitle, biz_context: bizContext, form_data: formData
        }, timeout: 10000
      })

      if (prepare.code !== 0) throw new Error(prepare.message || `Workflow prepare failed: code=${prepare.code ?? 'unknown'}`)

      const actionDefId = prepare.data?.action_def?.id
      const routeId = prepare.data?.matched_routes?.[0]?.id
      if (!actionDefId || !routeId) throw new Error('Workflow 未匹配到可用审批路由')

      const created = await $fetch<WorkflowCreateResponse>(`${baseUrl}/api/v1/instances`, {
        method: 'POST', headers,
        body: {
          action_def_id: actionDefId, route_id: routeId, biz_id: cleanString(row.code), biz_title: bizTitle,
          biz_url: `${financePublicBaseUrl()}${actionConfig.listPath}`, biz_context: bizContext, form_data: formData,
          attachments: [], callback_url: `${financePublicBaseUrl()}/api/v1/finance/workflow/callback`
        }, timeout: 10000
      })

      if (created.code !== 0) throw new Error(created.message || `Workflow create failed: code=${created.code ?? 'unknown'}`)

      const workflowInstanceId = cleanString(created.data?.instance_id)
      if (!workflowInstanceId) throw new Error('Workflow 创建实例成功但未返回 instance_id')

      return {
        workflowInstanceId, externalInstanceId: cleanString(created.data?.instance_no), platform: 'workflow',
        status: cleanString(created.data?.status) || 'running', errorMessage: null
      }
    }
  })
}

function buildBizTitle(row: RuntimeRow, target: ApprovalTargetConfig) {
...
function buildBizContext(target: ApprovalTargetConfig, row: RuntimeRow, body: Record<string, unknown>) {
  return {
    app_code: 'finance',
    biz_type: target.bizType,
    biz_code: cleanString(row.code),
...
function buildFormData(row: RuntimeRow, body: Record<string, unknown>) {
