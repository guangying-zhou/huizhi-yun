...
export function getFinanceConsoleAuth(event: H3Event) {
  return event.context.consoleAuth as ConsoleAuthContext | undefined
}

export async function ensureFinanceConsoleAuth(event: H3Event) {
  const existing = getFinanceConsoleAuth(event)
...
export function getRequestUid(event: H3Event) {
  const consoleAuth = getFinanceConsoleAuth(event)
