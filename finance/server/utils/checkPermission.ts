...
type PermissionAction = 'view' | 'edit' | 'approve' | 'issue' | 'confirm' | 'export' | 'replay' | 'admin'

async function loadAuthorizationSnapshot(
  event: H3Event,
  uid: string
): Promise<Pick<RuntimeAuthorizationSnapshot, 'resources' | 'actionPolicies'>> {
  if (isDevPermissionBypassEnabled(event)) {
    return {
      resources: {
        dashboard: ['view', 'edit', 'admin'],
        invoices: ['view', 'edit', 'approve', 'issue', 'admin'],
      ...
      actionPolicies: {} as NonNullable<RuntimeAuthorizationSnapshot['actionPolicies']>
    }
  }
