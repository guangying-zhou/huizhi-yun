...
import { createWorkflowApprovalInstance } from './financeWorkflow'
...
import {
  ...
  resolveFinanceResponsibilityAccessQuery
...
function sanitizeWorkflowSubmitPayload(body: unknown) {
  const workflowKeys = new Set([
    ...
    'approverUids',
    'approver_uids',
    'nonSelfApprovalActorUids',
    'non_self_approval_actor_uids',
    'approvalOperatorUid',
    'approval_operator_uid'
  ])
  const payload: Record<string, unknown> = {}
  for (const [key, value] of Object.entries(objectBody(body))) {
...
async function prepareMutationBody(event: H3Event, path: string, method: DataRuntimeMethod, body: unknown) {
  ...
  const payload = sanitizeWorkflowSubmitPayload(body)
  ...
  try {
    ...
    const submission = await createWorkflowApprovalInstance(event, submitTarget.target, row, payload)
    return {
      ...payload,
    ...
    throw createError({
      ...
      message: 'Workflow approval instance is required before submitting Finance approval documents.'
    })
  }
}
...
const runtimeAuthKeys = new Set([
  ...
  'currentUserInvoiceRequestAccess',
  'current_user_receipt_access',
  'currentUserReceiptAccess',
  // 付款/确认经办人身份键：runtime 的职责分离检查
  // （payment_confirmation_duty_separation.go）取 actor 时，这些键**优先于**
  // BFF 注入的受信 operator_uid。它们又都不是业务表的列，所以伪造后既不落库
  // 也不报错，唯一作用就是让「制单人 ≠ 付款人」检查取到假 actor 而放行。
  // 必须在 BFF 删除，由受信上下文重建（走查 ISSUE-B-004）。
  // 这六个键都不是任何业务表的列，删除不影响落库内容。
  'paymentConfirmedBy',
...
function invoiceResponsibilityAction(path: string, method: string) {
  if (method === 'GET' && (path === '/v1/finance/invoice-requests' || /^\/v1\/finance\/invoice-requests\/[^/]+$/.test(path))) {
    return 'view' as const
  }
  if (method === 'POST' && /^\/v1\/finance\/invoice-requests\/[^/]+\/issue$/.test(path)) {
    return 'issue' as const
  }
  return null
}

function receiptResponsibilityAction(path: string, method: string) {
  if (method === 'GET' && (path === '/v1/finance/receipts' || /^\/v1\/finance\/receipts\/[^/]+$/.test(path))) {
    return { resource: 'receipts' as const, action: 'view' as const }
  }
  if ((method === 'PATCH' || method === 'PUT') && /^\/v1\/finance\/receipts\/[^/]+$/.test(path)) {
    return { resource: 'receipts' as const, action: 'confirm' as const }
  }
  if (method === 'POST' && path === '/v1/finance/reconciliation') {
    return { resource: 'reconciliation' as const, action: 'confirm' as const }
  }
  return null
}
...
export async function buildFinanceRuntimeAuthQuery(
  ...
  const invoiceAction = invoiceResponsibilityAction(path, method)
  if (invoiceAction) {
    Object.assign(query, await resolveFinanceResponsibilityAccessQuery(event, uid, 'invoices', invoiceAction, 'invoice_request'))
  }
  const receiptAction = receiptResponsibilityAction(path, method)
  if (receiptAction) {
    Object.assign(query, await resolveFinanceResponsibilityAccessQuery(event, uid, receiptAction.resource, receiptAction.action, 'receipt'))
  }
  return query
}
...
export function buildFinanceRuntimeAuthBody(path: string, method: string, body: unknown, authQuery: Record<string, unknown>) {
  ...
  const invoiceRequestAccess = stringValue(authQuery.current_user_invoice_request_access)
  if (invoiceRequestAccess) payload.current_user_invoice_request_access = invoiceRequestAccess
  const receiptAccess = stringValue(authQuery.current_user_receipt_access)
  if (receiptAccess) payload.current_user_receipt_access = receiptAccess
...
export async function maybeCallFinanceDataRuntime<T>(
  ...
  // 必须整体透传调用方选项：此前这里逐字段枚举 scope/method/query/body，
  // 把 serviceCommandActor、idempotencyKey、serviceTokenSourceBinding、timeout
  // 等全部静默丢掉。类型是 Omit<TenantRuntimeCallOptions, 'appCode'>，调用方传
  // 这些字段不会有任何类型错误，因此丢弃完全无声。
  //
  // 走查 ISSUE-B-025 实测后果：开票申请 handler 传了 serviceCommandActor，
  // 被丢弃后 actor 委托签名头根本没发出去，data-runtime 回落到服务主体、
  // 清空 current_user，最终 403 trusted_actor_mismatch。
  return maybeCallTenantRuntime<T>(event, path, { ...options, appCode: 'finance' })
}
...
function isFinanceWriteMethod(method: DataRuntimeMethod) {
...
export async function maybeCallCurrentFinanceDataRuntime<T>(event: H3Event): Promise<DataRuntimeSkipped | DataRuntimeHandled<T>> {
  ...
  const runtime = await maybeCallFinanceDataRuntime<T>(event, path, { scope, method: normalizedMethod, query, body })
  if (!runtime.handled && isFinanceWriteMethod(normalizedMethod)) {
    throw createError({
      statusCode: 503,
      message: 'Finance tenant-runtime is required for Finance write API mutations.'
    })
  }
  return runtime
}
