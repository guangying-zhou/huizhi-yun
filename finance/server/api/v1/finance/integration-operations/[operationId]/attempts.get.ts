...
export default defineEventHandler(async (event) => {
  ...
  const data = await callFinanceIntegrationOperationAdmin<Record<string, unknown>>(event, `/v1/finance/integration-operations/${encodeURIComponent(operationId)}/attempts`, 'view', { query: { limit: 100 } })
