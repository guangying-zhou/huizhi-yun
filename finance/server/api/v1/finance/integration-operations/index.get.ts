...
export default defineEventHandler(async (event) => {
  ...
  for (const key of ['status', 'limit', 'cursor']) if (source[key] !== undefined) query[key] = source[key]
  const data = await callFinanceIntegrationOperationAdmin<Record<string, unknown>>(event, '/v1/finance/integration-operations', 'view', { query })
