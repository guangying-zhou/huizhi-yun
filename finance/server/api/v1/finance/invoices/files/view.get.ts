...
function sourceMatchesInvoiceFile(
  invoiceSource: string,
  requestedSource: string,
  config: Awaited<ReturnType<typeof getOssIntegrationConfig>> | null,
  requestedObjectKey = ''
) {
  if (!invoiceSource) return false
  if (invoiceSource === requestedSource) return true

  const invoiceUrl = normalizedUrl(invoiceSource)
  const requestedUrl = normalizedUrl(requestedSource)
  if (invoiceUrl && requestedUrl && invoiceUrl === requestedUrl) return true

  if (!config || !requestedObjectKey) return false

  try {
    return extractObjectKey(invoiceSource, config) === requestedObjectKey
  } catch {
    return normalizeObjectKey(invoiceSource) === requestedObjectKey
  }
}

async function assertInvoiceFileAccess(
  event: H3Event,
  invoiceCode: string,
  requestedSource: string,
  options: {
  ...
  if (!invoiceCode) {
    throw createError({ statusCode: 400, statusMessage: '缺少发票编码' })
  }

  const path = `/v1/finance/invoices/${encodeURIComponent(invoiceCode)}`
  const runtime = await maybeCallFinanceDataRuntime<RuntimeEnvelope<InvoiceRow>>(
    event,
    path,
    {
  ...
  if (!runtime.handled) {
    throw createError({ statusCode: 503, statusMessage: 'Finance tenant-runtime is required for invoice file previews.' })
  }

  const invoice = runtime.data.data
  if (!invoice) {
    throw createError({ statusCode: 404, statusMessage: '发票不存在' })
  }

  if (!sourceMatchesInvoiceFile(
...
export default defineEventHandler(async (event) => {
  ...
  const invoiceCode = stringValue(query.code || query.invoiceCode || query.invoice_code)
