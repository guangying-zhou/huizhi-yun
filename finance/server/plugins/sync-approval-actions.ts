...
export default defineNitroPlugin(() => {
  ...
  setTimeout(async () => {
    ...
    try {
      ...
      console.log(`[Finance SyncApprovalActions] Synced to Workflow: ${result.created} new, ${result.updated} updated, ${result.deprecated} deprecated`)
      if (result.errors?.length) {
        console.warn('[Finance SyncApprovalActions] Errors:', result.errors)
      }
    } catch {
      console.log('[Finance SyncApprovalActions] Workflow 不可达，跳过审批动作同步')
    }
  }, 5000)
})
