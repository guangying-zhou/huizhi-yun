...
DROP TABLE IF EXISTS finance_notification_checkpoint;
...
CREATE TABLE invoice_request (
    ...
    issuance_responsible_uid VARCHAR(50) DEFAULT NULL COMMENT '已批开票唯一直接责任人UID',
    issuance_due_at         DATETIME DEFAULT NULL COMMENT '已批开票办理时限',
    remark                  VARCHAR(500) DEFAULT NULL COMMENT '备注',
    created_by              VARCHAR(50) DEFAULT NULL COMMENT '创建人',
    updated_by              VARCHAR(50) DEFAULT NULL COMMENT '更新人',
    created_at              DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    ...
    CONSTRAINT ck_invoice_request_issuance_responsibility CHECK (
      (issuance_responsible_uid IS NULL AND issuance_due_at IS NULL)
      OR (
        issuance_responsible_uid IS NOT NULL AND issuance_due_at IS NOT NULL
        AND issuance_responsible_uid = TRIM(issuance_responsible_uid)
        AND issuance_responsible_uid <> ''
        AND LOWER(issuance_responsible_uid) <> '@all'
        AND issuance_responsible_uid NOT REGEXP '[[:cntrl:]]'
      )
    )
...
CREATE TABLE finance_receipt (
    ...
    reconciliation_responsible_uid VARCHAR(50) DEFAULT NULL COMMENT '未核销事项唯一直接责任人UID',
    reconciliation_due_at  DATETIME DEFAULT NULL COMMENT '到账核销办理时限',
    ...
    CONSTRAINT ck_finance_receipt_reconciliation_responsibility CHECK (
      (reconciliation_responsible_uid IS NULL AND reconciliation_due_at IS NULL)
      OR (
        reconciliation_responsible_uid IS NOT NULL AND reconciliation_due_at IS NOT NULL
        AND reconciliation_responsible_uid = TRIM(reconciliation_responsible_uid)
        AND reconciliation_responsible_uid <> ''
        AND LOWER(reconciliation_responsible_uid) <> '@all'
        AND reconciliation_responsible_uid NOT REGEXP '[[:cntrl:]]'
      )
    )
...
-- ------------------------------------------------------------
-- 2.7.1 开票/核销到期通知可靠检查点
-- ------------------------------------------------------------
CREATE TABLE finance_notification_checkpoint (
    ...
    INDEX idx_finance_notification_source (event_stream, source_type, source_id, state, condition_generation),
    INDEX idx_finance_notification_closure (event_stream, state, close_reason, lifecycle_closed_at, closed_at),
    INDEX idx_finance_notification_code (source_type, source_code, state, condition_generation),
...
CREATE TABLE project_finance_summary (
    ...
    gross_profit_amount     DECIMAL(18,2) DEFAULT NULL COMMENT '毛利额；成本未就绪时必须为NULL',
    gross_margin_rate       DECIMAL(8,4) DEFAULT NULL COMMENT '毛利率',
    cost_readiness_status   VARCHAR(20) NOT NULL DEFAULT 'not_ready' COMMENT '成本就绪状态：ready/not_ready',
    cost_readiness_reasons_json JSON DEFAULT NULL COMMENT '成本未就绪原因和同步证据',
    cost_input_hash         CHAR(64) DEFAULT NULL COMMENT '规范化成本输入SHA-256',
    cost_readiness_checked_at DATETIME(6) DEFAULT NULL COMMENT '最近完整成本预检/同步时间',
    calculated_at           DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '计算时间',
...
CREATE TABLE project_cost_allocation (
    ...
    currency_code       CHAR(3) DEFAULT NULL COMMENT '明确记录的分摊币种；历史未知保持NULL，不推断或换汇',
    ...
    source_refs_json    JSON DEFAULT NULL COMMENT '分摊输入与来源快照',
    status              VARCHAR(30) DEFAULT 'active' COMMENT '状态：active/reversed',
    created_by          VARCHAR(50) DEFAULT NULL COMMENT '创建人',
    created_at          DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at          DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
-- G3 可靠投递增量：Finance 作为调用方新增 integration_operation 与
-- integration_operation_attempt；完整、可重复执行 DDL 见
-- docs/migrations/20260710_finance_integration_operations.sql。
-- 核销事实与对应 operation 必须在同一事务提交；目标 Altoc 的
-- service_command_receipt 由各目标应用自身持有；Finance 目标收据如下。

-- G4 reliable dead-letter actionable lifecycle. Upgrade tenants use
-- docs/migrations/20260711_finance_dead_letter_actionable_lifecycle.sql.
CREATE TABLE IF NOT EXISTS integration_operation_dead_letter_actionable (
  ...
  UNIQUE KEY uk_fin_iopdla_actionable (tenant_code, deployment_code, source_app, actionable_key),
  UNIQUE KEY uk_fin_iopdla_notification (tenant_code, deployment_code, source_app, notification_id),
  INDEX idx_fin_iopdla_publish (tenant_code, deployment_code, source_app, publish_acked_at, dead_lettered_at),
  INDEX idx_fin_iopdla_closure (tenant_code, deployment_code, source_app, closure_acked_at, closure_pending_at),
  CONSTRAINT fk_fin_iopdla_operation FOREIGN KEY (operation_id) REFERENCES integration_operation(operation_id) ON DELETE RESTRICT,
  CONSTRAINT chk_fin_iopdla_closure CHECK (closure_state IS NULL OR closure_state IN ('resolved', 'cancelled')),
  CONSTRAINT chk_fin_iopdla_recipient CHECK (recipient_uids IS NULL OR JSON_TYPE(recipient_uids) = 'ARRAY')
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Finance dead-letter actionable lifecycle';
...
-- Product center period cost attribution (PC18)
