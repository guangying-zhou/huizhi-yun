...
finance/
  ...
  server/
    ...
    utils/
      db.ts                  # disabled stub only; Finance BFF must not connect to MySQL
      dataRuntime.ts         # tenant-runtime forwarding and scoped query injection
      financeScopedAuthorization.ts
      financePermissionRoutes.ts
      financeApproval.ts
      financeWorkflow.ts
      financeInstanceConflictExplanation.ts
    repositories/
    services/
    jobs/
  docs/
...
实现时优先复用现有模块模式，例如权限校验、审批动作同步和审计日志的实现方式，但不要把 Altoc 的经营概念复制到 Finance。Finance Nuxt BFF 不保留本地 MySQL fallback，所有 `/api/v1/finance/**` 业务数据读写必须经 `server/middleware/tenant-runtime.ts` 转发到 tenant-runtime；runtime 未命中时应 fail closed。

Finance 用户态 runtime 请求必须由 Foundation 以当前已验证用户的 actor delegation 重签，并将经 Console scoped authorization 求值的费用、项目、绩效、开票/到账责任范围放入被签名的完整 request target。tenant-runtime 只接受该 delegation；浏览器 body/query 的 `current_user`、`operator_uid`、`current_user_*_access`、项目或部门范围均不是事实。没有有效 delegation 时普通用户路径在 SQL/mutation 前失败关闭；`/service/**`、Workflow callback 和 scheduled notification 只走各自明确的 service/worker 合同，不能把 service client 作为用户 actor。due-notification scan/ack/closure-ack 专用 worker 仅使用 `finance.runtime` 的短期精确 grant、tenant/deployment enrollment 与 request-target HMAC purpose；不接受 `finance.write`、static token 或任意 body actor/lifecycle 断言。
...
| `/invoices/{code}/receipt-reconcile` | POST | 发票快捷登记到账并生成核销 |
...
| `/authorization/instance-conflict-explain` | POST | 当前用户费用/付款对象实例级职责冲突解释；Finance 先从 tenant-runtime 读取对象事实，再通过 Foundation 调 Console/Platform 解释自审批、同人经办审批和付款确认风险；费用报销、项目支出审批、付款申请和支出台账页面已提供行内解释入口 |
| `/workflow/actions/sync` | POST | 同步 Finance 审批动作 |
| `/workflow/callback` | POST | 接收 Workflow 审批回调并转发 tenant-runtime 处理 |
...
2. Finance BFF 将回调转发 tenant-runtime；runtime 未命中时返回 503，不执行 Nuxt 本地 SQL fallback。
3. tenant-runtime 根据 `workflow_instance_id` 和 `biz_type` 查找单据。
4. tenant-runtime 幂等写入 `approval_callback_log` 并更新单据状态。
5. 审批通过后由 tenant-runtime 按单据类型落账到 `finance_invoice` 或 `finance_expense`。
...
6. 实现提交审批：Finance BFF 调用 Workflow runtime 创建 instance，并把真实 `workflow_instance_id` 转发 tenant-runtime 保存；Workflow 创建失败或 tenant-runtime 未命中时返回 503，Nuxt BFF 不生成本地 `local` 审批实例，也不保留本地 `pending_approval` 写库分支。tenant-runtime `SubmitApproval` 只允许进入 `pending_approval`，审批通过/驳回/取消只能通过 Workflow callback 服务令牌路径触发。
7. 实现 Workflow 回调：接收 `instance_id/biz_id/resource_code/action_code` 回调字段，由 BFF 校验 service token 后转发 tenant-runtime 幂等记录、更新单据状态；Nuxt BFF 不保留本地 SQL fallback。
8. 实现审批通过落账：开票申请只进入 `approved` 待开票状态，必须由独立 `invoices:issue` 动作生成正式发票；报销/项目支出/付款仍按各自审批合同生成支出记录。
...
| People 职级、Console 日历、Finance 参数或 Aims 工时不足 | v0.3 毛利不准确或继续展示旧结果 | 单一 runtime 事务动作持久化 not_ready、反转旧 managed labor，并将毛利/毛利率置空；补齐输入后以 input hash 重放完整集合 |
| 核销撤销影响合同摘要 | Altoc 展示错误 | 核销和撤销都触发摘要重算，支持后台全量重算 |
| 财务权限过宽 | 敏感数据泄露 | 按资源动作和部门/项目范围控制，默认最小权限；direct `approved/issued/paid/confirmed` 状态写入必须命中 `approve` 或 `confirm`；费用/付款对象自审批风险通过 `/authorization/instance-conflict-explain` 基于真实 runtime 对象事实解释，并在费用/付款页面行内展示 |
