...
test('cost rules route requires its dedicated handler without generic forwarding fallback', async () => {
  for (const denied of [false, true]) {
    ...
    runInNewContext(compiled, { exports, defineEventHandler: (handler: unknown) => handler, require: (name: string) => {
      if (name === 'h3') return { createError, getRequestURL: () => ({ pathname: '/api/v1/finance/service/product-cost/replace-rules' }) }
      if (name.endsWith('/productCostRulesService')) return { handleProductCostRulesService: async () => {
        calls++
