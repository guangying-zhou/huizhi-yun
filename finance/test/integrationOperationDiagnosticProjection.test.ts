...
test('Finance browser diagnostic routes use the shared allow-list projection', () => {
  const list = read('server/api/v1/finance/integration-operations/index.get.ts')
  const attempts = read('server/api/v1/finance/integration-operations/[operationId]/attempts.get.ts')
  const replay = read('server/api/v1/finance/integration-operations/[operationId]/replay.post.ts')
