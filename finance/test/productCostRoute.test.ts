...
test('cost route requires its dedicated handler without generic forwarding fallback', async () => {
  for (const denied of [false, true]) {
    ...
    runInNewContext(compiled, { exports, defineEventHandler: (handler: unknown) => handler, require: (name: string) => {
      if (name === 'h3') return { createError, getRequestURL: () => ({ pathname: '/api/v1/finance/service/product-cost/read' }) }
      if (name.endsWith('/productCostService')) return { handleProductCostService: async () => {
        calls++
