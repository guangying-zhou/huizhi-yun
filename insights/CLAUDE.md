...
- 使用 `package.json` 中的脚本：`pnpm --dir insights dev`、`build`、`preview`、`lint`、`typecheck`、`test`。开发读取 `.env.dev`，构建读取 `.env`；当前没有 Drizzle、独立 Worker 或多品牌构建脚本。
- 测试使用 Vitest，位于 `tests/**/*.test.ts`，验证范围遵循根 Execution Style。修改保留的 `hostClassifier` 工具时运行其现有测试；该工具的存在不代表模块仍运行多租户路由。
...
- Brevo 仅用于本地认证的登录/密码重置验证码邮件，不是业务通知通道。保持固定
  收件人/主题/模板的 purpose-limited adapter；在 Insights 接入 Foundation 认证前，
  不要硬迁到业务 notification-runtime 协议，也不得扩展为任意邮件发送器。
