...
const rawNavigationItems: NavigationItem[] = [
  {
    label: '概览',
...
const navigationItems = computed(() => rawNavigationItems.map(item => ({
  ...item,
  defaultOpen: item.children?.length ? isRouteInMenu(item, route.path) : undefined
})))
</script>
