...
export default defineEventHandler(async (event) => {
  requireInsightsApiPermission(event, resolveInsightsSyncApiPermission('contributors', event.node.req.method || 'POST'))
  ...
  for (const user of users) {
    ...
    if (user.deptCode) {
      const dept = await queryRow<DepartmentIdRow>(
    ...
    const existing = await queryRow<PersonIdRow>(
