...
test('IP Assets BFF binds exact ip_assets:view scope before its separate read capability', async () => {
  ...
  globalThis.__ipAssetsReadTransport = async (_event, path, options) => { calls.push({ path, options }); return { handled: true, data: { code: 0, data: { items: [] } } } }
  ...
  try {
    ...
    const request = async path => { const response = await fetch(`http://127.0.0.1:${server.address().port}${path}`); return { status: response.status, body: await response.json() } }
    denied = true
    const deniedResponse = await request('/ip-assets')
    assert.equal(deniedResponse.status, 403); assert.equal(deniedResponse.body.data?.code, 'person_permission_denied'); assert.equal(calls.length, 0)
    denied = false
    assert.equal((await request('/ip-assets?status=active&page=2&pageSize=20')).status, 200)
    const list = calls.at(-1)
    assert.equal(globalThis.__ipAssetsReadPrepared.at(-1), 'assets.ip-assets-list'); assert.equal(list.path, 'assets.ip-assets-list')
    assert.equal(list.options.authorization.actorUid, session.uid); assert.equal(list.options.authorization.resource, 'ip_assets'); assert.equal(list.options.authorization.action, 'view')
    assert.equal(list.options.authorization.scope.current_user_assets_object_access, 'relation'); assert.equal(list.options.query.status, 'active')
    assert.equal((await request('/ip-assets?current_user=forged')).status, 400); assert.equal((await request('/ip-assets/0')).status, 400)
    assert.equal((await request('/ip-assets/42')).status, 200); assert.equal(calls.at(-1).path, 'assets.ip-assets-view')
    assert.equal(globalThis.__ipAssetsReadPrepared.at(-1), 'assets.ip-assets-view')
    assert.ok(checks.every(check => check.uid === session.uid && check.app === 'assets' && check.resourceCode === 'ip_assets' && check.action === 'view'))
  } finally {
