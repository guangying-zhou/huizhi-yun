...
test('pilot Host routes contain no parallel SQL or database state machine', () => {
  ...
  assert.equal(routes.filter(path => path.includes('/aims/')).length, 64)
  assert.equal(routes.filter(path => path.includes('/assets/')).length, 27)
...
test('every pilot route is a thin shared handler or declared Enterprise adapter', () => {
  ...
  const declaredAims = /enterpriseProduct(List|Onboard|Workspace|RequestRead|HandoffCandidates|Components)|'assets\.product-adoption-read'/
  const declaredAssets = /handleEnterprise(?:Assets(Products|Read|Categories|Links)|DigitalAssets(?:Read|Write)|IPAssets(?:Read|Write))/
...
test('31 Enterprise utility routes have explicit old-entry and Runtime-domain evidence', () => {
  ...
  const mappings = [
    ...
    ['enterpriseIPAssetsRead', 2, 'assets/server/api/v1/ip-assets/index.get.ts', 'data-runtime/internal/apps/assets/ip_asset_commands.go'],
