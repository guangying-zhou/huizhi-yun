...
function bff(mode = '', action = 'complete') {
  ...
  runInNewContext(compiled, { exports, require: (name: string) => {
    ...
    if (name.endsWith('/enterpriseRuntimeClient')) return { requireEnterpriseUser: async () => ({ uid: 'U1', tenant: 'T1', deployment: 'HOST' }), enterpriseRuntimePermitExpiresAt: () => 12345, prepareEnterpriseRuntime: async (_event: any, operation: string) => { calls.push({ prepared: operation }) }, callEnterpriseRuntime: async (_event: any, operation: string, body: any, options: any) => { calls.push({ operation, body, options }); return { accepted: true } } }
    if (name.endsWith('/platformBundleAuthorization')) return { loadScopedAuthorizationFromConsoleRuntime: async (_event: any, _uid: string, app: string, options: any) => { calls.push({ app, options }); return { decision: { allowed: mode !== 'denied' } } } }
    if (name === './enterpriseAimsPersonnel') return { enterpriseAimsPersonnel: async () => { throw Error('Completion must not borrow a Directory identity or personnel permit') } }
    throw Error(name)
  } })
...
test('missing user grant or raw status prevents preparation and Runtime dispatch', async () => {
  for (const mode of ['denied', 'raw-status']) { const { execute, calls } = bff(mode); await assert.rejects(execute()); assert.equal(calls.some(call => call.prepared || call.operation), false) }
})
