...
test('same local product path belongs to distinct prefixed routes', () => {
  ...
  assert.deepEqual(pages.map(page => page.path), businessModules.flatMap(module => [module.prefix, `${module.prefix}/products`]))
  assert.equal(new Set(pages.map(page => page.name)).size, pages.length)
})
test('every composed business route explicitly selects the Enterprise shell', () => {
  const pages = registerBusinessPages([], businessModules, 'entry.vue')
  const visit = routes => routes.flatMap(page => [page, ...visit(page.children || [])])
  for (const page of visit(pages)) assert.equal(page.meta.layout, 'enterprise', `${page.path} must use the Enterprise layout`)
})
...
test('product workspace and version plans preserve nested shells and metadata', () => {
  ...
  assert.deepEqual(workspace.children.map(page => page.path), ['', 'structure', 'features', 'components', 'adoption', 'documents', 'cycles', 'features/:featureId', 'features/:featureId/requests', 'features/:featureId/lifecycle', 'features/:featureId/roadmap', 'requests', 'planning-items/:itemId/handoff', 'execution-coordination', 'planning', 'versions', 'versions/:versionId'])
  const version = workspace.children.find(page => page.path === 'versions/:versionId')
  assert.deepEqual(version.children.map(page => page.path), ['', 'acceptance', 'acceptances', 'acceptances/:acceptanceId', 'releases', 'releases/:recordId', 'plan', 'features'])
...
test('Codocs registers the nine non-slide mydocs pages and defers slides', () => {
  const pages = registerBusinessPages([], businessModules, 'entry.vue')
  const codocs = pages.filter(page => page.path.startsWith('/codocs'))
  const expected = [
    '/codocs/mydocs',
  ...
  for (const route of expected) assert.ok(codocs.some(page => page.path === route), `missing Codocs route: ${route}`)
  ...
  assert.equal(codocs.find(page => page.path === '/codocs/mydocs/weekly-reports')?.file, journalFile)
})
...
test('every registered Host page derives its cached keys from the session scope', () => {
  ...
  for (const file of files) {
    ...
    const scopedKeyNames = new Set([...source.matchAll(/(?:const|let)\s+([A-Za-z_$][\w$]*)\s*=/g)]
      .map(match => match[1])
      .filter(name => new RegExp(`(?:const|let)\\s+${name}\\s*=\\s*[\\s\\S]{0,320}?cacheKey\\(`).test(source)))
    for (const call of source.matchAll(/use(?:State|AsyncData)\s*\(/g)) {
      ...
      if (!first.includes('cacheKey(') && ![...scopedKeyNames].some(name => first.includes(name))) {
        offenders.push(`${file.split('/huizhiyun/')[1]}: ${first.replace(/\s+/g, ' ').slice(0, 70)}`)
      }
    }
  }
