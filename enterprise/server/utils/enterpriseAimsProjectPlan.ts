...
export async function enterpriseAimsProjectPlan(event: H3Event) {
  ...
  const input = (query: Record<string, string>) => ({ tenant: user.tenant, deployment: user.deployment, projectId, query, authorization: { actorUid: user.uid, tenant: user.tenant, deployment: user.deployment, resource: 'project-plan', action: 'view', expiresAt: enterpriseRuntimePermitExpiresAt() } })
  ...
  async function all(operation: 'aims.project-plan-milestones' | 'aims.project-plan-items') {
    ...
    for (let page = 1; page <= 100; page++) {
      const response = await callEnterpriseRuntime(event, operation, input({ ...scope, page: String(page), pageSize: '100' })) as { data?: Record<string, unknown>[] | { items?: Record<string, unknown>[], total?: number } }
      const data = response?.data
      const batch = Array.isArray(data) ? data : (data?.items || [])
      rows.push(...batch)
      if (batch.length < 100 || rows.length >= Number((!Array.isArray(data) && data?.total) || 0)) break
    }
    return rows
  }
