export type PermissionAction = 'view' | 'request' | 'edit' | 'approve' | 'replay' | 'admin'
...
export type AssetsPermissionChecker = (resource: string, action: PermissionAction) => boolean
...
export const resources: ResourceDefinition[] = [
  ...
  { code: 'assignments', name: '资产操作', description: '分配、领用、归还、释放等操作', actions: ['view', 'request', 'edit', 'approve', 'admin'], sortOrder: 70 },
  { code: 'offboarding_recoveries', name: '离职资产回收', description: '离职员工未归还资产清单与回收责任维护', actions: ['view', 'edit', 'admin'], sortOrder: 75 },
  ...
  { code: 'integration_operations', name: '跨应用操作', description: '诊断并受控重放 Assets 发起的跨应用可靠操作', actions: ['view', 'replay'], sortOrder: 95 },
...
export const menus: MenuItemDefinition[][] = [[
  ...
  },
  {
    ...
    resource: 'offboarding_recoveries'
  },
  {
    label: '预警中心',
...
export const routeRules: RouteRule[] = [
  ...
  { pattern: '/digital-assets/**', resource: 'digital_assets', action: 'view' },
  { pattern: '/overview', resource: 'reports', action: 'view' },
  { pattern: '/physical', resource: 'asset_items', action: 'view' },
  { pattern: '/physical/**', resource: 'asset_items', action: 'view' },
  { pattern: '/resources', resource: 'asset_items', action: 'view' },
  { pattern: '/resources/**', resource: 'asset_items', action: 'view' },
  { pattern: '/items/**', resource: 'asset_items', action: 'view' },
  ...
  { pattern: '/deliveries/**', resource: 'deliveries', action: 'view' },
  { pattern: '/customer-delivery-assets/**', resource: 'deliveries', action: 'view' },
  ...
  { pattern: '/operations/assignments/**', resource: 'assignments', action: 'view' },
  { pattern: '/offboarding-recoveries', resource: 'offboarding_recoveries', action: 'view' },
  { pattern: '/offboarding-recoveries/**', resource: 'offboarding_recoveries', action: 'view' },
...
const landingRouteCandidates: Array<RouteRule & { to: string }> = [
  { to: '/', pattern: '/', resource: 'dashboard', action: 'view' },
  { to: '/operations/assignments', pattern: '/operations/assignments', resource: 'assignments', action: 'request' },
  { to: '/physical', pattern: '/physical', resource: 'asset_items', action: 'view' },
  { to: '/operations/assignments', pattern: '/operations/assignments', resource: 'assignments', action: 'view' },
  { to: '/procurement/orders', pattern: '/procurement/orders', resource: 'purchase_orders', action: 'view' },
  { to: '/alerts', pattern: '/alerts', resource: 'alerts', action: 'view' },
  { to: '/reports', pattern: '/reports', resource: 'reports', action: 'view' },
  { to: '/deliveries', pattern: '/deliveries', resource: 'deliveries', action: 'view' },
  { to: '/environments', pattern: '/environments', resource: 'environments', action: 'view' },
  { to: '/products', pattern: '/products', resource: 'products', action: 'view' }
]

export function resolveAssetsLandingRoute(hasPermission: AssetsPermissionChecker): string | null {
  return landingRouteCandidates.find(candidate => hasPermission(candidate.resource, candidate.action))?.to || null
}
