...
const canEditAsset = computed(() => permissionsLoaded.value && hasPermission('asset_items', 'edit'))
...
usePageTitle(computed(() => asset.value?.asset_name || '资产详情'))
...
const selfServiceActionTypes = ['claim', 'return', 'release']
const operationItems = computed<DropdownMenuItem[]>(() => {
  const items: Array<DropdownMenuItem & { actionType: string }> = [{
    label: '分配',
    actionType: 'assign',
  ...
  return items
    .filter(item => !requestOnlyAssignment.value || selfServiceActionTypes.includes(item.actionType))
    .map(({ actionType: _actionType, ...item }) => item)
})
...
<template>
  ...
  <AssetsAssignmentCreateModal
    ...
    :request-only="requestOnlyAssignment"
