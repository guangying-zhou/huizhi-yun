...
const quickLinkPermissions: Record<string, [string, string]> = {
  '/physical': ['asset_items', 'view'],
  '/resources': ['asset_items', 'view'],
  '/products': ['products', 'view'],
  '/environments': ['environments', 'view'],
  '/procurement/orders': ['purchase_orders', 'view']
}
const quickLinks = computed<QuickLinkItem[]>(() => (response.value?.data.quick_links || []).filter((link) => {
  const permission = quickLinkPermissions[link.to]
  return permissionsLoaded.value && permission && hasPermission(permission[0], permission[1])
}))
          ...
          <UCard>
            <template #header>
              <div class="flex items-center justify-between">
                ...
                <NuxtLink
                  v-if="permissionsLoaded && hasPermission('alerts', 'view')"
                  to="/alerts"
          ...
          <UCard>
            <template #header>
              <div class="flex items-center justify-between">
                ...
                <NuxtLink
                  v-if="permissionsLoaded && hasPermission('asset_items', 'view')"
                  to="/resources"
                  class="text-sm text-primary hover:underline"
                >
                  查看资源资产
