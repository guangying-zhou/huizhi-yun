...
const selfServiceActionTypes = ['claim', 'return', 'release'] as const
const actionTypeOptions = computed(() => getOptions('assignment_action_type').filter(option => (
  !props.requestOnly || selfServiceActionTypes.includes(option.value as typeof selfServiceActionTypes[number])
)))
...
function resetState() {
  ...
  state.effective_at = ''
  state.note = ''
  applyActionPreset(props.defaultActionType || (props.requestOnly ? 'claim' : 'assign'))
}
...
async function handleSubmit() {
  ...
  if (!props.requestOnly && ['assign', 'claim', 'transfer'].includes(state.action_type) && state.target_type === 'none') {
    toast.add({ title: '缺少目标类型', description: '该操作必须指定目标类型。', color: 'warning' })
    return
  }

  if (!props.requestOnly && isTargetRequired.value && !state.target_ref.trim() && state.target_type !== 'none') {
...
<template>
  <UModal
    v-model:open="isOpen"
    :title="requestOnly ? '发起资产申请' : '新增资产操作'"
    :description="requestOnly ? '仅可为本人发起领用、归还或释放申请，审批通过后生效。' : '发起分配、领用、转移、归还、释放、续费或报废操作。'"
    ...
    <template #body>
      <div class="space-y-4 p-4">
        <UAlert
          ...
          title="本人资产申请"
          description="领用目标由系统固定为当前登录人；归还和释放只允许当前由本人使用的资产。"
        />
        ...
        <div class="grid gap-4 md:grid-cols-2">
          ...
          <UFormField v-if="!requestOnly" label="目标类型">
            <USelect
          ...
          <UFormField v-if="!requestOnly" label="操作状态">
            <USelect
          ...
          <UFormField v-if="!requestOnly" label="流程实例号">
            <UInput
          ...
          <UFormField v-if="!requestOnly" :label="state.target_type === 'user' ? '目标 UID' : '目标编码'" :required="isTargetRequired && state.target_type !== 'none'">
            <UInput
          ...
          <UFormField v-if="!requestOnly" label="数量">
            <UInput
          ...
          <UFormField v-if="!requestOnly" class="md:col-span-2" label="计划生效时间">
            <UInput
