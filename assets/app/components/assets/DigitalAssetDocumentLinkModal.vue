...
async function handleSubmit() {
  ...
  if (!state.document_id.trim()) {
    toast.add({ title: '缺少文档编号', description: '请填写相关数字资产文档编号。', color: 'warning' })
    return
  }

  submitting.value = true
  try {
    await $fetch<ApiResponse<{ id: number }>>(`/api/v1/digital-assets/${props.asset.id}/documents`, {
      method: 'POST',
      body: {
    ...
    toast.add({ title: '文档已关联', description: '数字资产文档关联已保存。', color: 'success', icon: 'i-lucide-check' })
    emit('created')
    isOpen.value = false
  } catch (error) {
    console.error('[DigitalAssetDocumentLink] Failed:', error)
...
<template>
  <UModal
    v-model:open="isOpen"
    title="关联文档"
    description="挂接仓库说明、数据字典、模型说明或交付附件。"
      ...
      <div class="space-y-4 p-4">
        <UFormField label="文档编号" required>
          <UInput
            ...
            placeholder="例如：DOC-DA-AIOPS-001"
            class="w-full"
          />
        </UFormField>
