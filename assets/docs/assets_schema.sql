...
CREATE TABLE IF NOT EXISTS `asset_resource_details` (
  ...
  KEY `idx_resource_expire_cursor` (`expires_at`, `asset_id`),
...
CREATE TABLE IF NOT EXISTS `customer_delivery_assets` (
  ...
  `responsible_uid` VARCHAR(64) DEFAULT NULL COMMENT 'Assets 内维护的交付资产运营责任人；不得由跨模块合同负责人隐式推导',
  `responsible_dept_code` VARCHAR(64) DEFAULT NULL COMMENT '运营责任部门；仅用于用户目标页数据范围，不作为通知广播对象',
  ...
  `altoc_status_sync_revision` BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '仅目标相关事实变化时递增的Altoc同步revision',
  `altoc_status_sync_fingerprint` CHAR(64) DEFAULT NULL COMMENT '目标相关事实规范化SHA-256',
  `altoc_status_sync_occurred_at` DATETIME(3) DEFAULT NULL COMMENT '当前revision冻结发生时间',
  `created_by` VARCHAR(64) DEFAULT NULL,
  `updated_by` VARCHAR(64) DEFAULT NULL,
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  ...
  CONSTRAINT `chk_customer_delivery_asset_responsibility_pair` CHECK (
    (`responsible_uid` IS NULL AND `responsible_dept_code` IS NULL)
    OR (
      `responsible_uid` IS NOT NULL AND TRIM(`responsible_uid`) <> ''
      AND LOWER(TRIM(`responsible_uid`)) <> '@all'
      AND `responsible_uid` = TRIM(`responsible_uid`)
      AND `responsible_dept_code` IS NOT NULL AND TRIM(`responsible_dept_code`) <> ''
...
CREATE TABLE IF NOT EXISTS `product_assets` (
  ...
  `build_stage` VARCHAR(64) DEFAULT NULL,
  `current_version` VARCHAR(64) DEFAULT NULL,
  `target_version` VARCHAR(64) DEFAULT NULL,
  `productization_value_level` VARCHAR(64) DEFAULT NULL,
  `supported_terminals` JSON DEFAULT NULL,
  `covered_legacy_systems` JSON DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `ip_assets` (
  ...
  KEY `idx_ip_status_expire_cursor` (`status`, `expires_at`, `id`)
...
CREATE TABLE IF NOT EXISTS `asset_offboarding_recovery_cases` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `case_code` VARCHAR(64) NOT NULL COMMENT 'Assets 离职资产回收事项稳定编码',
  `source_app` VARCHAR(50) NOT NULL COMMENT '离职生命周期投影来源应用',
  `source_event_key` VARCHAR(191) NOT NULL COMMENT '来源稳定幂等事件键',
  `source_payload_sha256` CHAR(64) NOT NULL COMMENT '已验证 People 离职投影载荷摘要；同事件异载荷冲突',
  `departed_employee_uid` VARCHAR(64) NOT NULL COMMENT '已离职员工UID；禁止作为收件人',
  `departed_employee_name` VARCHAR(191) DEFAULT NULL COMMENT '离职时姓名快照，不作为身份事实',
  `offboarded_at` DATETIME NOT NULL COMMENT '离职生效时间',
  `recovery_due_at` DATE NOT NULL COMMENT '资产回收期限（UTC 日期语义）',
  `recovery_responsible_uid` VARCHAR(64) DEFAULT NULL COMMENT 'Assets 内显式维护的唯一回收责任人；NULL 表示待分配',
...
CREATE TABLE IF NOT EXISTS `assets_notification_checkpoint` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `event_stream` ENUM('resource_expiry', 'ip_expiry', 'delivery_expiry', 'delivery_warranty', 'delivery_support', 'offboarding_unrecovered') NOT NULL,
  `source_type` ENUM('asset_item', 'ip_asset', 'customer_delivery_asset', 'offboarding_recovery_case') NOT NULL,
...
-- Assets 作为调用方的 integration_operation / integration_operation_attempt。
-- 完整增量 DDL 见 docs/migrations/20260710_assets_integration_operations.sql；
-- dead-letter actionable lifecycle 见 docs/migrations/20260711_assets_dead_letter_actionable_lifecycle.sql。

-- Product catalog watermark (2026-09-07)
