...
> **当前运行契约（2026-07-11 核对）**：本文保留首版产品建模与范围判断；身份、目录、授权、业务数据访问与服务调用以现行代码及 [`MODULE_CONTRACTS.md`](../../docs/MODULE_CONTRACTS.md) 为准。Assets 用户登录走 Foundation 接入的 **Console OIDC**，目录/项目注册表走 **Console Directory + Foundation adapter**，业务数据走 tenant-runtime/data-runtime，跨应用调用使用 Foundation 申请的短期 Console `token_use=service` JWT 与精确 capability。文中 Account 仅指遗留迁移期兼容语义；当前 `/api/account/**` 仅为已验证 Console 会话后的兼容 BFF 命名，不调用或回退 legacy Account。
...
- 与 `Console Directory / Foundation`、`Aims`、`Altoc`、`Codocs`、`Workflow` 的集成方式
- 首版数据库表结构草案与后续扩展方向
...
|------|------------|--------------------|
| 用户、部门、角色、权限 | `Console Directory`、Console 授权运行时与 Foundation adapter | 负责人、使用人、保管人、申请人、部门归属、数据权限 |
| 平台项目注册表 | `Console Directory` | `project_code` 关联项目、项目负责人、项目成员、项目启停状态 |
...
- 项目主数据链路必须是 `Assets -> Console Directory`，而不是 `Assets -> Aims`。
- `Aims` 可以提供项目执行上下文，但不能替代 `Console Directory` 成为项目注册表。
...
下表使用应用内路由；生产部署时由 Nuxt `baseURL=/assets/` 自动形成 `/assets/...` 公开地址。

| 页面 | 路由建议 | 说明 |
|------|----------|------|
| 工作台 | `/` | 我的待办、待处理预警、即将到期资产、项目成本摘要 |
| 实物资产列表 | `/physical` | 列表、筛选、批量导出 |
| 资源资产列表 | `/resources` | 资源、Seat、额度、域名证书统一列表 |
| 产品资产列表 | `/products` | 同一列表中区分产品主档与技术底座子类别 |
| 知识产权资产列表 | `/ip-assets` | 软著、商标、专利、资质证照台账 |
| 数字资产列表 | `/digital-assets` | 代码、文档、数据、模型和交付物台账 |
| 实物/资源资产详情 | `/items/[id]` | 基础信息、关系、变更记录、预警记录 |
...
权限由 Console 运行时授权快照与 Foundation helper 提供，Assets 只做两层控制：
...
| `POST` | `/api/v1/authorization/instance-conflict-explain` | 采购单审批职责冲突解释 |
...
| `POST` | `/api/v1/authorization/instance-conflict-explain` | 资产操作审批职责冲突解释（`targetType=assignment`） |
...
### 11.1 与 Console Directory / Foundation

使用场景：

- Console OIDC 登录态、Console 授权快照、用户/部门/项目注册表
- Foundation Directory adapter、workflow proxy 与通知 helper
- Console 统一消息中心

设计要求：

- 前端只调 Assets API，不直连 Directory 或其他业务服务
- Assets 服务端通过 Foundation `directoryApi` 调用 Console Directory；遗留 `server/api/account/**` 仅为已验证会话后的兼容路由命名，禁止 Account fallback
- 项目选择器统一使用 `project_code`
- 服务端跨应用读写由 Foundation 以短期 Console service token 调用目标 service API；浏览器不得持有 service token 或静态跨应用 API key
...
- 通过 Console Directory / Foundation adapter 读取项目、用户、部门
- 读取 Altoc 合同、客户
- 读取 Aims 项目执行摘要
- 关联 Codocs 文档
- 接入 Workflow 审批
...
| 编号 | 决策 | 结论 |
|------|------|------|
| AD-01 | 项目主数据来源 | `Console Directory` |
