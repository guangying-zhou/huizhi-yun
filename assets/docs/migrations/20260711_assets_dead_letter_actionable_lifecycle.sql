-- Repeatable/idempotent source lifecycle for Assets integration-operation dead-letter actionables.
CREATE TABLE IF NOT EXISTS integration_operation_dead_letter_actionable (
  ...
  UNIQUE KEY uk_assets_iopdla_actionable (tenant_code, deployment_code, source_app, actionable_key),
  UNIQUE KEY uk_assets_iopdla_notification (tenant_code, deployment_code, source_app, notification_id),
  INDEX idx_assets_iopdla_publish (tenant_code, deployment_code, source_app, publish_acked_at, dead_lettered_at),
  INDEX idx_assets_iopdla_closure (tenant_code, deployment_code, source_app, closure_acked_at, closure_pending_at),
  CONSTRAINT fk_assets_iopdla_operation FOREIGN KEY (operation_id) REFERENCES integration_operation(operation_id) ON DELETE RESTRICT,
  CONSTRAINT chk_assets_iopdla_closure CHECK (closure_state IS NULL OR closure_state IN ('resolved','cancelled')),
  CONSTRAINT chk_assets_iopdla_recipient CHECK (recipient_uids IS NULL OR JSON_TYPE(recipient_uids) = 'ARRAY')
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Assets跨应用死信actionable generation发布与closure证据';
