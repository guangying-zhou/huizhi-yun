-- ADR-018: extend the existing Assets-owned receipt CHECK without replacing
-- earlier product, digital-asset or IP-asset operation clauses.
ALTER TABLE service_command_receipt
  ...
  ADD CONSTRAINT chk_scr_cross_app CHECK (source_app <> target_app OR (
    ...
    AND (
      ...
      OR (operation_code IN ('assets.products.link-base.v1', 'assets.products.link-asset.v1', 'assets.products.link-document.v1') AND required_capability = 'assets:product:edit')
      ...
      OR (operation_code = 'assets.ip-assets.link-product.v1' AND required_capability = 'assets:ip-asset:link-product')
    )
  ));
