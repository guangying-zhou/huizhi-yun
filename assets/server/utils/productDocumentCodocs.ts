...
interface DocumentMetadata { uuid: string, title: string, doc_type: string, updated_at: string }
...
export async function readAssetProductDocumentMetadata(event: H3Event, productCode: string, documentUuid: string): Promise<DocumentMetadata> {
  if (!validText(productCode, 64) || productCode.includes('/') || !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.test(documentUuid) || documentUuid === '00000000-0000-0000-0000-000000000000') throw createError({ statusCode: 400, message: '产品或文档身份无效' })
  await requirePermission(event, 'products', 'edit')
  const actorUid = requireRequestUid(event)
  if (!validText(actorUid, 64)) throw createError({ statusCode: 401, message: '用户身份无效' })
  ...
  const signature = await buildServiceCommandRuntimeHeaders({ token, method: 'POST', requestTarget: new URL(url).pathname, requestId, tenantCode: gateway.tenant, sourceDeploymentCode: gateway.deployment, targetDeploymentCode: route.deploymentCode, sourceApp: 'assets', sourceClientId: 'assets.runtime', targetApp: 'codocs', envelope: serviceCommand })
