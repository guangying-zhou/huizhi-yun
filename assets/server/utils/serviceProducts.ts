...
function mapProduct(row: Record<string, unknown>) {
  return {
    ...
    productLineLabel: row.product_line_label ?? row.product_line ?? null,
    productLineSortOrder: row.product_line_sort_order ?? null,
...
async function fetchProductPage(event: H3Event, query: Record<string, unknown>) {
  const runtime = await maybeCallTenantRuntime<RuntimeEnvelope<RuntimeProductPage>>(event, '/v1/assets/service/products', {
    appCode: 'assets',
    scope: 'assets.read assets:product:read',
    method: 'GET',
    query
  })
...
export async function resolveServiceProducts(event: H3Event, input: { keyword?: unknown, codes?: unknown }) {
  ...
  if (codes.length > 0) {
    const page = await fetchProductPage(event, { product_codes: codes.join(',') })
    const codeOrder = new Map(codes.map((code, index) => [code, index]))
    const items = (page.items || [])
      .map(mapProduct)
      .sort((left, right) => (codeOrder.get(normalize(left.productCode)) ?? codes.length)
        - (codeOrder.get(normalize(right.productCode)) ?? codes.length))
