...
export async function callAssetsIntegrationOperationAdmin<T>(
  event: H3Event,
  path: string,
  action: 'view' | 'replay',
  ...
  const scoped = uid
    ? await loadScopedAuthorizationFromConsoleRuntime(event, uid, 'assets', {
        resourceCode: 'integration_operations', action
      })
    : null
  if (!hasTenantGlobalIntegrationOperationGrant(scoped?.grants, { appCode: 'assets', action })) {
  ...
  const runtime = await maybeCallTenantRuntime<T>(event, path, {
    appCode: 'assets',
    scope: `${method === 'GET' ? 'assets.read' : 'assets.write'} assets:integration_operations:${action}`,
    method,
    query: options.query,
    body: options.body
  })
  if (!runtime.handled) {
    throw createError({ statusCode: 503, message: 'Assets tenant-runtime is required for integration operation administration.' })
  }
  return runtime.data
}
