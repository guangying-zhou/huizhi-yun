...
import { callAssetsIntegrationOperationAdmin } from '~~/server/utils/integrationOperationAdmin'
...
export default defineEventHandler(async (event) => {
  ...
  const data = await callAssetsIntegrationOperationAdmin<Record<string, unknown>>(
    event,
    `/v1/assets/integration-operations/${encodeURIComponent(operationId)}/attempts`,
    'view',
    { query: { limit: 100 } }
  )
