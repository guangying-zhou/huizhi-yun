...
export default defineNitroPlugin(() => {
  ...
  setTimeout(async () => {
    ...
    try {
      ...
      console.log(`[Assets SyncApprovalActions] Synced to Workflow: ${result.created} new, ${result.updated} updated, ${result.deprecated} deprecated`)
      if (result.errors?.length) {
        console.warn('[Assets SyncApprovalActions] Errors:', result.errors)
      }
    } catch {
      console.log('[Assets SyncApprovalActions] Workflow 不可达，跳过审批动作同步')
    }
  }, 5000)
})
