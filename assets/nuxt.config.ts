...
const appCode = process.env.HZY_APP_CODE || 'assets'
const isCloudflareBuild = process.env.HZY_CLOUDFLARE_BUILD === 'true'
const appBasePath = resolveAppBasePath(appCode)
const normalizedAppBasePath = appBasePath === '/' ? '' : appBasePath.replace(/\/+$/, '')
const consoleAuthBypassPaths = Array.from(new Set([
  '/api/v1/dictionaries',
  '/api/v1/dictionaries/',
  '/assets/api/v1/dictionaries',
  '/assets/api/v1/dictionaries/',
  `${normalizedAppBasePath}/api/v1/dictionaries`,
  `${normalizedAppBasePath}/api/v1/dictionaries/`
]))
...
export default defineNuxtConfig({
  ...
  runtimeConfig: {
    hzy: {
      ...
      dataAccessMode: process.env.HZY_ASSETS_DATA_ACCESS_MODE || process.env.HZY_DATA_ACCESS_MODE || 'tenant-runtime',
      tenantRuntime: {
        endpoint: process.env.HZY_ASSETS_TENANT_RUNTIME_URL || process.env.HZY_TENANT_RUNTIME_URL || process.env.HZY_ASSETS_DATA_RUNTIME_URL || process.env.HZY_DATA_RUNTIME_URL || '',
        token: process.env.HZY_ASSETS_TENANT_RUNTIME_TOKEN || process.env.HZY_TENANT_RUNTIME_TOKEN || process.env.HZY_ASSETS_DATA_RUNTIME_TOKEN || process.env.HZY_DATA_RUNTIME_TOKEN || '',
      ...
      consoleOidc: {
        bypassAuthPaths: consoleAuthBypassPaths
      },
      directory: {
      ...
      serviceClient: {
        clientId: process.env.HZY_ASSETS_SERVICE_CLIENT_ID || process.env.HZY_SERVICE_CLIENT_ID || '',
        clientSecret: process.env.HZY_ASSETS_SERVICE_CLIENT_SECRET || process.env.HZY_SERVICE_CLIENT_SECRET || '',
        tokenUrl: process.env.HZY_CONSOLE_TOKEN_URL || ''
      },
      notifications: {
        dueEnabled: process.env.HZY_ASSETS_DUE_NOTIFICATIONS_ENABLED || 'false'
      }
    },
    consoleOidc: {
      bypassAuthPaths: consoleAuthBypassPaths
    ...
    public: {
      appName: packageMeta.name || 'assets',
      appVersion: packageMeta.version || '0.0.0',
      appDisplayName: process.env.NUXT_PUBLIC_APP_DISPLAY_NAME || '汇智云资产',
      appIcon: process.env.NUXT_PUBLIC_APP_ICON || 'i-lucide-boxes',
      appLogo: process.env.NUXT_PUBLIC_APP_LOGO || withAppBase('/assets.png'),
      deploymentProfile,
      deploymentPublicUrl,
      appBasePath,
      appHomeUrl: process.env.HZY_APP_HOME_URL || process.env.NUXT_PUBLIC_APP_HOME_URL || '',
      rum: {
        enabled: process.env.NUXT_PUBLIC_RUM_ENABLED || 'false',
  ...
  nitro: {
    scheduledTasks: {
      '*/15 * * * *': ['notifications:due', 'integration-operations:delivery-asset-status']
    },
    alias: {
