...
状态：历史/目标设计参考（部分 Vault 能力已落地；不是当前迁移或发布验收依据）
原始日期：2026-04-30
最后事实核对：2026-07-11
定位：`console.credential-vault` 的凭证分类、零配置运行链路与迁移实施设计。

当前事实源：[`docs/Development-Execution-Checklist-2026-07.md`](../../docs/Development-Execution-Checklist-2026-07.md)、[`docs/MODULE_CONTRACTS.md`](../../docs/MODULE_CONTRACTS.md)、Console `CLAUDE.md`、`console/docs/hzy_console_schema.sql` 与当前 Console/Foundation 实现。业务应用不再以 app 级 `license.lic` bootstrap 作为新主路径；跨应用调用使用 Foundation 获取的短期 Console service token。真实 Vault、secret、轮换、迁移、Cloudflare 部署和真实租户验证均须另行授权。

> 2026-07-17 边界修订：`docs/ADR-017-Console-Tenant-Data-Plane-Separation.md`
> 已决定 Vault 表、主密钥和所有可恢复 Secret 全部位于客户侧 tenant-runtime。
> 本文的凭证分类、轮换、reveal/resolve 与审计规则继续有效；“Console 本地 Vault”
> 指逻辑领域，不再表示 Console UI/BFF 进程持有数据库或主密钥。

2026-07-11 P1 收口：`/api/v1/console/vault/resolve` 不再接收调用方提供的 `secretCode`、`secretRef`、版本或 purpose，而只接收 `integrationCode`。服务端以已验证的 service client + source app、可执行的 `scope_json.integrationCodes` allowlist 与 active integration/credential/固定 Vault version 绑定后才解析；`usage_type/owner_type/owner_key` 不匹配或 scope 缺失均拒绝。服务身份读取 integration 配置使用同一 allowlist。仍待真实 migration/seed 覆盖率、轮换和环境验收，不能据此声称已上线。
...
- 业务应用不在本地保存跨应用 client secret；通过 Foundation 获取短期 Console service token，再读取非 secret 配置并按授权 `resolve` 运行所需 secret。
...
| `bootstrap` | 启动凭证 | 历史离线兼容部署的启动 access key | 显式离线兼容流程 | 只用于兼容 bootstrap 换短期 token | 不建议 reveal，轮换重发 |
...
- `integration`
  ...
  - 程序化 `resolve` 必须校验调用方 service client/source app、可执行 `scope_json.integrationCodes` allowlist、active integration→credential→固定版本绑定与 secret 归属；调用方只提交 `integrationCode`，purpose 由服务端派生并审计。
...
历史离线兼容部署可使用 `license.lic`；它不是新业务应用的主信任链路：
...
```text
App server start
  -> Foundation 以已配置或受信运行环境提供的 service client 获取短期 Console service token
...
- secret 引用：Console 内部 `secretRef` / `secretCode`，业务模块不持久化或提交给 resolve
- secret 明文：仅服务端按已授权 `integrationCode` resolve
...
- GitLab 通过 Tenant Runtime 固定 operation 消费，不再返回 Runtime config/token
...
- resolver 内部统一做 token 获取、重试、缓存、服务端派生的审计 purpose 和错误脱敏。
...
- Foundation `integrationConfig.ts` adapter：兼容封装尚未迁完的 provider；GitLab 已改为 `integrationCode + fixed operation`，业务进程不再接收 token
...
1. Aims：GitLab、企业微信、OSS。企业微信 OAuth 已先接入 Foundation integration adapter：`corpid/agentid` 读取 Console integration 非 secret 配置，`corpsecret` 从 vault resolve，`WECOM_*` 仅作为 legacy fallback。
   - 初始化模板：`console/docs/sql/Console-SQL-Seed-v1.6-wecom-integration.sql`，用于创建 `wecom.default`、绑定 `integration.wecom.default.corpsecret`，并给 Aims/Codocs runtime service client 授权 `integration_config:view` / `credential_vault:resolve`。
...
- Aims 初始化模板：`console/docs/sql/Console-SQL-Seed-v1.7-aims-bootstrap.sql`，创建 Aims service client、`bootstrap.{deploymentCode}.access_key`，并授予 integration/vault scopes。
