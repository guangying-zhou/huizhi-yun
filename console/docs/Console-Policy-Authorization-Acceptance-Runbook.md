...
>
> - **最后事实核验：** 2026-07-11。
> - **当前事实源：** [`scripts/accept-platform-policy-roles.mjs`](../scripts/accept-platform-policy-roles.mjs)、[`scripts/accept-console-policy-consumption.mjs`](../scripts/accept-console-policy-consumption.mjs)、Console 与 Platform 当前 `CLAUDE.md`、[`docs/MODULE_CONTRACTS.md`](../../docs/MODULE_CONTRACTS.md) 及 [`docs/Development-Execution-Checklist-2026-07.md`](../../docs/Development-Execution-Checklist-2026-07.md)。
> - **执行边界：** 本文脚本会访问目标 Platform/Console，并可能生成 policy bundle 或使用真实会话；离线阅读或脚本存在不构成真实租户授权。任何角色/subject 变更、bundle 物化、真实登录会话使用、live acceptance 或回滚均须按目标 tenant/deployment 逐项、明确授权。
...
1. 全局企业角色存在（默认 `system_admin`，见 `platform/docs/sql/HZY-Platform-SQL-Seed-v2.16-enterprise-roles.sql`）；
2. `system-roles/{code}/enable` 物化成功；
3. 角色出现在租户可分配角色列表；
4. `subject-roles` 授权写入成功；
5. 生成的 bundle payload 使用 `schemaVersion=policy-bundle.v2`，且 `roles / roleAssignments / rolePermissionGrants / subjects` 包含对应数据。
   默认验收 `system_admin` 时，脚本还会断言 `simulate-role/simulate-user` 来自 `platform:authorization_simulator`，且没有回流到 `platform:authorization_admin`。
