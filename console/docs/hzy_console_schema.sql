...
--   Section H: Runtime Compatibility
--   Section I: Generic Audit
--
-- 约定：
--   - MySQL 8.0+
...
CREATE TABLE IF NOT EXISTS `org_profiles` (
  ...
  `revision` BIGINT UNSIGNED NOT NULL DEFAULT 1 COMMENT '企业资料乐观锁版本',
...
-- Console zero-direct-DB cutover exceptions are append-only review evidence.
-- A disposition is effective only while its source_fingerprint still matches
-- the current blocking row; any relevant source change fails closed.
...
CREATE TABLE IF NOT EXISTS `directory_lifecycle_scope_versions` (`employee_uid` VARCHAR(64) NOT NULL PRIMARY KEY,`applied_revision` BIGINT UNSIGNED NOT NULL,`snapshot_hash` CHAR(64) NOT NULL,`lifecycle_type` VARCHAR(32) NOT NULL,`created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),`updated_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
CREATE TABLE IF NOT EXISTS `service_command_receipt` (`receipt_id` CHAR(36) PRIMARY KEY,`operation_id` CHAR(36) NOT NULL,`operation_code` VARCHAR(191) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,`tenant_code` VARCHAR(100) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,`source_deployment_code` VARCHAR(100) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,`deployment_code` VARCHAR(100) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,`source_app` VARCHAR(50) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,`target_app` VARCHAR(50) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,`required_capability` VARCHAR(191) NOT NULL,`idempotency_key` VARCHAR(191) CHARACTER SET ascii COLLATE ascii_bin NOT NULL,`command_schema_version` VARCHAR(30) NOT NULL,`command_sha256` CHAR(64) NOT NULL,`status` VARCHAR(32) NOT NULL DEFAULT 'processing',`locked_by` VARCHAR(100) NULL,`locked_until` DATETIME(3) NULL,`fencing_token` BIGINT UNSIGNED NOT NULL DEFAULT 0,`version_no` BIGINT UNSIGNED NOT NULL DEFAULT 1,`first_request_id` VARCHAR(100) NULL,`last_request_id` VARCHAR(100) NULL,`correlation_id` VARCHAR(100) NULL,`original_actor_uid` VARCHAR(100) NULL,`service_client_id` VARCHAR(100) NULL,`target_biz_type` VARCHAR(100) NULL,`target_biz_code` VARCHAR(191) NULL,`response_http_status` SMALLINT UNSIGNED NULL,`response_summary_sha256` CHAR(64) NULL,`result_json` JSON NULL,`received_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),`last_received_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),`completed_at` DATETIME(3) NULL,`created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),`updated_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),UNIQUE KEY `uk_console_scr_identity` (`tenant_code`,`source_deployment_code`,`deployment_code`,`source_app`,`target_app`,`operation_code`,`idempotency_key`)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
CREATE TABLE IF NOT EXISTS `integration_operation` (`operation_id` CHAR(36) PRIMARY KEY,`operation_key` VARCHAR(191) NOT NULL,`tenant_code` VARCHAR(100) NOT NULL,`deployment_code` VARCHAR(100) NOT NULL,`source_app` VARCHAR(50) NOT NULL,`target_app` VARCHAR(50) NOT NULL,`operation_code` VARCHAR(191) NOT NULL,`required_capability` VARCHAR(191) NOT NULL,`source_biz_type` VARCHAR(100) NOT NULL,`source_biz_code` VARCHAR(191) NOT NULL,`idempotency_key` VARCHAR(191) NOT NULL,`command_schema_version` VARCHAR(30) NOT NULL,`command_json` JSON NOT NULL,`command_sha256` CHAR(64) NOT NULL,`status` VARCHAR(32) NOT NULL DEFAULT 'pending',`attempt_count` INT UNSIGNED NOT NULL DEFAULT 0,`fencing_token` BIGINT UNSIGNED NOT NULL DEFAULT 0,`next_attempt_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),`locked_until` DATETIME(3) NULL,`target_receipt_id` CHAR(36) NULL,`target_biz_type` VARCHAR(100) NULL,`target_biz_code` VARCHAR(191) NULL,`response_summary_sha256` CHAR(64) NULL,`original_request_id` VARCHAR(100) NULL,`original_actor_uid` VARCHAR(100) NULL,`service_client_id` VARCHAR(100) NULL,`last_error_code` VARCHAR(100) NULL,`last_error_class` VARCHAR(50) NULL,`last_error_summary` VARCHAR(1000) NULL,`created_by` VARCHAR(100) NULL,`updated_by` VARCHAR(100) NULL,`succeeded_at` DATETIME(3) NULL,`created_at` DATETIME(3) NOT NULL,`updated_at` DATETIME(3) NOT NULL,UNIQUE KEY `uk_console_iop_key` (`tenant_code`,`deployment_code`,`source_app`,`operation_key`),KEY `idx_console_iop_due` (`status`,`next_attempt_at`,`locked_until`)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
CREATE TABLE IF NOT EXISTS `integration_operation_attempt` (`attempt_id` CHAR(36) PRIMARY KEY,`operation_id` CHAR(36) NOT NULL,`attempt_no` INT UNSIGNED NOT NULL,`fencing_token` BIGINT UNSIGNED NOT NULL,`result_status` VARCHAR(32) NOT NULL,`error_code` VARCHAR(100) NULL,`started_at` DATETIME(3) NOT NULL,`finished_at` DATETIME(3) NULL,`created_at` DATETIME(3) NOT NULL,UNIQUE KEY `uk_console_ioa_attempt` (`operation_id`,`attempt_no`),KEY `idx_console_ioa_status` (`result_status`,`created_at`)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
CREATE TABLE IF NOT EXISTS `console_platform_lifecycle_actionables` (`operation_id` CHAR(36) NOT NULL,`generation` INT UNSIGNED NOT NULL DEFAULT 1,`actionable_key` VARCHAR(191) NOT NULL,`object_version` VARCHAR(191) NOT NULL,`recipient_uids_json` JSON NULL,`notification_id` VARCHAR(64) NULL,`published_at` DATETIME(3) NULL,`closure_state` VARCHAR(16) NULL,`closure_acknowledged_at` DATETIME(3) NULL,`created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),`updated_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),PRIMARY KEY (`operation_id`,`generation`),KEY `idx_console_platform_lifecycle_actionable_pending` (`notification_id`,`closure_acknowledged_at`,`updated_at`),CONSTRAINT `fk_console_platform_lifecycle_actionable_operation` FOREIGN KEY (`operation_id`) REFERENCES `integration_operation` (`operation_id`) ON DELETE CASCADE,CONSTRAINT `ck_console_platform_lifecycle_actionable_closure` CHECK (`closure_state` IS NULL OR `closure_state` IN ('resolved','cancelled'))) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- 外部组织源身份与汇智云稳定 dept_code 分离。供应商部门 ID 只能作为 identity，
-- 不得直接成为跨模块业务键，也不得迫使 canonical 部门改变 source_provider。
CREATE TABLE IF NOT EXISTS `directory_department_identities` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `provider_code` VARCHAR(64) NOT NULL,
  `external_department_id` VARCHAR(255) NOT NULL,
  `dept_code` VARCHAR(64) NOT NULL,
  `mapping_origin` VARCHAR(32) NOT NULL DEFAULT 'source_created' COMMENT 'migration_confirmed / source_created / admin_bound / path_matched',
  `source_payload_hash` CHAR(64) NULL,
  `manager_external_subject` VARCHAR(255) NULL COMMENT '供应商部门负责人 subject；解析到稳定 uid 后投影到 directory_departments.manager_uid',
  `first_seen_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `last_seen_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `last_snapshot_revision` VARCHAR(128) NULL COMMENT '最近一次成功组织快照水位；仅用于全量缺失停用判定',
  `status` VARCHAR(32) NOT NULL DEFAULT 'active',
  `active_dept_code` VARCHAR(64) GENERATED ALWAYS AS (
    CASE WHEN `status` = 'active' THEN `dept_code` ELSE NULL END
  ) STORED COMMENT '同一 provider 下 active identity 的 canonical 部门唯一键',
...
-- 双部门树迁移期保留旧 DT-* 编码的受控别名。新写入只使用 canonical_dept_code；
-- 旧行首个发布周期只在 Directory 软停用；alias subject 与非主 membership 继续兼容投影，不硬删除。
...
-- HR 组织完整快照的租户侧安全阈值。请求方不能在浏览器请求中覆盖这些值。
...
-- 只有 Connector final marker、快照 hash、根部门和计数全部验证通过后才会创建 run。
...
-- 缺失部门候选按已验证快照冻结；应用时仍重新检查人员主归属、子部门和更新快照。
...
-- 客户侧 hzy-data-runtime Directory Connector 的单次注册凭证消费记录。
...
-- 目录账号一次性激活凭据。账号以 Console 生成、谁都不知道也不留存的一次性口令
-- 创建，员工凭激活凭据自行设定真实密码；表中只保存令牌的 SHA-256。
CREATE TABLE IF NOT EXISTS `directory_activation_credentials` (
  ...
  `issued_operation_id` CHAR(36) NULL,
  `redeem_operation_id` CHAR(36) NULL,
...
-- 目录身份预留：开户前原子占位 UID/登录名/邮箱/外部主体，只阻止冲突，不建账号。
...
-- Connector 公钥用于在 Console 入库前加密初始密码/新密码；私钥只保留在客户 Agent。
CREATE TABLE IF NOT EXISTS `directory_connectors` (
  ...
  `runtime_started_at` DATETIME(3) NULL,
...
-- 企业连接运行时的短期、单次安装码；数据库只保存 SHA-256，不保存原始安装码。
...
-- 一个租户 Console deployment 只登记一个企业连接运行时实例。
...
-- 外部 OAuth 登录事务只保存 state/browser binding 的 SHA-256，且单次消费。
...
-- ============================================================
-- Section I: Portal Notifications
-- ============================================================
...
CREATE TABLE IF NOT EXISTS `portal_notifications` (
  ...
  CONSTRAINT `ck_portal_notifications_request_hash`
...
-- ============================================================
-- Section J: Generic Audit
-- ============================================================

CREATE TABLE IF NOT EXISTS `operation_logs` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `domain_code` VARCHAR(64) NOT NULL COMMENT 'tenant_profile / system_settings / integration / vault / service_client / directory / auth_runtime / runtime_compat',
