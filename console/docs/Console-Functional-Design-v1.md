...
状态：Draft  
日期：2026-04-23  
定位：目标设计，作为 `docs/Huizhi-yun-Platform-Target-Architecture.md` 与 `console/docs/sql/Console-SQL-DDL-Draft-v1.sql` 的配套文档

> 2026-07-17 边界修订：`docs/ADR-017-Console-Tenant-Data-Plane-Separation.md`
> 保留本文的 Console 功能域和客户数据归属；持久化实现统一迁入客户侧
> tenant-runtime。Console UI/BFF 无论部署在 Cloudflare、PM2 还是客户本地，
> 都不再直接持有 `hzy_console` 数据库凭证。
...
- `workflow.apiUrl`（`runtime`）：审批流服务基础地址，供业务应用经 Console runtime/app identity 与 runtime settings 解析；不得作为 app 级 license bootstrap 的新用途。
...
| `bootstrap` | 启动兼容凭证 | 仅供已部署 legacy 客户端以 Vault 精确 access key 换取短期 service token；新业务应用不得写入或依赖 `license.lic` |
| `custody` | 托管凭证 | 仅用于集中保管、授权展示和审计的客户第三方 Key，默认不允许程序化 resolve |

其中：

- `integration / service` 是当前业务应用的 runtime credentials；`bootstrap` 仅保留给已部署 legacy 客户端，必须在 Vault 精确匹配 access key 后使用。
