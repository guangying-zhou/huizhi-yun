...
--     aud=altoc, scope=altoc:write altoc:service-ticket:delivery-result:sync
...
SELECT
  sc.`id`,
  'altoc:service-ticket:delivery-result',
  'sync',
  JSON_OBJECT(
    'source', 'seed:v1.31',
...
SELECT
  ...
  MAX(scg.`resource_code` = 'altoc:service-ticket:delivery-result' AND scg.`action` = 'sync' AND scg.`status` = 'active') AS `has_altoc_service_ticket_delivery_result_sync`,
