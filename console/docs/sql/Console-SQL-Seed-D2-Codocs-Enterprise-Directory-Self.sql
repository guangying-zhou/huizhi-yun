...
-- Enterprise actor-bound department/project projection through the Runtime
...
SELECT c.id, expected.resource_code, 'read',
  JSON_OBJECT(
    'source', 'seed:d2-codocs-enterprise-directory-self-read',
    'tenantCode', 'C000001',
    'deploymentCode', 'C000001-test-enterprise',
    'audience', expected.audience,
    'semanticScope', 'console:directory-self:read',
    'purpose', 'codocs-current-user-document-transfer-targets'
  ),
  'active', UTC_TIMESTAMP(), UTC_TIMESTAMP()
FROM service_clients c
JOIN (
  SELECT 'console:directory-self' AS resource_code, 'data-runtime' AS audience
  UNION ALL
  SELECT 'tenant-runtime:console:directory-self', 'tenant-runtime'
) expected
