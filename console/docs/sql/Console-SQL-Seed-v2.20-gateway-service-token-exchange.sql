...
SELECT sc.id,'console:service-token','gateway-exchange',
  JSON_OBJECT('source','seed:gateway-service-token-exchange',
    'audiences',JSON_ARRAY('data-runtime','tenant-runtime'),
    'semanticScope','console:service-token:gateway-exchange',
    'tenantCode','C000001','deploymentCode','wiztek-test-console'),
  'active',UTC_TIMESTAMP(),UTC_TIMESTAMP()
FROM service_clients sc
WHERE EXISTS (SELECT 1 FROM org_profiles WHERE singleton_key=1 AND BINARY tenant_code='C000001')
  AND sc.client_code='console.runtime' AND sc.app_code='console'
  ...
  AND NOT EXISTS (
    ...
    WHERE existing.service_client_id=sc.id
      ...
      AND existing.action='gateway-exchange'
  );
COMMIT;
