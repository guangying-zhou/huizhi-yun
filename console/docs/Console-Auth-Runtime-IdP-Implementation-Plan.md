...
状态：当前目标设计与实施跟踪（非生产验收依据）
最后核对：2026-07-11
当前事实源：根 `CLAUDE.md`、`console/CLAUDE.md`、`docs/MODULE_CONTRACTS.md`、`console/docs/Console-API-Contract-v1.md`、`docs/Platform-Console-Prod-Dev-Isolation-Plan.md`、`docs/Development-Execution-Checklist-2026-07.md`
定位：把 `console` 升级为企业侧应用用户认证运行时与下游 OIDC IdP 的目标设计和实施跟踪。代码/离线测试证据不等同于 migration、Cloudflare 部署、真实登录会话验收或生产发布；这些外部操作须分别授权。

> 2026-07-17 边界修订：`docs/ADR-017-Console-Tenant-Data-Plane-Separation.md`
> 已决定 Auth Runtime 的 Session、Refresh Token、OIDC/Service Token 私钥和
> Service Client credential 全部迁入客户侧 tenant-runtime。本文的 OIDC 协议、
> Token 生命周期和 IdP 功能定义继续有效；其中“Console 本地数据库/文件”实现仅代表
> 当前迁移源，不再是目标部署形态。
...
Foundation、Aims 与 Codocs 的默认主路径已消费 Console OIDC 和已验签的 `consoleAuth` context；`account` / `auth_*` Cookie 与业务应用直连身份源只保留显式 legacy 兼容。Console 已具备本地 session、OIDC provider、JWKS、bundle 驱动 client 物化和 service token 能力；本计划继续记录未完成的协议级覆盖、密钥轮换、legacy 退场及真实环境验收。
...
现有 `console/docs/sql/Console-SQL-DDL-Draft-v1.sql` 已有 `local_sessions` 与 `auth_login_events`，需要增量补齐：
...
实现进度（2026-04-29）：已新增 `console/docs/sql/Console-SQL-Migration-v1.3-auth-runtime-core.sql`，补齐 `local_sessions` 与后续 OIDC Provider 所需核心表；已新增 `console/server/utils/authSession.ts` 与 `console/server/utils/authIdentity.ts`；CAS / 企业微信回调已改为先解析或绑定 `directory_identities`，再创建 Console 本地 session，并在 `CONSOLE_AUTH_COOKIE_MODE=dual` 下双写 `console_session` 与 legacy `token/auth_user/auth_*` Cookie；已新增 `/api/v1/console/auth/me` 与 `/api/v1/console/auth/logout`，Console 路由守卫和登录页已改为读取 `/auth/me`，`/api/auth/permissions` 也改为使用已校验的 Console session。
...
- [~] A5-T3：Codocs 默认通过 Foundation Console OIDC 与 verified `consoleAuth` context 读取身份；旧 WeCom callback 仅在显式 `HZY_AUTH_MODE=legacy` 或 `HZY_LEGACY_AUTH_BRIDGE=true` 时可用。Collab/真实多用户登录验收仍待单独完成。
...
- [~] A6-T3：refresh token reuse detection 已在代码中撤销 token family；仍需补齐 OIDC 协议级执行测试。
- [ ] A6-T4：实现 session 管理 UI：查看在线会话、强制下线、按用户/设备撤销。
- [~] A6-T5：Codocs 旧 WeCom callback 已默认 410，仅显式 legacy 兼容可用；所有业务应用旧入口的完全退场和真实兼容性验收仍待完成。
...
| 上游登录完成后的 return URL 被误当作 OIDC redirect URI | 经 CAS / 企业微信 callback 发生开放跳转 | Console upstream callback 仅接受安全相对路径；跨应用回跳只能走 OIDC client exact-match redirect URI。 |
