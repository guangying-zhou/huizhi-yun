...
状态：目标/待实施设计（不是当前运行、迁移或发布验收依据）
原始日期：2026-04-26
最后事实核对：2026-07-11
定位：把独立 `workflow` 审批流程引擎收敛为 `console.workflow-runtime` 的目标设计；配套 `docs/Huizhi-yun-Architecture.md`、`Console-Directory-Runtime-Integration-Plan.md`、`foundation/docs/Workflow-Integration-Guide.md`。

当前事实源：[`docs/Development-Execution-Checklist-2026-07.md`](../../docs/Development-Execution-Checklist-2026-07.md)、[`docs/MODULE_CONTRACTS.md`](../../docs/MODULE_CONTRACTS.md)、Workflow `CLAUDE.md`、当前 Workflow BFF 与 `data-runtime/internal/apps/workflow`。当前主路径仍是独立 Workflow BFF + tenant-runtime，而不是本计划中的 Console 内嵌 runtime；未执行任何 DDL/data migration、Cloudflare 部署、真实 callback、secret 或真实租户验收。

2026-07-11 当前安全边界：终态 callback 只允许 Codocs、Finance、People 的固定 Service API 路径，并以受信 app service origin 发送；实例/任务/动作/管理用户路径必须使用签名 actor，调用方 body/query 的 actor、scope 和 runtime 上下文会被剥离；按业务键实例读取只允许发起人、任务参与者或历史动作执行人。`request_app_code` 不再作为 runtime 信任输入，详情安全回退到跨应用外链视图。Console 内嵌迁移、完整回调可靠投递、角色型审批人目录合同、真实数据迁移与部署验收仍未完成，不能据此宣称上线。
