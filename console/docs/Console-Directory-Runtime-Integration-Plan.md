...
状态：目标/待实施设计（部分能力已落地；不是当前发布或迁移验收依据）
原始日期：2026-04-26
最后事实核对：2026-07-11
定位：把现有 `account` 的企业目录能力收敛到 `console` 的实施设计；配套 `docs/Huizhi-yun-Architecture.md`、`docs/Huizhi-yun-Platform-Target-Architecture.md`、`docs/Directory-Runtime-Contract.md`、`docs/Account-Directory-Runtime-Refactor-Plan.md` 与 `Console-Workflow-Runtime-Integration-Plan.md`。

当前事实源：[`docs/Development-Execution-Checklist-2026-07.md`](../../docs/Development-Execution-Checklist-2026-07.md)、[`docs/Huizhi-yun-Integrated-Operations-Roadmap.md`](../../docs/Huizhi-yun-Integrated-Operations-Roadmap.md)、[`docs/MODULE_CONTRACTS.md`](../../docs/MODULE_CONTRACTS.md)、Console `CLAUDE.md` 与 `docs/hzy_console_schema.sql`。本文件中未实现的 worker、迁移、环境清理和真实租户验收均不得视为已完成；任何 migration、secret、OAuth、Cloudflare 部署或真实环境操作须另行授权。

已核对的当前边界：Console 已有目录表、用户/部门/项目及成员管理接口、Account 兼容读路由与同步任务 UI；Account 仍是兼容入口和迁移源，尚未完成真实数据导入/对账/停机验收。兼容目录读路由与正式目录读路由同样要求当前用户的逐资源 `view` 权限；Foundation 不再因 `401/403` 降级到旧路由。现有 `HZY_CONSOLE_CLIENT_ID/SECRET` Basic 设想不构成已实现的跨应用授权链路，后续必须改为经 Console 签发、目标为 Console 的短期 service token 和细粒度 capability，且补齐契约测试后才能用于无人值守读取。
...
- 新增跨应用目录读取使用经 Console 签发、`aud=console` 的短期 service token 和细粒度 directory capability；不得把 `client_id:client_secret` 的 Basic header 作为已授权的运行时调用方式。
...
components:
  securitySchemes:
    ...
    ServiceCredential:
      ...
      scheme: bearer
      bearerFormat: JWT
      description: Short-lived Console-issued service token with an explicit directory read capability. Basic client credentials may only obtain a token at the token endpoint and must never authorize Directory reads directly.
  ...
  parameters:
    ...
    ProjectCodePath:
      ...
      required: true
      description: >
        Stable project registry code. Git-backed projects may use a namespaced
        code such as `huizhi-yun/account`; callers must URL-encode the complete
        value when placing it in this single path parameter.
      schema:
        type: string
...
### 5.5 目标：Platform Subject Sync Pull/Push API 鉴权与任务模型

本节描述的是后续 pull/push task 模型，尚未作为当前运行主链路交付。当前已实现 MVP 分两种模式：PM2 / 私有单租户 Console 在启动时重建 `directory_subject_exports` 后，使用 runtime token 调用 `POST /api/v1/runtime/subjects/sync`；Cloudflare 共享 Console 在 LDAP/Account/企业微信/钉钉/GitLab 或手工 subject 同步完成后，使用统一内部 token 调用 tenant-bound `POST /api/platform/internal/console/tenants/{tenantCode}/subjects/sync`。两条入口复用 Platform 同一入库事务，只写最小 subject 与 membership 投影；准确契约见 `docs/MODULE_CONTRACTS.md`。不得按本节未落地的 `subject-sync/pull`、`subject-sync/push` 或 5 分钟 worker 操作生产环境。Platform 不应直连客户侧 Console Directory API，避免 SaaS 控制面直连企业内网。
...
- Console Directory 的 service token 是企业侧内部短期凭证，来源于 Console `service_clients / service_client_credentials` 的 token 签发流程，不等同于 Platform runtime token；Basic client secret 不能直接调用 Directory API。
...
- Platform 不依赖该接口的凭证直连客户侧 Console。
- Console worker 可以直接读表，也可以通过该接口读投影；如果通过接口读，使用具备显式 capability 的短期 Console service token。
...
- `directory.mode = disabled | primary` 曾是设计项；当前未实现，不可作为现有运行开关或发布前提。后续专题需决定实现或删除该设计。
...
状态：部分完成。Console 已有用户、部门、项目及成员写接口、Account 兼容 facade、目录管理页、Directory Sync Job 最小框架、LDAP / 企业微信 / 钉钉目录源配置与 runner，以及同步任务详情/事件查看。尚无本计划可证明已完成的真实 Account 导入、hash 对账、数据切流或停机验收；provider 字段映射与运行可靠性仍需专项验证。
...
- 实现一次性导入脚本：Account → Console，脚本见 `console/docs/sql/Console-SQL-Migration-v1-account-to-directory.sql`。
- 实现增量同步任务框架。
...
- 将业务模块 env 模板中的 `HZY_ACCOUNT_API_URL / HZY_ACCOUNT_API_KEY / HZY_ACCOUNT_API_SECRET` 标为 legacy-only，并在完成迁移验收后移除；不得新增主路径依赖。
...
### Phase 5：目标改造为 Console 拉任务并回传

目标：在当前启动时 `POST /api/v1/runtime/subjects/sync` MVP 之外，评估 platform 不再接收企业端主动无任务推送、Console 使用 runtime token 拉取任务并回传的模型；Platform 不直连企业内网读取目录。未完成该评估和契约/运行验证前，现有 MVP 仍是唯一当前事实。
