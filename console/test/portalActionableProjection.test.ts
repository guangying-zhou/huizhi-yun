...
describe('portal actionable API contracts', () => {
  test('service lifecycle authenticates before payload and user summary binds current user', () => {
    ...
    const publisherBinding = readFileSync(new URL('../server/utils/localWorkflowNotificationBinding.ts', import.meta.url), 'utf8')
    assert.match(lifecycle, /resolveNotificationPublisherBinding\(event, serviceActor\)/)
    assert.match(publisherBinding, /localWorkflowServiceBinding\(\{/)
    assert.match(publisherBinding, /HZY0_WORKFLOW_LOCAL_ONLY/)
    assert.match(publisherBinding, /HZY0_NOTIFICATIONS_IN_APP_ONLY/)
    assert.match(lifecycle, /if \(!binding\) throw createError\(\{ statusCode: 403/)
    assert.match(lifecycle, /assertNotificationPublisherIdentity\(serviceActor, binding\)/)
