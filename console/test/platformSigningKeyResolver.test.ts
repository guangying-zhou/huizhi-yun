...
test('platform signing key resolver：managed cloud 在 kid 不同时按 kid 请求 Platform 内部公钥', async () => {
  ...
  const fetchSigningKey = async (url: string, options: Record<string, unknown>) => {
  ...
  const key = await resolvePlatformBundleSigningKey(config, 'psk_20260621_new', fetchSigningKey)
