...
function harness(backend = 'runtime', throwOnUseEvent = false, extraEnv: Record<string, string> = {}, verifiedAdapter: Record<string, unknown> = {}) {
  let now = 1_000_000
  let gets = 0
  ...
  runInNewContext(source, { exports, process, Buffer, Date: class extends Date { static now() { return now } }, require(name: string) {
    ...
    if (name === './verifiedPolicyRuntime') return verifiedAdapter
...
test('verified backend bypasses old memory/HMAC state and propagates dependency failure', async () => {
  let reads = 0
  const expected = { bundleVersion: 'verified' }
  const h = harness('runtime', false, {}, {
    async readVerifiedConsolePolicy() { if (++reads === 2) throw new Error('Runtime unavailable'); return expected },
    async writeVerifiedConsolePolicy() { return expected }
  })
  await h.api.writeCachedBundle('', h.bundle(), 'scope')
  const event = { context: { cloudflare: { env: { HZY_PLATFORM_BUNDLE_CACHE_BACKEND: 'verified-runtime' } } } }
  assert.equal(h.api.getRuntimeCacheDescriptor('', 'scope', event).table, 'verified_policy_snapshots')
  assert.equal(h.api.persistentPolicyStoreEnabled(event), true)
  assert.equal(await h.api.readCachedBundle('', 'scope', event), expected)
  await assert.rejects(h.api.readCachedBundle('', 'scope', event), /Runtime unavailable/)
  assert.equal(await h.api.writeCachedBundle('', h.bundle(), 'scope', event), expected)
})
...
test('verified-runtime authorization views are not re-aged by the legacy sync freshness rule', () => {
  const h = harness('runtime')
  const enterpriseEntitlement = {
    schemaVersion:'enterprise-entitlement.v1',productCode:'enterprise-full',tenantCode:'tenant',revision:1,
    status:'active',effectiveStatus:'active',effectiveFrom:'1970-01-01T00:00:00Z',end:{kind:'unlimited',evidenceReference:'test-reference'}
  }
  // Accepted 20 minutes ago under a 60-minute signed lease that is still valid.
  const accepted = h.bundle(1_000_000 - 20 * 60_000, new Date(1_000_000 + 40 * 60_000).toISOString())
  const legacy = { ...accepted, payload: { policyRevision: 1, enterpriseEntitlement } }
  assert.equal(h.api.getCachedBundleInvalidReason(legacy, 1_000_000, 300000), 'enterprise_entitlement_refresh_required')
  for (const policyValidity of ['valid', 'grace']) {
    assert.equal(h.api.getCachedBundleInvalidReason({ ...legacy, policyValidity }, 1_000_000, 300000), null)
    assert.match(h.api.getCachedBundleInvalidReason({ ...legacy, policyValidity }, 1_000_000 + 40 * 60_000, 300000), /expired/)
  }
})
