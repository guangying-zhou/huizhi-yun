...
describe('Console avatar OSS proxy boundary', () => {
  test('avatar proxy normalizes query path before calling the fixed Runtime operation', () => {
    ...
    assert.match(content, /getConsoleOSSAvatar\(event/)
    assert.match(content, /objectPath/)
    assert.match(content, /contentType\.toLowerCase\(\)\.startsWith\('image\/'\)/)
    assert.match(content, /X-Content-Type-Options/)
    assert.match(content, /message: 'Failed to fetch avatar'/)
    assert.doesNotMatch(content, /message: error\.message \|\| 'Failed to fetch avatar'/)
    assert.doesNotMatch(content, /ali-oss|resolveVaultSecret|accessKeySecret|getIntegration/)
    assertBefore(content, 'normalizeAvatarObjectPath(query.path)', 'getConsoleOSSAvatar(event')
  })
})
