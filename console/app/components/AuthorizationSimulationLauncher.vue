...
const roleSelectOptions = computed(() => enterpriseRoles.value.map(role => ({
  label: role.roleName && role.roleName !== role.roleCode ? `${role.roleName} (${role.roleCode})` : role.roleCode,
  value: role.roleCode
})))
...
watch(authenticated, (isAuthenticated) => {
  ...
  void loadEnterpriseRoles({ force: true })
...
function openModal() {
  ...
  if (canSimulateRole.value) {
    form.mode = 'role_simulation'
    form.roleCode ||= getAuthorization()?.activeRoleCode || availableRoles.value[0]?.roleCode || enterpriseRoles.value[0]?.roleCode || ''
  } else {
    form.mode = 'user_simulation'
  }
  open.value = true
      ...
      <div class="space-y-4">
        ...
        >
          <USelect
            ...
            :loading="enterpriseRolesLoading"
            class="w-full font-mono"
            placeholder="选择要模拟的企业角色"
          />
        </UFormField>
