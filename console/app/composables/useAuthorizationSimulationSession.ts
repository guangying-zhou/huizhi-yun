...
export interface AuthorizationEnterpriseRoleOption {
  roleCode: string
  roleName: string
}

interface AuthorizationEnterpriseRolesResponse {
  code: number
  data?: {
    items?: AuthorizationEnterpriseRoleOption[]
  }
}
...
type RefreshAuthorizationStateOptions = {
...
export function useAuthorizationSimulationSession() {
  ...
  const enterpriseRoles = useState<AuthorizationEnterpriseRoleOption[]>('console:authorization-simulation-enterprise-roles', () => [])
  const enterpriseRolesLoaded = useState('console:authorization-simulation-enterprise-roles-loaded', () => false)
  const enterpriseRolesLoading = useState('console:authorization-simulation-enterprise-roles-loading', () => false)
  ...
  async function refreshAuthorizationState(options: RefreshAuthorizationStateOptions = {}) {
    const { clearCache, loadPermissions } = usePermissions()
    const { clearApps, loadApps } = useUserApplications()
    clearCache()
    clearApps()

    const refreshes: Promise<unknown>[] = [
      loadPermissions({ force: true }),
      loadApps(true)
    ]

    if (options.operatorControls) {
      capabilityLoaded.value = false
      canSimulateRole.value = false
      canSimulateUser.value = false
      enterpriseRolesLoaded.value = false
      enterpriseRoles.value = []
      refreshes.push(
        loadPlatformSimulationCapabilities({ force: true }),
        loadEnterpriseRoles({ force: true })
      )
    }
  ...
  async function loadEnterpriseRoles(options: { force?: boolean } = {}) {
    if (enterpriseRolesLoading.value) {
      return enterpriseRoles.value
    }
    if (enterpriseRolesLoaded.value && !options.force) {
      return enterpriseRoles.value
    }

    enterpriseRolesLoading.value = true
    try {
      const response = await $fetch<AuthorizationEnterpriseRolesResponse>('/api/v1/console/authorization/enterprise-roles')
      enterpriseRoles.value = response.data?.items || []
      enterpriseRolesLoaded.value = true
    } catch (error) {
      if (![401, 403].includes(statusCode(error))) {
        console.warn('[AuthorizationSimulation] Failed to load enterprise role catalog:', error)
      }
      enterpriseRoles.value = []
      enterpriseRolesLoaded.value = true
    } finally {
      enterpriseRolesLoading.value = false
    }
  ...
  return {
    ...
    canSimulateUser,
    enterpriseRoles,
    enterpriseRolesLoaded,
    enterpriseRolesLoading,
    loadSession,
