...
function idempotencyKey(operation: string) {
  return `console:business-domain:${operation}:${globalThis.crypto?.randomUUID?.() || Date.now()}`
}
  ...
  try {
    ...
    if (toAdd.length > 0) {
      await $fetch(`/api/v1/companies/${code}/business-domains`, {
        method: 'POST',
        headers: { 'idempotency-key': idempotencyKey('preset-add') },
        body: {
    ...
    for (const domainCode of toRemove) {
      const domain = domains.value.find(item => item.domainCode === domainCode)
      if (!domain) continue
      await $fetch(`/api/v1/companies/${code}/business-domains/${encodeURIComponent(domainCode)}`, {
        method: 'DELETE',
        headers: { 'idempotency-key': idempotencyKey(`preset-remove:${domainCode}`) },
        body: { expectedRevision: domain.revision }
      })
    }
...
async function saveCustom() {
  ...
  try {
    ...
    await $fetch(`/api/v1/companies/${code}/business-domains`, {
      method: 'POST',
      headers: { 'idempotency-key': idempotencyKey('custom-create') },
      body: {
...
async function saveEdit() {
  ...
  try {
    ...
    await $fetch(`/api/v1/companies/${code}/business-domains/${encodeURIComponent(editingDomain.value.domainCode)}`, {
      method: 'PATCH',
      headers: { 'idempotency-key': idempotencyKey(`update:${editingDomain.value.domainCode}`) },
      body: {
        aliasName: editForm.value.aliasName || null,
        sortOrder: editForm.value.sortOrder,
        expectedRevision: editingDomain.value.revision
      }
    })
...
async function removeDomain(domain: CompanyDomain) {
  if (!(await confirm({
    title: '删除业务领域',
    message: `确定删除「${domain.displayName}」？删除后不可恢复。`,
    tone: 'danger'
  }))) return
  try {
    const code = await loadCompanyCode()
    await $fetch(`/api/v1/companies/${code}/business-domains/${encodeURIComponent(domain.domainCode)}`, {
      method: 'DELETE',
      headers: { 'idempotency-key': idempotencyKey(`delete:${domain.domainCode}`) },
      body: { expectedRevision: domain.revision }
    })
    ...
    <template #body>
      <div class="space-y-4">
        ...
        <UCard :ui="{ body: 'p-0' }">
          ...
          >
            <template #empty>
              <CommonEmptyState icon="i-lucide-network" title="暂无业务域" description="新建业务域后可用于组织与授权范围配置。" />
            </template>
