...
const divisionRevision = ref(0)

function idempotencyKey(operation: string) {
  return `console:region:${operation}:${globalThis.crypto?.randomUUID?.() || Date.now()}`
}
...
async function initFromTemplate() {
  if (!(await confirm({
    title: '初始化区域',
    message: '将从标准七大区模板初始化，已有区域不会被覆盖。是否继续？',
    confirmLabel: '初始化'
  }))) return
  saving.value = true
  try {
...
async function saveEdit() {
  ...
  try {
    ...
    await $fetch(`/api/v1/companies/${code}/regions/${encodeURIComponent(editingRegion.value.regionCode)}`, {
      method: 'PATCH',
      headers: { 'idempotency-key': idempotencyKey(`update:${editingRegion.value.regionCode}`) },
      body: {
        ...editForm.value,
        expectedRevision: editingRegion.value.revision
      }
    })
...
async function deleteRegion(region: Region) {
  if (!(await confirm({
    title: '删除区域',
    message: `确定删除区域「${region.regionName}」及其行政区划映射？删除后不可恢复。`,
    tone: 'danger'
  }))) return
  try {
    const code = await loadCompanyCode()
    await $fetch(`/api/v1/companies/${code}/regions/${encodeURIComponent(region.regionCode)}`, {
      method: 'DELETE',
      headers: { 'idempotency-key': idempotencyKey(`delete:${region.regionCode}`) },
      body: { expectedRevision: region.revision }
    })
...
async function openDivisionModal(region: Region) {
  ...
  try {
    const code = await loadCompanyCode()
    const res = await $fetch<ApiResponse<DivisionMapping[]> & { meta: { revision: number } }>(`/api/v1/companies/${code}/regions/${encodeURIComponent(region.regionCode)}/divisions`)
    divisionRevision.value = res.meta.revision
    const codes: string[] = []
...
async function saveDivisions() {
  ...
  try {
    ...
    await $fetch(`/api/v1/companies/${code}/regions/${encodeURIComponent(divisionRegion.value.regionCode)}/divisions`, {
      method: 'PUT',
      headers: { 'idempotency-key': idempotencyKey(`divisions:${divisionRegion.value.regionCode}`) },
      body: {
        divisions,
        expectedRevision: divisionRevision.value
      }
    })
