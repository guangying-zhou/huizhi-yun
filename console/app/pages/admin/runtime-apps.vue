...
const canAdminRuntimeApps = computed(() => permissionsLoaded.value && hasPermission('runtime_apps', 'admin'))
...
function openConfirm(app: RuntimeAppStatus, action: RuntimeAction) {
  if (!canAdminRuntimeApps.value) {
    toast.add({ color: 'error', title: '无权限', description: '需要应用运行管理权限' })
    return
  }
                ...
                <div class="col-span-6 md:col-span-3">
                  <div class="flex justify-end gap-2">
                    <UButton
                      ...
                      size="sm"
                      variant="soft"
                      :disabled="!canAdminRuntimeApps || !app.manageable || app.status === 'online'"
