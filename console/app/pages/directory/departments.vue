...
import { extractErrorMessage } from '~/utils/log'
...
async function submitDepartment() {
  ...
  const payload = modalMode.value === 'create'
    ? departmentPayload()
    : departmentChanges()
  if (modalMode.value === 'edit' && Object.keys(payload).length === 0) {
    toast.add({ title: '未检测到更改', color: 'neutral' })
  ...
  try {
    if (modalMode.value === 'create') {
      await $fetch('/api/v1/console/directory/departments', {
        method: 'POST',
        headers: { 'idempotency-key': `directory:department:create:${globalThis.crypto?.randomUUID?.() || Date.now()}` },
        body: payload
      })
    } else {
      await $fetch(`/api/v1/console/directory/departments/${encodeURIComponent(form.deptCode)}`, {
        method: 'PATCH',
        headers: { 'idempotency-key': `directory:department:update:${form.deptCode}:${globalThis.crypto?.randomUUID?.() || Date.now()}` },
        body: payload
      })
    }
...
async function deleteDepartment(dept: DirectoryDepartment) {
  if (!(await confirm({
    title: '删除部门',
    message: `确认删除部门「${dept.name}」？删除后不可恢复。`,
    tone: 'danger'
  }))) return
  ...
  try {
    await $fetch(`/api/v1/console/directory/departments/${encodeURIComponent(dept.deptCode)}`, {
      method: 'DELETE',
      headers: { 'idempotency-key': `directory:department:delete:${dept.deptCode}:${globalThis.crypto?.randomUUID?.() || Date.now()}` }
    })
  ...
  <UDashboardPanel id="directory-departments" :ui="dashboardPanelUi">
    ...
    <template #body>
      ...
      <div>
        ...
        >
          <template #empty>
            <CommonEmptyState icon="i-lucide-building-2" title="暂无部门" description="新建部门或运行目录同步后会显示在这里。" />
          </template>
        </UTable>
      </div>
