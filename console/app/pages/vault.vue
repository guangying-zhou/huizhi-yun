...
const canAdminVault = computed(() => permissionsLoaded.value && hasPermission('credential_vault', 'admin'))
...
function openReveal(secret: VaultSecret) {
  if (!canAdminVault.value) {
    toast.add({ title: '权限不足', description: '需要凭证库管理员权限。', color: 'warning' })
    return
  }
  selectedCode.value = secret.secretCode
  revealReason.value = ''
    ...
    <template #body>
      <div class="space-y-4">
        ...
        <div>
          ...
          >
            <template #empty>
              <CommonEmptyState icon="i-lucide-vault" title="暂无凭证" description="新建凭证后可供集成和服务安全引用。" />
            </template>
