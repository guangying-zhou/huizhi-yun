...
const canViewDataRuntime = computed(() => permissionsLoaded.value && hasPermission('data_runtime', 'view'))
const canEditDataRuntime = computed(() => permissionsLoaded.value && hasPermission('data_runtime', 'edit'))
const canDeployDataRuntime = computed(() => permissionsLoaded.value && hasPermission('data_runtime', 'deploy'))

const { data, pending, refresh } = await useFetch<ApiResponse<DataRuntimeOverview>>(
  '/api/v1/console/data-runtime/status',
  {
    immediate: canViewDataRuntime.value,
    default: () => ({
      code: 0,
      data: {
        parameters: {
...
const updateStatusDescription = computed(() => {
  ...
  return `最近检测：${formatDateTime(status.checkedAt)}`
})
...
async function refreshOverview() {
  if (!canViewDataRuntime.value) {
    toast.add({ color: 'error', title: '无权限', description: '需要数据运行时查看权限' })
    return
  }
  await refresh()
}
...
async function loadUpdateStatus(silent = false) {
  if (!canViewDataRuntime.value) return
  try {
...
async function saveParameters() {
  if (!canEditDataRuntime.value) {
    toast.add({ color: 'error', title: '无权限', description: '需要数据运行时编辑权限' })
    return
  }
  saving.value = true
  try {
...
async function triggerUpdate() {
  if (!canDeployDataRuntime.value) {
    toast.add({ color: 'error', title: '无权限', description: '需要数据运行时部署权限' })
    return
  }
  updating.value = true
  updateError.value = ''
  try {
    ...
    updateStatus.value = {
      operationId: String(response.data.result.operationId || '') || null,
      status: String(response.data.result.status || 'running'),
      phase: String(response.data.result.phase || 'queued'),
      ...
      packageSourceFingerprint: String(response.data.result.packageSourceFingerprint || '') || null,
...
<template>
  <UDashboardPanel id="data-runtime" :ui="dashboardPanelUi">
    ...
    <template #body>
      <UAlert
        v-if="!canViewDataRuntime"
        color="warning"
        variant="soft"
        icon="i-lucide-lock"
        title="无权查看数据运行时"
        description="当前账号缺少 data_runtime:view 权限。"
      />
        ...
        <div class="grid gap-4 xl:grid-cols-[1fr_1.2fr]">
          <div class="rounded-lg border border-default bg-default p-4">
            ...
            <dl class="grid grid-cols-[6rem_1fr] gap-2 text-sm">
              ...
              <dd>{{ formatDateTime(latest?.publishedAt || latest?.builtAt) }}</dd>
              <dt class="text-muted">
                Commit
              </dt>
              <dd class="font-mono">
        ...
        <div class="rounded-lg border border-default bg-default p-4">
          ...
          <div class="grid gap-4 xl:grid-cols-[1fr_1fr]">
            <dl class="grid grid-cols-[6rem_1fr] gap-2 text-sm">
              ...
              <dd>{{ formatDateTime(runtime?.builtAt) }}</dd>
              <dt class="text-muted">
                Commit
              </dt>
              <dd class="font-mono">
              ...
              <dd>{{ formatDateTime(runtime?.checkedAt) }}</dd>
            </dl>
