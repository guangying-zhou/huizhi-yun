...
export default defineEventHandler(async (event) => {
  ...
  const session = await measureRequestStage(event, 'console_session', () => resolveConsoleSession(event))
  const body = await readBody<Record<string, unknown>>(event)
  const targetAppCode = stringValue(body.appCode) || appCode

  try {
    const snapshot = await measureRequestStage(event, 'console_policy', () => loadPolicyScopedAuthorization(session.uid, targetAppCode, event, {
