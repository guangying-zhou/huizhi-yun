...
function memoryCacheTtlMs(event?: H3Event) {
...
export function persistentPolicyStoreEnabled(event?: H3Event) {
  return runtimeEnvValue('HZY_PLATFORM_BUNDLE_CACHE_BACKEND', event).toLowerCase() === 'runtime'
}
...
function runtimeCacheBackend(event?: H3Event): 'file' | 'memory' | 'runtime' {
...
function memoryCacheSet<T>(key: string, value: T, deadline = Infinity, event?: H3Event) {
  memoryCache.set(key, { value, expiresAt: Math.min(deadline, Date.now() + memoryCacheTtlMs(event)) })
}
