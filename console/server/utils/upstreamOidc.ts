...
type ExternalJsonFetch = <T>(url: string, options?: Record<string, unknown>) => Promise<T>
const fetchExternalJson = fetchExternal as unknown as ExternalJsonFetch
...
async function discover(issuer: string) {
  ...
  if (!discoveryByIssuer.has(normalizedIssuer)) {
    discoveryByIssuer.set(
      normalizedIssuer,
      fetchExternalJson<DiscoveryDocument>(`${normalizedIssuer}/.well-known/openid-configuration`).catch(() => ({}))
    )
  }
...
export async function startUpstreamOidcLogin(event: H3Event) {
  ...
  if (query.prompt === 'login' || (forceLogin && hasConsoleLogoutMarker(event))) {
...
async function exchangeCode(event: H3Event, config: UpstreamOidcConfig, code: string, verifier: string) {
  ...
  return await fetchExternalJson<TokenResponse>(config.tokenEndpoint, {
    method: 'POST',
...
async function resolveClaims(config: UpstreamOidcConfig, tokenSet: TokenResponse, nonce: string): Promise<UserClaims> {
  ...
  if (tokenSet.access_token && config.userinfoEndpoint) {
    return await fetchExternalJson<UserClaims>(config.userinfoEndpoint, {
      headers: {
