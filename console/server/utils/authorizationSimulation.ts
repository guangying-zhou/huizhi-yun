...
function simulationSecret(event: H3Event) {
  ...
  if (runMode === 'dev') {
    return 'hzy-console-dev-authorization-simulation'
  }
...
export async function writeAuthorizationSimulationAudit(event: H3Event, input: AuthorizationSimulationAuditInput) {
  try {
    const result = input.result
      || (input.action === 'denied' || input.action === 'failed' || input.action === 'expired' || input.action === 'invalidated' ? 'failed' : 'success')
    await appendConsoleHumanOperationLog(event, {
      sourceApp: 'console',
      action: `simulation.${input.action}`,
      targetType: input.mode || 'authorization_simulation',
      targetId: stringValue(input.roleCode) || stringValue(input.subjectCode) || null,
    ...
    }, `console:authorization-simulation:${crypto.randomUUID()}`)
  } catch (error) {
