...
function runtimeDisabledActivationStatus(): ActivationStatus {
  return Object.assign(emptyActivationStatus(), {
    mode: 'active',
    activated: true,
    envValid: true,
    licenseValid: true,
...
type PlatformRuntimeFetch = <T>(
  url: string,
  options: Record<string, unknown>
) => Promise<T>

/**
 * 托管云 Worker 调 Platform internal API 会经过公共边缘。
 * 生产 zone `huizhi.yun` 的 `CN_CA_JP` WAF 规则按 `ip.src.country` 拦截，
 * 而 cron 触发的 Worker 子请求没有访客上下文、被归属为 `country=US`——
 * 2026-08-23 firewall events 证实 policy bundle 拉取每 5 分钟被拦一次。
 *
 * 豁免条件是 `cf.worker.upstream_zone`（边缘设置、不可伪造）加本 UA。
 * UA 单独不构成安全边界（任何人都能伪造），只用于把豁免范围收窄到我们自己的服务调用。
 */
const PLATFORM_WORKER_USER_AGENT = 'HZY-Cloudflare-Worker/1.0'

const rawPlatformRuntimeFetch = fetchExternal as unknown as PlatformRuntimeFetch
export const platformRuntimeFetch: PlatformRuntimeFetch = async (url, options) => {
  const headers = {
  ...
  if (binding && typeof binding.fetch === 'function') {
    ...
    const response = await binding.fetch(target, {
      ...
      signal: typeof AbortSignal?.timeout === 'function'
        ? AbortSignal.timeout(Number(options.timeout || POLICY_BUNDLE_FETCH_TIMEOUT_MS))
        : undefined
    })
    ...
    if (!response.ok) {
      ...
      throw createError({
        ...
        message: message || response.statusText || 'Platform service request failed'
      })
    }
    return payload
  }
  return rawPlatformRuntimeFetch(url, { ...options, headers })
}
...
export async function fetchAndVerifyPolicyBundle(config: PlatformRuntimeConfig, event?: H3Event): Promise<CachedPolicyBundle> {
  ...
  if (!policyBundleDeploymentMatchesRuntime({
    ...
    bundleDeploymentCode: normalizeString(data.deploymentId)
  })) {
...
export async function postPlatformHeartbeat(config: PlatformRuntimeConfig, bundle: CachedPolicyBundle | null) {
  ...
  return platformRuntimeFetch<HeartbeatEnvelope>(
...
export async function fetchPlatformTenantProfile(config: PlatformRuntimeConfig): Promise<PlatformTenantProfileResponse> {
  const response = await platformRuntimeFetch<PlatformTenantProfileEnvelope>(
