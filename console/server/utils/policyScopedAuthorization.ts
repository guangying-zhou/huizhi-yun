...
function permissionMatches(
  ...
  if (action && !actionSatisfies(permission.action, action, policy)) return false
  return true
}
  ...
  return grants
    .map(grant => ({
      ...
      permissions: grant.permissions.filter(permission =>
        permissionMatches(permission, appCode, resourceCode, action, policy)
      )
    }))
...
export async function loadPolicyScopedAuthorization(
  ...
  const actionPolicy = buildPolicyBundleActionPolicy(snapshot.payload || {}, appCode, resourceCode)
  const grants = filterGrants(grantResult.grants, appCode, resourceCode, action, actionPolicy)
