...
type PlatformSigningKeyFetch = (
  url: string,
  options: Record<string, unknown>
) => Promise<PlatformSigningKeyEnvelope>
...
async function fetchPlatformSigningKey(
  config: PlatformSigningKeyResolverConfig,
  kid: string,
  fetchSigningKey: PlatformSigningKeyFetch
) {
  return await fetchSigningKey(
...
export async function resolvePlatformBundleSigningKey(
  config: PlatformSigningKeyResolverConfig,
  kid: string,
  fetchSigningKey: PlatformSigningKeyFetch = fetchExternal as unknown as PlatformSigningKeyFetch
  ...
  try {
    const envelope = await fetchPlatformSigningKey(config, normalizedKid, fetchSigningKey)
