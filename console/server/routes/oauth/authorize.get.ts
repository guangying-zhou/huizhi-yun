...
export default defineEventHandler(async (event) => {
  ...
  const client = await requireAuthorizeOidcClient(event, first(query.client_id))
  const redirectUri = await assertAuthorizeRedirectUri(event, client, first(query.redirect_uri), 'redirect')
