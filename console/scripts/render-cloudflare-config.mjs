...
function requireBase64Ed25519PublicKey(name, input) {
  try {
    const pem = Buffer.from(String(input || '').trim(), 'base64').toString('utf8')
    requireEd25519PublicKey(name, pem)
  } catch (error) {
    console.error(`Invalid ${name}. Expected base64 of an Ed25519 public-key PEM: ${error instanceof Error ? error.message : String(error)}`)
    process.exit(1)
  }
}
...
const deploymentProfile = value('HZY_DEPLOYMENT_PROFILE', value('NUXT_PUBLIC_DEPLOYMENT_PROFILE', value('DEPLOYMENT_PROFILE', 'managed-cloud-runtime')))
const appBasePath = value('HZY_APP_BASE_PATH', '/')
const bundleCacheBackend = value('HZY_PLATFORM_BUNDLE_CACHE_BACKEND', 'memory').toLowerCase()
...
const platformLifecycleSyncEnabled = booleanValue('HZY_CONSOLE_PLATFORM_LIFECYCLE_SYNC_ENABLED', 'false') ? 'true' : 'false'
const connectorPackageBaseUrl = trimTrailingSlash(value('HZY_CONNECTOR_RUNTIME_PACKAGE_BASE_URL', 'https://downloads.huizhi.yun/packages/hzy-connector-runtime'))
const connectorReleasePublicKey = value('HZY_CONNECTOR_RUNTIME_RELEASE_PUBLIC_KEY_PEM_BASE64')

if (!['memory', 'runtime'].includes(bundleCacheBackend)) {
  console.error([
    'Invalid Console Cloudflare runtime cache backend.',
    'Cloudflare Console must not own a database or Hyperdrive binding.',
    'Use memory or runtime for policy bundles; database access belongs to Tenant Runtime.'
  ].join('\n'))
  process.exit(1)
}
...
requireBase64Ed25519PublicKey('HZY_CONNECTOR_RUNTIME_RELEASE_PUBLIC_KEY_PEM_BASE64', connectorReleasePublicKey)
...
const config = {
  ...
  services: [
    {
      binding: 'HZY_PLATFORM_SERVICE',
      service: value('HZY_PLATFORM_WORKER_NAME', 'hzy-platform')
    }
  ],
  ...
  vars: {
    ...
    CONSOLE_OIDC_ISSUER: oidcIssuer,
    ...
    HZY_PLATFORM_BUNDLE_MEMORY_TTL_MS: value('HZY_PLATFORM_BUNDLE_MEMORY_TTL_MS', bundleCacheBackend === 'runtime' ? '300000' : '30000'),
    ...
    HZY_CONSOLE_PLATFORM_LIFECYCLE_SYNC_ENABLED: platformLifecycleSyncEnabled,
    HZY_CONNECTOR_RUNTIME_PACKAGE_BASE_URL: connectorPackageBaseUrl,
    HZY_CONNECTOR_RUNTIME_RELEASE_PUBLIC_KEY_PEM_BASE64: connectorReleasePublicKey,
