...
await withTemporaryMySql(plan, async context => {
  await new Promise((done, reject) => {
    const child = spawn('go', ['test', './internal/migrations/unified', '-run', '^TestProductMasterConflictReportMySQL$', '-count=1', '-v'], {
      cwd: resolve(rootDir, 'data-runtime'), stdio: 'inherit',
      env: { ...process.env, HZY_INT202_TEST_SOCKET: context.socketPath }
    })
    child.once('error', reject)
    child.once('exit', code => code === 0 ? done() : reject(new Error(`INT-202 MySQL test exited ${code}`)))
  })
