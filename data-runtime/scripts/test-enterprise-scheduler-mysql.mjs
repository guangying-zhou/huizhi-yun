...
await withTemporaryMySql(plan, async (context) => {
  await new Promise((resolveRun, reject) => {
    const child = spawn('go', ['test', '-race', './internal/enterprisescheduler', '-run', '^TestSchedulerRegistryMapped(Claim|Completion|MilestoneRollover|DueNotification|AssetsDueNotification)MySQL$', '-count=1', '-v'], {
