...
await withTemporaryMySql(plan, async (context) => {
  await new Promise((resolveRun, reject) => {
    const child = spawn('go', ['test', '-race', './internal/server', '-run', '^TestEnterpriseContractActivationHTTPMySQL$', '-count=1', '-v'], {
      cwd: resolve(rootDir, 'data-runtime'), stdio: 'inherit',
      env: { ...process.env, HZY_CONTRACT_ACTIVATION_HTTP_SOCKET: context.socketPath }
    })
