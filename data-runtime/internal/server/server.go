...
type Server struct {
	...
	snapshotRetention             snapshotRetentionCache
...
func New(cfg config.Config) (*Server, error) {
	var gatewayKeys *gatewaykeys.Store
	if cfg.GatewayKeyset.Enabled {
		var err error
		gatewayKeys, err = gatewaykeys.New(gatewaykeys.Binding{Tenant: cfg.Tenant, Environment: cfg.GatewayKeyset.Environment, RuntimeCode: cfg.Control.RuntimeCode, GatewayDeployment: cfg.GatewayKeyset.GatewayDeployment}, cfg.GatewayKeyset.PlatformKeyID, cfg.GatewayKeyset.PlatformPublicKey, filepath.Join(cfg.Control.ConfigDir, "gateway-keyset.json"))
		if err != nil {
			return nil, fmt.Errorf("gateway keyset configuration invalid")
		}
	}
...
func (s *Server) route(r *http.Request) (routeResult, error) {
	...
	if r.Method == http.MethodPost && (path == "/v1/enterprise/console/directory-self:departments" || path == "/v1/enterprise/console/directory-self:projects") {
		return s.routeEnterpriseDirectorySelf(r, path)
	}
	...
	if action, ok := enterpriseProductDocumentActions[path]; ok && r.Method == http.MethodPost {
		return s.routeEnterpriseProductDocuments(r, action)
	}
	if path == "/v1/enterprise/aims/product-documents:link" && r.Method == http.MethodPost {
		return s.routeEnterpriseProductDocumentLink(r)
	}
	if path == "/v1/enterprise/aims/project-products:list" && r.Method == http.MethodPost {
		return s.routeEnterpriseProjectProducts(r, "list")
	}
	if path == "/v1/enterprise/aims/project-products:link" && r.Method == http.MethodPost {
		return s.routeEnterpriseProjectProducts(r, "link")
	}
	if route, ok := enterpriseCodocsReadRoutes[path]; ok && r.Method == http.MethodPost {
		return s.routeEnterpriseCodocsRead(r, route)
	}
	if route, ok := enterpriseCodocsCollaborationRoutes[path]; ok && r.Method == http.MethodPost {
		return s.routeEnterpriseCodocsOperation(r, route, enterpriseCodocsCollaborationQuery)
	}
	if route, ok := enterpriseCodocsPublishExecutionRoutes[path]; ok && r.Method == http.MethodPost {
		return s.routeEnterpriseCodocsOperation(r, route, enterpriseCodocsPublishExecutionQuery)
	}
	if route, ok := enterpriseCodocsDocumentShareRoutes[path]; ok && r.Method == http.MethodPost {
		return s.routeEnterpriseCodocsOperation(r, route, enterpriseCodocsDocumentShareQuery)
	}
	if route, ok := enterpriseCodocsReviewHistoryRoutes[path]; ok && r.Method == http.MethodPost {
		return s.routeEnterpriseCodocsOperation(r, route, enterpriseCodocsReviewHistoryQuery)
	}
	if route, ok := enterpriseCodocsVersionRoutes[path]; ok && r.Method == http.MethodPost {
		return s.routeEnterpriseCodocsOperation(r, route, enterpriseCodocsVersionQuery)
	}
	if route, ok := enterpriseCodocsAnnotationsRoutes[path]; ok && r.Method == http.MethodPost {
		return s.routeEnterpriseCodocsOperation(r, route, enterpriseCodocsAnnotationsQuery)
	}
	if capability, ok := collaborationSnapshotActions[path]; ok && r.Method == http.MethodPost && s.cfg.Apps.Codocs.SnapshotV2Enabled && s.cfg.Apps.Codocs.CollaborationV2Enabled {
		return s.routeCodocsCollaborationSnapshot(r, path, capability)
	}
	if route, ok := enterpriseCodocsCollaborationSessionRoutes[path]; ok && r.Method == http.MethodPost && s.cfg.Apps.Codocs.SnapshotV2Enabled && s.cfg.Apps.Codocs.CollaborationV2Enabled {
		return s.routeEnterpriseCodocsOperation(r, route, enterpriseCodocsCollaborationSessionQuery)
	}
	if route, ok := enterpriseCodocsDocumentSnapshotRoutes[path]; ok && r.Method == http.MethodPost && s.cfg.Apps.Codocs.SnapshotV2Enabled {
		return s.routeEnterpriseCodocsOperation(r, route, enterpriseCodocsDocumentSnapshotQuery)
	}
	if route, ok := enterpriseCodocsDocumentUpdateRoutes[path]; ok && r.Method == http.MethodPost {
		return s.routeEnterpriseCodocsOperation(r, route, enterpriseCodocsDocumentUpdateQuery)
	}
	if route, ok := enterpriseCodocsDocumentTransferRoutes[path]; ok && r.Method == http.MethodPost {
		return s.routeEnterpriseCodocsOperation(r, route, enterpriseCodocsDocumentTransferQuery)
	}
	...
	if path == enterpriseProjectUpdatePath && r.Method == http.MethodPost {
	...
	if action, ok := map[string]string{
		...
		"/v1/enterprise/assets/ip-assets:products": "products",
	}[path]; ok && r.Method == http.MethodPost {
	...
	if path == "/v1/console/verified-policy/renewal" {
		return s.routeVerifiedPolicy(r)
	}
	...
	if r.Method == http.MethodPost && path == "/v1/console/auth/service-tokens/issue" {
		var authCtx auth.Context
		var err error
		// A Console key assertion is the Platform-independent alternative to
		// the Platform bootstrap token; it is accepted on this route only.
		if assertion := consoleAssertionBearer(r); assertion != "" {
			authCtx, err = s.authenticateConsoleAssertion(r, assertion)
		} else {
			authCtx, err = s.auth.Authenticate(r, auth.Requirement{
	...
	if r.Method == http.MethodPost && path == gatewaykeys.ExchangePath {
		return s.gatewayExchange(r)
	}

	if r.Method == http.MethodPost && path == "/v1/console/auth/service-tokens/exchange" {
		authCtx, err := s.auth.Authenticate(r, auth.Requirement{
			AppCode: "console", Scope: "console:service-token:exchange", SourceAppCode: "console",
		...
		if authCtx.ClientID != "console.runtime" || authCtx.Subject != "client:console.runtime" || authCtx.Mode != "jwt" {
			return routeResult{}, httperror.New(http.StatusForbidden, "console_exchange_identity_invalid", "Console Runtime service identity required")
		}
		adapter, err := s.requireConsole()
		if err != nil {
			return routeResult{}, err
		}
		if err := adapter.VerifyConsoleExchangeCaller(r.Context(), authCtx.CredentialID); err != nil {
			return routeResult{}, err
		}
		body, err := readJSONBody(r)
		if err != nil {
		...
		result, err := adapter.ExchangeConsoleServiceClientToken(
			r.Context(), body, authCtx.Tenant, authCtx.Deployment, consoleapp.AuditMutationMeta{
				RequestID: requestID(r), ActorType: "service",
				ActorID: authCtx.Subject, SourceApp: authCtx.AppCode,
			},
		)
		...
		return routeResult{Operation: "console.auth.service_token.exchange", Auth: &authCtx, Body: result}, err
	}
