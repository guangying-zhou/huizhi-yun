...
func TestEnterpriseVersionReleaseHTTPMySQL(t *testing.T) {
	...
	authenticator, template, _ := enterpriseContextFixture(t, func(c jwt.MapClaims) {
		c["scope"] = "aims:product-versions:edit aims:product-versions:delete aims:product-versions:archive aims:product-versions:reopen aims:product-versions:accept aims:product-versions:publish aims:product-versions:read"
	}, true)
	...
	call := func(action string, value any, mutate func(*enterpriseVersionInput, *http.Request)) (int, string) {
		...
		permission := action
		if action == "acceptance-preview" || action == "execution-coordination" || action == "execution-project-authorization" || action == "acceptance-list" || action == "acceptance-view" || action == "release-list" || action == "release-view" {
			permission = "view"
		}
