...
var enterpriseVersionActions = map[string]string{
	...
	"/v1/enterprise/aims/product-version:scope-list":                      "scope-list",
	"/v1/enterprise/aims/product-version:scope-history":                   "scope-history",
	"/v1/enterprise/aims/product-version:scope-deliver":                   "scope-deliver",
	"/v1/enterprise/aims/product-version:scope-reopen":                    "scope-reopen",
	"/v1/enterprise/aims/product-version:scope-edit":                      "scope-edit",
	"/v1/enterprise/aims/product-version:scope-visibility":                "scope-visibility",
	"/v1/enterprise/aims/product-version:scope-legacy-criteria":           "scope-legacy-criteria",
...
type enterpriseVersionInput struct {
	...
	PlanningAuthorization *pc.AuthorizationPermit `json:"planning_authorization,omitempty"`
...
func (s *Server) routeEnterpriseVersionCommand(r *http.Request, action string) (routeResult, error) {
	...
	capability := action
	if action != "scope-deliver" && action != "scope-reopen" && action != "scope-edit" && action != "scope-visibility" && action != "scope-legacy-criteria" {
		capability = permission
	}
	if action == "execution-coordination" || action == "acceptance-preview" || action == "execution-project-authorization" || action == "acceptance-list" || action == "acceptance-view" || action == "release-list" || action == "release-view" || action == "scope-list" || action == "scope-history" {
		permission, capability = "view", "read"
	}
	route := enterpriseRouteContext{Binding: enterprise.BindingKey{Tenant: s.cfg.Tenant, Environment: s.cfg.Enterprise.Environment, RuntimeDeployment: s.cfg.Deployment}, HostDeployment: s.cfg.DeploymentBindings["enterprise"], LogicalSource: "aims", LogicalTarget: "aims", Capability: "aims:product-versions:" + capability}
	...
	if action != "scope-edit" {
		if planning, present := body["planning_authorization"]; present && planning != nil {
			return result, httperror.New(400, "enterprise_version_planning_unexpected", "Planning authorization is not supported for this action")
		}
	}
	...
	if action == "scope-edit" {
		if input.PlanningAuthorization == nil {
			return result, httperror.New(403, "enterprise_version_planning_required", "Planning authorization is required")
		}
		if err = validateEnterpriseProductPermit(input.ProductCode, input.Tenant, input.Deployment, *input.PlanningAuthorization, "product_priorities", "prioritize", verified, time.Now()); err != nil {
			return result, err
		}
	}
	if action == "scope-list" || action == "scope-history" {
		var value struct {
		...
		if action == "scope-list" {
			if value.ScopeID != 0 {
				return result, httperror.New(400, "enterprise_version_scope_input_invalid", "Scope identity is not supported for list")
			}
			out, err = s.enterpriseVersions.ListScope(r.Context(), input.ProductCode, verified.ActorUID, input.Authorization, value.VersionID, pc.PlanningPageQuery{Page: value.Page, PageSize: value.PageSize, Keyword: value.Keyword})
		} else {
			if value.Keyword != "" {
				return result, httperror.New(400, "enterprise_version_scope_input_invalid", "Keyword is not supported for history")
			}
			out, err = s.enterpriseVersions.ListScopeHistory(r.Context(), input.ProductCode, verified.ActorUID, input.Authorization, value.VersionID, value.ScopeID, value.Page, value.PageSize)
		}
		if err != nil {
	...
	case "scope-deliver":
		var value pc.ProductVersionScopeDelivery
		if err = decodeEnterprisePlanningInput(input.Input, &value); err == nil {
			output, err = s.enterpriseVersions.DeliverScope(r.Context(), identity, input.Authorization, value)
		}
	case "scope-reopen":
		var value pc.ProductVersionScopeReopen
		if err = decodeEnterprisePlanningInput(input.Input, &value); err == nil {
			output, err = s.enterpriseVersions.ReopenScope(r.Context(), identity, input.Authorization, value)
		}
	case "scope-edit":
		var value pc.ProductVersionScopeEdit
		if err = decodeEnterprisePlanningInput(input.Input, &value); err == nil {
			output, err = s.enterpriseVersions.EditScope(r.Context(), identity, input.Authorization, *input.PlanningAuthorization, value)
		}
	case "scope-visibility":
		var value pc.ProductVersionScopeVisibility
		if err = decodeEnterprisePlanningInput(input.Input, &value); err == nil {
			output, err = s.enterpriseVersions.ChangeScopeVisibility(r.Context(), identity, input.Authorization, value)
		}
	case "scope-legacy-criteria":
		var value pc.LegacyProductVersionScopeCriteria
		if err = decodeEnterprisePlanningInput(input.Input, &value); err == nil {
			output, err = s.enterpriseVersions.UpdateLegacyScopeCriteria(r.Context(), identity, input.Authorization, value)
		}
	case "delete":
