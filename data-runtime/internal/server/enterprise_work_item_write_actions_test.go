...
func TestEnterpriseWorkItemWriteActionsCarryTheirOwnCapability(t *testing.T) {
	for path, action := range map[string]string{
		...
		"/v1/enterprise/aims/work-items:matter-complete": "matter-complete",
		"/v1/enterprise/aims/work-items:plan-ready":      "plan-ready",
	...
	if aimsapp.EnterpriseWorkItemCompleteCapability != "aims:work-item-complete:execute" {
		t.Fatal("matter completion must retain the existing explicit completion capability")
	}
