...
func (s *Server) routeEnterpriseCodocsOperation(r *http.Request, route enterpriseDelegatedRoute, buildQuery func(enterpriseDelegatedInput, string, string) (url.Values, error)) (routeResult, error) {
	...
	ctxRoute := enterpriseRouteContext{
		...
		Action: action.PermitAction, Capability: route.Spec.Domain + ":" + route.Spec.Resource + ":" + action.PermitAction,
	}
	...
	result.Body, _, err = s.codocs.HandleRuntime(r.Context(), action.Method, action.Target(input), query, input.Payload)
	return result, err
}
