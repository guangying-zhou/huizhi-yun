...
type enterpriseIPAssetsReadInput struct {
	ID                  string                            `json:"id"`
...
func (s *Server) routeEnterpriseIPAssetsRead(r *http.Request, action string) (routeResult, error) {
	...
	route := enterpriseRouteContext{Binding: enterprise.BindingKey{Tenant: s.cfg.Tenant, Environment: s.cfg.Enterprise.Environment, RuntimeDeployment: s.cfg.Deployment}, HostDeployment: s.cfg.DeploymentBindings["enterprise"], LogicalSource: "assets", LogicalTarget: "assets", Capability: "assets:ip-asset:read"}
	...
	var input enterpriseIPAssetsReadInput
	...
	if action != "products" && input.TargetAuthorization != nil {
		return result, httperror.New(400, "enterprise_ip_assets_input_invalid", "Product authorization is only accepted for relation reads")
	}
	...
	if action == "list" {
		...
		id, e := strconv.ParseInt(input.ID, 10, 64)
		if e != nil || id <= 0 || len(input.Query) != 0 || input.TargetAuthorization == nil {
			return result, httperror.New(400, "invalid_ip_asset_products_input", "IP product relation input invalid")
		}
		target, e := enterpriseIPAssetProductTargetScope(*input.TargetAuthorization, verified)
		if e != nil {
			return result, e
		}
		data, err = s.assets.EnterpriseIPAssetProductsView(r.Context(), q, target, id)
	} else {
