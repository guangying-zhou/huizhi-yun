...
func (s *Server) routeEnterpriseProjectMemberWrite(r *http.Request, spec struct{ action, capability string }) (routeResult, error) {
	...
	route := enterpriseRouteContext{Binding: enterprise.BindingKey{Tenant: s.cfg.Tenant, Environment: s.cfg.Enterprise.Environment, RuntimeDeployment: s.cfg.Deployment}, HostDeployment: s.cfg.DeploymentBindings["enterprise"], LogicalSource: "enterprise", LogicalTarget: "aims", Capability: spec.capability}
