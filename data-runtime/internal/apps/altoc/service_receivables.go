...
func (a *Adapter) syncContractFinanceSummary(ctx context.Context, contractCode string, body map[string]any) (map[string]any, error) {
	...
	receiptInput, command, err := integrationoperation.ReceiptCommandFromBody(
		body, "altoc", "finance.reconciliation.altoc-summary.v1", "altoc:contract:finance-summary:sync",
	)
	if err != nil {
		return nil, serviceCommandReceiptError(err)
	}
	if receiptInput.TrustedContext.SourceApp != "finance" {
		return nil, httperror.New(http.StatusForbidden, "service_command_source_forbidden", "service command source must be finance")
	}
	if strings.TrimSpace(fmt.Sprint(command["contractCode"])) != contractCode {
		return nil, httperror.New(http.StatusConflict, "service_command_path_mismatch", "service command contract does not match the target path")
	}
	...
	executed, err := repository.Execute(ctx, receiptInput, func(ctx context.Context, tx *sql.Tx, _ json.RawMessage) (integrationoperation.ReceiptBusinessResult, error) {
		result, err := a.syncContractFinanceSummaryTx(ctx, tx, contractCode, command)
		if err != nil {
			return integrationoperation.ReceiptBusinessResult{}, err
		}
		return integrationoperation.ReceiptBusinessResult{TargetBizType: "contract_finance_summary", TargetBizCode: contractCode, HTTPStatus: http.StatusOK, Value: result}, nil
	})
	if err != nil {
		return nil, serviceCommandReceiptError(err)
	}
...
func (a *Adapter) syncContractFinanceSummaryTx(ctx context.Context, tx *sql.Tx, contractCode string, body map[string]any) (map[string]any, error) {
...
func (a *Adapter) prepareReceivablePlanInvoiceRequest(ctx context.Context, receivablePlanCode string, body map[string]any) (map[string]any, error) {
	...
	idempotencyKey := firstBodyText(body, "idempotencyKey", "idempotency_key")
	if idempotencyKey == "" {
		derived, err := nextReceivableInvoiceRequestKeyTx(ctx, tx, body, receivablePlanCode)
		if err != nil {
			return nil, err
		}
	...
	operation, err := enqueueReceivableInvoiceRequestOperationTx(ctx, tx, plan, invoiceRequest, idempotencyKey, operator, body)
	if err != nil {
		return nil, err
	}
	if err := insertAltocAuditTx(ctx, tx, "receivable_plan", plan["id"], "invoice_request_freeze", nil, map[string]any{
		"idempotencyKey": idempotencyKey, "operationKey": operation["operationKey"],
	}, operator); err != nil {
		return nil, err
	}
	if err := tx.Commit(); err != nil {
...
func (a *Adapter) markReceivablePlansBillableByPaymentTermTx(ctx context.Context, tx *sql.Tx, paymentTermID int64, body map[string]any) (map[string]any, error) {
	...
	expectedContractCode := strings.TrimSpace(firstBodyText(body, "contractCode", "contract_code"))

	for _, plan := range plans {
		if expectedContractCode != "" && strings.TrimSpace(fmt.Sprint(plan["contract_code"])) != expectedContractCode {
			return nil, httperror.New(http.StatusConflict, "service_command_contract_mismatch", "payment term does not belong to the frozen contract")
		}
