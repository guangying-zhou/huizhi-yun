...
func (a *Adapter) createDocumentLink(ctx context.Context, body map[string]any) (map[string]any, error) {
	...
	// Existing Codocs attachment retries use the same entity/UUID business key.
	// Preserve the prior row instead of creating duplicate audit records or
	// turning a safe source-command retry into a duplicate-key failure.
	if documentUUID := documentNullableText(fields["document_uuid"]); documentUUID != nil {
		var existingID int64
		err := tx.QueryRowContext(ctx, `
			SELECT id FROM document_link
			WHERE entity_type = ? AND entity_id = ? AND document_uuid = ?
			LIMIT 1`, spec.Entity, entityID, documentUUID).Scan(&existingID)
		if err == nil {
			if err := tx.Commit(); err != nil {
				return nil, err
			}
			return map[string]any{"id": existingID, "idempotent": true}, nil
		}
