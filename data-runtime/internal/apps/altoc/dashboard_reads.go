...
func (a *Adapter) dashboardReceivables(ctx context.Context, query url.Values) (map[string]any, error) {
	...
	// 应收四象限：全部为时点存量，口径与下方逾期账龄分桶一致（均取 unreceived_amount）。
	// plan_total = received + receivable；overdue 与 uninvoiced 都是 receivable 的子集，
	// 两者可重叠（已过期且未开票），不可相加。
	quadrantWhere := []string{"rp.deleted_at IS NULL"}
	quadrantWhere = append(quadrantWhere, scopeWhere...)
	quadrantRow, err := altocQueryOneMap(ctx, a.DB(), `
		SELECT
		  SUM(COALESCE(rp.amount, 0)) AS plan_total,
		  SUM(COALESCE(rp.received_amount, 0)) AS received_amount,
		  SUM(CASE WHEN rp.status NOT IN ('received', 'bad_debt')
		      THEN COALESCE(rp.unreceived_amount, 0) ELSE 0 END) AS receivable_amount,
		  SUM(CASE WHEN rp.planned_payment_date < CURDATE() AND rp.status NOT IN ('received', 'bad_debt')
		      THEN COALESCE(rp.unreceived_amount, 0) ELSE 0 END) AS overdue_amount,
		  SUM(CASE WHEN rp.status IN ('pending', 'to_invoice')
		      THEN COALESCE(rp.unreceived_amount, 0) ELSE 0 END) AS uninvoiced_amount
		FROM receivable_plan rp
		LEFT JOIN contract ct ON ct.id = rp.contract_id
		WHERE `+strings.Join(quadrantWhere, " AND "), scopeArgs...)
	if err != nil {
		return nil, err
	}
	overdueWhere := []string{
	...
	return map[string]any{
		...
		"ar_quadrants": map[string]any{
			"plan_total": moneyValue(quadrantRow["plan_total"]),
			"received":   moneyValue(quadrantRow["received_amount"]),
			"receivable": moneyValue(quadrantRow["receivable_amount"]),
			"overdue":    moneyValue(quadrantRow["overdue_amount"]),
			"uninvoiced": moneyValue(quadrantRow["uninvoiced_amount"]),
		},
