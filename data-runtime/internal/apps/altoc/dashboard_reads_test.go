...
func TestDashboardKPIsApplyNonAdminScopeToAllAggregates(t *testing.T) {
	...
	assertDashboardQuery(t, queries,
		...
		[]string{"rp.owner_user_id = ?", "rp.collection_responsible_uid = ?", "ct.owner_user_id = ?", "ct.owner_dept_code IN"},
		[]any{"u1", "u1", "u1", "D1", "D2"},
	)
	assertDashboardQuery(t, queries,
...
func TestDashboardReceivablesAppliesNonAdminScopeToAllAggregates(t *testing.T) {
	adapter, capture := newDashboardScopeTestAdapter(t)
	query := dashboardScopeQuery()

	if _, err := adapter.dashboardReceivables(context.Background(), query); err != nil {
		t.Fatalf("dashboardReceivables returned error: %v", err)
	}

	queries := capture.snapshot()
	receivablePlanScope := []string{
		"rp.owner_user_id = ?",
		"rp.collection_responsible_uid = ?",
		"ct.owner_user_id = ?",
		"ct.owner_dept_code IN",
	}
	scopeArgs := []any{"u1", "u1", "u1", "D1", "D2"}
	...
	for _, tt := range []struct {
		...
		{
			name:      "month plan",
			selectors: []string{"FROM receivable_plan rp", "plan_amount"},
		},
		{
			name:      "ar quadrants",
			selectors: []string{"FROM receivable_plan rp", "uninvoiced_amount", "plan_total"},
		},
		{
			name:      "overdue aging breakdown",
			selectors: []string{"FROM receivable_plan rp", "AS aging"},
		},
		{
			name:      "upcoming list",
			selectors: []string{"FROM receivable_plan rp", "BETWEEN CURDATE()"},
		},
		{
			name:      "overdue list",
			selectors: []string{"FROM receivable_plan rp", "rp.overdue_days"},
		},
	} {
		t.Run(tt.name, func(t *testing.T) {
			assertDashboardQuery(t, queries, tt.selectors, receivablePlanScope, scopeArgs)
		})
	}
}

func TestDashboardReceivablesQuadrantsKeepOverdueConsistentWithAgingBreakdown(t *testing.T) {
	adapter, capture := newDashboardScopeTestAdapter(t)

	if _, err := adapter.dashboardReceivables(context.Background(), dashboardScopeQuery()); err != nil {
		t.Fatalf("dashboardReceivables returned error: %v", err)
	}

	quadrants := findDashboardQuery(t, capture.snapshot(), []string{"uninvoiced_amount", "plan_total"})
	aging := findDashboardQuery(t, capture.snapshot(), []string{"AS aging"})

	// 四象限的逾期口径必须与账龄分桶一致，否则同一屏上两个数字会互相矛盾。
	for _, part := range []string{
		"rp.planned_payment_date < CURDATE()",
		"rp.status NOT IN ('received', 'bad_debt')",
	} {
		if !strings.Contains(quadrants, compactDashboardSQL(part)) {
			t.Fatalf("ar quadrants overdue is missing %q:\n%s", part, quadrants)
		}
		if !strings.Contains(aging, compactDashboardSQL(part)) {
			t.Fatalf("aging breakdown is missing %q:\n%s", part, aging)
		}
	}
	// 未开票象限只能统计尚未进入开票流程的状态。
	if !strings.Contains(quadrants, compactDashboardSQL("rp.status IN ('pending', 'to_invoice')")) {
		t.Fatalf("ar quadrants uninvoiced must be limited to pending/to_invoice:\n%s", quadrants)
	}
}

func findDashboardQuery(t *testing.T, queries []capturedDashboardQuery, selectors []string) string {
	t.Helper()
	...
	t.Fatalf("no captured query matched %#v", selectors)
	return ""
}
