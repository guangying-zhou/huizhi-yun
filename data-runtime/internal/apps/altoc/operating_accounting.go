...
func (a *Adapter) handleOperatingAccountingPost(ctx context.Context, path string, body map[string]any) (any, string, bool, error) {
	...
	if lineCode, ok := pathParam(path, "/v1/altoc/service/contract-lines/", "/profit-summary:freeze"); ok {
		data, err := a.freezeContractLineProfitSummary(ctx, unescapePathParam(lineCode), body)
		return runtimeOK(data), "altoc.service.contract_lines.profit_summary.freeze", true, err
	}
...
func (a *Adapter) replaceContractLineCostAllocations(ctx context.Context, lineCode string, body map[string]any) (map[string]any, error) {
	...
	if err := ensureContractLineAllocationsMutableTx(ctx, tx, altocMapText(line, "code")); err != nil {
		return nil, err
	}
...
func (a *Adapter) freezeContractLineProfitSummary(ctx context.Context, lineCode string, body map[string]any) (map[string]any, error) {
	if err := altocRequireActionScope(body, "contract", "edit"); err != nil {
		return nil, err
	}
	...
	operator := altocActor(body)
	freezeKey := firstBodyText(body, "freezeKey", "freeze_key")
	...
	current, err := altocQueryOneMap(ctx, tx, `
		SELECT *
		...
		LIMIT 1
		FOR UPDATE
	`, line["code"], periodStart, periodEnd)
	if err != nil {
		return nil, err
	}
	if current == nil {
		return nil, httperror.New(http.StatusNotFound, "profit_summary_not_found", "current profit summary not found")
	}
	idempotent := altocMapText(current, "frozen_at") != ""
	if !idempotent {
		if _, err := tx.ExecContext(ctx, `
			UPDATE contract_line_profit_summary
			SET frozen_at = CURRENT_TIMESTAMP,
			    frozen_by = COALESCE(?, frozen_by),
			    freeze_key = COALESCE(?, freeze_key),
			    updated_by = COALESCE(?, updated_by),
			    updated_at = CURRENT_TIMESTAMP
			WHERE id = ?
		`, nullableText(operator), nullableText(freezeKey), nullableText(operator), current["id"]); err != nil {
			return nil, err
		}
	}
	summary, err := altocQueryOneMap(ctx, tx, "SELECT * FROM contract_line_profit_summary WHERE id = ? LIMIT 1", current["id"])
	if err != nil {
		return nil, err
	}
	if err := tx.Commit(); err != nil {
...
func (a *Adapter) recalculateContractProfitSummary(ctx context.Context, contractCode string, body map[string]any) (map[string]any, error) {
	...
	projectCosts, projectCostsProvided, err := projectCostsFromBody(body)
	if err != nil {
		return nil, err
	}
	operator := altocActor(body)
	...
	if err := ensureContractProfitPeriodMutableTx(ctx, tx, contract["id"], periodStart, periodEnd); err != nil {
		return nil, err
	}
	...
	materializedCount := 0
	needsMaterialization := contractCostAllocationsEmpty(allocations)
	...
	if err := ensureRequiredProjectCostSnapshot(projectCosts, projectCostsProvided, allocations); err != nil {
		return nil, err
	}
...
func validateContractLineCostAllocations(allocations []contractLineCostAllocationInput) error {
	...
	for _, allocation := range allocations {
		...
		case "direct":
		case "ratio":
			if allocation.AllocationRatio == nil || *allocation.AllocationRatio <= 0 || *allocation.AllocationRatio > 100 {
				return httperror.New(http.StatusBadRequest, "invalid_cost_allocation_ratio", "ratio allocation requires 0 < allocationRatio <= 100")
			}
		case "amount":
...
func projectCostsFromBody(body map[string]any) (map[string]float64, bool, error) {
	...
	for _, key := range []string{"projectCosts", "project_costs"} {
		...
		case map[string]any:
			for projectCode, value := range typed {
				...
				if projectCode == "" {
					return nil, provided, httperror.New(http.StatusBadRequest, "invalid_project_cost", "project cost projectCode is required")
				}
				cost, err := parseAltocFloat(value)
				if err != nil || cost < 0 {
					return nil, provided, httperror.New(http.StatusBadRequest, "invalid_project_cost", "project cost must be non-negative")
				}
		...
		case []any:
			for _, item := range typed {
				entry, ok := item.(map[string]any)
				if !ok {
					return nil, provided, httperror.New(http.StatusBadRequest, "invalid_project_cost", "project cost item is invalid")
				}
				projectCode := firstBodyText(entry, "projectCode", "project_code")
				cost, present, err := optionalAltocBodyFloat(entry, "totalCost", "total_cost", "cost")
				if projectCode == "" || !present || err != nil || cost < 0 {
					return nil, provided, httperror.New(http.StatusBadRequest, "invalid_project_cost", "project cost item requires projectCode and non-negative totalCost")
				}
				result[projectCode] = cost
			}
		default:
			return nil, provided, httperror.New(http.StatusBadRequest, "invalid_project_cost", "projectCosts must be an object or array")
		}
	}
	return result, provided, nil
}
...
func parseServiceCostInputs(body map[string]any) ([]serviceCostInput, error) {
	...
	projectCosts, _, err := projectCostsFromBody(body)
	if err != nil {
		return nil, err
	}
...
func ensureRequiredProjectCostSnapshot(projectCosts map[string]float64, projectCostsProvided bool, allocations map[string][]map[string]any) error {
	required := map[string]bool{}
	for _, rows := range allocations {
		for _, row := range rows {
			switch altocMapText(row, "allocation_type") {
			case "direct", "ratio", "workdays":
				if projectCode := altocMapText(row, "project_code"); projectCode != "" {
					required[projectCode] = true
				}
			}
		}
	}
	...
	missing := []string{}
	for projectCode := range required {
		if !projectCostsProvided {
			missing = append(missing, projectCode)
			continue
		}
		if _, ok := projectCosts[projectCode]; !ok {
			missing = append(missing, projectCode)
		}
	}
	sort.Strings(missing)
	if len(missing) > 0 {
		return httperror.New(http.StatusBadRequest, "missing_project_cost_snapshot", "explicit project cost snapshot is required for allocated projects: "+strings.Join(missing, ","))
	}
	return nil
}

func ensureContractProfitPeriodMutableTx(ctx context.Context, tx *sql.Tx, contractID any, periodStart string, periodEnd string) error {
	rows, err := altocQueryMaps(ctx, tx, `
		SELECT ps.*
		FROM contract_line_profit_summary ps
		INNER JOIN contract_line cl ON cl.code = ps.contract_line_code
	...
	for _, row := range rows {
		if altocMapText(row, "frozen_at") != "" {
			return httperror.New(http.StatusConflict, "profit_summary_frozen", "profit summary is frozen for this contract period")
		}
	}
	return nil
}

func ensureContractLineAllocationsMutableTx(ctx context.Context, tx *sql.Tx, lineCode string) error {
	rows, err := altocQueryMaps(ctx, tx, `
		SELECT *
		FROM contract_line_profit_summary
		WHERE contract_line_code = ?
	...
	for _, row := range rows {
		if altocMapText(row, "frozen_at") != "" {
			return httperror.New(http.StatusConflict, "profit_summary_frozen", "profit summary is frozen for this contract line")
		}
	}
	return nil
}
...
func contractCostAllocationsEmpty(allocations map[string][]map[string]any) bool {
	for _, rows := range allocations {
		if len(rows) > 0 {
			return false
		}
...
func insertContractLineProfitSummaryTx(ctx context.Context, tx *sql.Tx, line map[string]any, periodStart string, periodEnd string, revenue float64, cost float64, profit float64, margin any, projectCount int, calculationKey string, sourceVersion string, detailJSON string, operator string) (map[string]any, bool, error) {
	...
	current, err := altocQueryOneMap(ctx, tx, `
		...
		LIMIT 1
		FOR UPDATE
	`, lineCode, periodStart, periodEnd)
	if err != nil {
		return nil, false, err
	}
	if current != nil && altocMapText(current, "frozen_at") != "" {
		return nil, false, httperror.New(http.StatusConflict, "profit_summary_frozen", "profit summary is frozen for this contract line period")
	}
