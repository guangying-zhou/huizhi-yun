...
const tenderScopeOwnerDeptColumn = "scope_owner_dept_code"
const tenderScopeOwnerDeptExpr = "COALESCE(o.owner_dept_code, c.owner_dept_code)"
...
func (a *Adapter) listTenders(ctx context.Context, query url.Values) (map[string]any, error) {
	...
	scopeWhere, scopeArgs, err := tenderReadScopeWhere(query)
	if err != nil {
		return nil, err
	}
...
func tenderReadScopeWhere(query url.Values) ([]string, []any, error) {
	return altocReadScopeWhere(query, "quotation", "", "t.owner_user_id", tenderScopeOwnerDeptExpr)
}
...
func tenderDetailSQL(whereSQL string) string {
	return `
		SELECT
		  t.*,
		  ` + tenderScopeOwnerDeptExpr + ` AS ` + tenderScopeOwnerDeptColumn + `,
		  c.name AS customer_name,
		  o.name AS opportunity_name,
...
func lockTenderForWriteTx(ctx context.Context, tx *sql.Tx, tenderID int64, body map[string]any) (map[string]any, error) {
	...
	return tender, altocRequireRecordWrite(body, "quotation", tender, "owner_user_id", tenderScopeOwnerDeptColumn)
}
