...
func (a *Adapter) HandleRuntime(ctx context.Context, method string, path string, query url.Values, body map[string]any) (any, string, error) {
	...
	if response, operation, matched, err := a.handleProductFeedbackProgressRuntime(ctx, method, path, query, scopedBody); matched {
		return response, operation, err
	}
	if response, operation, matched, err := a.handleProductFeedbackStatusRuntime(ctx, method, path, query, scopedBody); matched {
		return response, operation, err
	}
	if response, operation, matched, err := a.handleReceivableNotificationRuntime(ctx, method, path, query, scopedBody); matched {
		return response, operation, err
	}
	if method == http.MethodGet {
	...
	if method == http.MethodPost && path == "/v1/altoc/payments" {
		if err := normalizeCollectionResponsibleMutation(scopedBody); err != nil {
			return nil, "", err
		}
	}
	if method == http.MethodGet {
...
func (a *Adapter) handleRuntimeGet(ctx context.Context, path string, query url.Values) (any, string, bool, error) {
	if path == "/v1/altoc/integration-operations" {
		data, err := a.listIntegrationOperationDiagnostics(ctx, query)
		return runtimeOK(data), "altoc.integration_operations.diagnostics.list", true, err
	}
	if operationID, ok := pathParam(path, "/v1/altoc/integration-operations/", "/attempts"); ok {
		data, err := a.listIntegrationOperationAttempts(ctx, unescapePathParam(operationID), query)
		return runtimeOK(data), "altoc.integration_operations.attempts.list", true, err
	}
	...
	if ticketCode, ok := pathParam(path, "/v1/altoc/service-tickets/", "/product-request"); ok {
		data, err := a.readServiceTicketProductFeedback(ctx, unescapePathParam(ticketCode), query)
		return runtimeOK(data), "altoc.service_tickets.product_request.read", true, err
	}
	if ticketCode, ok := pathParam(path, "/v1/altoc/service-tickets/", "/dispatch-context"); ok {
		data, err := a.serviceTicketDispatchContext(ctx, unescapePathParam(ticketCode), query)
		return runtimeOK(data), "altoc.service_tickets.dispatch_context", true, err
	}
...
func (a *Adapter) handleRuntimePost(ctx context.Context, path string, body map[string]any) (any, string, bool, error) {
	if path == "/v1/altoc/integration-operations:claim-next" {
		data, err := a.claimNextIntegrationOperation(ctx, body)
		return runtimeOK(data), "altoc.integration_operations.claim_next", true, err
	}
	if path == "/v1/altoc/integration-operations:pending-failure-notifications" {
		data, err := a.listPendingIntegrationOperationFailureNotifications(ctx, body)
		return runtimeOK(data), "altoc.integration_operations.failure_notifications.list", true, err
	}
	if path == "/v1/altoc/integration-operations:pending-dead-letter-actionables" {
		data, err := a.listPendingDeadLetterActionables(ctx, body)
		return runtimeOK(data), "altoc.integration_operations.dead_letter_actionables.list", true, err
	}
	if path == "/v1/altoc/integration-operations:pending-dead-letter-closures" {
		data, err := a.listPendingDeadLetterClosures(ctx, body)
		return runtimeOK(data), "altoc.integration_operations.dead_letter_closures.list", true, err
	}
	if operationKey, ok := pathParam(path, "/v1/altoc/integration-operations/", ":claim"); ok {
		data, err := a.claimIntegrationOperation(ctx, unescapePathParam(operationKey), body)
		return runtimeOK(data), "altoc.integration_operations.claim", true, err
	}
	if operationKey, ok := pathParam(path, "/v1/altoc/integration-operations/", ":fail"); ok {
		data, err := a.failIntegrationOperation(ctx, unescapePathParam(operationKey), body)
		return runtimeOK(data), "altoc.integration_operations.fail", true, err
	}
	if operationKey, ok := pathParam(path, "/v1/altoc/integration-operations/", ":succeed"); ok {
		data, err := a.succeedIntegrationOperation(ctx, unescapePathParam(operationKey), body)
		return runtimeOK(data), "altoc.integration_operations.succeed", true, err
	}
	if operationID, ok := pathParam(path, "/v1/altoc/integration-operations/", ":replay"); ok {
		data, err := a.replayIntegrationOperation(ctx, unescapePathParam(operationID), body)
		return runtimeOK(data), "altoc.integration_operations.replay", true, err
	}
	if operationID, ok := pathParam(path, "/v1/altoc/integration-operations/", ":failure-notified"); ok {
		data, err := a.markIntegrationOperationFailureNotified(ctx, unescapePathParam(operationID), body)
		return runtimeOK(data), "altoc.integration_operations.failure_notifications.mark", true, err
	}
	if operationID, ok := pathParam(path, "/v1/altoc/integration-operations/", ":dead-letter-actionable-published"); ok {
		data, err := a.markDeadLetterActionablePublished(ctx, unescapePathParam(operationID), body)
		return runtimeOK(data), "altoc.integration_operations.dead_letter_actionables.mark", true, err
	}
	if operationID, ok := pathParam(path, "/v1/altoc/integration-operations/", ":dead-letter-closure-acknowledged"); ok {
		data, err := a.markDeadLetterClosureAcknowledged(ctx, unescapePathParam(operationID), body)
		return runtimeOK(data), "altoc.integration_operations.dead_letter_closures.mark", true, err
	}
	...
	if paymentTermID, ok := pathParam(path, "/v1/altoc/service/payment-terms/", "/receivable-plan:mark-billable"); ok {
		data, err := a.executeMilestoneReceivableBillable(ctx, unescapePathParam(paymentTermID), body)
	...
	if ticketCode, ok := pathParam(path, "/v1/altoc/service-tickets/", "/ops-knowledge:reserve"); ok {
		data, err := a.reserveServiceTicketOpsKnowledge(ctx, unescapePathParam(ticketCode), body)
		return runtimeOK(data), "altoc.service_tickets.ops_knowledge.reserve", true, err
	}
	if ticketCode, ok := pathParam(path, "/v1/altoc/service-tickets/", "/product-request:freeze"); ok {
		data, err := a.freezeServiceTicketProductFeedback(ctx, unescapePathParam(ticketCode), body)
		return runtimeOK(data), "altoc.service_tickets.product_request.freeze", true, err
	}
	if ticketCode, ok := pathParam(path, "/v1/altoc/service-tickets/", "/aims-work-item:freeze"); ok {
		data, err := a.freezeServiceTicketAimsWorkItem(ctx, unescapePathParam(ticketCode), body)
		return runtimeOK(data), "altoc.service_tickets.aims_work_item.freeze", true, err
	}
	if ticketCode, ok := pathParam(path, "/v1/altoc/service-tickets/", "/aims-work-item:complete"); ok {
		data, err := a.completeServiceTicketAimsWorkItem(ctx, unescapePathParam(ticketCode), firstBodyText(body, "operationKey"), body)
		return runtimeOK(data), "altoc.service_tickets.aims_work_item.complete", true, err
	}
	if ticketCode, ok := pathParam(path, "/v1/altoc/service-tickets/", "/ops-knowledge:ack"); ok {
		data, err := a.ackServiceTicketOpsKnowledge(ctx, unescapePathParam(ticketCode), body)
		return runtimeOK(data), "altoc.service_tickets.ops_knowledge.ack", true, err
	}
	if ticketCode, ok := pathParam(path, "/v1/altoc/service-tickets/", "/ops-knowledge:complete"); ok {
		data, err := a.completeServiceTicketOpsKnowledge(ctx, unescapePathParam(ticketCode), body)
		return runtimeOK(data), "altoc.service_tickets.ops_knowledge.complete", true, err
	}
