...
func altocRuntimeBodyFromRequest(query url.Values, body map[string]any) map[string]any {
	...
	if altocQueryHasRuntimeContext(query) {
		return merged
	}
	for _, key := range []string{
		"current_user",
...
func altocCommandBodyFromRequest(query url.Values, body map[string]any) map[string]any {
	...
	for key, value := range body {
		if altocTrustedRuntimeContextKey(key) {
			continue
		}
		merged[key] = value
	}
...
func altocQueryHasRuntimeContext(query url.Values) bool {
	for _, key := range []string{
		"current_user",
		"currentUser",
		"operator_uid",
...
func altocTrustedRuntimeContextKey(key string) bool {
	switch key {
	case "current_user",
		"currentUser",
		"operator_uid",
		...
		return true
	default:
		return altocTrustedDataScopeKey(key)
	}
}

func altocTrustedDataScopeKey(key string) bool {
	switch key {
	case "current_user_altoc_access",
		"currentUserAltocAccess",
		"current_user_altoc_dept_codes",
		"currentUserAltocDeptCodes",
		"current_user_altoc_dept_code",
