...
func TestValidateContractLineCostAllocationsAllowsPartialLineRatio(t *testing.T) {
	r60 := float64(60)
	if err := validateContractLineCostAllocations([]contractLineCostAllocationInput{
		...
		t.Fatalf("partial ratio allocation rejected: %v", err)
	}

	r101 := float64(101)
	err := validateContractLineCostAllocations([]contractLineCostAllocationInput{
		{ProjectCode: "PRJ-1", AllocationType: "ratio", AllocationRatio: &r101},
	})
	httpErr, ok := err.(httperror.Error)
	if !ok || httpErr.Status != http.StatusBadRequest || httpErr.Code != "invalid_cost_allocation_ratio" {
		t.Fatalf("error = %#v, want invalid_cost_allocation_ratio", err)
	}
}
...
func TestEnsureRequiredProjectCostSnapshotRequiresExplicitZeroCost(t *testing.T) {
	allocations := map[string][]map[string]any{
		"CL-1": {{"project_code": "PRJ-1", "allocation_type": "direct"}},
	}
	err := ensureRequiredProjectCostSnapshot(map[string]float64{}, false, allocations)
	httpErr, ok := err.(httperror.Error)
	if !ok || httpErr.Status != http.StatusBadRequest || httpErr.Code != "missing_project_cost_snapshot" {
		t.Fatalf("error = %#v, want missing_project_cost_snapshot", err)
	}
	if err := ensureRequiredProjectCostSnapshot(map[string]float64{"PRJ-1": 0}, true, allocations); err != nil {
		t.Fatalf("explicit zero cost snapshot rejected: %v", err)
	}
}
...
func TestReplaceContractLineCostAllocationsRejectsFrozenLine(t *testing.T) {
	adapter, mock, closeDB := newAltocCoverageSQLMockAdapter(t)
	defer closeDB()
	...
	mock.ExpectQuery(`(?s)SELECT cl\.\*, c\.code AS contract_code, cu\.code AS customer_code\s+FROM contract_line cl`).
		WithArgs("CL-1").
		WillReturnRows(sqlmock.NewRows([]string{"id", "code", "contract_code"}).
			AddRow(int64(20), "CL-1", "CON-1"))
	mock.ExpectQuery(`(?s)SELECT \*\s+FROM contract_line_profit_summary\s+WHERE contract_line_code = \?`).
		WithArgs("CL-1").
		WillReturnRows(sqlmock.NewRows([]string{"id", "frozen_at"}).AddRow(int64(99), "2026-07-01 00:00:00"))
	mock.ExpectRollback()

	_, err := adapter.replaceContractLineCostAllocations(context.Background(), "CL-1", map[string]any{
		"allocations": []any{
			map[string]any{"projectCode": "PRJ-1", "allocationType": "direct"},
		},
	})
	httpErr, ok := err.(httperror.Error)
	if !ok || httpErr.Status != http.StatusConflict || httpErr.Code != "profit_summary_frozen" {
		t.Fatalf("error = %#v, want profit_summary_frozen", err)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatalf("expectations: %v", err)
	}
...
func TestFreezeContractLineProfitSummaryMarksCurrentSummary(t *testing.T) {
	adapter, mock, closeDB := newAltocCoverageSQLMockAdapter(t)
	defer closeDB()
	...
	mock.ExpectQuery(`(?s)SELECT \*\s+FROM contract_line_profit_summary\s+WHERE contract_line_code = \?`).
		...
		WillReturnRows(sqlmock.NewRows([]string{"id", "frozen_at"}).AddRow(int64(66), nil))
	mock.ExpectExec(`(?s)UPDATE contract_line_profit_summary\s+SET frozen_at = CURRENT_TIMESTAMP`).
		WithArgs("tester", "freeze-1", "tester", int64(66)).
		WillReturnResult(sqlmock.NewResult(0, 1))
	mock.ExpectQuery(`SELECT \* FROM contract_line_profit_summary WHERE id = \? LIMIT 1`).
		WithArgs(int64(66)).
		WillReturnRows(sqlmock.NewRows([]string{"id", "contract_line_code", "frozen_at", "freeze_key"}).
			AddRow(int64(66), "CL-1", "2026-07-01 00:00:00", "freeze-1"))
	mock.ExpectCommit()

	result, err := adapter.freezeContractLineProfitSummary(context.Background(), "CL-1", map[string]any{
	...
	if err != nil {
		t.Fatalf("freezeContractLineProfitSummary: %v", err)
	}
	if result["idempotent"] != false {
		t.Fatalf("result = %#v, want non-idempotent freeze", result)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatalf("expectations: %v", err)
	}
...
func TestRecalculateContractProfitSummaryRejectsFrozenPeriod(t *testing.T) {
	adapter, mock, closeDB := newAltocCoverageSQLMockAdapter(t)
	defer closeDB()
	...
	mock.ExpectQuery(`(?s)SELECT ps\.\*\s+FROM contract_line_profit_summary ps\s+INNER JOIN contract_line cl`).
		WithArgs(int64(10), "2026-06-01", "2026-06-30").
		WillReturnRows(sqlmock.NewRows([]string{"id", "frozen_at"}).AddRow(int64(66), "2026-07-01 00:00:00"))
	mock.ExpectRollback()

	_, err := adapter.recalculateContractProfitSummary(context.Background(), "CON-1", map[string]any{
		"periodStart": "2026-06-01",
		"periodEnd":   "2026-06-30",
...
func TestRecalculateContractProfitSummaryRequiresProjectCostSnapshot(t *testing.T) {
	adapter, mock, closeDB := newAltocCoverageSQLMockAdapter(t)
	defer closeDB()
	...
	mock.ExpectQuery(`(?s)SELECT ps\.\*\s+FROM contract_line_profit_summary ps\s+INNER JOIN contract_line cl`).
		WithArgs(int64(10), "2026-06-01", "2026-06-30").
		WillReturnRows(sqlmock.NewRows([]string{"id", "frozen_at"}))
	...
	mock.ExpectQuery(`(?s)SELECT\s+rel\.id AS relation_id,.*FROM contract_project_link cpl`).
		...
		WillReturnRows(sqlmock.NewRows([]string{
			"relation_id", "relation_type", "allocation_method", "allocation_ratio", "allocated_amount", "planned_workdays",
			"project_code", "project_role", "plan_key", "contract_code", "contract_line_id", "contract_line_code",
		}).AddRow(int64(501), "delivery", "direct", nil, nil, nil, "PRJ-1", "delivery", "delivery-main", "CON-1", int64(20), "CL-1"))
	mock.ExpectQuery(`(?s)SELECT id\s+FROM contract_line_cost_allocation\s+WHERE source_type = 'contract_project_line_rel'`).
		WithArgs("contract_project_line_rel:501").
		WillReturnRows(sqlmock.NewRows([]string{"id"}))
	mock.ExpectQuery(`(?s)SELECT id\s+FROM contract_line_cost_allocation\s+WHERE contract_line_code = \?`).
		WithArgs("CL-1", "PRJ-1", "direct").
		WillReturnRows(sqlmock.NewRows([]string{"id"}))
	mock.ExpectExec(`(?s)INSERT INTO contract_line_cost_allocation`).
		WithArgs(int64(20), "CL-1", "CON-1", "PRJ-1", "direct", nil, nil, nil, "contract_project_line_rel:501", sqlmock.AnyArg(), "system", "system").
		WillReturnResult(sqlmock.NewResult(31, 1))
	mock.ExpectQuery(`(?s)SELECT a\.\*\s+FROM contract_line_cost_allocation a\s+INNER JOIN contract_line cl`).
		WithArgs(int64(10), "2026-06-30", "2026-06-01").
		WillReturnRows(sqlmock.NewRows([]string{"id", "contract_line_code", "project_code", "allocation_type", "allocation_ratio", "allocated_amount", "allocated_workdays"}).
			AddRow(int64(30), "CL-1", "PRJ-1", "direct", nil, nil, nil))
	mock.ExpectRollback()
