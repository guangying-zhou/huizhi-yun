...
func (a *Adapter) syncCustomerDeliveryAssetStatus(ctx context.Context, deliveryAssetCode string, body map[string]any) (map[string]any, error) {
	...
	receiptInput, command, err := integrationoperation.ReceiptCommandFromBody(
		body, "altoc", "assets.delivery-asset.status-sync.v1", "altoc:contract:delivery-asset-status:sync",
	)
	if err != nil {
		return nil, serviceCommandReceiptError(err)
	}
	if receiptInput.TrustedContext.SourceApp != "assets" {
		return nil, httperror.New(http.StatusForbidden, "service_command_source_forbidden", "service command source must be assets")
	}
	if strings.TrimSpace(fmt.Sprint(command["deliveryAssetCode"])) != deliveryAssetCode {
		return nil, httperror.New(http.StatusConflict, "service_command_path_mismatch", "service command delivery asset does not match target path")
	}
	...
	executed, err := repository.Execute(ctx, receiptInput, func(ctx context.Context, tx *sql.Tx, _ json.RawMessage) (integrationoperation.ReceiptBusinessResult, error) {
		apply, err := claimAssetsStatusRevisionTx(ctx, tx, deliveryAssetCode, command, receiptInput.CommandSHA256)
		if err != nil {
			return integrationoperation.ReceiptBusinessResult{}, err
		}
		targetCode := fmt.Sprintf("%s:revision:%d", deliveryAssetCode, uint64(syncBodyNumber(command, "sourceRevision")))
		if !apply {
			return integrationoperation.ReceiptBusinessResult{TargetBizType: "contract_delivery_asset_status", TargetBizCode: targetCode, HTTPStatus: http.StatusOK, Value: map[string]any{"staleSkipped": true, "deliveryAssetCode": deliveryAssetCode}}, nil
		}
		result, err := a.syncCustomerDeliveryAssetStatusTx(ctx, tx, deliveryAssetCode, command)
		if err != nil {
			return integrationoperation.ReceiptBusinessResult{}, err
		}
		if err := recordAssetsStatusRevisionTx(ctx, tx, deliveryAssetCode, command, receiptInput.CommandSHA256); err != nil {
			return integrationoperation.ReceiptBusinessResult{}, err
		}
		return integrationoperation.ReceiptBusinessResult{TargetBizType: "contract_delivery_asset_status", TargetBizCode: targetCode, HTTPStatus: http.StatusOK, Value: result}, nil
	})
	if err != nil {
		return nil, serviceCommandReceiptError(err)
	}
...
func claimAssetsStatusRevisionTx(ctx context.Context, tx *sql.Tx, assetCode string, command map[string]any, commandSHA string) (bool, error) {
	revision := uint64(syncBodyNumber(command, "sourceRevision"))
	if revision == 0 {
		return false, httperror.New(http.StatusBadRequest, "source_revision_required", "sourceRevision must be positive")
	}
	// Materialize the per-asset lock row before selecting it. A locking read of a
	// missing row is not sufficient to serialize two first deliveries reliably;
	// the zero revision is transaction-local until the domain mutation records the
	// real applied revision below.
	if _, err := tx.ExecContext(ctx, `INSERT INTO assets_delivery_status_projection (delivery_asset_code,applied_revision,command_sha256,status,applied_at) VALUES (?,0,REPEAT('0',64),'pending',UTC_TIMESTAMP(3)) ON DUPLICATE KEY UPDATE delivery_asset_code=VALUES(delivery_asset_code)`, assetCode); err != nil {
		return false, err
	}
	var appliedRevision uint64
	var appliedSHA string
	err := tx.QueryRowContext(ctx, `SELECT applied_revision,command_sha256 FROM assets_delivery_status_projection WHERE delivery_asset_code=? FOR UPDATE`, assetCode).Scan(&appliedRevision, &appliedSHA)
	if err != nil {
		return false, err
	}
	...
	if revision == appliedRevision {
		if appliedSHA != commandSHA {
			return false, integrationoperation.ErrIdempotencyPayloadMismatch
		}
		return false, nil
	}
	return true, nil
...
func recordAssetsStatusRevisionTx(ctx context.Context, tx *sql.Tx, assetCode string, command map[string]any, commandSHA string) error {
	_, err := tx.ExecContext(ctx, `INSERT INTO assets_delivery_status_projection (delivery_asset_code,applied_revision,command_sha256,status,environment_code,applied_at) VALUES (?,?,?,?,?,UTC_TIMESTAMP(3)) ON DUPLICATE KEY UPDATE applied_revision=VALUES(applied_revision),command_sha256=VALUES(command_sha256),status=VALUES(status),environment_code=VALUES(environment_code),applied_at=VALUES(applied_at)`, assetCode, uint64(syncBodyNumber(command, "sourceRevision")), commandSHA, syncBodyText(command, "status"), nullableText(syncBodyText(command, "environmentCode", "environment_code")))
	return err
}

func syncBodyNumber(body map[string]any, key string) int64 {
	value, _ := strconv.ParseInt(strings.TrimSpace(fmt.Sprint(body[key])), 10, 64)
	return value
}

func (a *Adapter) syncCustomerDeliveryAssetStatusTx(ctx context.Context, tx *sql.Tx, deliveryAssetCode string, body map[string]any) (map[string]any, error) {
