...
func altocRequireOwnerWrite(body map[string]any, resource string, owner any) error {
	...
	if altocDataAccessMode(body) == "self_dept" {
		return httperror.New(http.StatusForbidden, "owner_scope_required", "Only the owner within scoped department can modify this record")
	}
...
func altocRequireRecordWrite(body map[string]any, resource string, record map[string]any, ownerColumn string, departmentColumn string) error {
	...
	if altocDataAccessMode(body) == "self_dept" {
		ownerMatches := ownerColumn != "" && strings.TrimSpace(altocMapText(record, ownerColumn)) == actor
		deptMatches := false
		if departmentColumn != "" {
		...
		if ownerMatches && deptMatches {
			return nil
		}
		return httperror.New(http.StatusForbidden, "write_scope_required", "Only the owner within scoped department can modify this record")
	}
...
func altocReadScopeWhereFromBody(body map[string]any, resource string, tableAlias string, ownerColumn string, departmentColumn string) ([]string, []any, error) {
	...
	mode := altocDataAccessMode(body)
	deptCodes := altocScopedDeptCodes(body)
	if mode == "self_dept" {
		if strings.TrimSpace(ownerColumn) == "" || strings.TrimSpace(departmentColumn) == "" || len(deptCodes) == 0 {
			return nil, nil, altocDataAccessForbidden()
		}
		placeholders := strings.TrimRight(strings.Repeat("?,", len(deptCodes)), ",")
		where := "(" + altocQualifiedColumn(tableAlias, ownerColumn) + " = ? AND " + altocQualifiedColumn(tableAlias, departmentColumn) + " IN (" + placeholders + "))"
		args := make([]any, 0, len(deptCodes)+1)
		args = append(args, actor)
		for _, deptCode := range deptCodes {
			args = append(args, deptCode)
		}
		return []string{where}, args, nil
	}
...
func altocRecordMatchesReadScope(body map[string]any, resource string, record map[string]any, ownerColumn string, departmentColumn string) (bool, error) {
	...
	if altocDataAccessMode(body) == "self_dept" {
		ownerMatches := strings.TrimSpace(ownerColumn) != "" && strings.TrimSpace(altocMapText(record, ownerColumn)) == actor
		deptMatches := false
		...
		return ownerMatches && deptMatches, nil
	}
...
func altocActorScopeAllowsAction(scope string, resource string, action string) bool {
	...
	capabilityResource := strings.ReplaceAll(resource, "_", "-")
	if scope == "altoc:"+capabilityResource+":admin" || scope == "altoc:"+resource+":admin" {
		return true
	}
	if scope == "altoc:"+capabilityResource+":"+action || scope == "altoc:"+resource+":"+action {
		return true
	}
	if altocActionCoveredByEdit(action) && (scope == "altoc:"+capabilityResource+":edit" || scope == "altoc:"+resource+":edit") {
		return true
	}
	return action == "view" && (scope == "altoc:"+capabilityResource+":edit" || scope == "altoc:"+resource+":edit")
}
...
func insertAltocAuditTx(ctx context.Context, tx *sql.Tx, entityType string, entityID any, action string, oldValue any, newValue any, operator string) error {
	_, err := insertAltocAuditRecordTx(ctx, tx, entityType, entityID, action, oldValue, newValue, operator)
	return err
}

func insertAltocNotificationAuditTx(ctx context.Context, tx *sql.Tx, entityType string, entityID any, action string, oldValue any, newValue any, operator string) (int64, error) {
	auditID, err := insertAltocAuditRecordTx(ctx, tx, entityType, entityID, action, oldValue, newValue, operator)
	if err != nil {
		return 0, err
	}
	if auditID <= 0 {
		return 0, fmt.Errorf("altoc audit insert returned invalid id for %s %v %s", entityType, entityID, action)
	}
	return auditID, nil
}

func insertAltocAuditRecordTx(ctx context.Context, tx *sql.Tx, entityType string, entityID any, action string, oldValue any, newValue any, operator string) (int64, error) {
	if operator = strings.TrimSpace(operator); operator == "" {
		operator = "system"
	}
	oldJSON, _ := json.Marshal(oldValue)
	newJSON, _ := json.Marshal(newValue)
	return altocInsertRecordTx(ctx, tx, "audit_log", map[string]any{
...
func altocNotificationEventVersion(auditID int64) string {
	return fmt.Sprintf("audit:%d", auditID)
}

func withAltocNotificationEventVersion(result map[string]any, auditID int64) map[string]any {
	result["notification_event_version"] = altocNotificationEventVersion(auditID)
	return result
}
