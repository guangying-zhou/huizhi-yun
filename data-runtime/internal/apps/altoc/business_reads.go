...
func (a *Adapter) updateReceivablePlan(ctx context.Context, identifier string, body map[string]any) (map[string]any, error) {
	...
	where, args := altocIdentityWhere("rp", identifier)
	filters := []string{where, "rp.deleted_at IS NULL"}
	scopeWhere, scopeArgs, err := altocReceivablePlanWriteScopeWhere(nil, body, "rp", "ct")
	if err != nil {
		return nil, err
	}
	filters = append(filters, scopeWhere...)
...
func altocReceivablePlanReadScopeWhere(query url.Values, body map[string]any, planAlias string, contractAlias string) ([]string, []any, error) {
	return altocReceivablePlanScopeWhere(query, body, planAlias, contractAlias, true)
}

// altocReceivablePlanWriteScopeWhere intentionally preserves the historical
// owner/contract-owner write boundary. Being the current collection
// responsible user grants read access to the target receivable plan, but does
// not by itself grant edit or payment-confirm authority.
func altocReceivablePlanWriteScopeWhere(query url.Values, body map[string]any, planAlias string, contractAlias string) ([]string, []any, error) {
	return altocReceivablePlanScopeWhere(query, body, planAlias, contractAlias, false)
}

func altocReceivablePlanScopeWhere(query url.Values, body map[string]any, planAlias string, contractAlias string, includeCollectionResponsible bool) ([]string, []any, error) {
	...
	if strings.TrimSpace(planAlias) != "" {
		...
		if includeCollectionResponsible {
			clauses = append(clauses, altocQualifiedColumn(planAlias, "collection_responsible_uid")+" = ?")
			args = append(args, actor)
		}
	}
