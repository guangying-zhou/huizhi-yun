...
func (a *Adapter) createOpportunity(ctx context.Context, body map[string]any) (map[string]any, error) {
	...
	auditID, err := insertAltocNotificationAuditTx(ctx, tx, "opportunity", opportunityID, "create", nil, map[string]any{
		"code":        code,
		"name":        fields["name"],
	...
	return withAltocNotificationEventVersion(map[string]any{
		"id":          opportunityID,
		"code":        code,
...
func (a *Adapter) assignOpportunity(ctx context.Context, identifier string, body map[string]any) (map[string]any, error) {
	...
	auditID, err := insertAltocNotificationAuditTx(ctx, tx, "opportunity", opportunityID, "assign", oldValue, newValue, operator)
	if err != nil {
		return nil, err
	}
	updated, err := a.opportunityByIDTx(ctx, tx, opportunityID)
