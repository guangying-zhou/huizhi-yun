...
func (a *Adapter) resolveServiceTicketAgreementTx(ctx context.Context, tx *sql.Tx, ticketID int64, body map[string]any) (map[string]any, error) {
	...
	if agreement != nil {
		...
		if entitlementStatus == "in_service" {
			quotaConsume = serviceTicketQuotaConsumption(ticket, agreement, body)
		}
	} else {
...
func matchingServiceAgreement(ctx context.Context, tx *sql.Tx, ticket map[string]any, assetCodes []string, columns map[string]bool) (map[string]any, error) {
	...
	if len(assetCodes) > 0 {
		...
		if coverageExists, existsErr := altocTableExists(ctx, tx, "service_agreement_coverage"); existsErr != nil {
			...
			coverageTargetWhere, coverageArgs := serviceAgreementCoverageTargetWhere(assetCodes, altocMapText(ticket, "environment_code"))
			coverageWhere = append(coverageWhere, coverageTargetWhere)
			if customerCode != "" {
			...
			agreement, err := altocQueryOneMap(ctx, tx, `
				...
				ORDER BY
				  CASE sac.target_type
				    WHEN 'delivery_asset_environment' THEN 0
				    WHEN 'delivery_asset' THEN 1
				    WHEN 'environment' THEN 2
				    ELSE 3
				  END ASC,
				  `+serviceAgreementMatchOrderSQL()+`
				LIMIT 1
				FOR UPDATE
			`, coverageArgs...)
			if err != nil || agreement != nil {
...
func serviceAgreementCoverageTargetWhere(assetCodes []string, environmentCode string) (string, []any) {
	placeholders := strings.TrimRight(strings.Repeat("?,", len(assetCodes)), ",")
	environmentCode = strings.TrimSpace(environmentCode)
	if environmentCode == "" {
		args := make([]any, 0, len(assetCodes))
		for _, code := range assetCodes {
			args = append(args, code)
		}
		return "(sac.target_type = 'delivery_asset' AND sac.delivery_asset_code IN (" + placeholders + "))", args
	}
	args := make([]any, 0, len(assetCodes)*2+2)
	for _, code := range assetCodes {
		args = append(args, code)
	}
	...
	args = append(args, environmentCode)
	return "((sac.target_type = 'delivery_asset_environment' AND sac.delivery_asset_code IN (" + placeholders + ") AND sac.environment_code = ?)" +
		" OR (sac.target_type = 'delivery_asset' AND sac.delivery_asset_code IN (" + placeholders + "))" +
		" OR (sac.target_type = 'environment' AND sac.environment_code = ?))", args
}
...
func serviceTicketQuotaConsumption(ticket map[string]any, agreement map[string]any, body map[string]any) float64 {
	...
	consumed := moneyValue(ticket["quota_consumed"])
	unit := strings.ToLower(strings.TrimSpace(altocMapText(agreement, "quota_unit")))
	if unit == "" || unit == "ticket" || unit == "tickets" || unit == "case" {
		if consumed >= 1 {
			return 0
		}
		return 1 - consumed
	...
	if unit == "hour" || unit == "hours" {
		desired := serviceResultNumber(body, "quotaConsumed", "quota_consumed", "actualHours", "actual_hours", "consumedHours", "consumed_hours")
		if desired <= consumed {
			return 0
		}
		return desired - consumed
	}
	return 0
}
...
func serviceTicketSLAStatus(ticket map[string]any, entitlementStatus string, responseDue any, resolutionDue any, now time.Time) string {
	...
	firstRespondedAt := parseAltocTime(ticket["first_responded_at"])
	if !responseDeadline.IsZero() && !firstRespondedAt.IsZero() && firstRespondedAt.After(responseDeadline) {
		return "breached"
	}
	if status == "resolved" || status == "closed" {
...
func serviceResultNumber(body map[string]any, keys ...string) float64 {
