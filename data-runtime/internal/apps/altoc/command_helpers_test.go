...
func TestAltocRequireRecordWriteAppliesDataAccessModes(t *testing.T) {
	...
	if err := altocRequireRecordWrite(
		...
		t.Fatalf("expected self_dept data access write to pass for owner inside scoped department, got %v", err)
	}
	...
	err = altocRequireRecordWrite(
		map[string]any{
			"current_user":                 "u1",
			"current_user_data_access":     "self_dept",
			"current_user_data_dept_codes": []string{"D9"},
		},
		"opportunity",
		record,
		"owner_user_id",
	...
	if err == nil {
		t.Fatal("expected self_dept data access to reject owner outside scoped department")
	}
	httpErr, ok = err.(httperror.Error)
	if !ok || httpErr.Status != http.StatusForbidden || httpErr.Code != "write_scope_required" {
		t.Fatalf("expected write_scope_required 403, got %#v", err)
...
func TestAltocRequireActionScopeAllowsServiceCommandActions(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "service ticket delivery result sync", resource: "service_ticket", action: "delivery-result:sync", scope: "altoc:service-ticket:delivery-result:sync"},
	} {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			err := altocRequireActionScope(
...
func TestAltocRuntimeBodyFromRequestPrefersQueryAuthContext(t *testing.T) {
	query := url.Values{}
	query.Set("current_user", "query-user")
	query.Set("current_user_scopes", "altoc:lead:edit")
	...
	if body["current_user"] != "query-user" {
		t.Fatalf("unexpected current_user %#v", body["current_user"])
	}
	if altocActorHasAdminScope(body, "customer") {
		t.Fatal("body scopes should not override trusted query scopes")
	}
	if !altocActorScopeAllowsAction(altocActorScopes(body)[0], "lead", "edit") {
		t.Fatalf("expected query scope to be preserved, got %#v", body["current_user_scopes"])
	}
	if got := altocActorDeptCodes(body); len(got) != 0 {
		t.Fatalf("body dept codes should not override trusted query context: %#v", got)
	}
}

func TestAltocRuntimeBodyFromRequestAllowsBodyFallbackWithoutQueryContext(t *testing.T) {
	body := altocRuntimeBodyFromRequest(url.Values{}, map[string]any{
		"current_user":            "body-user",
		"current_user_scopes":     []string{"altoc:customer:admin"},
		"current_user_dept_codes": []any{"D1", "D2"},
	})
	...
	if !altocActorHasAdminScope(body, "customer") {
		t.Fatal("expected body fallback scopes to grant customer admin")
	}
	if got := altocActorDeptCodes(body); !reflect.DeepEqual(got, []string{"D1", "D2"}) {
		t.Fatalf("unexpected fallback dept codes %#v", got)
	}
}
...
func TestAltocCommandBodyFromRequestKeepsBusinessFieldsAndQueryDataScope(t *testing.T) {
	...
	body := altocCommandBodyFromRequest(query, map[string]any{
		...
		"currentUserDataAccess":         "all",
		"current_user_altoc_dept_codes": "ROOT",
	...
	if body["current_user"] != "query-user" {
		t.Fatalf("expected trusted query actor to win, got %#v", body["current_user"])
	}
	if altocActorHasAdminScope(body, "customer") {
		t.Fatalf("body scope spoof should not survive trusted query merge: %#v", body)
	}
	...
	if _, ok := body["current_user_altoc_access"]; ok {
		t.Fatalf("body data access spoof should not survive trusted query merge: %#v", body)
	}
	if _, ok := body["currentUserDataAccess"]; ok {
		t.Fatalf("camel-case body data access spoof should not survive trusted query merge: %#v", body)
	}
	if _, ok := body["current_user_altoc_dept_codes"]; ok {
		t.Fatalf("body scoped dept spoof should not survive trusted query merge: %#v", body)
	}
}
...
func TestAltocReadScopeWhereAppliesDataAccessModes(t *testing.T) {
	...
	selfDeptQuery := url.Values{}
	selfDeptQuery.Set("current_user", "u1")
	selfDeptQuery.Set("current_user_data_access", "self_dept")
	selfDeptQuery.Set("current_user_data_dept_codes", "D3,D4")
	where, args, err = altocReadScopeWhere(selfDeptQuery, "opportunity", "op", "owner_user_id", "owner_dept_code")
	if err != nil {
		t.Fatalf("self_dept data access returned error: %v", err)
	}
	if len(where) != 1 || where[0] != "(op.owner_user_id = ? AND op.owner_dept_code IN (?,?))" {
		t.Fatalf("unexpected self_dept where %#v", where)
	}
	if !reflect.DeepEqual(args, []any{"u1", "D3", "D4"}) {
		t.Fatalf("unexpected self_dept args %#v", args)
	}
...
func TestAltocReceivablePlanReadScopeWhereIncludesCollectorOwnerAndContractScope(t *testing.T) {
	query := url.Values{}
	query.Set("current_user", "u1")
	query.Set("current_user_dept_codes", "D1,D2")
	...
	if len(where) != 1 || where[0] != "(rp.owner_user_id = ? OR rp.collection_responsible_uid = ? OR ct.owner_user_id = ? OR ct.owner_dept_code IN (?, ?))" {
		t.Fatalf("unexpected where %#v", where)
	}
	expectedArgs := []any{"u1", "u1", "u1", "D1", "D2"}
	if !reflect.DeepEqual(args, expectedArgs) {
		t.Fatalf("unexpected args %#v", args)
	}
}

func TestAltocReceivablePlanWriteScopePreservesOwnerBoundary(t *testing.T) {
	query := url.Values{}
	query.Set("current_user", "collector-not-owner")
	query.Set("current_user_dept_codes", "D9")

	where, args, err := altocReceivablePlanWriteScopeWhere(query, nil, "rp", "ct")
	if err != nil {
		t.Fatalf("altocReceivablePlanWriteScopeWhere returned error: %v", err)
	}
	if len(where) != 1 || where[0] != "(rp.owner_user_id = ? OR ct.owner_user_id = ? OR ct.owner_dept_code IN (?))" {
		t.Fatalf("unexpected write where %#v", where)
	}
	if strings.Contains(where[0], "collection_responsible_uid") {
		t.Fatalf("collector relation unexpectedly expanded write scope: %#v", where)
	}
	if !reflect.DeepEqual(args, []any{"collector-not-owner", "collector-not-owner", "D9"}) {
		t.Fatalf("unexpected write args %#v", args)
	}
}
...
func TestAltocRecordMatchesReadScopeRequiresOwnerAndDepartmentForSelfDeptMode(t *testing.T) {
	body := map[string]any{
		"current_user":                 "u1",
		"current_user_data_access":     "self_dept",
		"current_user_data_dept_codes": "D1,D2",
	}
	...
	if !allowed {
		t.Fatal("expected owner inside scoped department to match")
	}

	allowed, err = altocRecordMatchesReadScope(
		body,
		"customer",
		map[string]any{"owner_user_id": "u1", "owner_dept_code": "D9"},
		"owner_user_id",
		"owner_dept_code",
	)
	if err != nil {
	...
	if allowed {
		t.Fatal("expected owner outside scoped department to be rejected")
	}
	...
	if allowed {
		t.Fatal("expected scoped department record owned by another user to be rejected")
	}
}
