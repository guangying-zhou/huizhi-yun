...
func (a *Adapter) restoreCompletionInstance(ctx context.Context, command map[string]any, requestID, workItemID int64, actorUID string) (int64, string, error) {
	...
	err := a.db.QueryRowContext(ctx, `SELECT COUNT(*), COALESCE(MIN(id),0), COALESCE(MIN(instance_no),'') FROM flow_instances
		...
		AND JSON_UNQUOTE(JSON_EXTRACT(form_data,'$.snapshotSha256')) = ?`, strconv.FormatInt(workItemID, 10), actorUID, strconv.FormatInt(requestID, 10), strconv.FormatInt(workItemID, 10), fmt.Sprint(command["projectId"]), command["snapshotSha256"]).Scan(&count, &id, &number)
	if err != nil {
		return 0, "", err
	}
