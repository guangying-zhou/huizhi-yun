...
type WorkflowEffects struct {
	...
	ActionableLifecycles []WorkflowActionableLifecycle `json:"actionableLifecycles,omitempty"`
...
type WorkflowActionableLifecycle struct {
	...
	PrerequisiteNotifications []WorkflowNotification `json:"prerequisiteNotifications,omitempty"`
}
...
func (a *Adapter) CreateInstance(ctx context.Context, rawBody map[string]any) (InstanceAPIResponse, error) {
	...
	result, err := a.createInstanceTx(ctx, tx, rawBody)
	if err != nil {
		return InstanceAPIResponse{}, err
	}
...
// createInstanceTx keeps the Workflow business mutation inside a caller-owned
// transaction so service-command receipts can commit the instance and its
// idempotency evidence atomically.
func (a *Adapter) createInstanceTx(ctx context.Context, tx *sql.Tx, rawBody map[string]any) (InstanceAPIResponse, error) {
	...
	if existingRejected != nil && truthy(config["terminal_relaunch"]) {
		existingRejected = nil
	}
	...
	if existingRejected != nil {
		...
		oldPendingEffects, err := pendingActionableLifecycleEffects(ctx, tx, existingRejected.ID, "", "cancelled")
		if err != nil {
			return InstanceAPIResponse{}, err
		}
		...
		`, existingRejected.ID, request.CurrentUser)
		if err != nil {
			return InstanceAPIResponse{}, err
		}
		resubmitActionID, err = result.LastInsertId()
		if err != nil {
			return InstanceAPIResponse{}, err
		}
		...
		effects.ActionableLifecycles = append(effects.ActionableLifecycles, oldPendingEffects...)
	} else {
		result, err := tx.ExecContext(ctx, `
	...
	if firstNodeIndex < len(snapshotNodes) {
		target := workflowNotificationTarget{AppCode: actionDef.AppCode, ResourceCode: actionDef.ResourceCode, BizID: bizID, InstanceID: instanceID}
		notifications, finalStatus, finalNode, err := createInitialTasks(ctx, tx, target, firstNodeIndex, snapshotNodes, fullContext, request.CurrentUser, request.BizTitle, request.BizURL, resubmitActionID)
		if err != nil {
			return InstanceAPIResponse{}, err
		}
	...
	if status == "approved" {
		instance := map[string]any{
			"id": instanceID, "instance_no": instanceNo,
			"app_code": actionDef.AppCode, "resource_code": actionDef.ResourceCode,
			"action_code": actionDef.ActionCode, "biz_id": bizID,
			"callback_url": request.CallbackURL, "form_data": mustJSON(defaultMap(request.FormData)),
			"initiator_uid": request.CurrentUser,
		}
...
func (a *Adapter) actionDefByKey(ctx context.Context, appCode string, resourceCode string, actionCode string) (*actionDefRecord, error) {
	return actionDefByKeyOn(ctx, a.db, appCode, resourceCode, actionCode)
}

func actionDefByKeyOn(ctx context.Context, q queryRowContext, appCode string, resourceCode string, actionCode string) (*actionDefRecord, error) {
...
func (a *Adapter) matchRoutes(ctx context.Context, actionDefID int64, flowContext map[string]any) ([]routeRecord, error) {
	return matchRoutesOn(ctx, a.db, actionDefID, flowContext)
}

type workflowRowsQueryer interface {
	QueryContext(context.Context, string, ...any) (*sql.Rows, error)
}

func matchRoutesOn(ctx context.Context, q workflowRowsQueryer, actionDefID int64, flowContext map[string]any) ([]routeRecord, error) {
...
func createInitialTasks(ctx context.Context, tx *sql.Tx, target workflowNotificationTarget, firstNodeIndex int, nodes []map[string]any, flowContext map[string]any, initiatorUID string, bizTitle string, bizURL string, resubmitActionID int64) ([]WorkflowNotification, string, int, error) {
	...
	for currentIndex < len(nodes) {
		...
		taskIDs, err := createTasksForNode(ctx, tx, instanceID, currentIndex, node)
		if err != nil {
			return nil, "", 0, err
		}
		...
		if !isExplicitInitiatorApprovalNode(node, initiatorUID) {
			...
			if len(uids) > 0 {
				...
				eventVersion := taskEventVersion(taskIDs)
				metadata := map[string]any{"instanceId": instanceID, "taskIds": taskIDs, "nodeIndex": currentIndex}
				if resubmitActionID > 0 {
					eventType = "workflow.instance.resubmitted"
					metadata["actionId"] = resubmitActionID
				}
				notifications = append(notifications, newWorkflowNotification(
					uids, "您有新的审批待办",
					fmt.Sprintf("%s提交了「%s」，请审批", firstNonEmptyString(cleanAnyString(flowContext["initiator_name"]), initiatorUID), bizTitle),
					bizURL, eventType, eventVersion, "info", target, metadata,
				))
			}
...
func createTasksForNode(ctx context.Context, tx *sql.Tx, instanceID int64, nodeIndex int, node map[string]any) ([]int64, error) {
	...
	for _, assignee := range resolvedAssignees(node) {
		...
		`, instanceID, nodeIndex, cleanAnyString(node["name"]), assignee.UID, cleanAnyString(node["type"]))
		if err != nil {
			return nil, err
		}
	...
	if len(taskIDs) > 0 {
		key := taskActionableKey(taskIDs)
		version := taskEventVersion(taskIDs)
		for _, taskID := range taskIDs {
			if _, err := tx.ExecContext(ctx, "UPDATE flow_tasks SET generation_key = ?, actionable_key = ?, actionable_version = ?, updated_at = NOW() WHERE id = ?", key, key, version, taskID); err != nil {
				return nil, err
			}
		}
	}
