...
func (a *Adapter) ReplayEnterpriseWorkItemCompletion(ctx context.Context, id EnterpriseProjectUpdateIdentity, projectID, itemID string, command map[string]any) (map[string]any, error) {
	...
	result, err := receipts.ExecuteInTransaction(ctx, tx, ri, func(ctx context.Context, tx *sql.Tx, _ json.RawMessage) (iop.ReceiptBusinessResult, error) {
		...
		checkpoint, err := replayEnterpriseCompletionOperation(ctx, a.DB(), tx, outbox, iop.ReplayInput{TenantCode: id.Tenant, DeploymentCode: a.enterpriseWrites.workerDeployment, SourceApp: "aims", OperationID: operationID, ExpectedVersion: uint64(expected), ActorUID: id.ActorUID, Reason: reason, Now: time.Now().UTC()})
		if err != nil {
			return iop.ReceiptBusinessResult{}, completionReplayError(err)
		}
...
func replayEnterpriseCompletionOperation(ctx context.Context, db *sql.DB, tx *sql.Tx, outbox iop.TrustedContext, input iop.ReplayInput) (iop.RecordResult, error) {
	repo, err := aimsCompletionRepository(db, outbox)
	if err != nil {
		return iop.RecordResult{}, err
	}
	return repo.ReplayInTransaction(ctx, tx, input)
}

func completionReplayError(err error) error {
	if errors.Is(err, iop.ErrReplayRejected) {
		return httperror.New(http.StatusConflict, "completion_replay_rejected", "Operation changed or is not eligible for manual recovery")
	}
	return err
}
