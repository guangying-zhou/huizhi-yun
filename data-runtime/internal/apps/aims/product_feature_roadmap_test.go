...
func TestFeatureRoadmapRuntimeRejectsInsufficientCapabilityBeforeDatabase(t *testing.T) {
	var adapter Adapter
	for _, action := range []string{"view"} {
		for _, scope := range []string{"aims.read", "aims.write", "aims:product-features:*", "aims:product-requests:edit", "aims:product-features:edit"} {
			...
			_, _, handled, err := adapter.handleProductFeatureRoadmapRuntime(context.Background(), http.MethodPost, "/v1/aims/internal/products/P-A/feature-roadmap:"+action, query, nil)
