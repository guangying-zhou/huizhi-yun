...
type VerifiedWorkItemCompletionCallback struct {
	...
	instanceNo, actor, hash, status          string
	operator                                 string
...
func (a *Adapter) ApplyWorkItemCompletionCallback(ctx context.Context, c VerifiedWorkItemCompletionCallback) (map[string]any, error) {
	...
	var actor, hash, status, reviewVersion, instanceNo string
	var instance sql.NullInt64
	var snapshotJSON []byte
	err = tx.QueryRowContext(ctx, "SELECT requested_by,snapshot_sha256,status,review_version,workflow_instance_id,COALESCE(workflow_instance_no,''),snapshot_json FROM work_item_completion_requests WHERE id=? AND project_id=? AND work_item_id=? FOR UPDATE", c.requestID, c.projectID, c.itemID).Scan(&actor, &hash, &status, &reviewVersion, &instance, &instanceNo, &snapshotJSON)
	if err != nil {
		return nil, httperror.New(403, "work_item_completion_callback_object_mismatch", "Completion request binding does not match")
	}
	if actor != c.actor || hash != c.hash || fmt.Sprint(item["project_id"]) != fmt.Sprint(c.projectID) || (instance.Valid && (instance.Int64 != int64(c.instanceID) || instanceNo != c.instanceNo)) {
		return nil, httperror.New(403, "work_item_completion_callback_object_mismatch", "Completion request binding does not match")
	}
	if status == c.status {
	...
	// Approval only applies to the exact frozen children, not a changed plan.
	var snapshot struct {
		Children []struct {
			ID      any    `json:"id"`
	...
	if json.Unmarshal(snapshotJSON, &snapshot) != nil || len(snapshot.Children) == 0 {
...
func completeWorkItemCompletionWorkflowTx(ctx context.Context, tx *sql.Tx, command, body map[string]any) error {
	...
	var instance sql.NullInt64
	var existingNo, hash, actor string
	err := tx.QueryRowContext(ctx, "SELECT workflow_instance_id,COALESCE(workflow_instance_no,''),snapshot_sha256,requested_by FROM work_item_completion_requests WHERE id=? AND work_item_id=? AND project_id=? FOR UPDATE", command["completionRequestId"], command["workItemId"], command["projectId"]).Scan(&instance, &existingNo, &hash, &actor)
	if err != nil {
		return err
	}
	if hash != command["snapshotSha256"] || actor != command["actorUid"] || (instance.Valid && (instance.Int64 != int64(id) || existingNo != no)) {
