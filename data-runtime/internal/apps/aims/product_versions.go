...
func (a *Adapter) handleProductVersionRuntime(ctx context.Context, method string, path string, query url.Values, body map[string]any) (any, string, bool, error) {
	...
	if productCode, ok := pathParam(path, "/v1/aims/service/products/", "/version-summaries"); ok && method == http.MethodGet {
		data, err := a.serviceProductVersionSummaries(ctx, productCode, query)
		return data, "aims.service.product_version_summaries.list", true, err
	}
...
func (a *Adapter) listProjectProducts(ctx context.Context, projectIDText string, query url.Values) (map[string]any, error) {
	projectID, access, err := a.requireProjectReadable(ctx, projectIDText, query, nil)
	if err != nil {
		return nil, err
	}
...
func (a *Adapter) createProductVersion(ctx context.Context, projectIDText string, query url.Values, body map[string]any) (map[string]any, error) {
	return nil, httperror.New(http.StatusGone, "legacy_product_version_create_retired", "版本规划已迁移到产品中心，请使用带产品权限和幂等键的创建命令")
}
...
func (a *Adapter) createProjectWithProductBindingTx(ctx context.Context, tx *sql.Tx, query url.Values, body map[string]any) (map[string]any, error) {
	...
	leaderUID := firstBodyText(body, "leader_uid", "leaderUid")
	if leaderUID == "" {
		return nil, httperror.New(http.StatusBadRequest, "project_manager_required", "leader_uid is required")
	}
	var duplicate int64
	...
	// 区分「调用方显式指定」与「未指定」：项目集默认分类只在未指定时生效，
	// 但 routine 为强约束，显式指定其他分类会被拒绝（V1.1 §1.4）。
	requestedCategory := firstBodyText(body, "category")
	category := firstNonEmptyText(requestedCategory, "product_dev")
	portfolioID, hasPortfolioID, err := optionalBodyID(body, "portfolio_id", "portfolioId")
	if err != nil {
		return nil, httperror.New(http.StatusBadRequest, "invalid_portfolio_id", "portfolio_id must be a positive integer")
	}
	var portfolioValue any
	if hasPortfolioID && portfolioID > 0 {
		defaultCategory, err := a.projectPortfolioDefaultCategory(ctx, portfolioID)
		if err != nil {
			return nil, err
		}
		switch {
		case defaultCategory == "routine":
			// 日常事务容器的分类是强约束，不可覆盖：否则该容器的工时会脱离
			// routine 口径，§4.4 的工时闭合出现缺口。
			if requestedCategory != "" && requestedCategory != "routine" {
				return nil, httperror.New(http.StatusBadRequest, "routine_category_locked",
					"日常事务项目集下的项目分类固定为日常事务，不可更改")
			}
			category = "routine"
		case defaultCategory != "" && requestedCategory == "":
			// 其余项目集的默认分类仅在调用方未显式指定时生效
			category = defaultCategory
		}
		portfolioValue = portfolioID
	}
	...
	serviceYear, err := normalizeServiceYearInput(category, body, true)
	if err != nil {
		return nil, err
	}
	...
	result, err := tx.ExecContext(ctx, `
		INSERT INTO aims_projects (
		  project_code, name, short_name, internal_code, description, category, methodology, lifecycle_status,
		  portfolio_id, domain_code, dept_code, leader_uid, security_level, confidentiality_level, access_whitelist,
	...
	`, projectCode, name, shortName,
		...
		firstNonEmptyText(firstBodyText(body, "confidentiality_level", "confidentialityLevel"), "L1"),
		...
		serviceYear.LineCode,
		serviceYear.PeriodSeq,
		serviceYear.PeriodStart,
		serviceYear.PeriodEnd,
		serviceYear.PeriodLabel,
	...
	if err := validateServiceYearProjectTx(ctx, tx, strconv.FormatInt(projectID, 10)); err != nil {
		return nil, err
	}
	if _, err := tx.ExecContext(ctx, "INSERT INTO project_counters (project_id, counter) VALUES (?, 0)", projectID); err != nil {
		return nil, err
	}
	if _, err := tx.ExecContext(ctx, "INSERT INTO aims_project_members (project_id, uid, role, status) VALUES (?, ?, 'manager', 'active')", projectID, leaderUID); err != nil {
		return nil, err
	}
	if _, err := tx.ExecContext(ctx, `
		INSERT INTO project_lifecycle_events (
		  project_id, from_status, to_status, effective_at, actor_uid, source
		) VALUES (?, NULL, ?, UTC_TIMESTAMP(6), ?, 'aims.project.create')
	`, projectID, lifecycleStatus, uid); err != nil {
		return nil, err
	}
...
// 承接了原 projectPortfolioIsProductLine 的职责：产品线项目集等价于
// default_category='product_dev'（V1.1 §1.4）。
func (a *Adapter) projectPortfolioDefaultCategory(ctx context.Context, portfolioID int64) (string, error) {
	var defaultCategory sql.NullString
	var isProductLine int64
	err := a.DB().QueryRowContext(ctx,
		"SELECT default_category, is_product_line FROM project_portfolios WHERE id = ?",
		portfolioID).Scan(&defaultCategory, &isProductLine)
	if err == sql.ErrNoRows {
		return "", httperror.New(http.StatusBadRequest, "portfolio_not_found", "所选项目集不存在")
	}
	if err != nil {
		return "", err
	}
	if defaultCategory.Valid && strings.TrimSpace(defaultCategory.String) != "" {
		return strings.TrimSpace(defaultCategory.String), nil
	}
	// 兼容 default_category 尚未回填的存量项目集
	if isProductLine != 0 {
		return "product_dev", nil
	}
	return "", nil
}
...
func (a *Adapter) updateProductVersion(ctx context.Context, projectIDText string, versionIDText string, query url.Values, body map[string]any) (map[string]any, error) {
	return nil, httperror.New(http.StatusGone, "legacy_product_version_edit_retired", "版本编辑已迁移到产品中心，请使用带产品权限、修订校验和幂等键的命令")
}
...
func (a *Adapter) attachVersionItems(ctx context.Context, projectIDText string, versionIDText string, query url.Values, body map[string]any) (map[string]any, error) {
	...
	return a.attachVersionItemsTransaction(ctx, projectID, versionID, featureValue, itemIDs)
}
...
func (a *Adapter) detachVersionItem(ctx context.Context, projectIDText string, versionIDText string, workItemIDText string, query url.Values) (map[string]any, error) {
	...
	return a.detachVersionItemTransaction(ctx, projectID, versionID, workItemID)
}

func (a *Adapter) detachVersionItemTransaction(ctx context.Context, projectID, versionID, workItemID int64) (map[string]any, error) {
	tx, err := a.DB().BeginTx(ctx, nil)
	if err != nil {
		return nil, err
	}
	defer tx.Rollback()
	var status string
	if err := tx.QueryRowContext(ctx, "SELECT status FROM product_versions WHERE id = ? FOR UPDATE", versionID).Scan(&status); err != nil {
		return nil, err
	}
	if status != "planning" && status != "developing" {
		return nil, httperror.New(http.StatusConflict, "version_not_editable", "version does not accept work item changes")
	}
	result, err := tx.ExecContext(ctx, "UPDATE work_items SET version_id = NULL, feature_id = NULL WHERE id = ? AND project_id = ? AND version_id = ?", workItemID, projectID, versionID)
	if err != nil {
		return nil, err
	}
	affected, _ := result.RowsAffected()
	...
	if _, err := tx.ExecContext(ctx, "UPDATE product_versions SET revision = revision + 1, scope_revision = scope_revision + 1 WHERE id = ?", versionID); err != nil {
		return nil, err
	}
	if err := tx.Commit(); err != nil {
		return nil, err
...
type workItemVersionFieldsUpdate func(context.Context, *sql.Tx) error

func (a *Adapter) prepareWorkItemVersionFieldsUpdate(ctx context.Context, workItemIDText string, query url.Values, body map[string]any) (workItemVersionFieldsUpdate, error) {
	workItemID, err := parseID(workItemIDText, "work_item_id")
	if err != nil {
		return nil, err
	}
	...
	`, workItemID).Scan(&projectID, &tier, &currentVersionID); err == sql.ErrNoRows {
		return nil, httperror.New(http.StatusNotFound, "work_item_not_found", "work item not found")
	} else if err != nil {
		return nil, err
	}
	if tier != "target" {
		return nil, httperror.New(http.StatusBadRequest, "work_item_tier_not_allowed", "only target work items can be attached to versions")
	}
	...
	if !access.IsMember {
		return nil, httperror.New(http.StatusForbidden, "project_access_denied", "project member access required")
	}
	...
	if versionID == 0 {
		return func(ctx context.Context, tx *sql.Tx) error {
			return updateWorkItemVersionTx(ctx, tx, projectID, workItemID, currentVersionID, 0, nil)
		}, nil
	}
	if err := a.ensureVersionVisibleToProject(ctx, projectID, versionID); err != nil {
		return nil, err
	...
	return func(ctx context.Context, tx *sql.Tx) error {
		return updateWorkItemVersionTx(ctx, tx, projectID, workItemID, currentVersionID, versionID, featureValue)
	}, nil
}
...
func (a *Adapter) requireProjectReadable(ctx context.Context, projectIDText string, query url.Values, body map[string]any) (int64, aimsProjectAccess, error) {
	...
	visible, err := a.projectVisibleToUser(ctx, projectID, query, uid)
	if err != nil {
		return 0, aimsProjectAccess{}, err
	}
	if !visible {
		return 0, aimsProjectAccess{}, httperror.New(http.StatusForbidden, "project_access_denied", "project view access required")
	}
	return projectID, access, nil
}
...
func (a *Adapter) projectVisibleToUser(ctx context.Context, projectID int64, query url.Values, uid string) (bool, error) {
