...
func (a *Adapter) handleProjectEnvironmentRuntime(ctx context.Context, method string, path string, query url.Values, body map[string]any) (any, string, bool, error) {
	...
	if method == http.MethodPost {
		if projectCode, ok := pathParam(path, "/v1/aims/service/projects/", "/environments"); ok {
			projectCode, _ = url.PathUnescape(projectCode)
			data, err := a.upsertProjectEnvironmentByProjectCode(ctx, projectCode, query, body)
		...
		if projectCode, environmentCode, action, ok := projectEnvironmentCommandPath(path); ok {
			data, err := a.changeProjectEnvironment(ctx, projectCode, environmentCode, action, query, body)
...
func (a *Adapter) upsertProjectEnvironmentByProjectCode(ctx context.Context, projectCode string, query url.Values, body map[string]any) (map[string]any, error) {
	...
	if err := a.requireProjectEnvironmentManagerOrScopedAdmin(ctx, projectCode, query); err != nil {
		return nil, err
	}
...
func (a *Adapter) changeProjectEnvironment(ctx context.Context, projectCode string, environmentCode string, action string, query url.Values, body map[string]any) (map[string]any, error) {
	if err := a.requireProjectEnvironmentManagerOrScopedAdmin(ctx, projectCode, query); err != nil {
		return nil, err
	}
	tx, err := a.DB().BeginTx(ctx, nil)
...
func (a *Adapter) requireProjectEnvironmentManagerOrScopedAdmin(ctx context.Context, projectCode string, query url.Values) error {
	...
	if err := a.DB().QueryRowContext(ctx, `
		...
		LIMIT 1
	`, strings.TrimSpace(projectCode)).Scan(&projectID); err != nil {
