...
func (a *Adapter) RequestEnterpriseWorkItemCompletion(ctx context.Context, identity EnterpriseProjectUpdateIdentity, projectID, itemID string, command map[string]any) (map[string]any, error) {
	return a.RequestEnterpriseWorkItemCompletionKind(ctx, identity, projectID, itemID, "target", command)
}

func (a *Adapter) RequestEnterpriseWorkItemCompletionKind(ctx context.Context, identity EnterpriseProjectUpdateIdentity, projectID, itemID, kind string, command map[string]any) (map[string]any, error) {
	if kind != "target" && kind != "matter" {
		return nil, httperror.New(400, "work_item_completion_kind_invalid", "Unsupported completion kind")
	}
	...
	if kind == "matter" {
		if err := e.VerifyCompatibilityViews(ctx, a.DB(), a.enterpriseWrites.binding, "aims", []string{"deliverables", "gitlab_commits"}); err != nil {
			return nil, err
		}
	}
	...
	if kind == "matter" {
		ri.OperationCode = "enterprise.aims.work-items.complete.matter.v1"
		ri.CommandSchemaVersion = "work-item-complete.matter.v1"
	}
	...
	result, err := repo.ExecuteInTransaction(ctx, tx, ri, func(ctx context.Context, tx *sql.Tx, _ json.RawMessage) (integrationoperation.ReceiptBusinessResult, error) {
		...
		if kind == "target" && leader != identity.ActorUID {
		...
		if kind == "matter" && aimsMapText(item, "assignee_uid") != identity.ActorUID {
			return integrationoperation.ReceiptBusinessResult{}, httperror.New(403, "matter_completion_assignee_required", "Only the assigned person can request matter completion")
		}
		if version != expected {
		...
		var hash string
		if kind == "matter" {
			snapshot, hash, err = matterCompletionSnapshotTx(ctx, tx, item)
		} else {
		...
		if kind == "matter" {
			inserted, err = tx.ExecContext(ctx, "INSERT INTO work_item_completion_requests(project_id,work_item_id,kind,requested_by,snapshot_json,snapshot_sha256,review_version,status,operation_key) VALUES(?,?,'matter',?,?,?,?, 'queued',?)", projectID, itemID, identity.ActorUID, raw, hash, reviewVersion, temporaryKey)
		} else {
		...
		if kind == "matter" {
			cmd["kind"] = "matter"
			cmd["formData"].(map[string]any)["kind"] = "matter"
			cmd["formData"].(map[string]any)["evidenceSummary"] = matterCompletionEvidenceSummary(snapshot)
		}
		outboundBase := base
		...
		auditChanges := map[string]any{"requestId": requestID, "snapshotSha256": hash, "from": "in_progress", "to": "in_review"}
		if kind == "matter" {
			auditChanges["kind"] = "matter"
		}
		if err = writeCompletionAuditTx(ctx, tx, projectID, itemID, identity.ActorUID, identity.IdempotencyKey, "completion_request", auditChanges); err != nil {
			return integrationoperation.ReceiptBusinessResult{}, err
		}
...
func (a *Adapter) EnterpriseWorkItemCompletionState(ctx context.Context, itemID, actor string) (map[string]any, error) {
	...
	if aimsMapText(item, "tier") == "matter" {
		if err := e.VerifyCompatibilityViews(ctx, a.DB(), a.enterpriseWrites.binding, "aims", []string{"deliverables", "gitlab_commits"}); err != nil {
			return nil, err
		}
		state["kind"] = "matter"
		state["canRequest"] = false
		if aimsMapText(item, "assignee_uid") == actor {
			evidence, _, evidenceErr := matterCompletionEvidence(ctx, tx, item, false)
			if evidenceErr != nil {
				return nil, evidenceErr
			}
			state["canRequest"] = validateMatterCompletionReady(item, evidence) == nil
		}
	}
	outbox, err := a.enterpriseOutbox()
	if err != nil {
...
func enqueueWorkItemCompletionWorkflowTx(ctx context.Context, tx *sql.Tx, outbox integrationoperation.TrustedContext, i EnterpriseProjectCreateIdentity, requestID int64, key string, command map[string]any) error {
	...
	_, err = tx.ExecContext(ctx, outbox.SQL(`INSERT INTO integration_operation(operation_id,operation_key,correlation_key,sequence_no,tenant_code,deployment_code,source_app,target_app,operation_code,required_capability,source_biz_type,source_biz_code,idempotency_key,command_schema_version,command_json,command_sha256,status,original_request_id,original_actor_uid,service_client_id,created_by,updated_by,next_attempt_at) VALUES(?,?,?,1,?,?,'aims','workflow',?,?,'work_item_completion_request',?,?,?,?,?,'pending',?,?,'aims.runtime',?,?,UTC_TIMESTAMP(3))`), id, key, key, i.Tenant, i.TargetDeployment, workItemCompletionWorkflowOperation, workItemCompletionWorkflowCapability, fmt.Sprint(requestID), key, schemaVersion, raw, hash, i.RequestID, i.ActorUID, i.ActorUID, i.ActorUID)
	return err
}
...
func validWorkItemCompletionWorkflowCommand(c map[string]any) bool {
	...
	expectedKey := fmt.Sprintf("aims:work-item-completion:%d:workflow-submit:v1", request)
	if kind == "matter" {
		expectedKey = fmt.Sprintf("aims:work-item-completion:matter:%d:workflow-submit:v2", request)
	}
	if c["idempotencyKey"] != expectedKey {
		return false
	}
	...
	if !ok || len(form) != map[string]int{"target": 4, "matter": 6}[kind] {
		return false
	}
	if kind == "matter" && !validMatterCompletionEvidenceSummary(form["evidenceSummary"]) {
		return false
	}
	return (kind == "target" || form["kind"] == "matter") && completionPositiveNumber(form["completionRequestId"]) == request && completionPositiveNumber(form["workItemId"]) == item && completionPositiveNumber(form["projectId"]) == project && form["snapshotSha256"] == hash
}

func validMatterCompletionEvidenceSummary(value any) bool {
