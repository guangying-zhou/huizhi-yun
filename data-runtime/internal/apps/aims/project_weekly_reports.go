...
func (a *Adapter) saveProjectWeeklyReport(ctx context.Context, projectID string, query url.Values, body map[string]any) (projectWeeklyReportItem, error) {
	...
	if status != "draft" {
		return projectWeeklyReportItem{}, httperror.New(
			http.StatusBadRequest,
			"weekly_report_submit_command_required",
			"save only accepts draft; use the weekly report submit command",
		)
	}
	...
	if governanceObligationID > 0 {
		var lockedObligationID int64
		err := tx.QueryRowContext(ctx, `
			SELECT id
			FROM weekly_report_obligations
			WHERE id = ?
			  AND project_id = ?
			  AND frozen_at IS NULL
			FOR UPDATE
		`, governanceObligationID, projectID).Scan(&lockedObligationID)
		if errors.Is(err, sql.ErrNoRows) {
			return projectWeeklyReportItem{}, httperror.New(
				http.StatusConflict,
				"weekly_report_obligation_frozen",
				"weekly report obligation is missing or frozen",
			)
		}
		if err != nil {
			return projectWeeklyReportItem{}, err
	...
	lockErr := tx.QueryRowContext(ctx, `
		...
		WHERE project_id = ?
		  ...
		  AND report_week = ?
		FOR UPDATE
	`, projectID, reportYear, reportWeek).Scan(&lockedStatus)
	if lockErr != nil && !errors.Is(lockErr, sql.ErrNoRows) {
		return projectWeeklyReportItem{}, lockErr
	}
	if lockErr == nil {
		switch lockedStatus {
		case "draft", "returned":
		...
		case "correction_draft":
			projectionStatus = "correction_draft"
		default:
			return projectWeeklyReportItem{}, httperror.New(
				http.StatusConflict,
				"weekly_report_not_editable",
				"weekly report is not editable in its current state",
			)
		}
	}
	...
	`,
		projectID,
		nullablePositiveInt64(governanceObligationID),
		reportYear,
		reportWeek,
		weekStart,
		weekEnd,
	...
	if governanceObligationID > 0 {
		if _, err := tx.ExecContext(ctx, `
			...
			SET due_status = 'draft',
			    updated_at = UTC_TIMESTAMP(6)
			WHERE id = ?
			  AND frozen_at IS NULL
		`, governanceObligationID); err != nil {
			return projectWeeklyReportItem{}, err
		}
	}
...
func (a *Adapter) requireProjectWeeklyReportManager(ctx context.Context, projectID string, uid string, query url.Values) error {
	if !truthyQuery(
		...
		"currentUserCanSubmitWeeklyReport",
		"current_user_can_submit_assigned_weekly_report",
		"currentUserCanSubmitAssignedWeeklyReport",
	) {
		return httperror.New(http.StatusForbidden, "weekly_report_submit_permission_required", "weekly report submit permission required")
	}
	...
	err := a.DB().QueryRowContext(ctx, `
		SELECT COALESCE((
		  ...
		  LIMIT 1
		), p.leader_uid) AS responsible_uid
		FROM aims_projects p
		WHERE p.id = ?
	...
	if responsibleUID.Valid && responsibleUID.String == uid {
		return nil
	}
	return httperror.New(http.StatusForbidden, "forbidden_project_weekly_report_manage", "only the current project manager or acting project manager can edit project weekly reports")
}
