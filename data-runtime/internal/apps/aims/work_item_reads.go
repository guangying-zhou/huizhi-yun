...
func (a *Adapter) workItemTransitions(ctx context.Context, workItemID string, query url.Values) ([]workItemTransition, error) {
	...
	row := a.DB().QueryRowContext(ctx, `
		...
		WHERE id = ?
	`, parsedWorkItemID)
	if err := row.Scan(&entityType, &status); err != nil {
	...
	projectRules, err := a.queryWorkItemTransitions(ctx, "project_id = ? AND entity_type = ? AND from_status = ?", projectID, entityType, status)
	if err != nil {
		return nil, err
	}
