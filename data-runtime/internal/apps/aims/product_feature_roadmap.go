...
func (a *Adapter) handleProductFeatureRoadmapRuntime(ctx context.Context, method, path string, query url.Values, body map[string]any) (any, string, bool, error) {
	code, match := pathParam(path, "/v1/aims/internal/products/", "/feature-roadmap:view")
	if !match {
		return nil, "", false, nil
	}
	operation := "aims.product-priorities.feature-roadmap"
	...
	var input productcenter.FeatureRoadmapQuery
	...
	result, err := productcenter.ReadFeatureRoadmap(ctx, a.DB(), code, query.Get("current_user"), planningPermit, featurePermit, input)
