...
func (a *Adapter) createRequirementTask(ctx context.Context, rawReqID string, query url.Values, body map[string]any) (map[string]any, error) {
	...
	if err := a.requireRequirementProjectManagerOrScopedAdmin(ctx, reqID, uid, query); err != nil {
		return nil, err
	}
...
func (a *Adapter) requireRequirementProjectManagerOrScopedAdmin(ctx context.Context, reqID int64, uid string, query url.Values) error {
	var projectID int64
	err := a.DB().QueryRowContext(ctx,
		"SELECT project_id FROM requirement_items WHERE id = ?",
		reqID).Scan(&projectID)
	if errors.Is(err, sql.ErrNoRows) {
		return httperror.New(http.StatusNotFound, "requirement_not_found", "需求不存在")
	}
	if err != nil {
		return err
	}
...
func (a *Adapter) requireRequirementContentProjectManagerOrScopedAdmin(ctx context.Context, contentID int64, uid string, query url.Values) error {
	var projectID int64
	err := a.DB().QueryRowContext(ctx,
		"SELECT project_id FROM requirement_contents WHERE id = ?",
		contentID).Scan(&projectID)
	if errors.Is(err, sql.ErrNoRows) {
		return httperror.New(http.StatusNotFound, "content_not_found", "章节不存在")
	}
	if err != nil {
		return err
	}
