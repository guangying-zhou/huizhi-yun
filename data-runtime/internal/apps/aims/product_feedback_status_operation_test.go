...
func TestProductFeedbackStatusOperation(t *testing.T) {
	...
	if err := validateServiceTicketDeliveryOperation("altoc", productFeedbackStatusOperationCode, command); err != nil {
		t.Fatal(err)
	}
	if err := validateServiceTicketDeliveryOperation("codocs", productFeedbackStatusOperationCode, command); err == nil {
