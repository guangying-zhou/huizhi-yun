...
func TestPlanningFeatureRuntimeRejectsInsufficientCapabilityBeforeDatabase(t *testing.T) {
	var adapter Adapter
	for _, action := range []string{"view", "change"} {
		for _, scope := range []string{"aims.read", "aims.write", "aims:product-features:*", "aims:product-requests:edit", "aims:product-features:edit"} {
			...
			_, _, handled, err := adapter.handleProductPlanningFeatureRuntime(context.Background(), http.MethodPost, "/v1/aims/internal/products/P-A/planning-feature:"+action, query, nil)
