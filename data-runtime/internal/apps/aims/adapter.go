...
var requiredColumns = []string{
	...
	"aims_projects.service_line_code",
	"work_items.routine_scope",
	"milestone_cycle_snapshots.gate_result",
}
...
func New(cfg config.AimsConfig) (*Adapter, error) {
	adapter, err := compat.New(compat.Config{
		...
		Resources: []compat.ResourceSpec{
			...
			{Path: "documents", Table: "project_documents", SearchColumns: []string{"title", "doc_category", "codocs_uuid", "created_by"}, DefaultOrderBy: "`updated_at` DESC, `id` DESC", WriteDenyColumns: []string{"portfolio_id", "project_id", "project_code", "milestone_id", "work_item_id", "uuid"}},
