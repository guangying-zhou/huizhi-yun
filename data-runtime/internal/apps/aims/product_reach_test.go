...
func TestProductReachRuntimeBoundary(t *testing.T) {
	var adapter Adapter
	for _, action := range []string{"list", "view", "record"} {
		path := "/v1/aims/internal/products/P-A/reach-observations:" + action
		...
		for _, scope := range []string{"aims.read", "aims.write", "aims:product-priorities:*", "aims:product-features:edit"} {
			...
			_, _, handled, err := adapter.handleProductReachRuntime(context.Background(), http.MethodPost, path, query, nil)
		...
		_, _, handled, err := adapter.handleProductReachRuntime(context.Background(), http.MethodGet, path, query, nil)
		var response httperror.Error
		if !handled || !errors.As(err, &response) || response.Status != 405 {
			t.Fatalf("method: %v", err)
		}
		_, _, handled, err = adapter.handleProductReachRuntime(context.Background(), http.MethodPost, path, query, map[string]any{"authorization": "invalid"})
