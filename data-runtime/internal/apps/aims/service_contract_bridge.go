...
type serviceProjectCreation struct {
	ProjectCode, Name, ShortName, InternalCode, Description string
	Category, Methodology, LifecycleStatus                  string
	PortfolioID, ContractID, OppID                          any
	DomainCode, DeptCode, LeaderUID                         string
	SecurityLevel, ConfidentialityLevel, AccessWhitelist    string
	StartDate, EndDate, CustomerCode, CustomerName          string
	ContractCode, ModuleConfig, BoardConfig                 string
	WorkflowConfig, NotificationConfig, CreatedBy, Source   string
	TemplateSetID, TemplateVersionID                        any
	TemplateDefinition                                      *projectTemplateDefinition
	ExcludedTemplateWorkItemKeys                            map[string]bool
}

func createServiceProjectTx(ctx context.Context, tx *sql.Tx, input serviceProjectCreation) (map[string]any, error) {
	result, err := tx.ExecContext(ctx, `
		INSERT INTO aims_projects (
		  ...
		  confidentiality_level, access_whitelist, start_date, end_date, opp_id, contract_id,
		  customer_code, customer_name, contract_code, module_config, board_config, workflow_config,
		  notification_config, template_set_id, template_version_id, created_by
		) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, input.ProjectCode, input.Name, input.ShortName, nullableText(input.InternalCode), nullableText(input.Description),
		input.Category, input.Methodology, input.LifecycleStatus, input.PortfolioID, nullableText(input.DomainCode),
		nullableText(input.DeptCode), nullableText(input.LeaderUID), input.SecurityLevel, input.ConfidentialityLevel,
		nullableText(input.AccessWhitelist), nullableText(input.StartDate), nullableText(input.EndDate), input.OppID,
		input.ContractID, nullableText(input.CustomerCode), nullableText(input.CustomerName), nullableText(input.ContractCode),
		nullableText(input.ModuleConfig), nullableText(input.BoardConfig), nullableText(input.WorkflowConfig),
		nullableText(input.NotificationConfig), input.TemplateSetID, input.TemplateVersionID, input.CreatedBy)
	if err != nil {
		return nil, err
	}
	...
	if _, err := tx.ExecContext(ctx, `
		INSERT INTO project_lifecycle_events (project_id, from_status, to_status, effective_at, actor_uid, source)
		VALUES (?, NULL, ?, UTC_TIMESTAMP(6), ?, ?)
	`, projectID, input.LifecycleStatus, input.CreatedBy, input.Source); err != nil {
		return nil, err
	}
	...
	`, projectID, input.LeaderUID); err != nil {
		return nil, err
	}
	if input.TemplateDefinition != nil {
		if err := instantiateProjectFromTemplateTx(ctx, tx, projectID, input.ProjectCode, input.CreatedBy, *input.TemplateDefinition, input.ExcludedTemplateWorkItemKeys); err != nil {
			return nil, err
		}
	}
	return aimsQueryOneMap(ctx, tx, "SELECT * FROM aims_projects WHERE id = ? LIMIT 1", projectID)
}
...
func (a *Adapter) handleServiceContractRuntime(ctx context.Context, method string, path string, query url.Values, body map[string]any) (any, string, bool, error) {
	switch {
	case method == http.MethodPost && path == "/v1/aims/service/projects/from-opportunity":
		data, err := a.createProjectFromOpportunityCommand(ctx, body)
		return data, "aims.service.projects.from_opportunity", true, err
	case method == http.MethodPost && path == "/v1/aims/service/projects/from-contract":
		data, err := a.createProjectFromContractCommand(ctx, body)
	...
	case method == http.MethodPost:
		if projectID, ok := pathParam(path, "/v1/aims/service/projects/", "/people-contributions:freeze"); ok {
			data, err := a.freezePeopleContributionSnapshot(ctx, projectID, query, body)
			return data, "aims.service.projects.people_contributions.freeze", true, err
		}
		if path == "/v1/aims/service/milestones:rollover-due" {
			data, err := a.rolloverDueProjectMilestones(ctx, body)
			return data, "aims.service.milestones.rollover_due", true, err
		}
		if projectCode, milestoneID, ok := serviceMilestoneRolloverPath(path); ok {
			data, err := a.rolloverProjectMilestone(ctx, projectCode, milestoneID, body)
			return data, "aims.service.projects.milestones.rollover", true, err
		}
		if projectCode, ok := pathParam(path, "/v1/aims/service/projects/", "/payment-milestones:sync"); ok {
			...
			data, err := a.syncPaymentMilestonesCommand(ctx, projectCode, body)
		...
		if ticketCode, ok := pathParam(path, "/v1/aims/service/service-tickets/", "/work-item"); ok {
			...
			return nil, "aims.service.service_tickets.work_item.retired", true, httperror.New(http.StatusGone, "legacy_service_ticket_work_item_retired", "use the reliable receipt-only work-item receive endpoint")
		}
		if ticketCode, ok := pathParam(path, "/v1/aims/service/service-tickets/", "/work-item:receive"); ok {
			ticketCode, _ = url.PathUnescape(ticketCode)
			data, err := a.receiveAltocServiceTicketWorkItem(ctx, ticketCode, body)
			return data, "aims.service.service_tickets.work_item.receive", true, err
		}
	}
...
func (a *Adapter) eligibleProjectsForContract(ctx context.Context, query url.Values) (map[string]any, error) {
	...
	exactContract := strings.EqualFold(strings.TrimSpace(firstNonEmptyText(query.Get("contract_match"), query.Get("contractMatch"))), "exact")
	if exactContract && contractCode == "" {
		return nil, httperror.New(http.StatusBadRequest, "missing_contract_code", "contractCode is required for exact contract matching")
	}
	...
	if exactContract {
		where = append(where, "contract_code = ?")
		args = append(args, contractCode)
	} else if !includeLinked {
		if contractCode != "" {
	...
	return map[string]any{
		...
		"filters": map[string]any{
			...
			"contract_match": firstNonEmptyText(query.Get("contract_match"), query.Get("contractMatch")),
		},
	}, nil
}
...
func (a *Adapter) createProjectFromContract(ctx context.Context, body map[string]any) (map[string]any, error) {
	...
	defer tx.Rollback()
	result, err := a.createProjectFromContractTx(ctx, tx, body)
	if err != nil {
		return nil, err
	}
...
func (a *Adapter) createProjectFromContractTx(ctx context.Context, tx *sql.Tx, body map[string]any) (map[string]any, error) {
	...
	project, err := createServiceProjectTx(ctx, tx, serviceProjectCreation{
		...
		Category:    firstNonEmptyText(firstBodyText(body, "category"), categoryForProjectRole(firstBodyText(body, "projectRole", "project_role")), "delivery"),
		...
		TemplateSetID: nullableOptionalID(body, "templateSetId", "template_set_id"), TemplateVersionID: nullableOptionalID(body, "templateVersionId", "template_version_id"),
		CreatedBy: createdBy, Source: "aims.service.project.create",
	})
	if err != nil {
		return nil, err
	}
...
func (a *Adapter) syncPaymentMilestones(ctx context.Context, projectCode string, body map[string]any) (map[string]any, error) {
	...
	defer tx.Rollback()
	result, err := a.syncPaymentMilestonesTx(ctx, tx, projectCode, body)
	if err != nil {
		return nil, err
	}
	if err := tx.Commit(); err != nil {
...
func (a *Adapter) syncPaymentMilestonesTx(ctx context.Context, tx *sql.Tx, projectCode string, body map[string]any) (map[string]any, error) {
	...
	for index, term := range terms {
		...
		if existingID > 0 {
			if err := requireMilestoneCompletionUnlockedTx(ctx, tx, existingID); err != nil {
				return nil, err
			}
			if _, err := tx.ExecContext(ctx, `
				UPDATE milestones
