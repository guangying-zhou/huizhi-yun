...
func (a *Adapter) approveRequirementReviewBatch(ctx context.Context, rawBatchID string, query url.Values, body map[string]any) (map[string]any, error) {
	...
	for _, reqID := range batch.requirementIDs {
		...
		if batch.batchType == "change" && req.itemKind == "change" && req.parentRequirementID != nil {
			if err := a.approveChangeRequirement(ctx, tx, batch, req.id, *req.parentRequirementID, uid, uid, batchID, workflowInstanceID, changeReason, changeCode); err != nil {
				return nil, err
			}
			continue
		}
		...
		if _, err := tx.ExecContext(ctx, `
			...
			nullableStringValue(workflowInstanceID), uid, uid); err != nil {
			return nil, err
		}
	...
	if batch.batchType == "baseline" {
		...
		if baselineTargetID.Valid {
			if err := requireWorkItemMilestoneCompletionUnlockedTx(ctx, tx, baselineTargetID.Int64); err != nil {
				return nil, err
			}
			if _, err := tx.ExecContext(ctx,
...
func (a *Adapter) withdrawRequirementReviewBatch(ctx context.Context, rawBatchID string, query url.Values) (map[string]any, error) {
	...
	if err := a.requireProjectManagerOrScopedAdmin(ctx, batch.projectID, uid, query); err != nil {
		return nil, err
	}
	if batch.status != "pending" {
