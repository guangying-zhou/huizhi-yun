...
var EnterpriseWorkItemStateCapabilities = map[string]string{
	"plan-ready": "aims:work-item-plan-ready:execute", "start": "aims:work-item-start:execute", "reset": "aims:work-item-reset:execute", "reopen": "aims:work-item-reopen:execute",
}
...
var enterpriseWorkItemStateEdges = map[string][2]string{
	"plan-ready": {"planning", "todo"}, "start": {"todo", "in_progress"}, "reset": {"in_progress", "todo"}, "reopen": {"completed", "in_progress"},
}

// plan-ready is the V2 target transition_key=decompose. The public action has
// a distinct name because requirement decompose-submit creates new work items.
func enterpriseWorkItemTransitionKey(action string) string {
...
func targetPlanReadyTx(ctx context.Context, tx *sql.Tx, itemID string) (bool, error) {
	var start, due sql.NullString
	var hours sql.NullFloat64
	if err := tx.QueryRowContext(ctx, "SELECT DATE_FORMAT(start_date,'%Y-%m-%d'),DATE_FORMAT(due_date,'%Y-%m-%d'),estimated_hours FROM work_items WHERE id=?", itemID).Scan(&start, &due, &hours); err != nil {
		return false, err
	}
	if !start.Valid || !due.Valid || start.String > due.String || !hours.Valid || hours.Float64 <= 0 {
		return false, nil
	}
	var count int
	if err := tx.QueryRowContext(ctx, "SELECT COUNT(*) FROM deliverables WHERE target_id=?", itemID).Scan(&count); err != nil {
		return false, err
	}
	return count > 0, nil
}
...
func validateEnterpriseWorkItemTransitionTx(ctx context.Context, tx *sql.Tx, project, tier, action, from, to string) error {
	...
	args := []any{project, tier, enterpriseWorkItemTransitionKey(action), from, to}
	if projectRules == 0 {
...
func (a *Adapter) TransitionEnterpriseWorkItem(ctx context.Context, id EnterpriseProjectUpdateIdentity, projectID, itemID, action string, command map[string]any) (map[string]any, error) {
	...
	result, err := receipts.ExecuteInTransaction(ctx, tx, receipt, func(ctx context.Context, tx *sql.Tx, _ json.RawMessage) (iop.ReceiptBusinessResult, error) {
		...
		if action == "plan-ready" {
			if tier != "target" || aimsMapText(item, "type") == "requirement" {
				return iop.ReceiptBusinessResult{}, httperror.New(409, "work_item_plan_ready_target_required", "Only a non-requirement target can be made ready")
			}
			if err := requireEnterpriseProjectManagerTx(ctx, tx, id, projectID, leader); err != nil {
				return iop.ReceiptBusinessResult{}, err
			}
			ready, err := targetPlanReadyTx(ctx, tx, itemID)
			if err != nil {
				return iop.ReceiptBusinessResult{}, err
			}
			if !ready {
				return iop.ReceiptBusinessResult{}, httperror.New(409, "work_item_plan_incomplete", "Target requires planned hours, valid dates, and a deliverable")
			}
		}
...
func (a *Adapter) EnterpriseWorkItemStateActions(ctx context.Context, itemID, actor string, scope url.Values) ([]string, error) {
	...
	if allowed == 1 && lifecycle == "active" {
		...
		if !locked {
			...
			for _, action := range []string{"plan-ready", "start", "reset", "reopen"} {
				edge := enterpriseWorkItemStateEdges[action]
				...
				args := []any{item["project_id"], item["tier"], enterpriseWorkItemTransitionKey(action), edge[0], edge[1]}
				if projectRules == 0 {
				...
				if action == "plan-ready" {
					if aimsMapText(item, "tier") != "target" || aimsMapText(item, "type") == "requirement" {
						continue
					}
					var manager int
					if err = tx.QueryRowContext(ctx, "SELECT COUNT(*) FROM aims_projects p LEFT JOIN aims_project_members m ON m.project_id=p.id AND m.uid=? AND COALESCE(m.status,'active')='active' WHERE p.id=? AND (p.leader_uid=? OR m.role='manager' OR ("+where+"))", append([]any{actor, item["project_id"], actor}, scopeArgs...)...).Scan(&manager); err != nil {
						return nil, err
					}
					if manager == 0 {
						continue
					}
					ready, checkErr := targetPlanReadyTx(ctx, tx, itemID)
					if checkErr != nil {
						return nil, checkErr
					}
					if !ready {
