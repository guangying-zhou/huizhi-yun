...
func mergeMilestoneDeliverableState(current *milestoneDeliverableState, incoming milestoneDeliverableState) {
	if incoming.Required {
		...
		if incoming.DeliverableType == "document" && incoming.QualityStatus != "passed" && incoming.QualityStatus != "waived" {
			current.DeliverableType = incoming.DeliverableType
			current.QualityStatus = incoming.QualityStatus
		}
		return
	}

	if current.Required || !incoming.Completed {
		return
	}
...
func (a *Adapter) milestoneDetail(ctx context.Context, milestoneID string, query url.Values) (map[string]any, error) {
	...
	if err := a.requireProjectReadAccess(ctx, strconv.FormatInt(milestone.ProjectID, 10), query); err != nil {
		return nil, err
	}
...
func (a *Adapter) milestoneDeliverableStates(ctx context.Context, projectID int64, milestoneID int64) ([]milestoneDeliverableState, error) {
	rows, err := a.DB().QueryContext(ctx, `
		SELECT d.id, d.name, d.`+"`required`"+`, d.status, d.deliverable_type, d.quality_status
		FROM deliverables d
	...
	for rows.Next() {
		...
		var required int64
		var status string
		if err := rows.Scan(&id, &name, &required, &status, &deliverableType, &qualityStatus); err != nil {
		...
		if idx, ok := byName[name]; ok {
			mergeMilestoneDeliverableState(&states[idx], milestoneDeliverableState{
	...
	for reqRows.Next() {
		...
		states = append(states, milestoneDeliverableState{
			Name: title, Required: required != 0, Completed: status == "completed",
			Status: status, DeliverableType: "task", QualityStatus: "not_required",
		})
	}
