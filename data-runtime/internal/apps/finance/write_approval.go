...
func (a *Adapter) SubmitApproval(ctx context.Context, target approvalTarget, code string, body jsonBody) (DataResult[map[string]any], error) {
	...
	workflowInstanceID, externalInstanceID, platform, err := validateApprovalSubmissionWorkflowMetadata(body)
	if err != nil {
		return DataResult[map[string]any]{}, err
	}
	...
	`, target.BizType, code, workflowInstanceID, platform, externalInstanceID, submittedBy, nil); err != nil {
		return DataResult[map[string]any]{}, err
	}
...
func validateApprovalSubmissionWorkflowMetadata(body jsonBody) (string, string, string, error) {
	workflowInstanceID := cleanStringValue(bodyValue(body, "workflowInstanceId", "workflow_instance_id"))
	if workflowInstanceID == "" {
		return "", "", "", httperror.New(http.StatusBadRequest, "workflow_instance_required", "Finance approval submission requires a Workflow instance")
	}

	platform := strings.ToLower(firstNonEmpty(cleanStringValue(bodyValue(body, "workflowPlatform", "workflow_platform", "externalPlatform", "external_platform")), "workflow"))
	if platform != "workflow" {
		return "", "", "", httperror.New(http.StatusForbidden, "workflow_platform_required", "Finance approval submission must be backed by Workflow")
	}

	status := strings.ToLower(cleanStringValue(bodyValue(body, "workflowStatus", "workflow_status", "status", "result", "approvalStatus", "approval_status")))
	switch status {
	case "", "running", "pending", "created", "processing":
	case "approved", "approve", "passed", "rejected", "reject", "failed", "canceled", "cancelled", "withdrawn", "issued", "paid":
		return "", "", "", httperror.New(http.StatusForbidden, "approval_result_requires_workflow_callback", "Approval results must be applied through Workflow callback")
	default:
		return "", "", "", httperror.New(http.StatusBadRequest, "unsupported_workflow_status", "unsupported Workflow submission status")
	}

	return workflowInstanceID, cleanStringValue(bodyValue(body, "externalInstanceId", "external_instance_id")), platform, nil
}
...
func (a *Adapter) applyApprovalResultTx(ctx context.Context, tx *sql.Tx, target approvalTarget, code string, result string, options approvalOptions) error {
	...
	if result == "approved" {
		...
		// reject_reason 是可选列：生产租户存在建库早于 finance_schema.sql 声明该列的
		// 情况（走查 ISSUE-B-002）。四张审批表共用这段 SQL，硬写该列会抛 MySQL 1054，
		// 经 writeError 兜底成 500 且被判为可重试，导致 Workflow 回调无限重投。
		// 缺列时降级为不写原因，保证驳回本身仍然成功。
		columns, err := financeTableColumns(ctx, tx, target.Table)
		if err != nil {
			return err
		}
		assignments := []string{
		...
		args := []any{nullableStringArg(options.WorkflowInstanceID)}
		if columns["reject_reason"] {
			assignments = append(assignments, "reject_reason = ?")
			args = append(args, options.RejectReason)
		}
		assignments = append(assignments,
			"updated_by = COALESCE(?, updated_by)",
			"updated_at = CURRENT_TIMESTAMP",
		)
		args = append(args, nullableStringArg(options.Operator), row["id"])
		if _, err := tx.ExecContext(ctx,
			"UPDATE "+target.Table+" SET "+strings.Join(assignments, ", ")+" WHERE id = ?",
			args...,
		); err != nil {
			return err
		}
...
func (a *Adapter) approveInvoiceRequest(ctx context.Context, tx *sql.Tx, row map[string]any, options approvalOptions) error {
	if cleanStringValue(row["status"]) == "issued" || valueFrom(row, "issued_invoice_id") != nil {
		return nil
	}
	if _, err := tx.ExecContext(ctx, `
