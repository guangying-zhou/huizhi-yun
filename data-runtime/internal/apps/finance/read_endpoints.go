...
type ProjectTotals struct {
	...
	GrossProfitAmount   any    `json:"grossProfitAmount"`
	CostReadinessStatus string `json:"costReadinessStatus"`
	NotReadyPeriodCount int    `json:"notReadyPeriodCount"`
}
...
func sumProjectPeriods(rows []map[string]any) ProjectTotals {
	notReadyPeriodCount := 0
	for _, row := range rows {
		if cleanStringValue(row["cost_readiness_status"]) != "ready" {
			notReadyPeriodCount++
		}
	}
	var grossProfitAmount any
	readinessStatus := "ready"
	if len(rows) == 0 || notReadyPeriodCount > 0 {
	...
	return ProjectTotals{
		...
		CostReadinessStatus: readinessStatus,
		NotReadyPeriodCount: notReadyPeriodCount,
	}
}
