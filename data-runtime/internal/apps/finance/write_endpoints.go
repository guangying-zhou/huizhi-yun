...
func (a *Adapter) HandleMutation(ctx context.Context, method string, path string, rawBody map[string]any) (DataResult[map[string]any], string, error) {
	return a.HandleMutationWithQuery(ctx, method, path, nil, rawBody)
}

func (a *Adapter) HandleMutationWithQuery(ctx context.Context, method string, path string, query url.Values, rawBody map[string]any) (DataResult[map[string]any], string, error) {
	method = strings.ToUpper(method)
	if path == "/v1/finance/internal/product-cost:read-rules" {
		data, err := a.readProductCostRulesRuntime(ctx, method, query, rawBody)
		return data, "finance.product_cost.read_rules", err
	}
	if path == "/v1/finance/internal/product-cost:replace-rules" {
		result, err := a.replaceProductCostRulesRuntime(ctx, method, query, rawBody)
		return result, "finance.product_cost.replace_rules", err
	}
	if path == "/v1/finance/internal/product-cost:read" {
		result, err := a.readProductCostRuntime(ctx, method, query, rawBody)
		return result, "finance.product_cost.read", err
	}
	body := financeMutationBodyFromRequest(query, jsonBody(rawBody))
	if result, operation, handled, err := a.handleFinanceIntegrationOperation(ctx, method, path, body); handled {
		return result, operation, err
	}
	if method == http.MethodPost && path == "/v1/finance/service/invoice-requests:create" {
		result, err := a.receiveAltocInvoiceRequest(ctx, body)
		return result, "finance.service.invoice_requests.create", err
	}
	if err := normalizeFinanceDueResponsibilityMutation(method, path, body); err != nil {
		return DataResult[map[string]any]{}, "finance.due_responsibility.validate", err
	}
	...
	if method == http.MethodPost && strings.HasPrefix(path, "/v1/finance/invoices/") && strings.HasSuffix(path, "/receipt-reconcile") {
		code := strings.TrimSuffix(strings.TrimPrefix(path, "/v1/finance/invoices/"), "/receipt-reconcile")
		result, err := a.CreateInvoiceReceiptReconciliation(ctx, code, body)
		return result, "finance.invoices.receipt_reconcile", err
	}
	...
	case method == http.MethodPost && strings.HasPrefix(path, "/v1/finance/invoice-requests/") && strings.HasSuffix(path, "/assign-issuance"):
		code := strings.TrimSuffix(strings.TrimPrefix(path, "/v1/finance/invoice-requests/"), "/assign-issuance")
		result, err := a.AssignInvoiceIssuance(ctx, code, body)
		return result, "finance.invoice_requests.assign_issuance", err
	...
	case method == http.MethodPost && path == "/v1/finance/project-accounting/labor-costs:sync":
		result, err := a.SyncProjectLaborCosts(ctx, body)
		return result, "finance.project_accounting.labor_costs.sync", err
...
var createSpecs = map[string]createSpec{
	...
	"/v1/finance/invoice-requests": {
		...
		Fields: []mutationField{
			...
			allowedDefaultField("status", []string{"draft"}, "draft", "status"),
			stringField("requested_by", "requestedBy", "requested_by"),
			stringField("issuance_responsible_uid", "issuanceResponsibleUid", "issuance_responsible_uid"),
			dateTimeField("issuance_due_at", "issuanceDueAt", "issuance_due_at"),
			stringField("remark", "remark"),
			stringField("created_by", "createdBy", "created_by"),
			stringField("updated_by", "updatedBy", "updated_by"),
		},
...
func receiptCreateSpec() createSpec {
	return createSpec{Operation: "finance.receipts.create", Scope: "finance.receipts.write", Table: "finance_receipt", CodePrefix: "RCV", Fields: []mutationField{
		...
		stringField("reconciliation_responsible_uid", "reconciliationResponsibleUid", "reconciliation_responsible_uid"),
		dateTimeField("reconciliation_due_at", "reconciliationDueAt", "reconciliation_due_at"),
...
func paymentRequestCreateSpec() createSpec {
	return createSpec{Operation: "finance.payment_requests.create", Scope: "finance.payment_requests.write", Table: "payment_request", CodePrefix: "PAY", Fields: []mutationField{
		...
		numberField("bank_account_id", "bankAccountId", "bank_account_id"),
		// 创建只允许落在 draft：approved / paid / rejected 必须分别由 Workflow 审批
		// 回调和付款确认动作写入，否则一次 POST 就能落地一条「已付款」的付款申请，
		// 跳过整条申请—审批—付款链（走查 ISSUE-B-003）。与 invoice_request 创建
		// 白名单保持一致，前端 createFields 本就不传 status。
		allowedDefaultField("status", []string{"draft"}, "draft", "status"),
		stringField("remark", "remark"),
		stringField("created_by", "createdBy", "created_by"),
		stringField("updated_by", "updatedBy", "updated_by"),
