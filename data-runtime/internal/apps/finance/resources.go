...
var listResourceSpecs = map[string]ListResourceSpec{
	...
	"/v1/finance/invoices": {
		...
		Select: []string{
			"id", "code", "invoice_no", "customer_code", "customer_name", "contract_code", "project_code",
			"receivable_plan_code", "invoice_type", "invoice_medium", "invoice_item", "invoice_amount", "tax_amount",
			"invoice_date", "status", "invoice_file_url", "invoice_file_name", "invoice_file_mime_type",
			"invoice_file_size",
			"COALESCE((SELECT SUM(reconciliation.reconciled_amount) FROM finance_reconciliation reconciliation WHERE reconciliation.invoice_id = finance_invoice.id AND reconciliation.status = 'active'), 0) AS reconciled_amount",
			"GREATEST(finance_invoice.invoice_amount - COALESCE((SELECT SUM(reconciliation.reconciled_amount) FROM finance_reconciliation reconciliation WHERE reconciliation.invoice_id = finance_invoice.id AND reconciliation.status = 'active'), 0), 0) AS unreconciled_amount",
			"CASE WHEN COALESCE((SELECT SUM(reconciliation.reconciled_amount) FROM finance_reconciliation reconciliation WHERE reconciliation.invoice_id = finance_invoice.id AND reconciliation.status = 'active'), 0) <= 0 THEN 'unreconciled' WHEN COALESCE((SELECT SUM(reconciliation.reconciled_amount) FROM finance_reconciliation reconciliation WHERE reconciliation.invoice_id = finance_invoice.id AND reconciliation.status = 'active'), 0) + 0.000001 >= finance_invoice.invoice_amount THEN 'reconciled' ELSE 'partially_reconciled' END AS reconciliation_status",
	...
	"/v1/finance/project-accounting": {
		...
		Select:         []string{"id", "project_code", "project_name", "customer_code", "contract_code", "period_month", "contract_amount", "invoice_amount", "received_amount", "direct_expense_amount", "labor_cost_amount", "allocated_cost_amount", "gross_profit_amount", "gross_margin_rate", "cost_readiness_status", "cost_readiness_reasons_json", "cost_input_hash", "cost_readiness_checked_at", "calculated_at", "created_at"},
	...
	"/v1/finance/project-cost-allocations": {
		...
		Select:         []string{"id", "code", "project_code", "period_month", "allocation_type", "employee_uid", "amount", "currency_code", "allocation_basis", "basis_value", "rule_code", "source_refs_json", "status", "created_at"},
	...
	"/v1/finance/receipts": {
		...
		Select:         []string{"id", "code", "receipt_no", "customer_code", "customer_name", "contract_code", "project_code", "receivable_plan_code", "receipt_source_type", "accounting_object_type", "accounting_object_code", "received_amount", "reconciled_amount", "unreconciled_amount", "received_at", "channel", "payer_name", "handler_user_id", "reconciliation_responsible_uid", "reconciliation_due_at", "status", "created_at", "deleted_at"},
