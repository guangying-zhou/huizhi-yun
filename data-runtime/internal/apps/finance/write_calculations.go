...
func (a *Adapter) RecalculateProjectFinance(ctx context.Context, body jsonBody) (DataResult[map[string]any], error) {
	...
	rows, err := queryMaps(ctx, a.db, `
		...
		ORDER BY metric.project_code, metric.period_month
	`, args...)
	if err != nil {
		return DataResult[map[string]any]{}, err
	}
	...
	for _, row := range rows {
		...
		`, row["project_code"], row["period_month"]); err != nil {
			_ = tx.Rollback()
			return DataResult[map[string]any]{}, err
		}
		var readinessStatus string
		if err := tx.QueryRowContext(ctx, `
			SELECT cost_readiness_status
			FROM project_finance_summary
			WHERE project_code = ? AND period_month = ?
			FOR UPDATE
		`, row["project_code"], row["period_month"]).Scan(&readinessStatus); err != nil {
			_ = tx.Rollback()
			return DataResult[map[string]any]{}, err
		}
		receivedAmount := amountFromMap(row, "received_amount")
