...
type SchemaStatus struct {
	...
	CheckedColumns []string `json:"checkedColumns"`
...
type DashboardSummary struct {
	...
	ProjectCostNotReadyCount  int64   `json:"projectCostNotReadyCount"`
...
var requiredTables = []string{
	...
	"finance_notification_checkpoint",
...
var requiredColumns = []struct {
	...
	{Table: "project_finance_summary", Column: "cost_readiness_status"},
	{Table: "project_finance_summary", Column: "cost_readiness_reasons_json"},
	{Table: "project_finance_summary", Column: "cost_input_hash"},
	{Table: "project_finance_summary", Column: "cost_readiness_checked_at"},
	{Table: "project_cost_allocation", Column: "source_refs_json"},
	{Table: "project_cost_allocation", Column: "currency_code"},
	{Table: "invoice_request", Column: "issuance_responsible_uid"},
	{Table: "invoice_request", Column: "issuance_due_at"},
	{Table: "finance_receipt", Column: "reconciliation_responsible_uid"},
	{Table: "finance_receipt", Column: "reconciliation_due_at"},
	// 四张审批表共用 applyApprovalResultTx 的驳回分支，缺列会让驳回整体失败。
	// 生产 payment_request 曾缺 reject_reason 而未被任何检查发现（ISSUE-B-002），
	// 纳入 schema status 后可通过 /runtime/schema/status 提前暴露同类漂移。
	{Table: "invoice_request", Column: "reject_reason"},
	{Table: "expense_claim", Column: "reject_reason"},
	{Table: "project_expense_request", Column: "reject_reason"},
	{Table: "payment_request", Column: "reject_reason"},
}
...
func (a *Adapter) SchemaStatus(ctx context.Context) (SchemaStatus, error) {
	...
	checkedColumns := make([]string, 0, len(requiredColumns))
	missingColumns := make([]string, 0)
	for _, required := range requiredColumns {
		qualified := required.Table + "." + required.Column
		checkedColumns = append(checkedColumns, qualified)
		var marker int
		err := a.db.QueryRowContext(ctx, `
			...
			WHERE TABLE_SCHEMA = DATABASE()
			  ...
			  AND COLUMN_NAME = ?
			LIMIT 1
		`, required.Table, required.Column).Scan(&marker)
...
func (a *Adapter) DashboardSummary(ctx context.Context, query url.Values) (DataResult[DashboardSummary], error) {
	...
	if projectScoped && len(projectCodes) == 0 {
		return DataResult[DashboardSummary]{Data: DashboardSummary{
			...
			ProjectGrossProfitAmount:  nil,
			ProjectCostNotReadyCount:  0,
	...
	var row struct {
		...
		ProjectGrossProfitAmount  sql.NullString
		ProjectCostNotReadyCount  int64
	...
	return DataResult[DashboardSummary]{
		Data: DashboardSummary{
			...
			ProjectGrossProfitAmount:  nullableString(row.ProjectGrossProfitAmount),
			ProjectCostNotReadyCount:  row.ProjectCostNotReadyCount,
...
func dashboardSummaryArgs(projectCodes []string) []any {
	args := make([]any, 0, len(projectCodes)*8)
	for i := 0; i < 8; i++ {
...
func dashboardSummarySQL(projectCodeCount int, projectScoped bool) string {
	...
	return `
		SELECT
		  ...
		  (SELECT CASE
		     WHEN COUNT(*) = 0 OR SUM(CASE WHEN cost_readiness_status <> 'ready' THEN 1 ELSE 0 END) > 0 THEN NULL
		     ELSE SUM(gross_profit_amount)
		   END
		   FROM project_finance_summary
		   WHERE period_month = DATE_FORMAT(CURRENT_DATE(), '%Y-%m')` + projectFilter + `) AS projectGrossProfitAmount,
		  COALESCE((SELECT COUNT(*) FROM project_finance_summary WHERE period_month = DATE_FORMAT(CURRENT_DATE(), '%Y-%m') AND cost_readiness_status <> 'ready'` + projectFilter + `), 0) AS projectCostNotReadyCount,
