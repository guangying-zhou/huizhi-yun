...
func TestDashboardSummarySQLAppliesProjectFilterToProjectScopedMetrics(t *testing.T) {
	...
	if count := strings.Count(sql, "project_code IN (?, ?)"); count != 8 {
		t.Fatalf("project_code filter count = %d, want 8\n%s", count, sql)
	}
	...
	if !strings.Contains(sql, "WHEN COUNT(*) = 0 OR SUM(CASE WHEN cost_readiness_status <> 'ready'") {
		t.Fatalf("dashboard gross profit should fail closed when summaries are missing or not ready\n%s", sql)
	}
}
...
func TestMaintenanceProjectScopedWhereUsesAuthorizedProjectsAsAndConstraint(t *testing.T) {
	...
	if want := "customer_code = ? AND (contract_code IN (?) OR project_code IN (?)) AND project_code IN (?, ?)"; where != want {
		t.Fatalf("where = %q, want %q", where, want)
	}
}

func TestMaintenanceServiceScopedWhereRequiresRequestedContractAndProjectScope(t *testing.T) {
	query := url.Values{}
	query.Set("contract_codes", "C1")
	...
	projectWhere, _ := maintenanceProjectSummaryWhere(scope)
	if want := "customer_code = ? AND (contract_code IN (?) OR project_code IN (?))"; projectWhere != want {
		t.Fatalf("project summary where = %q, want %q", projectWhere, want)
	}
}

func TestMaintenanceServiceScopeRejectsCustomerOnlyQuery(t *testing.T) {
	_, err := maintenanceFinancialScopeFromQuery("CUSTOMER-1", url.Values{})
	if err == nil {
		t.Fatal("customer-only maintenance scope must fail closed")
	}
	var httpErr httperror.Error
	if !errors.As(err, &httpErr) || httpErr.Status != http.StatusBadRequest || httpErr.Code != "maintenance_scope_required" {
		t.Fatalf("error = %#v, want 400 maintenance_scope_required", err)
	}
}

func TestMaintenanceServiceScopeSupportsContractOnlyAndProjectOnlyFacts(t *testing.T) {
	contractScope, err := maintenanceFinancialScopeFromQuery("CUSTOMER-1", url.Values{"contract_codes": {"C1"}})
	if err != nil {
		t.Fatalf("contract-only scope: %v", err)
	}
	if got := maintenanceFinanceWhere(contractScope, "customer_code", "contract_code", "project_code", "period_month"); got != "customer_code = ? AND (contract_code IN (?))" {
		t.Fatalf("contract-only where = %q", got)
	}

	projectScope, err := maintenanceFinancialScopeFromQuery("CUSTOMER-1", url.Values{"project_codes": {"P1"}})
	if err != nil {
		t.Fatalf("project-only scope: %v", err)
	}
	if got := maintenanceFinanceWhere(projectScope, "customer_code", "contract_code", "project_code", "period_month"); got != "customer_code = ? AND (project_code IN (?))" {
		t.Fatalf("project-only where = %q", got)
	}
}
