...
func (a *Adapter) ListResource(ctx context.Context, spec ListResourceSpec, query url.Values) (ListResult[map[string]any], error) {
	...
	applyFinanceResponsibilityListAccess(&where, &args, query, spec.Table)
...
func (a *Adapter) GetRecordByCode(ctx context.Context, spec DetailResourceSpec, code string, query url.Values) (DataResult[map[string]any], error) {
	...
	if responsibilityWhere, responsibilityArgs := financeResponsibilityDetailWhere(query, spec.Table); responsibilityWhere != "" {
		whereSQL += " AND " + responsibilityWhere
		args = append(args, responsibilityArgs...)
	}

	rows, err := a.db.QueryContext(ctx, "SELECT * FROM "+spec.Table+" WHERE "+whereSQL+" LIMIT 1", args...)
	if err != nil {
...
// financeTableColumns 探测表的实际列集合，用于对可选列做失败降级而不是抛
// MySQL 1054。未映射的 SQL 错误会被 writeError 兜底成 500 internal_error，
// 且 retryableHTTPStatus 判其可重试，调用方会持续重投（见走查 ISSUE-B-002）。
func financeTableColumns(ctx context.Context, conn interface {
