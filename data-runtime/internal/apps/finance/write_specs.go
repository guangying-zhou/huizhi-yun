...
func (a *Adapter) createBySpec(ctx context.Context, spec createSpec, body jsonBody) (DataResult[map[string]any], error) {
	return createBySpecOn(ctx, a.db, spec, body)
}

func createBySpecOn(ctx context.Context, conn execQueryer, spec createSpec, body jsonBody) (DataResult[map[string]any], error) {
	...
	_, err := conn.ExecContext(ctx, "INSERT INTO "+spec.Table+" ("+strings.Join(columns, ", ")+") VALUES ("+strings.Join(placeholders, ", ")+")", args...)
	if err != nil {
		return DataResult[map[string]any]{}, err
	}
	...
	row, err := queryOneMap(ctx, conn, sqlText, code)
	if err != nil {
		return DataResult[map[string]any]{}, err
	}
...
func (a *Adapter) updateBySpec(ctx context.Context, spec updateSpec, code string, body jsonBody) (DataResult[map[string]any], error) {
	...
	if spec.Table == "finance_receipt" {
		if err := requireFinanceResponsibilityBodyAccess(body, before, financeReceiptResponsibilityTarget); err != nil {
			return DataResult[map[string]any]{}, err
		}
	}
