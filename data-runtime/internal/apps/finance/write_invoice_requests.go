...
func (a *Adapter) IssueInvoiceRequest(ctx context.Context, code string, body jsonBody) (DataResult[map[string]any], error) {
	...
	operator := cleanStringValue(bodyValue(body, "current_user"))
	if operator == "" {
		return DataResult[map[string]any]{}, httperror.New(http.StatusUnauthorized, "trusted_actor_required", "trusted current_user is required to issue an invoice")
	}
	status := cleanStringValue(request["status"])
	if status != "approved" && status != "issued" {
		return DataResult[map[string]any]{}, httperror.New(http.StatusConflict, "invalid_status", "only approved invoice requests can be issued")
	}
	if err := requireFinanceResponsibilityBodyAccess(body, request, financeInvoiceRequestResponsibilityTarget); err != nil {
		return DataResult[map[string]any]{}, err
	}
	if status == "issued" {
		if valueFrom(request, "issued_invoice_id") == nil {
			return DataResult[map[string]any]{}, httperror.New(http.StatusConflict, "issued_invoice_missing", "issued invoice request is missing its formal invoice reference")
		}
...
func (a *Adapter) AssignInvoiceIssuance(ctx context.Context, code string, body jsonBody) (DataResult[map[string]any], error) {
	if err := requireCodePath(code); err != nil {
		return DataResult[map[string]any]{}, err
	}
	if err := normalizeFinanceDueResponsibilityMutation(http.MethodPost, "/v1/finance/invoice-requests/"+code+"/assign-issuance", body); err != nil {
		return DataResult[map[string]any]{}, err
	}
	operator := cleanStringValue(bodyValue(body, "current_user"))
	if operator == "" {
		return DataResult[map[string]any]{}, httperror.New(http.StatusUnauthorized, "trusted_actor_required", "trusted current_user is required to assign invoice issuance")
	}
	if !bodyHas(body, "issuanceResponsibleUid", "issuance_responsible_uid") || !bodyHas(body, "issuanceDueAt", "issuance_due_at") {
		return DataResult[map[string]any]{}, invalidFinanceDueResponsibility("responsible UID and due time must be supplied together")
	}
	...
	defer tx.Rollback()
	request, err := queryOneMap(ctx, tx, `SELECT * FROM invoice_request WHERE code=? AND deleted_at IS NULL LIMIT 1 FOR UPDATE`, code)
	if err != nil {
		return DataResult[map[string]any]{}, err
	}
	if request == nil {
	...
	if cleanStringValue(request["status"]) != "approved" || valueFrom(request, "issued_invoice_id") != nil {
		return DataResult[map[string]any]{}, httperror.New(http.StatusConflict, "invalid_status", "only approved unissued invoice requests can assign issuance responsibility")
	}

	responsible := nilString(bodyValue(body, "issuanceResponsibleUid", "issuance_responsible_uid"))
	dueAt := optionalDateTimeValue(bodyValue(body, "issuanceDueAt", "issuance_due_at"))
	if cleanStringValue(request["issuance_responsible_uid"]) == cleanStringValue(responsible) && cleanStringValue(request["issuance_due_at"]) == cleanStringValue(dueAt) {
		request["idempotent"] = true
		if err := tx.Commit(); err != nil {
			return DataResult[map[string]any]{}, err
		}
		return resultData(request), nil
	}
	if _, err := tx.ExecContext(ctx, `UPDATE invoice_request SET issuance_responsible_uid=?,issuance_due_at=?,updated_by=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`, responsible, dueAt, operator, request["id"]); err != nil {
		return DataResult[map[string]any]{}, err
	}
	...
	if err := insertFinanceAuditLog(ctx, tx, "invoice_request", request["id"], code, "assign_issuance", request, updated, operator); err != nil {
		return DataResult[map[string]any]{}, err
	}
