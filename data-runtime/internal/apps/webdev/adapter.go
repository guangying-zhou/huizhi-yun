...
func (a *Adapter) HandleRuntime(ctx context.Context, method string, path string, query url.Values, body map[string]any) (any, string, error) {
	...
	case method == http.MethodPost && path == "/v1/webdev/service/jobs":
		result, err := a.CreateJob(ctx, body)
		return result, "webdev.service_jobs.create", err
	...
	case method == http.MethodGet && path == "/v1/webdev/service/issues/mine":
		result, err := a.ListIssues(ctx, query)
		return result, "webdev.service_issues.mine", err
	case method == http.MethodGet && path == "/v1/webdev/service/issues/settings":
		result, err := a.GetIssueSettings(ctx, trustedRuntimeTenant(query, nil))
		return result, "webdev.service_issues.settings", err
	case method == http.MethodPost && path == "/v1/webdev/issues":
		result, err := a.CreateIssue(ctx, body)
		return result, "webdev.issues.create", err
	case method == http.MethodPost && path == "/v1/webdev/service/issues/intake":
		result, err := a.CreateIssue(ctx, body)
		return result, "webdev.service_issues.intake", err
	case method == http.MethodPost && isWebDevServiceIssueClaimPath(path):
		id := strings.TrimSuffix(strings.TrimPrefix(path, "/v1/webdev/service/issues/"), "/claim")
		result, err := a.ClaimIssue(ctx, id, body)
		return result, "webdev.service_issues.claim", err
	case method == http.MethodGet && isWebDevServiceIssueDetailPath(path):
		id := strings.TrimPrefix(path, "/v1/webdev/service/issues/")
		result, err := a.GetIssue(ctx, id, trustedRuntimeTenant(query, nil))
		return result, "webdev.service_issues.get", err
	case method == http.MethodPatch && isWebDevServiceIssueDetailPath(path):
		id := strings.TrimPrefix(path, "/v1/webdev/service/issues/")
		result, err := a.UpdateIssue(ctx, id, body)
		return result, "webdev.service_issues.update", err
...
func isWebDevServiceIssueDetailPath(path string) bool {
	id := strings.TrimPrefix(path, "/v1/webdev/service/issues/")
	return id != path && id != "" && !strings.Contains(id, "/") && id != "mine" && id != "settings"
}

func isWebDevServiceIssueClaimPath(path string) bool {
...
func trustedRuntimeTenant(query url.Values, body map[string]any) string {
	if query != nil {
		if tenant := strings.TrimSpace(query.Get("hzy_runtime_tenant_code")); tenant != "" {
			return tenant
		}
	}
	if body != nil {
		if tenant := strings.TrimSpace(stringValue(body, "hzy_runtime_tenant_code")); tenant != "" {
			return tenant
		}
	}
	return ""
...
func requireTrustedRuntimeTenant(query url.Values, body map[string]any) (string, error) {
	tenant := trustedRuntimeTenant(query, body)
	if tenant == "" {
		return "", httperror.New(http.StatusForbidden, "trusted_webdev_tenant_required", "trusted runtime tenant context is required")
	}
	return tenant, nil
}
...
func (a *Adapter) ListIssues(ctx context.Context, query url.Values) (DataResult[IssueList], error) {
	tenant, err := requireTrustedRuntimeTenant(query, nil)
	if err != nil {
...
func (a *Adapter) getIssueRow(ctx context.Context, id string, tenant string) (Issue, error) {
	row := a.db.QueryRowContext(ctx, "SELECT "+issueColumns+" FROM webdev_issues WHERE issue_id = ? AND tenant = ?", id, tenant)
	return scanIssue(row)
}

func (a *Adapter) GetIssue(ctx context.Context, id string, tenant string) (DataResult[Issue], error) {
	if invalidID(id) {
		return DataResult[Issue]{}, httperror.New(http.StatusNotFound, "issue_not_found", "Issue not found")
	}
	if strings.TrimSpace(tenant) == "" {
		return DataResult[Issue]{}, httperror.New(http.StatusForbidden, "trusted_webdev_tenant_required", "trusted runtime tenant context is required")
	}
	issue, err := a.getIssueRow(ctx, id, tenant)
	if err != nil {
		if err == sql.ErrNoRows {
			return DataResult[Issue]{}, httperror.New(http.StatusNotFound, "issue_not_found", "Issue not found")
		}
	...
	events, err := a.listIssueEvents(ctx, id, tenant)
	if err != nil {
		return DataResult[Issue]{}, err
	}
	issue.Events = events
...
func (a *Adapter) listIssueEvents(ctx context.Context, issueID string, tenant string) ([]IssueEvent, error) {
	rows, err := a.db.QueryContext(ctx, `
		SELECT id, issue_id, actor, action, detail_json, created_at
		FROM webdev_issue_events
		WHERE issue_id = ? AND tenant = ?
		ORDER BY id ASC
		LIMIT 500
	`, issueID, tenant)
	if err != nil {
...
func (a *Adapter) CreateIssue(ctx context.Context, body map[string]any) (DataResult[Issue], error) {
	...
	tenant, err := requireTrustedRuntimeTenant(nil, body)
	if err != nil {
		return DataResult[Issue]{}, err
	}
	createdAt := stringDefault(stringValue(body, "createdAt"), time.Now().UTC().Format(time.RFC3339Nano))
	...
	return a.GetIssue(ctx, issueID, tenant)
}
...
func (a *Adapter) UpdateIssue(ctx context.Context, id string, body map[string]any) (DataResult[Issue], error) {
	...
	result, err := a.db.ExecContext(ctx, "UPDATE webdev_issues SET "+strings.Join(sets, ", ")+" WHERE issue_id = ? AND tenant = ?", args...)
	if err != nil {
		return DataResult[Issue]{}, err
	}
	...
	if state := stringValue(body, "state"); state != "" {
		...
		_, _ = a.db.ExecContext(ctx, `
			...
			SELECT tenant, issue_id, ?, 'state_changed', ?, ? FROM webdev_issues WHERE issue_id = ? AND tenant = ?
		`, actor, string(detail), time.Now().UTC().Format(time.RFC3339Nano), id, tenant)
	}
	return a.GetIssue(ctx, id, tenant)
}
...
func (a *Adapter) ClaimIssue(ctx context.Context, id string, body map[string]any) (DataResult[ClaimResult], error) {
	...
	result, err := a.db.ExecContext(ctx, `
		...
		WHERE issue_id = ? AND tenant = ? AND state = 'open' AND linked_job_id = ''
	`, claimToken, boolToInt(body, "autoClaimed"), id, tenant)
	if err != nil {
		return DataResult[ClaimResult]{}, err
	}
	affected, _ := result.RowsAffected()
	...
	if affected == 1 {
		_, _ = a.db.ExecContext(ctx, `
			INSERT INTO webdev_issue_events (tenant, issue_id, actor, action, created_at)
			SELECT tenant, issue_id, ?, 'claimed', ? FROM webdev_issues WHERE issue_id = ? AND tenant = ?
		`, actor, now, id, tenant)
		issue, err := a.getIssueRow(ctx, id, tenant)
		if err != nil {
			return DataResult[ClaimResult]{}, err
...
func (a *Adapter) CreateIssueEvent(ctx context.Context, issueID string, body map[string]any) (DataResult[IssueEvent], error) {
	...
	result, err := a.db.ExecContext(ctx, `
		INSERT INTO webdev_issue_events (tenant, issue_id, actor, action, detail_json, created_at)
		SELECT tenant, issue_id, ?, ?, ?, ? FROM webdev_issues WHERE issue_id = ? AND tenant = ?
	`, actor, action, detailArg, createdAt, issueID, tenant)
	if err != nil {
		return DataResult[IssueEvent]{}, err
	}
	if affected, _ := result.RowsAffected(); affected == 0 {
...
func (a *Adapter) GetIssueSettings(ctx context.Context, tenant string) (DataResult[IssueSettings], error) {
	...
	if tenant == "" {
		return DataResult[IssueSettings]{}, httperror.New(http.StatusForbidden, "trusted_webdev_tenant_required", "trusted runtime tenant context is required")
	}
	row := a.db.QueryRowContext(ctx, `
