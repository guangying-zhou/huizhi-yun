...
type SchemaStatus struct {
	...
	CheckedColumns []string `json:"checkedColumns,omitempty"`
	MissingColumns []string `json:"missingColumns,omitempty"`
}

// RuntimeUpdateTxHook extends a generic resource update with an app-specific
// mutation that must commit or roll back with the resource row. Hook metadata
// is merged into the returned resource after the transaction commits.
type RuntimeUpdateTxHook func(context.Context, *sql.Tx, string) (map[string]any, error)
type RuntimeCreateTxHook func(context.Context, *sql.Tx, string) (map[string]any, error)
...
func New(cfg Config) (*Adapter, error) {
	...
	return &Adapter{
		...
		requiredColumns: append([]string(nil), cfg.RequiredColumns...),
		resources:       resources,
...
func (a *Adapter) SchemaStatus(ctx context.Context) (SchemaStatus, error) {
	if len(a.requiredTables) == 0 && len(a.requiredColumns) == 0 {
		return SchemaStatus{
			App:      a.appCode,
			Database: a.dbName,
			Status:   "ok",
	...
	checkedColumns := normalizedRequiredColumns(a.requiredColumns)
	missingColumns := make([]string, 0)
	if len(checkedColumns) > 0 {
		tables := make([]string, 0)
		tableSeen := map[string]bool{}
		for _, qualified := range checkedColumns {
			table, _, _ := strings.Cut(qualified, ".")
			if !tableSeen[table] {
				tableSeen[table] = true
		...
		for rows.Next() {
			...
			existingColumns[table+"."+column] = true
		}
		if err := rows.Err(); err != nil {
			_ = rows.Close()
			return SchemaStatus{}, err
		...
		_ = rows.Close()
		for _, qualified := range checkedColumns {
			if !existingColumns[qualified] {
				missingColumns = append(missingColumns, qualified)
			}
		}
	}

	status := "ok"
	if len(missing) > 0 || len(missingColumns) > 0 {
...
func normalizedRequiredColumns(values []string) []string {
	seen := map[string]bool{}
	result := make([]string, 0, len(values))
	for _, value := range values {
		qualified := strings.TrimSpace(value)
		table, column, ok := strings.Cut(qualified, ".")
		if !ok || strings.TrimSpace(table) == "" || strings.TrimSpace(column) == "" || strings.Contains(column, ".") {
			continue
		}
		qualified = strings.TrimSpace(table) + "." + strings.TrimSpace(column)
		if seen[qualified] {
			continue
		}
		seen[qualified] = true
...
// HandleRuntimeUpdateWithTxHook preserves the generic update allowlist and
// authorization behavior while giving complex app adapters one transaction in
// which to persist a caller-owned operation/outbox record.
func (a *Adapter) HandleRuntimeUpdateWithTxHook(
	ctx context.Context,
	method string,
	path string,
	query url.Values,
	...
	if method != http.MethodPut && method != http.MethodPatch {
		return nil, "", httperror.New(http.StatusMethodNotAllowed, "runtime_update_required", "Transactional update hooks only support PUT or PATCH")
	}
	if hook == nil {
		return nil, "", httperror.New(http.StatusInternalServerError, "runtime_update_hook_required", "Transactional update hook is required")
	}
	suffix, ok := a.runtimeSuffix(path)
	if !ok {
		return nil, "", httperror.New(http.StatusNotFound, "not_found", "Route not found")
	...
	result, err := a.updateWithTxHook(ctx, match, query, match.rest[0], body, hook)
	return a.wrap(result), a.operation(match.spec, "update"), err
}

// HandleRuntimeCreateWithTxHook preserves the generic create allowlist and
// authorization behavior while committing caller-owned integration metadata
// in the same transaction as the new resource row.
func (a *Adapter) HandleRuntimeCreateWithTxHook(
	ctx context.Context,
	method string,
	path string,
	query url.Values,
	body map[string]any,
	hook RuntimeCreateTxHook,
) (any, string, error) {
	if method != http.MethodPost {
		return nil, "", httperror.New(http.StatusMethodNotAllowed, "runtime_create_required", "Transactional create hooks only support POST")
	}
	if hook == nil {
		return nil, "", httperror.New(http.StatusInternalServerError, "runtime_create_hook_required", "Transactional create hook is required")
	}
	suffix, ok := a.runtimeSuffix(path)
	if !ok {
		return nil, "", httperror.New(http.StatusNotFound, "not_found", "Route not found")
	...
	match, ok := a.matchResource(suffix)
	if !ok || len(match.rest) != 0 {
		return nil, "", httperror.New(http.StatusNotFound, "not_found", "Route not found")
	}
	result, err := a.createWithTxHook(ctx, match, query, body, hook)
	return a.wrap(result), a.operation(match.spec, "create"), err
}
...
func (a *Adapter) createWithTxHook(ctx context.Context, match resourceMatch, query url.Values, body map[string]any, hook RuntimeCreateTxHook) (map[string]any, error) {
	if match.spec.ReadOnly {
		return nil, httperror.New(http.StatusMethodNotAllowed, "resource_read_only", "Resource is read-only")
	}
	columns, err := a.tableColumns(ctx, match.spec.Table)
	...
	defer tx.Rollback()
	inserted, err := tx.ExecContext(ctx, "INSERT INTO "+quoteID(match.spec.Table)+" ("+quoteIDList(names)+") VALUES ("+strings.Join(placeholders, ", ")+")", args...)
	if err != nil {
		return nil, err
	}
	...
	if match.spec.IDColumn != "" && match.spec.IDColumn != "id" {
		if value, ok := fields[match.spec.IDColumn]; ok && strings.TrimSpace(fmt.Sprint(value)) != "" {
			identifier = strings.TrimSpace(fmt.Sprint(value))
		}
	}
	metadata, err := hook(ctx, tx, identifier)
	if err != nil {
		return nil, err
	}
	for key := range metadata {
		if hasColumn(columns, toSnakeCase(key)) {
			return nil, fmt.Errorf("transactional create hook metadata conflicts with resource field %q", key)
		}
	}
	if err := tx.Commit(); err != nil {
		return nil, err
	}
	result, err := a.get(ctx, match, identifier, query)
	if err != nil {
		return nil, err
	}
...
func (a *Adapter) updateWithTxHook(
	ctx context.Context,
	match resourceMatch,
	query url.Values,
	identifier string,
...
func (a *Adapter) updateWithExternalTxHook(ctx context.Context, match resourceMatch, query url.Values, identifier string, body map[string]any, hook RuntimeUpdateTxHook, externalTx *sql.Tx) (map[string]any, error) {
	...
	if len(fields) > 0 {
		...
		args = append(args, whereArgs...)
		result, err := tx.ExecContext(ctx, "UPDATE "+quoteID(match.spec.Table)+" SET "+strings.Join(set, ", ")+" WHERE "+strings.Join(where, " AND "), args...)
		if err != nil {
			return nil, err
		}
		...
		if affected == 0 {
			...
			err := tx.QueryRowContext(
				ctx,
				"SELECT 1 FROM "+quoteID(match.spec.Table)+" WHERE "+strings.Join(where, " AND ")+" LIMIT 1",
				whereArgs...,
			).Scan(&exists)
...
func (a *Adapter) whereParts(match resourceMatch, columns map[string]columnInfo, query url.Values) ([]string, []any) {
	...
	for queryKey, column := range match.spec.ListInFilters {
		values := uniqueStrings(delimitedStrings(query[queryKey]))
		if len(values) == 0 {
			continue
		}
		if !hasColumn(columns, column) {
		...
		where = append(where, quoteID(column)+" IN ("+placeholders+")")
		for _, value := range values {
			args = append(args, value)
		}
	}
	...
	for key, values := range query {
		...
		if _, configured := match.spec.ListInFilters[key]; configured {
			continue
		}
...
func (a *Adapter) requireOwnerWrite(ctx context.Context, match resourceMatch, columns map[string]columnInfo, where []string, args []any, query url.Values, body map[string]any) error {
	...
	if !runtimeActorCanWriteScopedRecordForMode(runtimeDataAccess(query, body), actor, runtimeActorDeptCodes(query, body), owner, hasOwnerScope, department, hasDepartmentScope) {
...
func runtimeActorCanWriteScopedRecordForMode(mode string, actor string, deptCodes []string, owner sql.NullString, hasOwnerScope bool, department sql.NullString, hasDepartmentScope bool) bool {
	if strings.TrimSpace(strings.ToLower(mode)) != "self_dept" {
		return runtimeActorCanWriteScopedRecord(actor, deptCodes, owner, hasOwnerScope, department, hasDepartmentScope)
	}
	...
	if actor == "" || ownerText == "" || departmentText == "" || ownerText != actor {
		return false
	}
	for _, deptCode := range deptCodes {
		if departmentText == strings.TrimSpace(deptCode) {
			return true
		}
	}
	return false
...
func (a *Adapter) readScopeWhere(match resourceMatch, columns map[string]columnInfo, query url.Values, body map[string]any) ([]string, []any, error) {
	...
	mode := runtimeDataAccess(query, body)
	deptCodes := runtimeActorDeptCodes(query, body)
	if mode == "self_dept" {
		clauses := make([]string, 0, 2)
		args := make([]any, 0, 4)
		if hasOwnerScope && hasDepartmentScope && len(deptCodes) > 0 {
			placeholders := strings.TrimRight(strings.Repeat("?,", len(deptCodes)), ",")
			clauses = append(clauses, "("+quoteID(ownerColumn)+" = ? AND "+quoteID(departmentColumn)+" IN ("+placeholders+"))")
			args = append(args, actor)
			for _, deptCode := range deptCodes {
		...
		if hasParentScope {
			parentWhere, parentArgs := parentScopeExistsWhere(parentScope, actor, deptCodes, mode)
			if parentWhere != "" {
...
func parentScopeExistsWhere(scope ParentScopeSpec, actor string, deptCodes []string, mode string) (string, []any) {
	parentAlias := "scope_parent"
	scopeClauses, scopeArgs := parentScopeRecordClauses(scope, parentAlias, actor, deptCodes, mode)
	...
	where = append(where, "("+strings.Join(scopeClauses, runtimeScopeClauseJoiner(mode))+")")
	return "EXISTS (SELECT 1 FROM " + quoteID(scope.Table) + " " + parentAlias + " WHERE " + strings.Join(where, " AND ") + ")", scopeArgs
}

func parentScopeRecordClauses(scope ParentScopeSpec, tableAlias string, actor string, deptCodes []string, mode string) ([]string, []any) {
	columnPrefix := ""
	...
	clauses := make([]string, 0, 2)
	args := make([]any, 0, 4)
	if runtimeScopeClauseJoiner(mode) == " AND " && (scope.OwnerColumn == "" || scope.DepartmentColumn == "" || len(deptCodes) == 0) {
		return nil, nil
	}
...
func runtimeScopeClauseJoiner(mode string) string {
	if strings.TrimSpace(strings.ToLower(mode)) == "self_dept" {
		return " AND "
	}
	return " OR "
...
func (a *Adapter) requireParentWriteScope(ctx context.Context, match resourceMatch, parentScope ParentScopeSpec, where []string, args []any, query url.Values, body map[string]any) error {
	...
	actor := actorFrom(query, body)
	deptCodes := runtimeActorDeptCodes(query, body)
	parentWhere, parentArgs := parentScopeExistsWhere(parentScope, actor, deptCodes, runtimeDataAccess(query, body))
	if parentWhere == "" {
		return httperror.New(http.StatusForbidden, "owner_scope_required", "Only the owner or scoped department can modify this record")
	}
...
func (a *Adapter) requireParentRecordScope(ctx context.Context, parentScope ParentScopeSpec, parentID string, query url.Values, body map[string]any) error {
	...
	deptCodes := runtimeActorDeptCodes(query, body)
	mode := runtimeDataAccess(query, body)
	scopeClauses, scopeArgs := parentScopeRecordClauses(parentScope, "", actor, deptCodes, mode)
	if len(scopeClauses) == 0 {
		return httperror.New(http.StatusForbidden, "owner_scope_required", "Only the owner or scoped department can modify this record")
	}
	scopedWhere := append([]string(nil), parentWhere...)
	scopedWhere = append(scopedWhere, "("+strings.Join(scopeClauses, runtimeScopeClauseJoiner(mode))+")")
...
func runtimeActorScopes(query url.Values, body map[string]any) []string {
	if queryHasRuntimeAuthContext(query) {
		if query == nil {
			return nil
		}
		return scopeStrings(query.Get("current_user_scopes"))
	}
...
func runtimeActorDeptCodes(query url.Values, body map[string]any) []string {
	if access := runtimeDataAccess(query, body); access == "self" {
		return nil
	} else if access == "dept" || access == "self_dept" {
...
func runtimeDataAccess(query url.Values, body map[string]any) string {
	if queryHasRuntimeAuthContext(query) {
		return strings.ToLower(queryValue(query, "current_user_data_access", "currentUserDataAccess"))
	}
...
func actorFrom(query url.Values, body map[string]any) string {
	if actor := firstNonEmpty(queryValue(query, "operator_uid", "operatorUid"), queryValue(query, "current_user", "currentUser")); actor != "" {
		return actor
	}
	if queryHasRuntimeAuthContext(query) {
		return ""
	...
	if body != nil {
		for _, key := range []string{"operator_uid", "operatorUid", "currentUser", "current_user"} {
...
func queryHasRuntimeAuthContext(query url.Values) bool {
	if query == nil {
		return false
	}
	for _, key := range []string{
		...
		if strings.TrimSpace(query.Get(key)) != "" || len(query[key]) > 0 {
			return true
		}
	}
	return false
