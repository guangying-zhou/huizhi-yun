...
func TestRuntimeAuthContextPrefersTrustedQueryOverBody(t *testing.T) {
	query := url.Values{}
	query.Set("current_user", "trusted-user")
	query.Set("operator_uid", "trusted-operator")
	query.Set("current_user_scopes", "app:records:edit")
	query.Set("current_user_data_access", "dept")
	query.Set("current_user_data_dept_codes", "D1,D2")
	query.Set("current_user_dept_codes", "D3")
	body := map[string]any{
		"current_user":                  "body-user",
		"operator_uid":                  "body-operator",
		"current_user_scopes":           "app:admin",
		"current_user_data_access":      "all",
		"current_user_data_dept_codes":  "DX",
		"current_user_dept_codes":       "DY",
		"current_user_department_codes": []any{"DZ"},
	}

	if actor := actorFrom(query, body); actor != "trusted-operator" {
		t.Fatalf("expected query actor to win, got %#v", actor)
	}
	if access := runtimeDataAccess(query, body); access != "dept" {
		t.Fatalf("expected query data access to win, got %#v", access)
	}
	if scopes := runtimeActorScopes(query, body); !reflect.DeepEqual(scopes, []string{"app:records:edit"}) {
		t.Fatalf("expected query scopes only, got %#v", scopes)
	}
	if depts := runtimeDataDeptCodes(query, body); !reflect.DeepEqual(depts, []string{"D1", "D2"}) {
		t.Fatalf("expected query data departments only, got %#v", depts)
	}
	if actorDepts := runtimeActorDeptCodes(query, body); !reflect.DeepEqual(actorDepts, []string{"D1", "D2"}) {
		t.Fatalf("expected query actor departments only in dept mode, got %#v", actorDepts)
	}
}

func TestRuntimeAuthContextAllowsBodyFallbackWithoutQueryContext(t *testing.T) {
	body := map[string]any{
		"operator_uid":                 "body-operator",
		"current_user_scopes":          []any{"app:admin"},
		"current_user_data_access":     "dept",
		"current_user_data_dept_codes": "D1,D2",
	}

	if actor := actorFrom(url.Values{}, body); actor != "body-operator" {
		t.Fatalf("expected body actor fallback, got %#v", actor)
	}
	if access := runtimeDataAccess(url.Values{}, body); access != "dept" {
		t.Fatalf("expected body data access fallback, got %#v", access)
	}
	if scopes := runtimeActorScopes(url.Values{}, body); !reflect.DeepEqual(scopes, []string{"app:admin"}) {
		t.Fatalf("expected body scopes fallback, got %#v", scopes)
	}
	if depts := runtimeDataDeptCodes(url.Values{}, body); !reflect.DeepEqual(depts, []string{"D1", "D2"}) {
		t.Fatalf("expected body departments fallback, got %#v", depts)
	}
}

func TestWherePartsAppliesConfiguredListInFilter(t *testing.T) {
	adapter := &Adapter{}
	query := url.Values{}
	query.Set("employee_uids", "u1,u2,u1")
	query.Set("employment_status", "active")

	where, args := adapter.whereParts(resourceMatch{
		spec: ResourceSpec{
	...
	if !reflect.DeepEqual(where, expectedWhere) {
		t.Fatalf("whereParts() = %#v, expected %#v", where, expectedWhere)
	}
	expectedArgs := []any{"u1", "u2", "active"}
	if !reflect.DeepEqual(args, expectedArgs) {
		t.Fatalf("whereParts() args = %#v, expected %#v", args, expectedArgs)
	}
}
...
func TestReadScopeWhereAppliesDataAccessModes(t *testing.T) {
	...
	where, args, err = adapter.readScopeWhere(match, columns, selfDeptQuery, nil)
	if err != nil {
		t.Fatalf("self_dept data access returned error: %v", err)
	}
	if len(where) != 1 || where[0] != "((`owner_user_id` = ? AND `owner_dept_code` IN (?,?)))" {
...
func TestReadScopeWhereUsesSelfDeptModeForParentScope(t *testing.T) {
	adapter := &Adapter{appCode: "altoc"}
	parentScope := ParentScopeSpec{
		Table:            "contract",
		IDColumn:         "id",
		LocalColumn:      "contract_id",
		Resource:         "contract",
		OwnerColumn:      "owner_user_id",
		DepartmentColumn: "owner_dept_code",
	...
	match := resourceMatch{
		spec: ResourceSpec{
			Path:        "contracts/{contract_id}/lines",
			Table:       "contract_line",
			ParentScope: &parentScope,
		},
	}
	columns := map[string]columnInfo{"contract_id": {}}
	query := url.Values{}
	query.Set("current_user", "u1")
	query.Set("current_user_data_access", "self_dept")
	query.Set("current_user_data_dept_codes", "D1,D2")

	where, args, err := adapter.readScopeWhere(match, columns, query, nil)
	if err != nil {
		t.Fatalf("readScopeWhere returned error: %v", err)
	}
	expectedWhere := "(EXISTS (SELECT 1 FROM `contract` scope_parent WHERE scope_parent.`id` = `contract_id` AND scope_parent.`deleted_at` IS NULL AND (scope_parent.`owner_user_id` = ? AND scope_parent.`owner_dept_code` IN (?,?))))"
	if len(where) != 1 || where[0] != expectedWhere {
		t.Fatalf("unexpected parent self_dept where %#v", where)
	}
	if !reflect.DeepEqual(args, []any{"u1", "D1", "D2"}) {
		t.Fatalf("unexpected parent self_dept args %#v", args)
	}
}
...
func TestRuntimeActorCanWriteScopedRecordForModeRequiresOwnerAndDepartmentForSelfDept(t *testing.T) {
	allowed := runtimeActorCanWriteScopedRecordForMode(
		"self_dept",
		"u1",
		[]string{"D1", "D2"},
		sql.NullString{String: "u1", Valid: true},
	...
	if !allowed {
		t.Fatal("expected owner inside scoped department to allow self_dept write")
	}
	...
	allowed = runtimeActorCanWriteScopedRecordForMode(
		...
		sql.NullString{String: "u1", Valid: true},
		true,
		sql.NullString{String: "D9", Valid: true},
		true,
	)
	if allowed {
		t.Fatal("expected owner outside scoped department to reject self_dept write")
	}
	...
	if allowed {
		t.Fatal("expected other owner inside scoped department to reject self_dept write")
	}
}
