...
func (a *Adapter) handleAssignmentRoutes(ctx context.Context, method string, path string, query url.Values, body map[string]any) (bool, any, string, error) {
	...
	if method == http.MethodPost && path == "/v1/assets/assignments" {
		id, err := a.createAssignment(ctx, query, body, actorFromRequest(query, body))
	...
	if rawID, ok := singlePathParam(path, "/v1/assets/assignments/"); ok && method == http.MethodGet {
		id, err := parsePositiveID(rawID)
		if err != nil {
			return true, nil, "", err
		}
		if err := a.requireAssignmentObjectAccess(ctx, query, id); err != nil {
			return true, nil, "assets.assignments.get", err
		}
		data, err := a.getAssignment(ctx, id)
		return true, okWithMessage(data, "ok"), "assets.assignments.get", err
	}
	return false, nil, "", nil
}

func (a *Adapter) getAssignment(ctx context.Context, id int64) (map[string]any, error) {
	item, err := a.queryRowMap(ctx, `
		SELECT assignment.id, assignment.assignment_no, assignment.asset_id,
		       asset.asset_code, asset.asset_name, asset.dept_code, asset.project_code,
		       assignment.action_type, assignment.target_type, assignment.target_ref,
		       assignment.status, assignment.workflow_instance_id, assignment.requested_by,
		...
		WHERE assignment.id=? AND asset.archived_at IS NULL`, id)
	if err != nil {
		return nil, err
	}
	if item == nil {
...
func (a *Adapter) listAssignments(ctx context.Context, query url.Values) (map[string]any, error) {
	...
	where := []string{"asset.archived_at IS NULL"}
	args := []any{}
	if access == "relation" {
	...
	if status := strings.TrimSpace(query.Get("status")); status != "" {
		where = append(where, "assignment.status=?")
		args = append(args, status)
	}
	if keyword := strings.TrimSpace(firstText(query.Get("search"), query.Get("keyword"))); keyword != "" {
		like := "%" + keyword + "%"
		where = append(where, "(assignment.assignment_no LIKE ? OR asset.asset_code LIKE ? OR asset.asset_name LIKE ? OR assignment.action_type LIKE ? OR assignment.target_ref LIKE ?)")
		args = append(args, like, like, like, like, like)
	}
	whereSQL := strings.Join(where, " AND ")
	var total int64
	var active int64
	if err := a.DB().QueryRowContext(ctx, `SELECT COUNT(*), COALESCE(SUM(assignment.status='active'), 0) FROM asset_assignments assignment INNER JOIN asset_items asset ON asset.id=assignment.asset_id WHERE `+whereSQL, args...).Scan(&total, &active); err != nil {
		return nil, err
	}
	items, err := a.queryMaps(ctx, `
		SELECT
	...
	return map[string]any{
		"summary": []summaryMetric{
			metric("操作记录", total, "分配/归还/释放", "primary"),
...
func (a *Adapter) handleAlertRoutes(ctx context.Context, method string, path string, query url.Values, body map[string]any) (bool, any, string, error) {
	...
	if rawID, ok := pathParamWithSuffix(path, "/v1/assets/alerts/", "/actions"); ok && method == http.MethodPost {
		...
		if err := a.requireAlertObjectAccess(ctx, query, id); err != nil {
			return true, nil, "assets.alerts.actions", err
		}
...
func (a *Adapter) listAlerts(ctx context.Context, query url.Values) (map[string]any, error) {
	...
	if status := strings.TrimSpace(query.Get("status")); status != "" {
		where = append(where, "alert.status=?")
		args = append(args, status)
	}
	if keyword := strings.TrimSpace(firstText(query.Get("search"), query.Get("keyword"))); keyword != "" {
		like := "%" + keyword + "%"
		where = append(where, "(alert.alert_no LIKE ? OR alert.title LIKE ? OR alert.alert_type LIKE ? OR alert.project_code LIKE ?)")
	...
	if err := a.DB().QueryRowContext(ctx, `SELECT COUNT(*), COALESCE(SUM(alert.status='pending'), 0), COALESCE(SUM(alert.status='acknowledged'), 0) FROM asset_alerts alert LEFT JOIN asset_items asset ON asset.id=alert.asset_id WHERE `+whereSQL, args...).Scan(&total, &pending, &acknowledged); err != nil {
		return nil, err
	}
	items, err := a.queryMaps(ctx, `
		SELECT
		  ...
		  alert.status,
		  alert.project_code,
		  DATE_FORMAT(alert.due_at, '%Y-%m-%d') AS due_at,
		  DATE_FORMAT(alert.triggered_at, '%Y-%m-%d %H:%i:%s') AS triggered_at
	...
	return map[string]any{
		"summary": []summaryMetric{
			metric("预警总数", total, "数据库记录", "primary"),
