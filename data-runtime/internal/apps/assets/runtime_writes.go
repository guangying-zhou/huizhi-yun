...
func (a *Adapter) linkDocument(ctx context.Context, objectType string, objectID int64, body map[string]any, operatorUID string) error {
	...
	return linkDocumentWithSchema(ctx, a.DB(), objectType, objectID, body, operatorUID, hasArtifactType, hasSourceContext)
}
...
type assetDocumentExecer interface {
	ExecContext(context.Context, string, ...any) (sql.Result, error)
}

func linkDocumentWithSchema(ctx context.Context, execer assetDocumentExecer, objectType string, objectID int64, body map[string]any, operatorUID string, hasArtifactType bool, hasSourceContext bool) error {
...
func (a *Adapter) createPurchaseOrder(ctx context.Context, body map[string]any, operatorUID string) (int64, error) {
	...
	status, err := purchaseOrderCreateStatus(body)
	if err != nil {
		return 0, err
	}
	if err := rejectPurchaseOrderWorkflowInstanceInOrdinaryWrite(body); err != nil {
		return 0, err
	}
	result, err := a.withTx(ctx, func(tx *sql.Tx) (any, error) {
		insert, err := tx.ExecContext(ctx, `
...
func (a *Adapter) updatePurchaseOrder(ctx context.Context, id int64, body map[string]any, operatorUID string) error {
	...
	if err := rejectPurchaseOrderControlFieldsInOrdinaryWrite(body); err != nil {
		return err
	}
	_, err := a.DB().ExecContext(ctx, `
...
func purchaseOrderCreateStatus(body map[string]any) (string, error) {
	status := strings.ToLower(strings.TrimSpace(bodyText(body, "status")))
	if status == "" || status == "draft" {
		return "draft", nil
	}
	return "", httperror.New(http.StatusForbidden, "asset_purchase_order_status_requires_workflow", "Purchase order status transitions must use submit, Workflow sync, or receipt actions")
}

func rejectPurchaseOrderControlFieldsInOrdinaryWrite(body map[string]any) error {
	if _, ok := body["status"]; ok && strings.TrimSpace(bodyText(body, "status")) != "" {
		return httperror.New(http.StatusForbidden, "asset_purchase_order_status_requires_workflow", "Purchase order status transitions must use submit, Workflow sync, or receipt actions")
	}
	return rejectPurchaseOrderWorkflowInstanceInOrdinaryWrite(body)
}

func rejectPurchaseOrderWorkflowInstanceInOrdinaryWrite(body map[string]any) error {
	for _, key := range []string{"workflow_instance_id", "workflowInstanceId", "instance_no", "instanceNo"} {
		if _, ok := body[key]; ok && strings.TrimSpace(bodyText(body, key)) != "" {
			return httperror.New(http.StatusForbidden, "asset_purchase_order_workflow_requires_submit", "Purchase order Workflow instance must be bound through submit action")
		}
	}
	return nil
}
...
func (a *Adapter) createAssignment(ctx context.Context, query url.Values, body map[string]any, operatorUID string) (int64, error) {
	assetID, err := requireIDBody(body, "asset_id", "缺少 asset_id")
	if err != nil {
		return 0, err
	}
	status, err := assignmentCreateStatus(body)
	if err != nil {
		return 0, err
	}
	result, err := a.withTx(ctx, func(tx *sql.Tx) (any, error) {
		asset := assignmentAssetRecord{}
		if err := tx.QueryRowContext(ctx, `
			SELECT id, asset_category, status, COALESCE(owner_uid, ''), COALESCE(custodian_uid, ''),
			       COALESCE(user_uid, ''), COALESCE(dept_code, ''), COALESCE(project_code, '')
			FROM asset_items
			WHERE id = ? AND archived_at IS NULL
			LIMIT 1`, assetID).Scan(
			&asset.ID,
			&asset.Category,
			&asset.Status,
			&asset.OwnerUID,
		...
		if err := authorizeAssignmentCreate(query, body, asset); err != nil {
			return nil, err
		}
...
func assignmentCreateStatus(body map[string]any) (string, error) {
	status := strings.ToLower(strings.TrimSpace(bodyText(body, "status")))
	if status == "" {
		return "pending", nil
	}
	...
	case "pending":
		return status, nil
	case "active", "returned", "released", "completed", "cancelled", "canceled", "approved", "rejected":
		return "", httperror.New(http.StatusForbidden, "asset_assignment_status_requires_workflow", "Asset assignment status transitions must use Workflow sync")
	default:
		return "", httperror.New(http.StatusBadRequest, "invalid_assignment_status", "不支持的资产操作状态")
	}
}
