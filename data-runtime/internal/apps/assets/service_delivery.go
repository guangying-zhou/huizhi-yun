...
func (a *Adapter) handleServiceDeliveryRoutes(ctx context.Context, method string, path string, query url.Values, body map[string]any) (bool, any, string, error) {
	...
	if method == http.MethodPost {
		if deliveryCode, ok := pathParamWithSuffix(path, "/v1/assets/service/deliveries/", "/documents"); ok {
			...
			data, err := a.linkServiceDeliveryDocument(ctx, deliveryCode, body)
...
func (a *Adapter) linkDeliveryDocument(ctx context.Context, deliveryCode string, body map[string]any, operatorUID string) (map[string]any, error) {
	if err := a.ensureDeliveryDocumentSchema(ctx); err != nil {
		return nil, err
	}
	result, err := a.withTx(ctx, func(tx *sql.Tx) (any, error) {
		return a.linkDeliveryDocumentTx(ctx, tx, deliveryCode, body, operatorUID)
	})
	if err != nil {
		return nil, err
	}
...
func (a *Adapter) linkServiceDeliveryDocument(ctx context.Context, deliveryCode string, body map[string]any) (map[string]any, error) {
	receiptInput, command, err := integrationoperation.ReceiptCommandFromBody(
		body,
		"assets",
		"altoc.ops-knowledge.assets-link.v1",
		"assets:write",
	)
	if err != nil {
	...
	if firstText(bodyText(command, "deliveryCode"), bodyText(command, "delivery_code")) != strings.TrimSpace(deliveryCode) {
		return nil, httperror.New(http.StatusConflict, "service_command_path_mismatch", "service command delivery does not match the target path")
	}
	...
	command["sourceBizType"] = "service_ticket"
	command["sourceBizCode"] = firstText(bodyText(command, "ticketCode"), bodyText(command, "ticket_code"))
	if err := a.ensureDeliveryDocumentSchema(ctx); err != nil {
		return nil, err
	}
	...
	executed, err := repository.Execute(ctx, receiptInput, func(ctx context.Context, tx *sql.Tx, _ json.RawMessage) (integrationoperation.ReceiptBusinessResult, error) {
		result, err := a.linkDeliveryDocumentTx(ctx, tx, deliveryCode, command, actorFromRequest(nil, command))
		if err != nil {
			return integrationoperation.ReceiptBusinessResult{}, err
		}
		documentUUID := firstText(bodyText(command, "documentUuid"), bodyText(command, "document_uuid"))
...
func (a *Adapter) linkDeliveryDocumentTx(ctx context.Context, tx *sql.Tx, deliveryCode string, body map[string]any, operatorUID string) (map[string]any, error) {
	...
	delivery, err := assetsTxQueryOneMap(ctx, tx, `
		SELECT id, customer_code, contract_code, project_code
		FROM asset_delivery_views
		WHERE delivery_code = ?
		LIMIT 1
		FOR UPDATE`, deliveryCode)
	if err != nil {
		return nil, err
	}
	if err != nil {
	...
	if artifactType == "ops_knowledge" {
		if err := validateOpsKnowledgeAssetEnvironmentTx(ctx, tx, delivery, body); err != nil {
			return nil, err
		}
	}
	...
	existing, err := assetsTxQueryOneMap(ctx, tx, `
			SELECT id
			FROM asset_documents
			WHERE object_type = 'delivery_view'
			  AND object_id = ?
			  AND document_id = ?
			LIMIT 1
			FOR UPDATE`, deliveryID, documentID)
	if err != nil {
		return nil, err
	}
	if err := linkDocumentWithSchema(ctx, tx, "delivery_view", deliveryID, linkBody, operatorUID, true, true); err != nil {
		return nil, err
	}
	if existing == nil {
		if _, err := tx.ExecContext(ctx, `
	...
	return map[string]any{
		...
		"idempotent":    existing != nil,
	}, nil
}

func (a *Adapter) ensureDeliveryDocumentSchema(ctx context.Context) error {
	...
	if !hasArtifactType || !hasSourceContext {
		return httperror.New(http.StatusServiceUnavailable, "delivery_document_schema_unavailable", "asset_documents artifact_type and source_context columns are required")
	}
	return nil
}

func assetsServiceCommandReceiptError(err error) error {
	switch {
...
func validateOpsKnowledgeAssetEnvironmentTx(ctx context.Context, tx *sql.Tx, delivery map[string]any, body map[string]any) error {
	deliveryAssetCode := firstText(bodyText(body, "deliveryAssetCode"), bodyText(body, "delivery_asset_code"))
	...
	if deliveryAssetCode == "" || environmentCode == "" {
		return httperror.New(http.StatusBadRequest, "ops_knowledge_asset_environment_required", "deliveryAssetCode and environmentCode are required for ops knowledge")
	}
	where := []string{
		"cda.delivery_asset_code = ?",
		"env.environment_code = ?",
		"cda.deleted_at IS NULL",
		"rel.deleted_at IS NULL",
		"rel.status = 'active'",
	}
	args := []any{deliveryAssetCode, environmentCode}
	for _, field := range []struct {
		column string
		value  string
	}{
		{"cda.customer_code", cleanAnyString(delivery["customer_code"])},
		{"cda.contract_code", cleanAnyString(delivery["contract_code"])},
		{"cda.project_code", cleanAnyString(delivery["project_code"])},
	} {
		if field.value != "" {
	...
	matched, err := assetsTxQueryOneMap(ctx, tx, `
		SELECT cda.id
		FROM customer_delivery_assets cda
		INNER JOIN customer_delivery_asset_environment_rel rel ON rel.delivery_asset_id = cda.id
	...
	if matched == nil {
		return httperror.New(http.StatusConflict, "ops_knowledge_asset_environment_mismatch", "delivery asset and environment do not belong to the delivery context")
	}
	return nil
}
...
func (a *Adapter) serviceDeliveryPackage(ctx context.Context, query url.Values) (map[string]any, error) {
	...
	if customerCode == "" {
		return nil, httperror.New(http.StatusBadRequest, "customer_code_required", "customer_code is required")
	}
	hasArtifactType, err := a.tableColumnExists(ctx, "asset_documents", "artifact_type")
	if err != nil {
		return nil, err
	...
	if !hasArtifactType || !hasSourceContext {
		return nil, httperror.New(http.StatusServiceUnavailable, "delivery_document_schema_unavailable", "asset_documents artifact_type and source_context columns are required")
	}
	...
	formalAssets, err := a.serviceDeliveryFormalAssets(ctx, customerCode, firstText(query.Get("contract_code"), query.Get("contractCode")), firstText(query.Get("project_code"), query.Get("projectCode")))
	if err != nil {
		return nil, err
	}
	return map[string]any{
...
func (a *Adapter) serviceDeliveryFormalAssets(ctx context.Context, customerCode string, contractCode string, projectCode string) ([]map[string]any, error) {
	where := []string{"customer_code = ?", "deleted_at IS NULL"}
	...
	for _, asset := range assets {
		environments, err := a.queryMaps(ctx, `
			...
			ORDER BY rel.is_primary DESC, rel.id ASC`, asset["id"])
		if err != nil {
			return nil, err
		}
...
func deliveryDocumentSourceContext(deliveryCode string, delivery map[string]any, body map[string]any, documentID string, artifactType string) map[string]any {
	...
	put("customer_code", firstText(cleanAnyString(delivery["customer_code"]), bodyText(body, "customerCode"), bodyText(body, "customer_code")))
	put("contract_code", firstText(cleanAnyString(delivery["contract_code"]), bodyText(body, "contractCode"), bodyText(body, "contract_code")))
	put("project_code", firstText(cleanAnyString(delivery["project_code"]), bodyText(body, "projectCode"), bodyText(body, "project_code")))
	put("delivery_asset_code", firstText(bodyText(body, "deliveryAssetCode"), bodyText(body, "delivery_asset_code")))
	put("environment_code", firstText(bodyText(body, "environmentCode"), bodyText(body, "environment_code")))
