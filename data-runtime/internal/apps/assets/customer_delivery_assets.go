...
func (a *Adapter) activateCustomerDeliveryAsset(ctx context.Context, assetCode string, body map[string]any, operatorUID string) (map[string]any, error) {
	...
	result, err := a.withTx(ctx, func(tx *sql.Tx) (any, error) {
		...
		updated, err := a.customerDeliveryAssetByIDTx(ctx, tx, asInt(asset["id"]))
		if err != nil {
			return nil, err
		}
		operation, err := a.enqueueAltocDeliveryAssetStatusOperationTx(ctx, tx, updated, body, operatorUID)
		if err != nil {
			return nil, err
		}
		updated["altocStatusSyncOperation"] = operation
		return updated, nil
	})
	if err != nil {
		return nil, err
