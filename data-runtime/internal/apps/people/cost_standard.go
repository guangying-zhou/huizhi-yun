...
func (a *Adapter) generateCostSnapshots(ctx context.Context, query url.Values, body map[string]any) (map[string]any, error) {
	...
	operator := peopleRuntimeActorFromRequest(query, body, "operator_uid", "operatorUid", "currentUser", "current_user", "created_by", "createdBy", "updated_by", "updatedBy")
	if operator == "" {
	...
	for _, employee := range employees {
		if employeeUID := cleanAnyString(employee["employee_uid"]); employeeUID != "" {
			employeeUIDs = append(employeeUIDs, employeeUID)
		}
	}
	assignments, err := a.effectivePrimaryAssignmentsByEmployee(ctx, employeeUIDs, monthEnd)
	if err != nil {
		return nil, err
	}
	...
	for _, employee := range employees {
		...
		assignment, hasAssignment := assignments[employeeUID]
		if !hasAssignment {
			skipped = append(skipped, map[string]any{
				"employee_uid": employeeUID,
				"reason":       "missing_effective_primary_assignment",
				"as_of_date":   monthEnd,
			})
			continue
		}
		...
		refs := map[string]any{
			...
			"assignment": map[string]any{
				"assignment_code": cleanAnyString(assignment["assignment_code"]),
		...
		`,
			...
			nullableString(cleanAnyString(assignment["assignment_code"])),
			nullableString(cleanAnyString(resolved["dept_code"])),
			nullableString(cleanAnyString(resolved["dept_name"])),
			nullableString(cleanAnyString(resolved["position_code"])),
			nullableString(cleanAnyString(resolved["position_name"])),
			nullableString(cleanAnyString(resolved["rank_code"])),
			nullableString(cleanAnyString(resolved["rank_name"])),
			sourceBizType,
			sourceBizID,
...
func (a *Adapter) resolveStandardCosts(ctx context.Context, query url.Values) (map[string]any, error) {
	...
	assignments, err := a.effectivePrimaryAssignmentsByEmployee(ctx, employeeUIDs, effectiveDate)
	if err != nil {
		return nil, err
	}
	rates, err := a.queryMaps(ctx, `
...
func standardCostRateMatches(employee map[string]any, rate map[string]any) bool {
	...
	return optionalEqual(rate["employment_type"], employee["employment_type"]) &&
