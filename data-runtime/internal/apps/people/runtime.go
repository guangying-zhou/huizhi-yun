...
func (a *Adapter) dashboardOverview(ctx context.Context, query url.Values) (map[string]any, error) {
	activeEmployeeWhere, activeEmployeeArgs, err := scopedEmployeeWhere(query, "people_employees.employee_uid", []string{"archived_at IS NULL", "employment_status = 'active'"}, nil)
	if err != nil {
		return nil, err
	}
	activeEmployees, err := a.count(ctx, "people_employees", activeEmployeeWhere, activeEmployeeArgs...)
	if err != nil {
		return nil, err
	}

	assignmentWhere, assignmentArgs, err := scopedEmployeeWhere(query, "people_assignments.employee_uid", []string{
		"is_primary = 1",
	...
	currentAssignments, err := a.count(ctx, "people_assignments", assignmentWhere, assignmentArgs...)
	if err != nil {
		return nil, err
	}

	cycleWhere, cycleArgs, err := dashboardPerformanceCycleWhere(query)
	if err != nil {
		return nil, err
	}
	activeCycles, err := a.count(ctx, "people_performance_cycles", cycleWhere, cycleArgs...)
	if err != nil {
		return nil, err
	}

	currentCostMetric := map[string]any{"label": "本月实际成本", "value": "未授权", "hint": "需要职级成本查看权限", "color": "neutral"}
	if hasStandardCostReadAccess(query) {
		currentCost, err := a.currentMonthActualCost(ctx, query)
		if err != nil {
			return nil, err
		}
		currentCostMetric = map[string]any{"label": "本月实际成本", "value": currentCost, "hint": "月度成本快照", "color": "secondary"}
	}
	...
	redactEmployeeSensitiveCostFieldsInResponse(query, assignments)

	return map[string]any{
		"metrics": []map[string]any{
			{"label": "在职员工", "value": activeEmployees, "hint": "员工事实", "color": "success"},
			{"label": "当前任职", "value": currentAssignments, "hint": "当前有效主任职", "color": "primary"},
			{"label": "进行中周期", "value": activeCycles, "hint": "草稿 / 采集中 / 计算中", "color": "warning"},
			currentCostMetric,
		},
...
func scopedEmployeeWhere(query url.Values, employeeColumn string, baseWhere []string, baseArgs []any) (string, []any, error) {
	where := append([]string{}, baseWhere...)
	args := append([]any{}, baseArgs...)

	scopeWhere, scopeArgs, err := employeeScopedRowClause(query, employeeColumn)
	if err != nil {
		return "", nil, err
	}
	if scopeWhere != "" {
...
func dashboardPerformanceCycleWhere(query url.Values) (string, []any, error) {
	where := []string{"status IN ('draft', 'collecting', 'calculating')"}
	args := []any{}
...
func (a *Adapter) count(ctx context.Context, table string, where string, args ...any) (int64, error) {
...
func (a *Adapter) currentMonthActualCost(ctx context.Context, query url.Values) (float64, error) {
	if !hasStandardCostReadAccess(query) {
		return 0, nil
	}
	...
	where, args, err := scopedEmployeeWhere(
		query,
		"people_cost_snapshots.employee_uid",
		[]string{"period_month = DATE_FORMAT(CURRENT_DATE(), '%Y-%m')"},
		nil,
	)
	if err != nil {
		return 0, err
...
func (a *Adapter) employeeProfile(ctx context.Context, employeeUID string, query url.Values) (map[string]any, error) {
	...
	redactEmployeeSensitiveCostFields(query, employee)
	if !canReadEmployeeProfileAssignments(query, employee) {
		assignments = []map[string]any{}
	}
	redactEmployeeSensitiveCostFieldsInResponse(query, assignments)
	if !canReadEmployeeProfileCostSnapshots(query, employee) {
		costSnapshots = []map[string]any{}
	}
	if !canReadEmployeeProfilePerformanceCycles(query, employee) {
		contributions = []map[string]any{}
		cycles = []map[string]any{}
	}
	if !canReadEmployeeProfileDocuments(query, employee) {
...
func (a *Adapter) performanceCycleDetail(ctx context.Context, cycleCode string, query url.Values) (map[string]any, error) {
	...
	redactEmployeeSensitiveCostFieldsInResponse(query, contributions)
	summary, err := a.queryRowMap(ctx, `
		SELECT
			...
			ROUND(AVG(pcs.contribution_score), 1) AS avg_score
		FROM people_contribution_snapshots pcs
		WHERE `+contributionWhereSQL+`
	`, contributionArgs...)
	if err != nil {
...
func (a *Adapter) projectPeopleCosts(ctx context.Context, projectCode string, query url.Values) (map[string]any, error) {
	...
	items, err := a.queryMaps(ctx, `
		SELECT
			...
			ROUND(AVG(pcs.contribution_score), 1) AS contribution_score
		FROM people_contribution_snapshots pcs
