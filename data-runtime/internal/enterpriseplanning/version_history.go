...
func (s *VersionService) ListScope(ctx context.Context, code, uid string, permit pc.AuthorizationPermit, versionID int64, q pc.PlanningPageQuery) (pc.ProductVersionScopePage, error) {
	var out pc.ProductVersionScopePage
	if s == nil || s.registry == nil {
		return out, e.ErrBindingNotFound
	}
	reader := s.writer
	reader.Operation = e.Read
	resolved, err := s.registry.Resolve(reader)
	if err != nil {
		return out, err
	}
	return pc.ListProductVersionScope(ctx, resolved.DB, code, uid, permit, versionID, q)
}

func (s *VersionService) ListScopeHistory(ctx context.Context, code, uid string, permit pc.AuthorizationPermit, versionID, scopeID int64, page, pageSize int) (pc.ProductVersionScopeHistoryPage, error) {
	var out pc.ProductVersionScopeHistoryPage
	if s == nil || s.registry == nil {
		return out, e.ErrBindingNotFound
	}
	reader := s.writer
	...
	return pc.ListProductVersionScopeHistory(ctx, resolved.DB, code, uid, permit, versionID, scopeID, page, pageSize)
}
