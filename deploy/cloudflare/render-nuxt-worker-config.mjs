...
function positiveIntegerValue(name, fallback = '') {
...
function cronTriggerConfig(prefix, options) {
  const configured = value(`HZY_${prefix}_CRON_TRIGGERS`, value(`HZY_${prefix}_CRONS`))
  const defaultCrons = typeof options.crons === 'function' ? options.crons() : (options.crons || [])
  const crons = (configured
    ? configured.split(/[;|]/)
    : defaultCrons
  )
...
export function renderNuxtWorkerConfig(options) {
  ...
  const cpuMs = positiveIntegerValue(`HZY_${prefix}_CPU_MS`, value('HZY_WORKER_CPU_MS'))
