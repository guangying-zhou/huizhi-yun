...
export function generateReleaseArtifactDescriptor({ repo, commit }) {
  ...
  return {
    ...
    sourceCommit: commit,
    // Pin the declared shared source/build inputs, including Console and the route
    // projection imported from outside the Gateway directory. This descriptor
    // is source-only: it does not claim built binaries or active deployments.
    sources: Object.fromEntries(releaseSourcePaths.map(path => [path, { commit, treeSha1: treeId(root, commit, path) }])),
    buildFiles: Object.fromEntries(releaseBuildFiles.map(path => [path, { blobSha1: treeId(root, commit, path) }])),
    enterpriseHost: {
      commit,
