...
try {
  ...
  const registry = { domains: { [new URL(origin).hostname]: {
    ...
    apps: Object.fromEntries(['console', 'aims', 'assets', 'finance'].map(app => [app,
  ...
  const config = {
    ...
    services: ['console', 'aims', 'assets', 'finance'].map(app => ({ binding: `HZY_${app.toUpperCase()}_SERVICE`, service: `hzy-test-${app}` })),
