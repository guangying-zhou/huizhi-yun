...
export function generateReadinessPolicies(repoRoot = root) {
  ...
  const consolePolicy = externalPolicy(read(repoRoot, paths.directoryTransport), /audience:\s*'([^']+)'/, /scope:\s*'(console:directory-users:read)'/, json(repoRoot, paths.consoleManifest))
  if (!read(repoRoot, paths.directoryRoute).includes(`'${consolePolicy.capabilities[0]}'`)) fail('READINESS_POLICY_DIRECTORY_CONTRACT_MISMATCH')
