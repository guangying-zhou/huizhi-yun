...
function grantScope(capability,audience) {
  return {
    source:'enterprise-test-pilot',tenantCode:p.tenantCode,deploymentCode:p.deploymentCode,audience,semanticScope:capability,
    ...(['integration_config:view','credential_vault:resolve'].includes(capability)?{integrationCodes:['oss.default']}:{}),
    ...(capability==='credential_vault:resolve'?{usageTypes:['integration']}: {})
  }
}
function restrictionsMatch(value,capability) {
  const scope=typeof value==='string'?JSON.parse(value):value
  if(!['integration_config:view','credential_vault:resolve'].includes(capability))return true
  return Array.isArray(scope?.integrationCodes)&&scope.integrationCodes.length===1&&scope.integrationCodes[0]==='oss.default'
    &&(capability!=='credential_vault:resolve'||(Array.isArray(scope?.usageTypes)&&scope.usageTypes.length===1&&scope.usageTypes[0]==='integration'))
}
export async function verifyRegistration(db) {
  ...
  const grantActive = ({capability: cap, audience}) => { const i=cap.lastIndexOf(':'); return grants.some(g=>g.resource_code===cap.slice(0,i)&&g.action===cap.slice(i+1)&&g.status==='active'&&scopeMatches(g.scope_json)&&semanticMatches(g.scope_json,cap,audience)&&restrictionsMatch(g.scope_json,cap)) }
  ...
  try {
    ...
    for (const {capability: cap, audience} of registrationGrants) {
      ...
      for(const row of existing) {
        ...
        if(!semanticMatches(scope,cap,audience)||!restrictionsMatch(scope,cap))await db.query("UPDATE service_client_grants SET scope_json=? WHERE service_client_id=? AND resource_code=? AND action=? AND status='active'",[JSON.stringify(grantScope(cap,audience)),service.id,resource,action])
      }
      if (!existing.length) await db.query("INSERT INTO service_client_grants(service_client_id,resource_code,action,scope_json,status) VALUES(?,?,?,?,'active')",[service.id,resource,action,JSON.stringify(grantScope(cap,audience))])
    }
    await db.commit()
