...
| `GET` | `/codocs/documents/:uuid/content` | 已验证当前用户、项目 member/leader/creator/scoped-admin 与精确 Aims `project_documents.codocs_uuid` / 文档型 deliverable 关联后，以固定 `codocs:project-document:content:read` one-shot service-command 读取正文；Codocs 必须再通过其 owner/share/relation ACL，项目成员关系不单独授予正文读取。 |
...
`content`、`section` 与 `source-sections` 只复用上述单 UUID 读取命令；Aims 不再调用 `codocs:documents:read` 的通用正文/摘要接口。命令固定 actor、projectCode、UUID、`content:read`、source/target tenant/deployment，并用刚签发的短时 `aud=codocs` service token 对 canonical method/path/envelope/hash 做 60 秒 HMAC；客户端不可传入角色、project scope、OSS path 或目标 capability。Codocs 先验 token 与首跳 HMAC，才会用自己的 runtime bearer 重签第二跳。Codocs 返回的内容 DTO 不包含 OSS path、签名 URL、策略或 grant。
