...
>
> **当前事实边界（2026-07-11 核对）**：资源能力的当前技术事实源是 Platform release/import 的应用 manifest；审批动作事实源是 Workflow `flow_action_defs`。同步调用经 Foundation 取得短期 Console service token，不使用 Account、静态 API key/secret 或业务应用直连 Platform。本文遗留迁移叙述只作设计背景；以 `docs/MODULE_CONTRACTS.md`、`aims/CLAUDE.md` 与当前代码为准。
...
调用方通过 Foundation `requestServiceAccessToken({ audience: 'workflow',
scope: 'workflow:action_defs:sync' })` 获取短期 Console service token；不得为该
同步链路新增静态 API key/secret 或复用 Account 凭据。
...
- `resources -> Platform release/import`
...
- Platform manifest / policy bundle 是资源与授权治理事实源，Console 是运行时授权消费边界
- Workflow 是审批动作中心
