...
import { projectStatusConfig, projectCategoryConfig, selectableProjectCategoryOptions, getProjectCategoryLabel } from '~/config/project'
...
function openEditModal() {
  ...
  editForm.value = {
    ...
    defaultCategory: portfolio.value.defaultCategory ?? undefined,
        ...
        <div class="shrink-0 border-b border-default px-6 py-4">
          ...
          <div v-else-if="portfolio" class="flex items-center justify-between">
            <div class="flex items-center gap-3 min-w-0">
              ...
              <UBadge
                v-if="portfolio.defaultCategory"
                color="primary"
                variant="soft"
                size="sm"
              >
                {{ getProjectCategoryLabel(portfolio.defaultCategory) }}
              </UBadge>
            </div>
  ...
  <UModal v-model:open="showEditModal" :ui="{ content: 'sm:max-w-lg' }">
    ...
    <template #body>
      <div class="p-4 space-y-4">
        ...
        >
          <USelectMenu
            v-model="editForm.defaultCategory"
            :items="selectableProjectCategoryOptions"
            value-key="value"
            placeholder="不预设"
            class="w-full"
            :disabled="portfolio?.isSystem === true"
          />
          <p
            v-if="portfolio?.isSystem"
