...
interface RoutineProjectBatchResponse {
  code: number
  message?: string
  data: {
    year: number
...
const routineBatchCreating = ref(false)
...
const routineBatchModalOpen = ref(false)
const routineBatchYear = ref(new Date().getFullYear())
...
const canDeleteProjects = computed(() => hasPermission('admin', 'admin'))
...
const canBatchCreateRoutineProjects = computed(() => hasPermission('admin', 'admin'))
const canSubmitRoutineBatch = computed(() => {
  const year = Number(routineBatchYear.value)
...
async function deleteProject() {
  if (!canDeleteProjects.value || !deletingProject.value || !canConfirmDelete.value) return
...
function openRoutineBatchModal() {
  routineBatchYear.value = new Date().getFullYear()
  routineBatchModalOpen.value = true
}
...
async function batchCreateRoutineProjects() {
  if (routineBatchCreating.value || !canBatchCreateRoutineProjects.value || !canSubmitRoutineBatch.value) return

  routineBatchCreating.value = true
  try {
    const response = await $fetch<RoutineProjectBatchResponse>(
      '/api/v1/admin/projects/batch-create-routine',
      {
        method: 'POST',
        body: { year: Number(routineBatchYear.value) }
      }
    )
    if (response.code !== 0) {
    ...
    toast.add({
      title: `${response.data.year} 年部门事务项目处理完成`,
      description: `创建 ${summary.created} 个，已存在跳过 ${summary.existing} 个，未配置部门负责人跳过 ${summary.missingManager} 个。`,
      color: summary.missingManager > 0 ? 'warning' : 'success'
    })
    routineBatchModalOpen.value = false
    await Promise.all([
      loadProjects(),
      portfolioStore.fetchPortfolios()
    ])
    ...
    routineBatchCreating.value = false
  }
}
...
<template>
  <Teleport to="#aims-layout-header-actions">
    <UButton
      v-if="canBatchCreateRoutineProjects"
      icon="i-lucide-layers-plus"
      label="批量创建部门事务项目"
      color="neutral"
      variant="outline"
      aria-label="批量创建部门事务项目"
      title="批量创建部门事务项目"
      :ui="{ label: 'hidden sm:inline' }"
      @click="openRoutineBatchModal"
    />
    <UButton
      v-if="canCreatePortfolios"
      ...
      title="新建项目集"
      :ui="{ label: 'hidden sm:inline' }"
      @click="createPortfolioModalOpen = true"
    />
    <UButton
  ...
  <UDashboardPanel id="admin-projects" :ui="{ body: 'flex flex-col flex-1 min-h-0 p-0 overflow-hidden' }">
    <template #body>
      <div class="flex min-h-0 flex-1 flex-col">
        ...
        <UModal v-if="canDeleteProjects" v-model:open="deleteModalOpen" :ui="{ content: 'sm:max-w-xl' }">
          <template #header>
            <div>
        ...
        <UModal
          v-if="canBatchCreateRoutineProjects"
          v-model:open="routineBatchModalOpen"
          :dismissible="!routineBatchCreating"
          ...
          <template #body>
            <div class="space-y-4">
              <UAlert
                ...
                icon="i-lucide-info"
                title="创建规则"
                description="项目名称为“部门名称 + 年度”，负责人取部门负责人，成员同步当前有效部门成员。已有项目自动跳过；没有部门负责人的部门不会创建。"
              />
              <UFormField
                label="年度"
                description="将作为项目周期，开始日期为 1 月 1 日，结束日期为 12 月 31 日。"
                required
              >
                <UInput
                  v-model.number="routineBatchYear"
                  class="w-full"
                  type="number"
                  :min="2000"
                  :max="2100"
                  ...
                  placeholder="例如：2026"
                  autofocus
                  @keydown.enter="batchCreateRoutineProjects"
                />
              </UFormField>
              <p v-if="!canSubmitRoutineBatch" class="text-sm text-error">
                请输入 2000 至 2100 之间的完整年度。
              </p>
            </div>
          </template>
          <template #footer>
            <div class="flex w-full justify-end gap-2">
              <UButton
                ...
                color="neutral"
                variant="ghost"
                :disabled="routineBatchCreating"
                @click="routineBatchModalOpen = false"
              />
              <UButton
                label="开始创建"
                icon="i-lucide-layers-plus"
                color="primary"
                :disabled="routineBatchCreating || !canSubmitRoutineBatch"
                :loading="routineBatchCreating"
                @click="batchCreateRoutineProjects"
              />
            </div>
          </template>
        </UModal>
  ...
  <PortfolioCreateModal
    v-if="canCreatePortfolios"
    v-model:open="createPortfolioModalOpen"
    @created="handlePortfolioCreated"
  />
  <ProjectCreateModal
    v-if="canCreateProjects"
