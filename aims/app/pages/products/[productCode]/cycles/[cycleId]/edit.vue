...
definePageMeta({ layoutHeader: true, layoutHeaderTitle: '修改周期草案', layoutHeaderProjectSwitcher: false })
const route = useRoute()
const code = computed(() => String(route.params.productCode || ''))
const id = computed(() => String(route.params.cycleId || ''))
const { data, status, error, refresh } = await useAsyncData(() => `cycle-edit:${code.value}:${id.value}`, async () => {
...
const alert = useApiErrorAlert(error, { fallbackTitle: '周期编辑数据加载失败' })
const toast = useToast()
async function back() {
  await navigateTo(`/products/${encodeURIComponent(code.value)}/cycles`)
}
async function saved() {
  toast.add({ title: '周期草案已更新', color: 'success' })
  await back()
}
</script>
