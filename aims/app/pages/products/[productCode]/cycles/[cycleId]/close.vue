...
definePageMeta({ layoutHeader: true, layoutHeaderTitle: '关闭规划周期', layoutHeaderProjectSwitcher: false })
const route = useRoute()
const code = computed(() => String(route.params.productCode || ''))
const id = computed(() => String(route.params.cycleId || ''))
const { data, status, error, refresh } = await useAsyncData(() => `cycle-close:${code.value}:${id.value}`, async () => {
  ...
  if (permission.code !== 0 || permission.data?.product_code !== code.value || !permission.data.prioritize || permission.data.status !== 'active') throw new Error('当前产品不可关闭周期或您没有周期决策权限')
...
const alert = useApiErrorAlert(error, { fallbackTitle: '周期关闭数据加载失败' })
const toast = useToast()
async function back() {
  await navigateTo(`/products/${encodeURIComponent(code.value)}/cycles`)
}
async function saved() {
  toast.add({ title: '规划周期已关闭', color: 'success' })
  await back()
}
</script>
