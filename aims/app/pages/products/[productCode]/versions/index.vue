...
<template>
  <div class="mx-auto min-w-0 max-w-5xl space-y-4 p-4 sm:p-6">
    ...
    <form class="flex flex-wrap items-end gap-3" @submit.prevent="flush">
      <UFormField label="搜索版本" class="w-full min-w-0 sm:w-auto sm:flex-1">
    ...
    <template v-if="status === 'success' && data">
      ...
      >
        ...
        <div class="flex flex-wrap items-center gap-2">
          <NuxtLink v-if="item.planning_mode === 'simple' || !hosted" :to="{ path: moduleUrl(`/products/${encodeURIComponent(code)}/versions/${item.id}/${item.planning_mode === 'simple' ? 'plan' : 'features'}`), query: versionQuery }" class="min-w-0 break-words font-medium text-primary hover:underline">
          ...
          <UBadge v-if="hosted && item.planning_mode === 'cycle'" color="neutral" variant="subtle">
            旧版范围暂不可查看
          </UBadge>
        </div>
