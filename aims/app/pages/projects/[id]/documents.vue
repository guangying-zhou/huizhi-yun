...
function ensureCanManageDocuments() {
  if (canManageDocuments.value) return true
  toast.add({ title: '仅项目经理可以执行该操作', color: 'warning' })
  return false
}

function canDeleteDocument(doc: DocumentNode | ProjectDocument | null | undefined) {
  if (!doc || doc.virtual) return false
  if (canManageDocuments.value) return true
  if (doc.type === 'folder') return false
  const uid = currentUid.value
  return Boolean(uid && doc.createdBy === uid)
}

function ensureCanDeleteDocument(doc: ProjectDocument | null | undefined) {
  if (canDeleteDocument(doc)) return true
  toast.add({ title: '仅项目经理或上传人可以删除项目文档', color: 'warning' })
  return false
}
...
const createSourceFileName = ref('')
...
function displayUserName(uid: string | null | undefined, empty = '-') {
  if (!uid) return empty
...
const activeDocumentTree = computed(() => buildTree(activeDocuments.value.map(withUploaderName)))
...
const searchResults = computed(() => {
  ...
  return documents.value
    ...
    .filter(doc =>
      ...
      || displayUploaderName(doc).toLowerCase().includes(keyword)
    )
    .map(withUploaderName)
})
...
function displayUploaderName(doc: ProjectDocument | DocumentNode | null | undefined) {
  const uid = String(doc?.createdBy || '').trim()
  return String(doc?.createdByName || '').trim()
    || (uid ? accountUserNameMap.value.get(uid) : '')
    || uid
    || '-'
}

function withUploaderName(doc: ProjectDocument): ProjectDocument {
  return {
    ...doc,
    createdByName: displayUploaderName(doc)
  }
}
...
async function openAccessModal(doc: ProjectDocument) {
  if (!ensureCanManageDocuments()) return
  accessDoc.value = doc
...
function openAccessModalForPreview() {
  ...
  if (previewDoc.value.virtual) {
    toast.add({ title: '任务交付文档沿用原文档访问控制', color: 'secondary' })
    return
  }
...
async function loadOtherDocumentPreview(doc: ProjectDocument) {
  resetOtherPreview()
...
async function submitCreate() {
  ...
  try {
    if (createKind.value === 'folder') {
      ...
      await createMarkdownDocument(createContent.value || undefined, createSourceFileName.value || undefined)
...
async function handleMarkdownUpload(event: Event) {
  ...
  try {
    ...
    createCategory.value = 'general'
    createSourceFileName.value = file.name
...
async function handleOtherUpload(event: Event) {
  ...
  toast.add({ title: '正在上传其他文档到文件柜，请稍候', color: 'secondary' })
  try {
...
function confirmDelete(id: number) {
  ...
  if (!ensureCanDeleteDocument(deleteTarget.value)) {
    deleteTarget.value = null
    return
  }
...
onMounted(async () => {
  setRefresh(loadDocuments)
  const projectRequest = !project.value || project.value.id !== projectId.value
            ...
            <div class="min-h-0 flex-1 overflow-y-auto p-3">
              <div class="space-y-2">
                ...
                >
                  ...
                  <span class="min-w-0 flex-1 truncate">其他格式文档</span>
          ...
          <main class="flex min-h-0 flex-col overflow-hidden">
            <div class="flex flex-wrap items-center justify-between gap-3 border-b border-default px-5 py-3">
              <div class="min-w-0">
                <div class="flex items-center gap-2">
                  ...
                  <h2 class="truncate text-base font-semibold text-highlighted">
                    {{ search ? '搜索结果' : (selectedLibrary === 'other' ? '其他格式文档' : activeTarget.label) }}
                  </h2>
                  <UBadge
                ...
                <div v-if="projectCode" class="mt-1 text-xs text-muted">
                  {{ selectedLibrary === 'other' ? '支持txt、docx、xlsx、pptx、pdf等' : '' }}
                </div>
              </div>
              ...
              <div class="flex items-center gap-2">
                ...
                <UButton
                  ...
                  :label="uploadingOther ? '上传中' : '上传其他格式文档'"
                  color="primary"
                  size="sm"
            ...
            <div class="min-h-0 flex-1 overflow-y-auto p-5">
              <UAlert
                ...
                variant="soft"
                class="mb-4"
                :title="uploadingMarkdown ? 'Markdown 文档正在上传' : '正在上传文档到项目文件柜'"
                ...
                >
                  ...
                  <div class="min-w-0 flex-1">
                    ...
                    <div class="mt-0.5 flex flex-wrap items-center gap-2 text-xs text-muted">
                      ...
                      <span>上传人：{{ displayUploaderName(doc) }}</span>
                      <span>{{ formatDate(doc.updatedAt) }}</span>
                    </div>
                  </div>
                  <UBadge color="neutral" variant="subtle" size="xs">
                    {{ displayUploaderName(doc) }}
                  </UBadge>
                  <UButton
                    v-if="canManageDocuments && !doc.virtual"
              ...
              <div v-else class="rounded-lg border border-default">
                ...
                <DocumentTree
                  ...
                  :loading="false"
                  :readonly="!canWriteDocuments && !canManageDocuments"
                  :can-create="canWriteDocuments"
                  :can-delete="canManageDocuments"
                  :can-delete-document="canDeleteDocument"
                  :can-manage-access="canManageDocuments"
    ...
    <template #header>
      <div class="flex min-w-0 flex-1 items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-1">
          <UButton
            v-if="canManageDocuments && previewDoc && !previewDoc.virtual"
            icon="i-lucide-shield-check"
            label="访问控制"
            color="neutral"
            variant="soft"
        ...
        >
          ...
          >
            <div class="flex shrink-0 items-center gap-3 border-b border-default px-3 py-2 text-sm">
              <UIcon :name="otherDocumentIcon(previewDoc)" class="size-4 text-primary" />
              <span class="min-w-0 flex-1 truncate text-highlighted">
                {{ otherPreviewInfo?.originalName || previewDoc.title }}
              </span>
              <span class="shrink-0 text-muted">
                {{ (otherPreviewInfo?.fileExt || fileExtension(previewDoc.repoFilePath || previewDoc.ossPath || previewDoc.title)).toUpperCase() }}
              </span>
              <UButton
          ...
          <div v-else class="space-y-2">
            ...
            >
              ...
              <div class="mt-0.5 text-xs text-muted">
                操作人：{{ displayUserName(item.actorUid, 'system') }}
              </div>
            </div>
          </div>
