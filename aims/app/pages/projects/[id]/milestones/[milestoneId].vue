...
const completionWorkflowForm = computed(() => completionRequest.value
  ? {
      completionRequestId: completionRequest.value.requestId,
      requestNo: completionRequest.value.requestNo,
      snapshotSha256: completionRequest.value.snapshotSha256,
      projectDirectorUid: completionRequest.value.projectDirectorUid,
      projectDirectorRevision: completionRequest.value.projectDirectorRevision,
...
usePageWorkflow({
  ...
  actions: computed(() => {
    ...
    const issues = computed(() => {
      ...
      const incompleteDeliverables = (milestone.value?.deliverables || [])
        .filter(item => item.required && !item.completed)
      if (incompleteDeliverables.length > 0) {
        list.push(`仍有 ${incompleteDeliverables.length} 项必交成果未审核通过`)
      }
      const incompleteDocumentQuality = (milestone.value?.deliverables || [])
        .filter(item =>
          item.required
          && item.deliverableType === 'document'
          && item.qualityStatus !== 'passed'
          && item.qualityStatus !== 'waived'
        )
      if (incompleteDocumentQuality.length > 0) {
        list.push(`仍有 ${incompleteDocumentQuality.length} 份必交文档未通过 QA 检查或获得豁免`)
      }
      return list
    })
    ...
    return [{
      ...
      formData: completionWorkflowForm,
      callbackUrl: computed(() => '/api/v1/service/workflow/callback'),
      async beforeSubmit() {
        ...
        }>(`/api/v1/milestones/${milestoneId.value}/completion-requests`, {
          method: 'POST',
          body: {}
        })
        completionRequest.value = response.data
        await nextTick()
      },
      async onSubmitted(payload) {
        const requestId = completionRequest.value?.requestId
        if (!requestId) throw new Error('里程碑完成申请未创建')
        await $fetch(`/api/v1/milestone-completion-requests/${requestId}/bind-workflow`, {
          method: 'POST',
          body: { workflowInstanceId: payload.instanceId }
        })
      },
      async onApproved() {
        toast.add({ title: '项目总监已批准，里程碑完成状态正在同步', color: 'success' })
        completionRequest.value = null
        await fetchDetail()
      },
      async onRejected() {
        toast.add({ title: '里程碑完成申请被驳回，修复后可重新申请', color: 'warning' })
        completionRequest.value = null
        await fetchDetail()
      }
    }]
              ...
              >
                ...
                <div class="flex flex-wrap gap-2">
                  ...
                  >
                    ...
                    <span
                      v-if="d.deliverableType === 'document'"
                      class="ml-1 text-xs"
                    >
                      · {{ d.qualityStatus === 'passed' ? 'QA通过' : d.qualityStatus === 'waived' ? '已豁免' : '待质量检查' }}
                    </span>
                  </UBadge>
                </div>
              </div>
