...
const canManageProjectLifecycle = computed(() => currentUserRole.value === 'manager')
const canManageAccessControl = computed(() =>
  currentUserRole.value === 'manager'
  || project.value?.currentUserIsProjectAdmin === true
)
const canManageProjectModules = computed(() =>
  currentUserRole.value === 'manager'
  || project.value?.currentUserIsProjectAdmin === true
)
...
const normalizedProjectModuleConfig = computed(() =>
  normalizeProjectModuleConfig(project.value?.moduleConfig, project.value?.category || 'custom_dev')
)
const moduleConfigDraft = reactive<Record<ProjectModuleKey, boolean>>(
  Object.fromEntries(projectModuleKeys.map(key => [key, false])) as Record<ProjectModuleKey, boolean>
)
const moduleConfigChanged = computed(() =>
  projectModuleKeys.some(key => moduleConfigDraft[key] !== normalizedProjectModuleConfig.value[key])
)

function resetModuleConfigDraft() {
  const normalized = normalizedProjectModuleConfig.value
  for (const key of projectModuleKeys) {
    moduleConfigDraft[key] = normalized[key]
  }
}

watch(normalizedProjectModuleConfig, resetModuleConfigDraft, { immediate: true })

async function saveModuleConfig() {
  if (!project.value || !canManageProjectModules.value) return
  moduleSaving.value = true
  try {
    await projectStore.updateProject(projectId.value, {
      moduleConfig: toPersistedProjectModuleConfig(moduleConfigDraft, project.value.category)
    })
    await projectStore.fetchProject(projectId.value)
    resetModuleConfigDraft()
    toast.add({ title: '项目模块已更新', color: 'success' })
  } catch (err) {
    toast.add({
      title: getErrorMessage(err, '保存项目模块失败'),
      color: 'error',
...
const confidentialityLevelConfig = computed(() => {
  const level = project.value?.confidentialityLevel || 'L1'
  return projectConfidentialityLevelConfig[level]
})
...
const accessControlPolicyReview = computed(() => {
  const confidentiality = securityForm.value.confidentialityLevel
  const effectiveLevel = effectiveSecurityLevel.value
  const tightened = effectiveLevel !== securityForm.value.securityLevel
  const rules: string[] = [
    `生效范围：${effectiveSecurityFormConfig.value.label}`,
    '项目负责人、创建人和项目成员始终可访问'
  ]

  if (effectiveLevel === 'whitelist') {
    rules.push('白名单用户可访问')
  }
  if (confidentiality === 'L2') {
    rules.push('L2 不允许公司范围放行')
  }
  if (confidentiality === 'L3') {
    rules.push('L3 不走部门或管理部门范围放行')
  }
  if (tightened) {
    rules.push(`保存时将从「${selectedSecurityFormConfig.value.label}」收紧为「${effectiveSecurityFormConfig.value.label}」`)
  }

  return {
    tightened,
    title: `${selectedConfidentialityFormConfig.value.label} / ${effectiveSecurityFormConfig.value.label}`,
    description: selectedConfidentialityFormConfig.value.description,
    rules,
    icon: tightened ? 'i-lucide-shield-alert' : selectedConfidentialityFormConfig.value.icon
  }
})
...
function buildWorkflowAction(
  ...
  const baseCanSubmit = overrides?.canSubmit ?? computed(() => !!project.value)
  return {
    actionCode,
    ...
    canSubmit: computed(() => canManageProjectLifecycle.value && baseCanSubmit.value),
...
const initiationIssues = computed(() => {
  const issues: string[] = []
  if (!projectRequiresInitiation(project.value)) return issues
  if (!proposal.value) issues.push('未关联项目立项书')
  const repositoryIssue = getProjectInitiationRepositoryIssue(project.value)
  if (repositoryIssue) issues.push(repositoryIssue)
...
const workflowActions = computed(() => {
  ...
  if (!status) return []
  if (!canManageProjectLifecycle.value) return []
  if (!projectRequiresInitiation(project.value) && (status === 'draft' || status === 'approval_pending')) return []
  switch (status) {
    case 'draft':
...
const { isReadonly: isWorkflowReadonly } = usePageWorkflow({
  ...
  resourceCode: 'projects',
  loadingTitle: '项目审批',
  initializing: workflowInitializing,
...
function openSecurityModal() {
  if (!canManageAccessControl.value) {
    toast.add({ title: '仅项目负责人、项目总监或系统管理员可以设置访问控制', color: 'warning' })
    return
  }
  securityForm.value = {
    securityLevel: project.value?.securityLevel || 'company',
    confidentialityLevel: project.value?.confidentialityLevel || 'L1',
...
async function handleSecuritySave() {
  ...
  try {
    ...
    await projectStore.updateProject(projectId.value, {
      ...
      confidentialityLevel: securityForm.value.confidentialityLevel,
      accessWhitelist: effectiveLevel === 'whitelist'
  ...
  <UDashboardPanel id="project-settings" :ui="{ root: 'relative flex flex-col min-w-0 h-full shrink-0', body: 'flex flex-col flex-1 min-h-0 p-0 overflow-hidden' }">
    <template #body>
      <div class="flex flex-col h-full min-h-0">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <UCard>
                ...
                <div class="space-y-3">
                  <div class="grid grid-cols-[100px_1fr] gap-x-4 gap-y-3 text-sm">
                    ...
                    <span class="text-muted">密级</span>
                    <span>
                      <UBadge
                        :color="(confidentialityLevelConfig.color as any)"
                        variant="subtle"
                        size="xs"
                        :icon="confidentialityLevelConfig.icon"
                      >
                        {{ confidentialityLevelConfig.label }}
                      </UBadge>
                    </span>
            ...
            <div class="space-y-4">
              ...
              <UCard v-if="project">
                <template #header>
                  <div class="flex items-center justify-between gap-3">
                    <span class="font-semibold">项目模块</span>
                    <div class="flex items-center gap-2">
                      <UButton
                        label="重置"
                        color="neutral"
                        variant="ghost"
                        size="xs"
                        :disabled="moduleSaving || !moduleConfigChanged"
                        @click="resetModuleConfigDraft"
                      />
                      <UButton
                        label="保存"
                        color="primary"
                        ...
                        :disabled="!canManageProjectModules || !moduleConfigChanged"
                ...
                <div class="space-y-3">
                  <div
                    v-if="!canManageProjectModules"
                    class="rounded-md bg-elevated px-3 py-2 text-xs text-muted"
                  >
                    仅项目负责人、项目总监或系统管理员可以调整项目模块。
                  </div>
                  <div
                    v-for="key in projectModuleKeys"
                    :key="key"
                    class="flex items-start justify-between gap-3 rounded-md border border-default px-3 py-2.5"
                  >
                    <div class="min-w-0 flex items-start gap-2">
                      <UIcon
                        :name="projectModuleMeta[key].icon"
                        class="size-4 text-primary mt-0.5 shrink-0"
                      />
                      <div class="min-w-0">
                        <div class="text-sm font-medium">
                          {{ projectModuleMeta[key].label }}
                        </div>
                    ...
                    <USwitch
                      v-model="moduleConfigDraft[key]"
                      :disabled="!canManageProjectModules || moduleSaving"
                    />
                  </div>
                </div>
              </UCard>
              ...
              <UCard v-if="project">
                <template #header>
                  <div class="flex items-center justify-between">
                    ...
                    <UButton
                      v-if="canManageAccessControl && !isApprovalMode"
                      label="设置"
                ...
                <div class="space-y-3">
                  ...
                  <div class="flex items-center justify-between gap-3">
                    <span class="text-sm text-muted">密级</span>
                    <UBadge
                      :color="(confidentialityLevelConfig.color as any)"
                      variant="subtle"
                      :icon="confidentialityLevelConfig.icon"
                  ...
                  <p class="text-xs text-muted">
                    {{ confidentialityLevelConfig.description }}
                  </p>
                  <div
        ...
        <!-- 访问控制设置弹窗 -->
        <UModal v-model:open="showSecurityModal" :ui="{ content: 'sm:max-w-xl' }">
          <template #header>
            <h3 class="text-lg font-semibold">
              访问控制设置
            </h3>
              ...
              <div class="rounded-lg border border-muted bg-elevated/50 p-3">
                <div class="mb-2 flex items-start justify-between gap-3">
                  <div class="flex min-w-0 items-center gap-2">
                    <UIcon
                      :name="accessControlPolicyReview.icon"
                      :class="accessControlPolicyReview.tightened ? 'text-warning' : 'text-muted'"
                      class="size-4 shrink-0"
                    />
                    <div class="min-w-0">
                      <div class="truncate text-sm font-medium">
                        {{ accessControlPolicyReview.title }}
                      </div>
                      <div class="text-xs text-muted">
                        {{ accessControlPolicyReview.description }}
                      </div>
                    </div>
                  </div>
                  <UBadge
                    v-if="accessControlPolicyReview.tightened"
                    color="warning"
                    variant="subtle"
                    size="xs"
                  >
                ...
                <ul class="space-y-1 text-xs text-muted">
                  <li
                    v-for="rule in accessControlPolicyReview.rules"
                    :key="rule"
                    class="flex gap-2"
                  >
