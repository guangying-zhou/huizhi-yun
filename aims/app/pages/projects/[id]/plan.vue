...
const moduleEnabled = computed(() =>
  projectModuleEnabled(project.value?.moduleConfig, project.value?.category, 'milestones')
)
...
const canManageProjectPlan = computed(() =>
  isProjectLeader.value
  || project.value?.currentUserRole === 'manager'
  || Boolean(project.value?.currentUserIsProjectAdmin)
)
...
const canCreateWorkItem = computed(() =>
  ...
  && (createWorkItemForm.value.milestoneId || 0) > 0
)
...
const rolloverMilestone = ref<Milestone | null>(null)
const rolloverCarryover = ref<'auto' | 'manual'>('auto')
const rollingOverMilestone = ref(false)
const rolloverCarryoverOptions = [
  { label: '自动结转未完成项', value: 'auto' },
  { label: '手动处理未完成项', value: 'manual' }
]

function canRolloverMilestone(milestone: Milestone) {
...
function openRolloverMilestone(milestone: Milestone) {
  rolloverMilestone.value = milestone
  rolloverCarryover.value = 'auto'
  showRolloverModal.value = true
}

async function handleRolloverMilestone() {
  if (!rolloverMilestone.value) return
  ...
  try {
    ...
    milestoneWorkItems.value.clear()
    milestoneDeliverables.value.clear()
    deliverablesLoaded.value = false
    await milestoneStore.fetchMilestones(projectId.value)
    await Promise.all(milestoneStore.milestones.map(m => loadMilestoneWorkItems(m.id)))
    await loadProjectDeliverables()
    toast.add({ title: '已开启下一周期', color: 'success' })
  } catch (error: unknown) {
    ...
    rollingOverMilestone.value = false
  }
}
...
onMounted(async () => {
  const projectRequest = !projectStore.currentProject || projectStore.currentProject.id !== projectId.value
    ...
    : Promise.resolve()
  const milestonesRequest = milestoneStore.fetchMilestones(projectId.value)

  await projectRequest
  if (!moduleEnabled.value) {
    await milestonesRequest
    return
  }

  const templateRequest = loadProjectTemplateVersion()
  await milestonesRequest
  // 默认展开进行中的里程碑
  ...
  // 模板和里程碑工作项互不依赖，避免阻塞首屏计划数据。
  await Promise.all([
    templateRequest,
    Promise.all(milestoneStore.milestones.map(m => loadMilestoneWorkItems(m.id)))
  ])
  ...
  <UDashboardPanel id="project-plan" :ui="{ root: 'relative flex flex-col min-w-0 h-full shrink-0', body: 'flex flex-col flex-1 min-h-0 p-0 overflow-hidden' }">
    <template #body>
      <div class="flex flex-col h-full min-h-0">
        ...
        <div
          v-else
          class="flex-1 min-h-0 overflow-y-auto px-4 pt-4 pb-12 space-y-4"
        >
          <!-- 加载中 -->
                ...
                >
                  <div class="flex items-center gap-3">
                    ...
                    >
                      ...
                      />
                      <UButton
                        v-if="canRolloverMilestone(milestone)"
                        icon="i-lucide-rotate-cw"
                        color="warning"
                        variant="ghost"
                        size="xs"
                        title="开启下一周期"
                        @click.stop="openRolloverMilestone(milestone)"
                      />
                      <!-- <UButton
          ...
          <UModal v-model:open="showRolloverModal">
            <template #header>
              ...
              <div class="space-y-4 p-4">
                <div class="rounded-lg border border-default p-3">
                  <div class="text-sm font-medium">
                    {{ rolloverMilestone?.name }}
                  </div>
                  <div class="text-xs text-muted mt-1">
                    {{ formatDate(rolloverMilestone?.startDate || null) }} ~ {{ formatDate(rolloverMilestone?.endDate || null) }}
                  </div>
                </div>
                <UFormField label="未完成工作项">
                  <USelect
                    v-model="rolloverCarryover"
                    :items="rolloverCarryoverOptions"
                    class="w-full"
                  />
                </UFormField>
              </div>
            ...
            <template #footer>
              <div class="flex justify-end gap-2">
                ...
                <UButton
                  ...
                  label="确认开启"
                  color="primary"
                  :loading="rollingOverMilestone"
                  @click="handleRolloverMilestone"
                />
              </div>
            </template>
          </UModal>
          ...
          <UModal v-model:open="showCreateWorkItemModal">
            ...
            <template #body>
              <div class="space-y-4 p-4">
                ...
                >
                  <USelect
                    :model-value="createWorkItemForm.milestoneId ?? undefined"
                    :items="milestoneSelectOptions"
                    placeholder="选择里程碑"
                    class="w-full"
                    :disabled="createMilestoneLocked"
                    @update:model-value="createWorkItemForm.milestoneId = Number($event) || null"
                  />
                </UFormField>
                <UFormField
                  label="类型"
