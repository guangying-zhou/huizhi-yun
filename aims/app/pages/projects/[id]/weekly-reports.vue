...
const editableByCurrentUser = ref(false)
const mainWork = ref('')
...
const milestoneStageOptions = [
  { label: '规划 P', value: '规划 P' },
  { label: '实施 I', value: '实施 I' },
  { label: '验证 V', value: '验证 V' },
  { label: '发布 R', value: '发布 R' }
]
const progressStatusOptions = [
  { label: '正常推进', value: '正常推进' },
  { label: '超出计划', value: '超出计划' },
  { label: '落后计划', value: '落后计划' },
  { label: '严重滞后', value: '严重滞后' }
]
...
const canManage = computed(() => {
  return Boolean(authUser.value) && !isApprovalMode.value && editableByCurrentUser.value
})
...
const inferredCurrentMilestone = computed(() => {
  ...
  const weekEnd = startOfDay(selectedWeekRange.value.end)
  const datedMatches = milestoneStore.milestones.filter((milestone) => {
    const start = parseDateOnly(milestone.startDate)
    const end = parseDateOnly(milestone.endDate)
    if (!start || !end) return false
    return start <= weekEnd && end >= weekStart
  })

  if (datedMatches.length === 1) return datedMatches[0] || null

  const activeMatches = milestoneStore.milestones.filter(milestone => milestone.status === 'active')
  if (activeMatches.length === 1) return activeMatches[0] || null

  return null
})
const inferredCurrentStage = computed(() => milestoneStageDisplayValue(inferredCurrentMilestone.value?.pivrStage))
const currentStageReadonly = computed(() => Boolean(inferredCurrentStage.value))
const previousProgressSnapshot = computed(() => resolvePreviousChangedProgress())
const previousProgressDisplay = computed(() => {
  const snapshot = previousProgressSnapshot.value
  if (!snapshot) return '-'
...
function normalizeReport(raw: RawProjectWeeklyReport): ProjectWeeklyReport {
  ...
  return {
    ...
    remarks: raw.remarks ?? '',
    status: (raw.status || 'draft') as WeeklyReportStatus,
...
function resetForm(report: ProjectWeeklyReport | null) {
  ...
  summaryFields.currentStage = inferredCurrentStage.value || normalizeMilestoneStage(report?.currentStage) || ''
  summaryFields.progressStatus = normalizeProgressStatus(report?.progressStatus)
  summaryFields.completionPercent = report?.completionPercent ?? null
  summaryFields.contractStatus = normalizeContractStatus(report?.contractStatus || (project?.contractCode ? '已签订' : '未签订'))
...
async function submitReport() {
  ...
  try {
    ...
    if (res.code === 0) {
      toast.add({ title: '周报已提交，等待项目总监审阅', color: 'success' })
      await loadSelectedReport()
      await fetchReportCalendar()
    }
  } catch (error) {
    console.error('[ProjectWeeklyReports] submit failed:', error)
    toast.add({ title: getErrorMessage(error, '提交周报失败'), color: 'error' })
  } finally {
    submitting.value = false
  }
}

function weeklyReportStatusLabel(status: WeeklyReportStatus) {
  return {
    draft: '草稿',
    submitted: '待总监审阅',
    returned: '已退回',
    reviewed: '已审阅',
    frozen: '已汇总锁定',
    correction_draft: '更正草稿'
  }[status]
}

function weeklyReportStatusColor(status: WeeklyReportStatus) {
...
function weeksBetween(previousStart: Date, currentStart: Date) {
  const diff = startOfDay(currentStart).getTime() - startOfDay(previousStart).getTime()
  return Math.max(1, Math.round(diff / (7 * 24 * 60 * 60 * 1000)))
}

function milestoneStageDisplayValue(stage: unknown) {
  const text = String(stage || '').trim()
  if (!text) return ''
  return pivrStageLabel[text] || milestoneStageOptions.find(option => option.value === text)?.value || text
}

function normalizeMilestoneStage(value: unknown) {
  const text = String(value || '').trim()
  if (!text) return ''
  const upper = text.toUpperCase()
  return pivrStageLabel[upper] || milestoneStageOptions.find(option => option.value === text)?.value || ''
}

function normalizeProgressStatus(value: unknown) {
  const text = String(value || '').trim()
  return progressStatusOptions.some(option => option.value === text) ? text : ''
}

function normalizeContractStatus(value: unknown) {
  const text = String(value || '').trim()
  if (!text) return '未签订'
  return contractStatusOptions.some(option => option.value === text) ? text : '未签订'
}
                ...
                <div class="flex flex-wrap items-start justify-between gap-3">
                  <div class="min-w-0">
                    <div class="flex flex-wrap items-center gap-2">
                      ...
                      <UBadge
                        v-if="currentReport"
                        :color="weeklyReportStatusColor(currentReport.status)"
                        variant="subtle"
                        size="sm"
                      >
                        {{ weeklyReportStatusLabel(currentReport.status) }}
                      </UBadge>
                      <UBadge
                        v-else
                        variant="outline"
                ...
                <div class="rounded-lg border border-default bg-default p-4">
                  ...
                  <div class="grid gap-3 md:grid-cols-2 2xl:grid-cols-4">
                    ...
                    <UFormField label="当前阶段">
                      <UInput
                        v-if="currentStageReadonly"
                        :model-value="summaryFields.currentStage"
                        disabled
                        class="w-full"
                      />
                      <USelect
                    ...
                    <UFormField label="上期进度">
                      <UInput
                        :model-value="previousProgressDisplay"
                        disabled
                        class="w-full"
                      />
                    </UFormField>
                    ...
                    <UFormField label="已填报人力成本">
                      <UInput
                        :model-value="previousLaborCostDisplay"
                        disabled
                        class="w-full"
                      />
                    </UFormField>
