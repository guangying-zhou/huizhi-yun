...
} from '~/utils/requirementsTargetSelection'
import { requirementReviewMilestoneIssues } from '~/utils/requirementReviewMilestoneValidation'
...
const moduleEnabled = computed(() =>
  projectModuleEnabled(project.value?.moduleConfig, project.value?.category, 'requirements')
)
const {
  conflictModalOpen,
  conflictLoading,
  ...
  isConflictLoading
} = useAimsInstanceConflictExplanation()
...
const {
  ...
  activeTargetId,
  activeTarget,
  refresh: fetchRequirementTargets
} = useRequirementTargets<RequirementTargetInfo>({
  projectId,
  activeMilestoneId: computed(() =>
    milestoneStore.milestones.find(milestone => milestone.status === 'active')?.id ?? null
  ),
  initialTargetId: requirementTargetIdFromRouteQuery(route.query.workItemId),
  fetchTargets: async (currentProjectId) => {
    const res = await $fetch<{ code: number, data: { items: RequirementTargetInfo[] } }>(
      `/api/v1/projects/${currentProjectId}/requirement-targets`,
      {
        params: {
          type: filters.type,
    ...
    return res.code === 0 ? res.data.items : null
  },
  resolveTargetId: resolveRequirementTargetId,
  setWorkItemId: (targetId) => {
    filters.workItemId = requirementTargetFilterValue(targetId)
  }
})
...
usePageWorkflow({
  ...
  actions: computed(() => {
    ...
    const issues = computed(() => {
      const list: string[] = []
      list.push(...requirementReviewMilestoneIssues({
        context: 'batch',
        currentMilestone: activeMilestone.value,
        requirementIds: batch.requirementIds,
        requirements: resolveBatchRequirements(batch)
      }))
...
async function submitReviewFromList(batchType: 'baseline' | 'change') {
  ...
  const milestoneIssues = requirementReviewMilestoneIssues({
    context: 'selection',
    currentMilestone: activeMilestone.value,
    requirementIds: ids,
    requirements: ids
  ...
  >
    <template #body>
      <div class="flex flex-col h-full min-h-0">
        ...
        <!-- 需求工作项（基线/变更）切换条 -->
        <RequirementTargetSwitcher
          v-if="moduleEnabled && requirementTargets.length > 0"
          :targets="requirementTargets"
          :active-target-id="activeTargetId"
          :active-target="activeTarget"
          :all-requirement-count="allRequirementCount"
          @update:active-target-id="activeTargetId = $event"
        />
                  ...
                  <div v-else>
                    <div class="flex items-start justify-between gap-3 mb-2">
                      ...
                      <div class="flex shrink-0 flex-wrap items-center justify-end gap-2">
                        <UButton
                          v-if="activeBatch.status === 'pending'"
                          icon="i-lucide-shield-alert"
                          label="冲突"
                          color="warning"
                          variant="soft"
                          size="xs"
                          :loading="isConflictLoading('requirement_review', activeBatch.id)"
                          @click="openConflictExplanation({
                            targetType: 'requirement_review',
                            id: activeBatch.id,
                            action: 'approve',
                            label: activeBatch.title
                          })"
                        />
                        <UButton
