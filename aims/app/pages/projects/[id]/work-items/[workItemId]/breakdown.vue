...
/** 任务分配生效后，为目标提供一个不依赖看板拖拽的明确执行入口。 */
const canStartTargetExecution = computed(() => {
  const item = context.value?.item
  const children = context.value?.children || []
  if (!item || item.tier !== 'target' || item.type === 'requirement') return false
  return item.status === 'todo'
    && children.length > 0
    && children.every(child => child.status !== 'planning')
    && isProjectLeader.value
    && isWorkItemEditable.value
            ...
            <template #header>
              <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="flex flex-wrap items-center gap-2 ml-auto">
                  <UButton
                    v-if="canStartTargetExecution"
                    icon="i-lucide-play"
                    label="开始执行"
                    color="primary"
                    size="sm"
                    :loading="startingExecution"
                    @click="startTargetExecution"
                  />
