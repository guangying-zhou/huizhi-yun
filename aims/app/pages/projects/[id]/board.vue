...
const isCurrentUserProjectMember = computed(() => {
  ...
  return (projectStore.currentProject?.members || []).some(member =>
    member.uid === uid && member.status !== 'suspended'
  )
})

const canCreateRoutineTask = computed(() =>
  isRoutineProject.value && isWorkItemEditable.value && isCurrentUserProjectMember.value
)
...
async function handleRoutineTaskCreated() {
  ...
  <UDashboardPanel v-else id="project-board" :ui="{ root: 'relative flex flex-col min-w-0 h-full shrink-0', body: 'flex flex-col flex-1 min-h-0 p-0 overflow-hidden' }">
    <template #body>
      <div class="flex flex-col h-full min-h-0">
        <ProjectNavbar>
          <template v-if="isRoutineProject" #actions>
            <UButton
              v-if="canCreateRoutineTask"
              label="新建事务"
              icon="i-lucide-plus"
              color="primary"
              size="sm"
        ...
        <div class="flex-1 min-h-0 overflow-y-auto px-6 pt-4 pb-12 space-y-4">
          <RoutineQuarterReview v-if="isRoutineProject" :project-id="projectId" />
          <!-- 工具栏 -->
