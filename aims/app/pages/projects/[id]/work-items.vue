...
const canCreateWorkItem = computed(() => {
  const lifecycleAllowsCreate = isRoutineProject.value ? isWorkItemEditable.value : canEditTargetStructure.value
  return lifecycleAllowsCreate && isCurrentUserProjectMember.value && !activeMilestoneCompleted.value
})
...
const createFormErrors = computed(() => {
  ...
  if (!isRoutineProject.value && !createForm.value.milestoneId) errors.milestone = '请选择里程碑'
  if (!createForm.value.title.trim()) errors.title = '请输入标题'
  if (!createForm.value.dueDate) errors.dueDate = '请选择截止日期'
  if (isRoutineProject.value && createForm.value.routineScope === 'cross_dept' && !createForm.value.beneficiaryDeptCode) {
    errors.beneficiaryDeptCode = '请选择受益部门'
  }
  return errors
})
...
function openCreateModal(type?: 'task' | 'bug' | 'requirement') {
  ...
  if (isRoutineProject.value) {
    createForm.value.milestoneId = null
    createForm.value.routineScope = 'department'
    createForm.value.beneficiaryDeptCode = null
    createForm.value.isUnplanned = false
  } else if (mid !== 'all' && mid !== 'none') {
        ...
        <ProjectNavbar>
          <template v-if="!isApprovalMode" #actions>
            <UButton
              ...
              :label="isRoutineProject ? '新建事务' : '新建工作目标'"
              color="primary"
              size="sm"
          ...
          <template v-if="viewMode === 'list'">
            ...
            >
              ...
              <template #assigneeUid-cell="{ row }">
                {{ row.original.assigneeName || getUserName(row.original.assigneeUid) }}
              </template>
                ...
                <div v-if="workItemStore.currentItem" class="p-4 space-y-4">
                  ...
                  <div class="space-y-2 text-sm">
                    ...
                    <div class="flex justify-between">
                      ...
                      <span>{{ workItemStore.currentItem.reporterName || (workItemStore.currentItem.reporterUid ? getUserName(workItemStore.currentItem.reporterUid) : '-') }}</span>
                    </div>
                    <div class="flex justify-between">
                      <span class="text-muted">截止日期</span>
                    ...
                    <div v-if="workItemStore.currentItem.comments?.length" class="space-y-3">
                      ...
                      >
                        <div class="flex items-center justify-between mb-1">
                          <span class="text-sm font-medium">{{ comment.authorName || getUserName(comment.authorUid) }}</span>
              ...
              <div class="space-y-4 p-4">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <UFormField
                    ...
                    label="里程碑"
                    required
                    :error="createFormErrors.milestone"
                  >
                    <URadioGroup
                      :model-value="createForm.milestoneId ?? undefined"
                ...
                <div v-if="isRoutineProject" class="space-y-4 rounded-lg border border-default bg-elevated/30 p-4">
                  <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <UFormField label="事务归属" required>
                      <URadioGroup
                        :model-value="createForm.routineScope ?? undefined"
                        :items="[
                          { label: '部门事务', value: 'department' },
                          { label: '跨部门协助', value: 'cross_dept' }
                        ]"
                        orientation="horizontal"
                        @update:model-value="createForm.routineScope = $event as 'department' | 'cross_dept'"
                      />
                    </UFormField>
                    <UFormField label="计划属性">
                      <UCheckbox v-model="createForm.isUnplanned" label="计划外事务" />
                    </UFormField>
                  </div>
                  <UFormField
                    v-if="createForm.routineScope === 'cross_dept'"
                    label="受益部门"
                    required
                  ...
                  >
                    <div class="max-h-52 overflow-y-auto rounded-lg border border-default bg-default p-2">
                      <DeptTreeSelector
                        ...
                        :node="department"
                        :selected-dept-code="createForm.beneficiaryDeptCode || ''"
                        @select="createForm.beneficiaryDeptCode = $event"
                      />
                    </div>
                  </UFormField>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                  ...
                  <UFormField v-if="!isRoutineProject" label="影响范围">
                    <URadioGroup
