...
function hasLinkedDocument(d: DeliverableItem) {
  if (d.documentSource === 'repo') {
    return Boolean(d.repoProjectCode && d.repoFilePath)
  }
  return Boolean(d.documentUuid)
}
...
function openSubmitDocModal(d: DeliverableItem) {
  ...
  // 恢复当前已绑定的来源
  submittingInitialDocRef.value = !hasLinkedDocument(d)
    ? null
    : d.documentSource === 'repo'
      ? {
          source: 'repo',
...
async function handleDeliverableDocSelected(docRef: import('~/composables/useAimsDocumentPicker').DocumentRef) {
  ...
  try {
    ...
    if (updatedDeliverable && context.value) {
      const index = context.value.deliverables.findIndex(item => item.id === deliverableId)
      if (index >= 0) {
        context.value.deliverables.splice(index, 1, updatedDeliverable)
      }
    }
              ...
              >
                <div class="flex items-center justify-between gap-2">
                  ...
                  <UButton
                    ...
                    :icon="hasLinkedDocument(d) ? 'i-lucide-file-edit' : 'i-lucide-file-plus'"
                    size="xs"
                    color="primary"
                    variant="soft"
                ...
                >
                  ...
                  <div class="min-w-0 flex-1">
                    ...
                    <div v-if="d.documentSource === 'repo'" class="mt-0.5 flex items-center gap-1.5 text-xs text-muted font-mono">
                      <span class="truncate">{{ d.repoFilePath }}</span>
                      <span v-if="d.repoCommitId" class="shrink-0">@ {{ d.repoCommitId.slice(0, 8) }}</span>
                    </div>
