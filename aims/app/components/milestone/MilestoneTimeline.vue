...
import { pivrPhases, pivrTypeMapping, pivrStageOrder, hasPivrStages } from '~/config/milestone'
...
const stages: PivrStage[] = pivrStageOrder

// 不使用 PIVR 阶段语义的分类（如 routine 日常事务）不渲染时间线
const showTimeline = computed(() => hasPivrStages(props.projectCategory as ProjectCategory))
...
<template>
  <div v-if="showTimeline" class="flex items-start gap-0 w-full">
    <div
