...
const props = defineProps<{
  ...
  canCreate?: boolean
  canDelete?: boolean
  canDeleteDocument?: (doc: DocumentNode) => boolean
...
const canCreate = computed(() => props.canCreate ?? !isReadonly.value)
const canDelete = computed(() => props.canDelete ?? !isReadonly.value)
const canManageAccess = computed(() => props.canManageAccess ?? !isReadonly.value)
const showActions = computed(() => canCreate.value || canDelete.value || Boolean(props.canDeleteDocument) || canManageAccess.value)
...
function canDeleteNode(doc: DocumentNode) {
  return canDelete.value || Boolean(props.canDeleteDocument?.(doc))
}
  ...
  <div v-else class="space-y-0.5">
    ...
    >
      ...
      >
        ...
        <!-- Uploader -->
        <span class="text-xs text-muted shrink-0 w-20 text-right truncate">
        ...
        >
          <UButton
            v-if="canManageAccess && doc.type === 'document' && !doc.virtual"
          ...
          />
          <UButton
            v-if="canCreate && doc.type === 'folder'"
          ...
          />
          <UButton
            v-if="canDeleteNode(doc) && !doc.virtual"
