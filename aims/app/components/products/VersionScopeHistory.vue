...
const page = ref(1)
const { data, status, error, refresh } = await useFetch(() => moduleUrl(`/api/v1/products/${encodeURIComponent(props.productCode)}/versions/${encodeURIComponent(props.versionId)}/features/${props.scopeId}/history`), { ...(hosted ? { key: computed(() => cacheKey(`version-scope-history:${props.productCode}:${props.versionId}:${props.scopeId}`)) } : {}), server: false, query: { page, pageSize: 10 }, transform: (r: { code: number, data: { version_id: number, scope_id: number, total: number, items: Entry[] } }) => {
