...
const projectCategory = computed(() => {
  const projectId = Number(projectIdValue.value)
  if (!projectId) return null

  if (projectStore.currentProject?.id === projectId) {
    return projectStore.currentProject.category
  }

  return projectStore.projects.find(project => project.id === projectId)?.category ?? null
})

const projectMenuItems = computed(() => {
  if (!projectIdValue.value) return []
  const items = getProjectMenuItems(projectIdValue.value, projectCategory.value)
