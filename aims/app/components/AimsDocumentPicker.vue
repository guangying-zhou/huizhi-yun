...
const {
  ...
  repoTree, repoLoading, repoError, loadRepoTree
...
async function ensureTabLoaded(tab: DocumentSourceTab) {
  if (tab === 'codocs_dept' && deptDocuments.value.length === 0) {
    ...
    await loadRepoTree(selectedRepoCode.value, props.aimsProjectId)
  }
}
...
watch(selectedRepoCode, async (v) => {
  if (!props.open) return
  if (sourceTab.value === 'repo' && v) await loadRepoTree(v, props.aimsProjectId)
  repoPreview.value = null
})
            ...
            <div class="flex flex-col">
              ...
              <div v-else class="flex flex-col">
                ...
                <div class="max-h-72 overflow-y-auto">
                  ...
                  <div v-else-if="repoError" class="text-center py-6 px-3 text-sm text-error">
                    {{ repoError }}
                  </div>
