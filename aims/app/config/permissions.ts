...
const legacyResources = [
  ...
  { code: 'projects', name: '项目管理', description: '项目列表、详情、成员与仓库管理', sortOrder: 20, supportedActions: ['view', 'create', 'edit', 'approve', 'close', 'admin'] as const },
  { code: 'work_items', name: '工作项管理', description: '工作项列表、详情与执行过程', sortOrder: 30, supportedActions: ['view', 'create', 'edit', 'delete', 'assign', 'confirm'] as const },
  ...
  { code: 'reports', name: '报表统计', description: '统计报表与分析视图', sortOrder: 70, supportedActions: ['view', 'edit', 'admin', 'export'] as const },
  { code: 'weekly_reports', name: '项目周报', description: '项目周报填报、独立审阅、公司汇总发布与导出', sortOrder: 71, supportedActions: ['view', 'submit', 'review', 'publish', 'export', 'configure'] as const },
  { code: 'quality_reviews', name: '项目文档质量检查', description: '必交文档完整性、确定版本质量检查与项目总监豁免', sortOrder: 72, supportedActions: ['view', 'review', 'waive', 'configure'] as const },
  { code: 'project_templates', name: '项目模板', description: '项目模板与流程模板的定义与维护', sortOrder: 80, supportedActions: ['view', 'edit', 'admin'] as const },
  { code: 'integration_operations', name: '跨应用操作', description: '诊断并受控重放 AIMS 发起的跨应用可靠操作', sortOrder: 85, supportedActions: ['view', 'replay'] as const },
  { code: 'admin', name: '系统管理', description: 'AIMS 应用级后台管理', sortOrder: 90, supportedActions: ['view', 'edit', 'admin'] as const }
] as const

// Product resources and project handoff permissions are projected from the
// deployed manifest. Existing legacy resource aliases are migrated separately.
export const productManifestResources = manifest.resources.filter(resource =>
  resource.code === 'products' || resource.code.startsWith('product_')
)
export const manifestResources = [
  ...legacyResources,
  ...manifest.resources.filter(resource =>
    resource.code === 'requirements' || productManifestResources.includes(resource)
  ).map(resource => ({ ...resource, supportedActions: resource.actions }))
]
...
export const appManifest = {
  ...
  recommendedRoles: manifest.recommendedRoles
} as const
...
export const approvalActions = [
  ...
  },
  {
    ...
    name: '里程碑完成审批',
    description: '项目经理申请完成，项目总监最终批准；通过后自动激活下一个里程碑',
...
type RouteAction = 'view' | 'edit' | 'admin' | 'review' | 'waive' | 'configure' | 'onboard'
...
export const routeRules: RouteRule[] = [
  { pattern: '/product-setup', resource: 'products', action: 'onboard' },
  ...
  {
    pattern: '/quality-reviews',
    resource: 'quality_reviews',
    action: 'view',
    anyOf: [
      { resource: 'quality_reviews', action: 'view' },
      { resource: 'quality_reviews', action: 'review' },
      { resource: 'quality_reviews', action: 'waive' },
      { resource: 'quality_reviews', action: 'configure' }
    ]
  },
  {
...
function normalizeRoutePath(path: string): string {
  ...
  if (normalized === '' || normalized === '/aims') return '/'
  if (normalized.startsWith('/aims/')) return normalized.slice('/aims'.length)
  return normalized
}
