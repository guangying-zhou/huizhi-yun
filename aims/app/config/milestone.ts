...
// 本表是里程碑名称的唯一来源。前端模板种子（project-template-defaults.ts）
// 与 data-runtime 的 defaultProjectMilestoneSeeds 均须与本表保持一致，
// 由 test/milestoneTemplateContract.test.ts 断言。
//
// 未列入本表的分类不使用 PIVR 阶段语义：
//   - routine（日常事务）：不使用 PIVR，不生成里程碑，时间线不渲染
//
// improvement（改进）已于 V1.1 停用，此处保留仅供存量项目正确渲染。

export const pivrTypeMapping: Partial<Record<ProjectCategory, Record<PivrStage, { title: string, description: string }>>> = {
  product_dev: {
  ...
  // maintenance 的四阶段是「周期内的节奏标签」，用于组织周期内动作与关期检查项，
  // 不各自生成里程碑。里程碑结构见 buildDefaultProjectTemplateDefinition 的
  // maintenance 分支：周期单元(monthly_cycle) + 常驻工单容器(service_ops)。
  maintenance: {
    P: { title: '周期规划', description: '本期重点与结转项接收' },
    I: { title: '任务处理', description: '日常巡检与 Bug 修复' },
    V: { title: '质量抽检', description: '服务指标(SLA)核对' },
    R: { title: '复盘优化', description: 'SOP 更新与关期' }
  },
  sales: {
...
// PIVR 里程碑生成策略（按项目类型）
// ============================================================
// 只有列入本表的分类才把四个 PIVR 阶段各生成一个里程碑。
// 未列入的分类：
//   - maintenance：走专用模板（周期单元 + 工单容器），四阶段仅作节奏标签
//   - routine：不生成任何里程碑

export const pivrMilestoneModes: Partial<Record<ProjectCategory, Record<PivrStage, MilestoneMode>>> = {
  product_dev: { P: 'strong_constraint', I: 'strong_constraint', V: 'strong_constraint', R: 'rolling_plan' },
  custom_dev: { P: 'strong_constraint', I: 'strong_constraint', V: 'strong_constraint', R: 'rolling_plan' },
  // R「项目结项」为强约束：交付结项有明确截止日期
  delivery: { P: 'strong_constraint', I: 'strong_constraint', V: 'strong_constraint', R: 'strong_constraint' },
  sales: { P: 'rolling_plan', I: 'rolling_plan', V: 'strong_constraint', R: 'rolling_plan' },
  presales: { P: 'rolling_plan', I: 'strong_constraint', V: 'strong_constraint', R: 'rolling_plan' },
  /** @deprecated V1.1 已停用，保留供存量模板渲染 */
  improvement: { P: 'rolling_plan', I: 'rolling_plan', V: 'strong_constraint', R: 'rolling_plan' },
  compliance: { P: 'strong_constraint', I: 'strong_constraint', V: 'strong_constraint', R: 'rolling_plan' }
}

export const pivrStageOrder: PivrStage[] = ['P', 'I', 'V', 'R']

/** 该分类是否使用 PIVR 阶段语义（用于时间线渲染判断） */
export function hasPivrStages(category: ProjectCategory | undefined | null): boolean {
  return !!category && !!pivrTypeMapping[category]
}
