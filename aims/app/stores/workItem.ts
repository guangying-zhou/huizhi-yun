...
function normalizeWorkItem(raw: RawWorkItem): WorkItem {
  ...
  return {
    id: Number(raw.id) || 0,
    projectId,
    milestoneId: nullableNumber(raw.milestoneId ?? raw.milestone_id),
    ...
    routineScope: nullableString(raw.routineScope ?? raw.routine_scope) as WorkItem['routineScope'],
    beneficiaryDeptCode: nullableString(raw.beneficiaryDeptCode ?? raw.beneficiary_dept_code),
    isUnplanned: booleanValue(raw.isUnplanned ?? raw.is_unplanned),
    carryoverOriginItemKey: nullableString(raw.carryoverOriginItemKey ?? raw.carryover_origin_item_key),
    carryoverOriginMilestoneId: nullableNumber(raw.carryoverOriginMilestoneId ?? raw.carryover_origin_milestone_id),
    carryoverCount: Number(raw.carryoverCount ?? raw.carryover_count) || 0,
    carryoverGovernanceAbnormal: booleanValue(raw.carryoverGovernanceAbnormal ?? raw.carryover_governance_abnormal),
