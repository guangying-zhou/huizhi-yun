...
export const useProjectStore = defineStore('project', () => {
  type RawAimsProject = Partial<AimsProject> & {
    ...
    confidentiality_level?: ProjectConfidentialityLevel
    ...
    service_period_seq?: number | null
    ...
    templateDefaultModuleConfig?: ModuleConfig | string | null
    template_default_module_config?: ModuleConfig | string | null
    ...
    current_user_is_project_admin?: boolean | number | string
  ...
  function normalizeProject(raw: RawAimsProject): AimsProject {
    return {
      ...
      confidentialityLevel: raw.confidentialityLevel ?? raw.confidentiality_level ?? 'L1',
      ...
      serviceLineCode: raw.serviceLineCode ?? raw.service_line_code ?? null,
      servicePeriodSeq: raw.servicePeriodSeq ?? raw.service_period_seq ?? null,
      ...
      servicePeriodLabel: raw.servicePeriodLabel ?? raw.service_period_label ?? null,
      ...
      moduleConfig: normalizeProjectModuleConfig(
        raw.moduleConfig ?? raw.module_config,
        raw.category ?? 'custom_dev',
        raw.templateDefaultModuleConfig ?? raw.template_default_module_config
      ) as ModuleConfig,
      ...
      currentUserIsProjectAdmin: raw.currentUserIsProjectAdmin ?? booleanValue(raw.current_user_is_project_admin, false),
  ...
  async function fetchProjectPage(query?: ProjectListQuery) {
    ...
    if (query?.participatingOnly) params.set('participating_only', '1')
  ...
  async function createProject(data: CreateProjectRequest) {
    const normalizedData: CreateProjectRequest = {
      ...data,
  ...
  async function updateProject(id: number, data: UpdateProjectRequest) {
    const normalizedData: UpdateProjectRequest = data.moduleConfig
      ? {
          ...data,
          moduleConfig: toPersistedProjectModuleConfig(
            data.moduleConfig,
            data.category || currentProject.value?.category || 'custom_dev'
          )
        }
      : data
    ...
    if (res.code === 0) {
      ...
      if (currentProject.value?.id === id) {
        currentProject.value = { ...currentProject.value, ...normalizedData } as typeof currentProject.value
      }
      const idx = projects.value.findIndex(p => p.id === id)
      if (idx !== -1) {
        projects.value[idx] = { ...projects.value[idx], ...normalizedData } as typeof projects.value[0]
      }
    }
    return res.data
  }
