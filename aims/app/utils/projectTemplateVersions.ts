...
function getFallbackTemplateSetName(raw: RawProjectTemplateVersion) {
  ...
  return `${getProjectCategoryLabel(category)} 默认模板`
}
...
function normalizeDefinitionObject(value: unknown): ProjectTemplateDefinition {
  const definition = value && typeof value === 'object' && !Array.isArray(value)
    ...
    : {}
  const defaultModuleConfig = parseModuleConfig(definition.defaultModuleConfig ?? definition.default_module_config) as ModuleConfig
  return {
    ...definition,
    schemaVersion: Number(definition.schemaVersion ?? definition.schema_version ?? 1),
    schema_version: Number(definition.schema_version ?? definition.schemaVersion ?? 1),
    defaultModuleConfig,
    default_module_config: defaultModuleConfig,
    milestones: Array.isArray(definition.milestones)
      ? definition.milestones as ProjectTemplateMilestoneDefinition[]
      : []
  }
}
