...
const appCode = process.env.HZY_APP_CODE || 'aims'
...
export default defineNuxtConfig({
  ...
  runtimeConfig: {
    ...
    public: {
      appName: packageMeta.name || 'aims',
      appVersion: packageMeta.version || '0.0.0',
      appDisplayName: process.env.NUXT_PUBLIC_APP_DISPLAY_NAME || '汇智云项目',
      appIcon: process.env.NUXT_PUBLIC_APP_ICON || 'i-lucide-package',
      ...
      codocsUrl: process.env.NUXT_PUBLIC_CODOCS_URL || 'http://localhost:3001/codocs',
  ...
  nitro: {
    ...
    scheduledTasks: {
      ...
      '*/15 * * * *': ['notifications:due'],
      '15 2 * * *': ['milestones:rollover']
    },
    alias: {
  ...
  typescript: {
    // Pure domain inputs are also imported by Node's native TypeScript tests.
    tsConfig: { compilerOptions: { allowImportingTsExtensions: true } }
  },
