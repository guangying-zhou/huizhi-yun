...
export default defineEventHandler(async (event) => {
  ...
  const monthParams = new URLSearchParams({ calendarCode, yearMonth })
  const headers = {
    ...trustedServiceRequestHeaders(event),
    authorization: `Bearer ${token}`
  }
  const [daysResponse, monthResponse] = await Promise.all([
    fetchConsoleServiceJson<ServiceEnvelope<WorkCalendarDayPage>>(
      event,
      `${appendPath(consoleBaseUrl, `/api/v1/console/service/work-calendar/${encodeURIComponent(calendarCode)}/days`)}?${params.toString()}`,
      {
        headers,
        timeout: 10000
      }
    ),
    fetchConsoleServiceJson<ServiceEnvelope<WorkCalendarMonth>>(
      event,
      `${appendPath(consoleBaseUrl, '/api/v1/console/service/work-calendar/month')}?${monthParams.toString()}`,
      {
        headers,
        timeout: 10000
      }
