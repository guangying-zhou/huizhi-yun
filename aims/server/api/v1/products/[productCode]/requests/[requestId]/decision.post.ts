...
import { productRequestDecisionInput } from '../../../../../../utils/productRequestInput'
...
export default defineEventHandler(async (event) => {
  ...
  const input = productRequestDecisionInput(await readBody(event), getRouterParam(event, 'requestId') || '')
  if (!key || !input) throw createError({ statusCode: 400, message: '需求字段或幂等键无效' })
  const facts = await requireProductPermission(event, code, 'product_requests', 'decide')
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/requests:decide`, {
    appCode: 'aims', method: 'POST', scope: 'aims.write aims:product-requests:decide',
    query: { current_user: facts.actor_uid }, idempotencyKey: key,
    body: { input, authorization: { resource: 'product_requests', action: 'decide', facts, expires_at: Date.now() + 15000 } }
  })
