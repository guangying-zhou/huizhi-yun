...
import { productRequestMergeInput } from '../../../../../../utils/productRequestInput'
...
export default defineEventHandler(async (event) => {
  ...
  const input = productRequestMergeInput(await readBody(event), getRouterParam(event, 'requestId') || '')
  ...
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/requests:merge`, {
    appCode: 'aims', method: 'POST', scope: 'aims.write aims:product-requests:merge',
