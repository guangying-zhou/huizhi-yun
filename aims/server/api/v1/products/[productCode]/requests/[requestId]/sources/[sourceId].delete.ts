...
import { productRequestSourceDeleteInput } from '../../../../../../../utils/productRequestSourceInput'
...
export default defineEventHandler(async (event) => {
  ...
  const input = productRequestSourceDeleteInput(await readBody(event), getRouterParam(event, 'requestId') || '', getRouterParam(event, 'sourceId') || '')
  if (!key || !input) throw createError({ statusCode: 400, message: '需求字段或幂等键无效' })
  const facts = await requireProductPermission(event, code, 'product_requests', 'delete')
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/request-sources:delete`, {
    appCode: 'aims', method: 'POST', scope: 'aims.write aims:product-requests:source-delete',
    query: { current_user: facts.actor_uid }, idempotencyKey: key,
    body: { input, authorization: { resource: 'product_requests', action: 'delete', facts, expires_at: Date.now() + 15000 } }
  })
