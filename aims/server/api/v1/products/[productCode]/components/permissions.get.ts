...
export default defineEventHandler(async (event) => {
  ...
  const facts = await requireProductPermission(event, code, 'product_components', 'view')
  const edit = await checkProductPermission(event, code, 'product_components', 'edit')
  const deletion = await checkProductPermission(event, code, 'product_components', 'delete')
