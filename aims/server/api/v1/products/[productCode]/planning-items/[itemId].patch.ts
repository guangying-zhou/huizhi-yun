...
import { productPlanningEditInput } from '../../../../../utils/productPlanningInput'
...
export default defineEventHandler(async (event) => {
  ...
  const input = productPlanningEditInput(await readBody(event), getRouterParam(event, 'itemId') || '')
  ...
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/planning-items:edit`, {
