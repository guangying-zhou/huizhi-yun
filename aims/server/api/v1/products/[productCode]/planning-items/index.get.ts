...
import { productPlanningPageInput } from '../../../../../utils/productPlanningInput'
...
export default defineEventHandler(async (event) => {
  ...
  const input = productPlanningPageInput(getQuery(event))
  ...
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/planning-items:list`, {
