...
import { productPlanningCreateInput } from '../../../../../utils/productPlanningInput'
...
export default defineEventHandler(async (event) => {
  ...
  const input = productPlanningCreateInput(await readBody(event))
  ...
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/planning-items:create`, {
