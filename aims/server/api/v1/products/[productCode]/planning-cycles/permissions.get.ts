...
export default defineEventHandler(async (event) => {
  ...
  const observe = await checkProductPermission(event, code, 'product_priorities', 'observe')
  return { code: 0, data: { product_code: code, status: facts.status, revision: facts.revision, edit: edit.allowed, prioritize: prioritize.allowed, assess: assess.allowed, observe: observe.allowed } }
})
