...
import { productPlanningCycleCreateInput } from '../../../../../utils/productPlanningCycleInput'
...
export default defineEventHandler(async (event) => {
  ...
  const input = productPlanningCycleCreateInput(await readBody(event))
  ...
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/planning-cycles:create`, {
    appCode: 'aims', method: 'POST', scope: 'aims.write aims:product-priorities:create',
