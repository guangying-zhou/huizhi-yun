...
import { productPlanningCycleEditInput } from '../../../../../utils/productPlanningCycleInput'
...
export default defineEventHandler(async (event) => {
  ...
  const input = productPlanningCycleEditInput(await readBody(event), getRouterParam(event, 'cycleId') || '')
  ...
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/planning-cycles:edit`, {
