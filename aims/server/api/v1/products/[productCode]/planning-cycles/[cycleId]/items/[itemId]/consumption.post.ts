...
import { productConsumptionInput } from '../../../../../../../../utils/productConsumptionInput'
...
export default defineEventHandler(async (event) => {
  ...
  const input = productConsumptionInput(await readBody(event), getRouterParam(event, 'cycleId') || '', getRouterParam(event, 'itemId') || '')
  if (!key || !input) throw createError({ statusCode: 400, message: '投入、核验依据、版本或幂等键无效' })
  const facts = await requireProductPermission(event, code, 'product_priorities', 'assess')
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/planning-consumption:confirm`, {
    appCode: 'aims', method: 'POST', scope: 'aims.write aims:product-priorities:consumption-confirm',
