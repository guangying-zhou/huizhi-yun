...
import { productObservationPageInput } from '../../../../../../utils/productObservationInput'
...
export default defineEventHandler(async (event) => {
  ...
  const input = productObservationPageInput(getQuery(event), getRouterParam(event, 'cycleId') || '')
  ...
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/planning-observations:list`, {
