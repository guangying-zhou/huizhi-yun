...
export default defineEventHandler(async (event) => {
  ...
  if (!key || !input) throw createError({ statusCode: 400, message: '撤回版本、影响、理由、例外或幂等键无效' })
  const facts = await requireProductPermission(event, code, 'product_priorities', 'prioritize')
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/planning-withdrawal:withdraw`, {
    appCode: 'aims', method: 'POST', scope: 'aims.write aims:product-priorities:withdraw',
