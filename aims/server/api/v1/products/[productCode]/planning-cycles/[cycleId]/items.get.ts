...
import { productPlanningCandidatePageInput } from '../../../../../../utils/productPlanningCycleInput'
...
export default defineEventHandler(async (event) => {
  ...
  const input = productPlanningCandidatePageInput(getQuery(event), getRouterParam(event, 'cycleId') || '')
  if (!input) throw createError({ statusCode: 400, message: '周期候选查询条件无效' })
  const facts = await requireProductPermission(event, code, 'product_priorities', 'view')
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/planning-candidates:list`, {
