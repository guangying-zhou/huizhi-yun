...
import { productAssessmentPageInput } from '../../../../../../../../utils/productAssessmentInput'
...
export default defineEventHandler(async (event) => {
  ...
  const input = productAssessmentPageInput(getQuery(event), getRouterParam(event, 'cycleId') || '', getRouterParam(event, 'itemId') || '')
  if (!input) throw createError({ statusCode: 400, message: '评估历史查询条件无效' })
  const facts = await requireProductPermission(event, code, 'product_priorities', 'view')
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/planning-assessments:list`, {
