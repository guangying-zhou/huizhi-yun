...
import { productObservationDetailInput } from '../../../../../../../utils/productObservationInput'
...
export default defineEventHandler(async (event) => {
  ...
  const input = productObservationDetailInput(getQuery(event), getRouterParam(event, 'cycleId') || '', getRouterParam(event, 'observationId') || '')
  if (!input) throw createError({ statusCode: 400, message: '观测查询条件无效' })
  const facts = await requireProductPermission(event, code, 'product_priorities', 'view')
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/planning-observations:view`, {
