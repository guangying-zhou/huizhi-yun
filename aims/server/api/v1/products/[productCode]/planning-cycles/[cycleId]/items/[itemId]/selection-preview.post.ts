...
export default defineEventHandler(async (event) => {
  ...
  if (!input) throw createError({ statusCode: 400, message: '选入预览版本、评估或例外说明无效' })
  const facts = await requireProductPermission(event, code, 'product_priorities', 'view')
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/planning-selection:preview`, {
