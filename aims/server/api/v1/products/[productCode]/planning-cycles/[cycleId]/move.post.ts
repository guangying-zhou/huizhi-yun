...
import { productQueueMoveInput } from '../../../../../../utils/productQueueInput'
...
export default defineEventHandler(async (event) => {
  ...
  const input = productQueueMoveInput(await readBody(event), getRouterParam(event, 'cycleId') || '')
  if (!key || !input) throw createError({ statusCode: 400, message: '调序位置、版本、理由或幂等键无效' })
  const facts = await requireProductPermission(event, code, 'product_priorities', 'prioritize')
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/planning-queue:move`, {
    appCode: 'aims', method: 'POST', scope: 'aims.write aims:product-priorities:move',
