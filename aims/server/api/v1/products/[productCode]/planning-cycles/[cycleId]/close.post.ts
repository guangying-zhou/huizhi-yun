...
export default defineEventHandler(async (event) => {
  ...
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/planning-cycles:close`, {
    appCode: 'aims', method: 'POST', scope: 'aims.write aims:product-priorities:cycle-close',
