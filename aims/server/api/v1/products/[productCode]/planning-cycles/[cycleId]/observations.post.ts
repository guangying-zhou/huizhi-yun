...
import { productObservationCreateInput } from '../../../../../../utils/productObservationInput'
...
export default defineEventHandler(async (event) => {
  ...
  const input = productObservationCreateInput(await readBody(event), getRouterParam(event, 'cycleId') || '')
  if (!key || !input) throw createError({ statusCode: 400, message: '周期字段或幂等键无效' })
  const facts = await requireProductPermission(event, code, 'product_priorities', 'observe')
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/planning-observations:create`, {
    appCode: 'aims', method: 'POST', scope: 'aims.write aims:product-priorities:observe',
    query: { current_user: facts.actor_uid }, idempotencyKey: key,
    body: { input, authorization: { resource: 'product_priorities', action: 'observe', facts, expires_at: Date.now() + 15000 } }
  })
