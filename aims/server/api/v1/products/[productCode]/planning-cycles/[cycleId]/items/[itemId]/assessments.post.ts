...
import { productAssessmentCreateInput } from '../../../../../../../../utils/productAssessmentInput'
...
export default defineEventHandler(async (event) => {
  ...
  const input = productAssessmentCreateInput(await readBody(event), getRouterParam(event, 'cycleId') || '', getRouterParam(event, 'itemId') || '')
  ...
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/planning-assessments:create`, {
    appCode: 'aims', method: 'POST', scope: 'aims.write aims:product-priorities:assess',
