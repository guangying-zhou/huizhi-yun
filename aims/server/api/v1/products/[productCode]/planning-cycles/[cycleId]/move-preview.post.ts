...
export default defineEventHandler(async (event) => {
  ...
  if (!input) throw createError({ statusCode: 400, message: '调序预览位置、版本或理由无效' })
  const facts = await requireProductPermission(event, code, 'product_priorities', 'view')
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/planning-queue:preview`, {
