...
export default defineEventHandler(async (event) => {
  ...
  if (!input) throw createError({ statusCode: 400, message: '复评历史查询条件无效' })
  const facts = await requireProductPermission(event, code, 'product_priorities', 'view')
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/planning-reviews:list`, {
