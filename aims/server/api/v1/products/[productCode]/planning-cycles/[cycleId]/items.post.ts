...
import { productPlanningCandidateAddInput } from '../../../../../../utils/productPlanningCycleInput'
...
export default defineEventHandler(async (event) => {
  ...
  const input = productPlanningCandidateAddInput(await readBody(event), getRouterParam(event, 'cycleId') || '')
  if (!key || !input) throw createError({ statusCode: 400, message: '候选事项、版本或幂等键无效' })
  const facts = await requireProductPermission(event, code, 'product_priorities', 'edit')
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/planning-candidates:add`, {
