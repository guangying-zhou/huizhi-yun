...
import { productBudgetInput } from '../../../../../../utils/productBudgetInput'
...
export default defineEventHandler(async (event) => {
  ...
  const input = productBudgetInput(await readBody(event), getRouterParam(event, 'cycleId') || '')
  ...
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/planning-budget:change`, {
    appCode: 'aims', method: 'POST', scope: 'aims.write aims:product-priorities:budget-change',
