...
import { productFeatureRoadmapInput } from '../../../../../../utils/productFeatureRoadmapInput'
...
export default defineEventHandler(async (event) => {
  ...
  const planningFacts = await requireProductPermission(event, code, 'product_priorities', 'view')
  const featureFacts = await requireProductPermission(event, code, 'product_features', 'view')
  const expires = Date.now() + 15000
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/feature-roadmap:view`, {
