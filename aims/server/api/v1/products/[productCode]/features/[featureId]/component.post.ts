...
import { productFeatureComponentInput } from '../../../../../../utils/productFeatureInput'
...
export default defineEventHandler(async (event) => {
  ...
  const facts = await requireProductPermission(event, code, 'product_features', 'edit')
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/features:component-assign`, {
