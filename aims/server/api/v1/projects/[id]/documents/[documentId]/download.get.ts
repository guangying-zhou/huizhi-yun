import { checkCodocsDocumentAccess, getCodocsProjectCabinetDownloadUrl } from '~~/server/utils/codocsApi'
...
export default defineEventHandler(async (event) => {
  ...
  if (!context.projectCode) {
    throw createError({ statusCode: 500, message: '项目编码缺失，无法下载项目文件' })
  }
  ...
  const download = await getCodocsProjectCabinetDownloadUrl({
    event,
    fileUuid: context.documentUuid,
    projectCode: context.projectCode,
    expectedOssPath
  })
  return sendRedirect(event, download.url)
})
