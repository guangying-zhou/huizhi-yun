 ...
 *
 * Uploads non-Markdown project documents into Codocs project cabinet (OSS)
 * and registers the returned file metadata in Aims project_documents.
 */
import { createError } from 'h3'
import { maybeCallTenantRuntime } from '@hzy/foundation/server/utils/tenantRuntimeClient'
import type { H3Event } from 'h3'
import { uploadCodocsProjectCabinetFile } from './codocsApi'
...
export async function uploadProjectOtherDocument(event: H3Event, uid: string, projectId: number, input: ProjectDocumentUpload) {
  if (!projectId || Number.isNaN(projectId)) {
    throw createError({ statusCode: 400, message: '无效的项目ID' })
  }

  const file = { filename: input.fileName, data: input.data, type: input.contentType }
  if (!file.filename || !file.data.length || file.data.length > 100 * 1024 * 1024) {
    throw createError({ statusCode: 400, message: '请选择不超过 100MB 的文件' })
  }
  ...
  const uploaded = await uploadCodocsProjectCabinetFile({
    event,
    ownerUid: uid,
    fileUuid: input.documentUuid,
    projectCode,
    deptCode,
