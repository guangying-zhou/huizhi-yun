...
export async function handleProductRequestRead(event: H3Event, action: 'list' | 'view' | 'sources') {
  ...
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/${action === 'sources' ? 'request-sources:list' : `requests:${action}`}`, {
    appCode: 'aims', method: 'POST', scope: 'aims.read aims:product-requests:read',
    query: { current_user: facts.actor_uid },
    body: { input, authorization: { resource: 'product_requests', action: 'view', facts, expires_at: Date.now() + 15000 } }
  })
