...
export async function onboardProductLine(event: H3Event, raw: unknown) {
  setHeader(event, 'Cache-Control', 'no-store')
  await requireProductOnboardPermission(event)
  const input = productLineOnboardInput(raw)
  ...
  const source = await fetchProductLineCatalog(event, input.line_code, input.expected_watermark)
  ...
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, '/v1/aims/internal/product-line-onboard', {
