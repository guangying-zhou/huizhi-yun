...
export function runtimeEnvelopeError(envelope: RuntimeEnvelope<unknown>) {
...
function sanitizeRuntimeBody(body?: Record<string, unknown>) {
  const runtimeBody = { ...(body || {}) }
  delete runtimeBody.current_user
  delete runtimeBody.operator_uid
  return runtimeBody
}
