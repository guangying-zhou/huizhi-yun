...
import { productFeatureComponentInput } from './productFeatureInput'
...
export async function handleProductFeatureComponent(event: H3Event, bridge?: ProductCommandBridge) {
  ...
  const input = productFeatureComponentInput(await readBody(event), getRouterParam(event, 'featureId') || '')
  if (!key || !input || Object.keys(getQuery(event)).length) throw createError({ statusCode: 400, message: '功能字段或幂等键无效' })
  const facts = await requireProductPermission(event, code, 'product_features', 'edit', bridge?.authorizationSource)
  const runtime = bridge
    ? { handled: true as const, data: await bridge.call(code, 'component-assign', { input, authorization: { resource: 'product_features', action: 'edit', facts, expires_at: Date.now() + 15000 } }, key) }
    : await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/features:component-assign`, {
        appCode: 'aims', method: 'POST', scope: 'aims.write aims:product-features:component-assign',
