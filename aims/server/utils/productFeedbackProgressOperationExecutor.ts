...
export interface ProductFeedbackProgressOperationIO extends ServiceTicketDeliveryOperationIO {
  callAltocProductFeedbackProgress: (envelope: RuntimeRow, operation: ClaimedDeliveryOperation) => Promise<RuntimeRow>
}

export async function executeClaimedProductFeedbackProgressOperation(operation: ClaimedDeliveryOperation, io: ServiceTicketDeliveryOperationIO) {
  ...
  try {
    ...
    if (operation.sourceApp !== 'aims' || operation.targetApp !== 'altoc' || operation.operationCode !== 'aims.altoc.product-feedback.update-progress.v1' || operation.requiredCapability !== 'altoc:product-feedback:update-progress' || operation.commandSchemaVersion !== 'product-feedback-progress.v1' || operation.idempotencyKey !== key || !operation.tenantCode || !operation.deploymentCode || !c || !validProductFeedbackProgress(c) || typeof c.productCode !== 'string' || !c.productCode || typeof c.ticketCode !== 'string' || !c.ticketCode || typeof c.requestBizId !== 'string' || typeof c.canonicalRequestBizId !== 'string' || !Number.isSafeInteger(c.sourceRevision) || Number(c.sourceRevision) < 1 || !['submitted', 'evaluating', 'accepted', 'deferred', 'rejected', 'merged'].includes(String(c.decisionStatus)) || (c.decisionStatus === 'merged') !== (c.requestBizId !== c.canonicalRequestBizId) || await hashServiceCommandPayload(c) !== operation.commandSha256) throw createError({ statusCode: 409, message: '反馈状态回流冻结命令无效' })
    if (!io.callAltocProductFeedbackProgress) throw createError({ statusCode: 503, message: '反馈状态回流投递通道暂不可用' })
    const response = await io.callAltocProductFeedbackProgress(buildServiceCommandEnvelope(operation), operation)
