...
export async function handleProductComponent(event: H3Event, action: 'list' | 'create' | 'move' | 'edit' | 'delete', bridge?: ProductCommandBridge) {
  ...
  const facts = await requireProductPermission(event, code, 'product_components', permission, bridge?.authorizationSource)
  const body = { input, authorization: { resource: 'product_components', action: permission, facts, expires_at: Date.now() + 15000 } }
  const runtime = bridge
    ? { handled: true as const, data: await bridge.call(code, action, body, key || undefined) }
    : await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/components:${action}`, {
