...
import { productFeatureCreateInput } from './productFeatureInput'
...
export async function handleProductFeatureCreate(event: H3Event, bridge?: ProductCommandBridge) {
  ...
  const input = productFeatureCreateInput(await readBody(event))
  ...
  const runtime = bridge
    ? { handled: true as const, data: await bridge.call(code, 'create', { input, authorization: { resource: 'product_features', action: 'edit', facts, expires_at: Date.now() + 15000 } }, key) }
    : await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/features:create`, {
        appCode: 'aims', method: 'POST', scope: 'aims.write aims:product-features:create',
