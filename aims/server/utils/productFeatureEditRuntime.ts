...
import { productFeatureEditInput } from './productFeatureInput'
...
export async function handleProductFeatureEdit(event: H3Event, bridge?: ProductCommandBridge) {
  ...
  const input = productFeatureEditInput(await readBody(event), getRouterParam(event, 'featureId') || '')
  ...
  const runtime = bridge
    ? { handled: true as const, data: await bridge.call(code, 'edit', { input, authorization: { resource: 'product_features', action: 'edit', facts, expires_at: Date.now() + 15000 } }, key) }
    : await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/features:edit`, {
        appCode: 'aims', method: 'POST', scope: 'aims.write aims:product-features:edit',
