...
export async function handleProductFeatureRead(event: H3Event, action: 'list' | 'view', bridge?: ProductCommandBridge) {
  ...
  const facts = await requireProductPermission(event, code, 'product_features', 'view', bridge?.authorizationSource)
  const runtime = bridge
    ? { handled: true as const, data: await bridge.call(code, action, { input, authorization: { resource: 'product_features', action: 'view', facts, expires_at: Date.now() + 15000 } }) }
    : await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/features:${action}`, {
