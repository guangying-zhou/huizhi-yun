...
export async function handleProductHandoff(event: H3Event, source: 'planning' | 'request', bridge?: ProductCommandBridge) {
  ...
  const planningFacts = await requireProductPermission(event, code, 'product_priorities', 'handoff', bridge?.authorizationSource)
  const requestFacts = input.request_biz_id ? await requireProductPermission(event, code, 'product_requests', 'handoff', bridge?.authorizationSource) : null
  const versionFacts = input.planned_version_id ? await requireProductPermission(event, code, 'product_versions', 'view', bridge?.authorizationSource) : null
  const uid = planningFacts.actor_uid
  const viewFacts = bridge ? await requireProductPermission(event, code, 'product_priorities', 'view', bridge.authorizationSource) : null
  const project = bridge
    ? { handled: true as const, data: await bridge.call(code, 'project-authorization', { input: { project_code: input.project_code }, authorization: { resource: 'product_priorities', action: 'view', facts: viewFacts, expires_at: Date.now() + 15000 } }) as { code: number, data: ProductHandoffProjectFacts } }
    : await maybeCallTenantRuntime<{ code: number, data: ProductHandoffProjectFacts }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/handoff-project:authorization`, {
  ...
  const runtime = bridge
    ? { handled: true as const, data: await bridge.call(code, 'create', commandBody, key) }
    : await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/planning-handoff:create`, {
