...
export async function handleProductExecutionCoordination(event: H3Event) {
  ...
  const result = await maybeCallTenantRuntime<{ code: number, data: ExecutionCoordination }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/versions:execution-coordination`, {
