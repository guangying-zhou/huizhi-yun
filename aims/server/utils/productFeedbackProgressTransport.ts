...
export async function sendProductFeedbackProgress(event: H3Event | null, operation: ClaimedDeliveryOperation, envelope: Row, targetDeploymentOverride = ''): Promise<Row> {
  ...
  const url = `${base.replace(/\/+$/, '')}/api/v1/service/product-feedback/progress`
  return await requestWithServiceAccessToken({ audience: 'altoc', scope: 'altoc:product-feedback:update-progress', event, async request(token) {
