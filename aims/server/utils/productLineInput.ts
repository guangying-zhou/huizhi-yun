...
export function productLineOnboardInput(raw: unknown, options: { allowCurrentAssetsWatermark?: boolean } = {}) {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return null
  const v = raw as Record<string, unknown>
  if (Object.keys(v).some(k => !['productLine', 'managerUid', 'productCodes', 'expectedWatermark'].includes(k)) || !productIdentifier(v.productLine) || !productIdentifier(v.managerUid) || typeof v.expectedWatermark !== 'string' || !(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}:[1-9]\d*$/.test(v.expectedWatermark) || (options.allowCurrentAssetsWatermark && /^current-assets:v1:[0-9a-f]{64}$/.test(v.expectedWatermark)))) return null
