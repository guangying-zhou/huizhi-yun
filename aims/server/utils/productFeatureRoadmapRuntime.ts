...
import { productFeatureRoadmapInput } from './productFeatureRoadmapInput'
import { hasProductControlCharacter } from './productWorkspaceInput'
import { runtimeEnvelopeError } from './aimsRuntimeForward'

export async function handleProductFeatureRoadmap(event: H3Event, bridge?: ProductCommandBridge) {
  ...
  const input = productFeatureRoadmapInput(getQuery(event), getRouterParam(event, 'featureId') || '')
  if (!input) throw createError({ statusCode: 400, message: '功能路线查询条件无效' })
  ...
  const runtime = bridge
    ? { handled: true as const, data: await bridge.call(code, 'roadmap', { input, planning_authorization: { resource: 'product_priorities', action: 'view', facts: planningFacts, expires_at: expires }, feature_authorization: { resource: 'product_features', action: 'view', facts: featureFacts, expires_at: expires } }) }
    : await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/feature-roadmap:view`, {
