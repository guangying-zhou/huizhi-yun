...
import { onboardProductLine } from './productLineOnboard'
...
import { productOnboardInput } from './productOnboardInput'
...
export async function handleProductOnboard(event: H3Event, bridge?: ProductOnboardBridge) {
  ...
  if (raw && typeof raw === 'object' && !Array.isArray(raw) && 'productLine' in raw) return onboardProductLine(event, raw, bridge)
  ...
  const catalog = await (bridge?.catalog || fetchProductCatalog)(event, { code: productCode, page: '1', pageSize: '1' })
  ...
  const runtime = bridge
    ? { handled: true, data: await bridge.execute(event, 'onboard', productCode, input, { product_code: productCode, actor_uid: actor, resource: 'products', action: 'onboard', expires_at: authorizedUntil }, { active_uids: [input.manager_uid], expires_at: Date.now() + 15000 }, key) }
    : await maybeCallTenantRuntime<{ code: number, data: unknown }>(event,
