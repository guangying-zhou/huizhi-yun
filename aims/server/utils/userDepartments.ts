...
export async function fetchAccessibleDepartments(event: H3Event, uid: string): Promise<RawDeptNode[]> {
  const normalizedUid = String(uid || '').trim()
  if (!normalizedUid) return []

  const response = await fetchDirectoryApi<{ code: number, data?: RawDeptNode[] }>(
    '/api/v1/departments/accessible',
    { event, params: { uid: normalizedUid }, timeout: 10000 }
  )
...
export async function fetchUserDepartments(event: H3Event, uid: string): Promise<UserDepartmentsResult> {
  if (!uid) {
...
export async function hasDepartmentAccess(event: H3Event, uid: string, deptCode: string): Promise<boolean> {
  const normalizedUid = String(uid || '').trim()
  const normalizedDeptCode = String(deptCode || '').trim()
  if (!normalizedUid || !normalizedDeptCode) return false

  const departments = await fetchAccessibleDepartments(event, normalizedUid)
