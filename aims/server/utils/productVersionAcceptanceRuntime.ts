...
export async function handleProductVersionAcceptance(event: H3Event, action: 'preview' | 'accept' | 'list' | 'view' | 'publish' | 'reopen' | 'archive' | 'delete') {
  ...
  const canViewProject = async (projectID: number) => {
    const object = await resolveAimsProjectAuthorizationObject(event, { projectId: String(projectID), uid: facts.actor_uid, requireCompleteFacts: true })
  ...
  if (action === 'accept' || action === 'publish') {
    const viewFacts = await requireProductPermission(event, code, 'product_versions', 'view')
    const review = await maybeCallTenantRuntime<{ code: number, data: ProductVersionAcceptancePreview }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/versions:acceptance-preview`, {
      appCode: 'aims', method: 'POST', scope: 'aims.read aims:product-versions:read', query: { current_user: facts.actor_uid },
      body: { input: { version_id: id }, authorization: { resource: 'product_versions', action: 'view', facts: viewFacts, expires_at: Date.now() + 15000 } }
    })
  ...
  const runtime = await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/versions:${writing ? action : `acceptance-${action}`}`, {
    ...
    body: { input, ...(executionReviewHash ? { execution_review_hash: executionReviewHash } : {}), authorization: { resource: 'product_versions', action: permission, facts, expires_at: Date.now() + 15000 } }
  })
