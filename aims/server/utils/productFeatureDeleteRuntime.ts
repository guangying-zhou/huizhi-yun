...
export async function handleProductFeatureDelete(event: H3Event, bridge?: ProductCommandBridge) {
  ...
  const input = productFeatureDeleteInput(await readBody(event), getRouterParam(event, 'featureId') || '')
  if (!key || !input) throw createError({ statusCode: 400, message: '删除字段或幂等键无效' })
  const facts = await requireProductPermission(event, code, 'product_features', 'delete', bridge?.authorizationSource)
  const runtime = bridge
    ? { handled: true as const, data: await bridge.call(code, 'delete', { input, authorization: { resource: 'product_features', action: 'delete', facts, expires_at: Date.now() + 15000 } }, key) }
    : await maybeCallTenantRuntime<{ code: number, data: unknown }>(event, `/v1/aims/internal/products/${encodeURIComponent(code)}/features:delete`, {
        appCode: 'aims', method: 'POST', scope: 'aims.write aims:product-features:delete',
        query: { current_user: facts.actor_uid }, idempotencyKey: key,
        body: { input, authorization: { resource: 'product_features', action: 'delete', facts, expires_at: Date.now() + 15000 } }
      })
