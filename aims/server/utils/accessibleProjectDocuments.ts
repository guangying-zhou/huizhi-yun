 ...
 * 来源。Codocs ACL 复核、仓库快照的项目成员判定都在本文件内部完成。
 */
import { createError, type H3Event } from 'h3'
import { checkCodocsDocumentAccess } from './codocsApi'
import { buildAimsProjectRuntimeAccessQuery } from './aimsProjectRuntimeAccess'
import { callAimsRuntime, getProjectContext } from './projectDocumentAccess'
...
async function loadWorkItemMilestones(
  event: Parameters<typeof callAimsRuntime>[0],
  projectId: number,
  uid: string,
...
async function loadUnifiedProjectDocuments(event: Parameters<typeof callAimsRuntime>[0], projectId: number, projectCode: string, uid: string) {
  ...
  const [documentPages, workItemMilestones] = await Promise.all([
    ...
    loadWorkItemMilestones(event, projectId, uid, projectAccessQuery)
  ])
...
export async function listAccessibleProjectDocuments(event: H3Event, uid: string, projectIdInput: unknown) {
  const projectId = numberValue(projectIdInput)
  if (!projectId) {
  ...
  // Folders belong to the project, not Codocs; retain empty folders for members.
  const allowedDocumentIds = new Set<number>(allDocs
    .filter(doc => doc.isFolder && projectContext.isMember
      && isDirectProjectDocument(doc, projectId, projectContext.projectCode))
    .map(doc => doc.id))
  ...
  await Promise.all(allDocs
    ...
    .map(async (doc) => {
      // Repository snapshots are authorized by the AIMS project/repository
      // relationship. They are not Codocs or cabinet objects and therefore do
      // not have a UUID that Codocs can validate.
      if (doc.documentSource === 'repo') {
        const access = directProjectMemberAccess(doc, projectContext, projectId)
        if (!access) return
        allowedDocumentIds.add(doc.id)
        accessById.set(doc.id, access)
        return
      }
      ...
      try {
        ...
        // Listing must neither change policy nor override Codocs denial.
        ...
        // Omit inaccessible/stale references; dependency failures remain visible.
