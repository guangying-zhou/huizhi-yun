...
const code = ts.transpileModule(readFileSync(new URL('../server/utils/productFeedbackStatusTransport.ts', import.meta.url), 'utf8'), { compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022 } }).outputText
...
test('feedback transport signs separate deployments and rejects missing routes before token issuance', async () => {
  for (const mode of ['request', 'scheduled', 'missing-route', 'wrong-tenant', 'scheduled-missing']) {
    ...
    const envelope = { operationId: '123e4567-e89b-42d3-a456-426614174001', operationCode: 'aims.altoc.product-feedback.update-status.v1', targetApp: 'altoc', requiredCapability: 'altoc:product-feedback:update-status', idempotencyKey: 'key', commandSchemaVersion: 'product-feedback-status.v1', commandSha256: await hashServiceCommandPayload(command), command }
    ...
    runInNewContext(code, { exports, URL, crypto: { randomUUID: () => 'request-id' }, require: (name: string) => {
      ...
      if (name.endsWith('/serviceOidc')) return { requestWithServiceAccessToken: async (options: { audience: string, scope: string, request: (token: string) => Promise<unknown> }) => {
        ...
        assert.equal(options.scope, 'altoc:product-feedback:update-status')
      ...
      if (name.endsWith('/tenantRuntimeClient')) return { buildServiceCommandRuntimeHeaders: async (options: Record<string, unknown>) => {
        ...
        assert.equal(options.requestTarget, '/api/v1/service/product-feedback/status')
      ...
      if (name.endsWith('/appServiceBinding')) return { serviceAppFetch: async (_event: unknown, _app: string, _url: string, options: { headers: Record<string, string> }) => {
        const verifyInput = {
          token: 'test-token', method: 'POST' as const, requestTarget: '/api/v1/service/product-feedback/status', requestId: 'request-id',
    ...
    const pending = exports.sendProductFeedbackStatus!(mode.startsWith('scheduled') ? null : {}, { tenantCode: 'TENANT', deploymentCode: 'AIMS', idempotencyKey: 'key' }, { serviceCommand: envelope }, mode === 'scheduled' ? 'ALTOC' : '')
