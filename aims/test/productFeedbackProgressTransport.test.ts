...
const code = ts.transpileModule(readFileSync(new URL('../server/utils/productFeedbackProgressTransport.ts', import.meta.url), 'utf8'), { compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022 } }).outputText
...
test('feedback transport signs separate deployments and rejects missing routes before token issuance', async () => {
  for (const mode of ['request', 'scheduled', 'missing-route', 'wrong-tenant', 'scheduled-missing']) {
    ...
    let tokens = 0
    const command = { productCode: 'P1', ticketCode: 'ST-1', requestBizId: '123e4567-e89b-42d3-a456-426614174000', canonicalRequestBizId: '123e4567-e89b-42d3-a456-426614174000', decisionStatus: 'accepted', sourceRevision: 3 }
    const envelope = { operationId: '123e4567-e89b-42d3-a456-426614174001', operationCode: 'aims.altoc.product-feedback.update-progress.v1', targetApp: 'altoc', requiredCapability: 'altoc:product-feedback:update-progress', idempotencyKey: 'key', commandSchemaVersion: 'product-feedback-progress.v1', commandSha256: await hashServiceCommandPayload(command), command }
    ...
    runInNewContext(code, { exports, URL, crypto: { randomUUID: () => 'request-id' }, require: (name: string) => {
      ...
      if (name.endsWith('/serviceAppUrl')) return {
        ...
        resolveTrustedServiceAppRoute: () => mode === 'missing-route' ? null : { deploymentCode: 'ALTOC' }
      }
      ...
      if (name.endsWith('/serviceOidc')) return { requestWithServiceAccessToken: async (options: { audience: string, scope: string, request: (token: string) => Promise<unknown> }) => {
        tokens++
        assert.equal(options.audience, 'altoc')
        assert.equal(options.scope, 'altoc:product-feedback:update-progress')
      ...
      if (name.endsWith('/tenantRuntimeClient')) return { buildServiceCommandRuntimeHeaders: async (options: Record<string, unknown>) => {
        ...
        assert.equal(options.targetDeploymentCode, 'ALTOC')
        assert.equal(options.sourceClientId, 'aims.runtime')
        assert.equal(options.requestTarget, '/api/v1/service/product-feedback/progress')
      ...
      if (name.endsWith('/appServiceBinding')) return { serviceAppFetch: async (_event: unknown, _app: string, _url: string, options: { headers: Record<string, string> }) => {
        const verifyInput = {
          token: 'test-token', method: 'POST' as const, requestTarget: '/api/v1/service/product-feedback/progress', requestId: 'request-id',
          tenantCode: 'TENANT', sourceDeploymentCode: 'AIMS', targetDeploymentCode: 'ALTOC', sourceApp: 'aims', sourceClientId: 'aims.runtime', targetApp: 'altoc', envelope,
        ...
        const changedHash = await hashServiceCommandPayload({ ...command, sourceRevision: 4 })
        await assert.rejects(verifyServiceCommandRuntimeHeaders({ ...verifyInput, envelope: { ...envelope, commandSha256: changedHash } }))
        assert.equal(options.headers['x-hzy-deployment'], 'ALTOC')
        return { code: 0, data: { ok: true } }
      } }
      throw new Error(name)
    } })
    const pending = exports.sendProductFeedbackProgress!(mode.startsWith('scheduled') ? null : {}, { tenantCode: 'TENANT', deploymentCode: 'AIMS', idempotencyKey: 'key' }, { serviceCommand: envelope }, mode === 'scheduled' ? 'ALTOC' : '')
