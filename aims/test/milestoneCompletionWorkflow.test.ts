...
test('only trusted Workflow terminal callback can finalize a milestone and enqueue Altoc work', () => {
  ...
  assert.match(callback, /HZY_AIMS_ENTERPRISE_ENABLE_MILESTONE_RECEIVABLE/)
  assert.match(callback, /selectWorkflowCallbackRuntime/)
  assert.match(callback, /selectMilestoneReceivableDispatch/)
  assert.match(callback, /forwardAimsMilestoneReceivableWorkflowCallback/)
  assert.match(callback, /forwardAimsRuntimePost<CallbackResult>/)
