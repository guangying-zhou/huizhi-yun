...
async function run(mode = '') {
  ...
  const matter = mode.startsWith('matter')
  const key = matter
    ? 'aims:work-item-completion:matter:3:workflow-submit:v2'
    : 'aims:work-item-completion:3:workflow-submit:v1'
  ...
  if (matter) {
    command.kind = 'matter'
    command.formData.kind = 'matter'
    command.formData.evidenceSummary = { deliverableCount: 0, requiredDeliverableCount: 0, commitCount: 0, timeEntryCount: 1 }
  }
  const operation: any = { operationId: '00000000-0000-4000-8000-000000000003', operationKey: key, idempotencyKey: key, tenantCode: 'TENANT', deploymentCode: 'AIMS', sourceApp: 'aims', targetApp: 'workflow', operationCode: 'aims.work-item.completion.workflow-submit.v1', requiredCapability: 'workflow:work-item-complete:create', commandSchemaVersion: matter
    ? 'v2'
    : 'v1', commandSha256: await hashServiceCommandPayload(command), originalActorUid: 'U1', fencingToken: 2, command }
  if (mode === 'matter-schema-mix') operation.commandSchemaVersion = 'v1'
  if (mode === 'matter-v3') operation.commandSchemaVersion = 'v3'
  if (mode === 'target-v2') operation.commandSchemaVersion = 'v2'
  if (mode === 'target-v3') operation.commandSchemaVersion = 'v3'
  if (mode === 'target-explicit-kind') command.kind = 'target'
  if (mode === 'matter-kind-mix') command.formData.kind = 'target'
  if (mode === 'matter-bad-summary') command.formData.evidenceSummary.timeEntryCount = -1
  ...
  if (['target-explicit-kind', 'matter-kind-mix', 'matter-bad-summary'].includes(mode)) operation.commandSha256 = await hashServiceCommandPayload(command)
  ...
  const result = await dispatcher.executeClaimedAimsOperation(operation, {
    callWorkflowWorkItemCompletion: mode === 'missing-transport'
      ? undefined
      : async (body: any) => {
...
test('matter v2 frozen command reaches Workflow with evidence summary and old target v1 remains accepted', async () => {
  const matter = await run('matter')
  assert.equal(matter.result.synced, true)
  assert.equal(matter.calls[0].target.serviceCommand.commandSchemaVersion, 'v2')
  assert.equal(matter.calls[0].target.serviceCommand.command.formData.evidenceSummary.timeEntryCount, 1)
  assert.equal((await run()).result.synced, true)
})
test('completion executor accepts only target v1 and matter v2 schema pairs', async () => {
  for (const mode of ['matter-schema-mix', 'matter-v3', 'target-v2', 'target-v3', 'target-explicit-kind', 'matter-kind-mix', 'matter-bad-summary']) {
