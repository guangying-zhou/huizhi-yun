...
import * as input from '../server/utils/productFeatureInput'
...
function harness(options: { body?: unknown, query?: Record<string, unknown>, key?: string | null, denied?: boolean, response?: unknown } = {}) {
  ...
  runInNewContext(compiled, { exports, defineEventHandler: (handler: unknown) => handler, require: (name: string) => {
    if (name === 'h3') return { createError, getRouterParam: (_event: unknown, key: string) => (({ productCode: '产品 A', featureId: '11111111-1111-4111-8111-111111111111' } as Record<string, string | undefined>)[key]), getQuery: () => options.query || {}, readBody: async () => options.body || { expectedRevision: 1, expectedFeatureRevision: 2, componentId: null, reason: '功能归类' }, getHeader: () => options.key === undefined ? 'component-key' : options.key, setHeader: (_event: unknown, key: string, value: string) => {
      headers[key] = value
    } }
    if (name.endsWith('/productFeatureInput')) return input
    if (name === './productVersionInput') return versionInput
    if (name.endsWith('/productWorkspaceInput')) return workspace
    if (name.endsWith('/productAuthorization')) return { requireProductPermission: async (_event: unknown, code: string, resource: string, action: string) => {
...
test('feature classification BFF binds trusted actor, product scope and idempotency without browser authority', async () => {
  const h = harness()
  await h.run('move')
  assert.deepEqual(h.permissions, ['产品 A:product_features:edit'])
  const call = h.calls[0]!
  assert.equal(call.path, `/v1/aims/internal/products/${encodeURIComponent('产品 A')}/features:component-assign`)
  assert.equal(call.args.scope, 'aims.write aims:product-features:component-assign')
  ...
  assert.equal(call.args.body.input.biz_id, '11111111-1111-4111-8111-111111111111')
  assert.equal(call.args.body.input.component_id, null)
