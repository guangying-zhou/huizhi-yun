...
function transport(mode = '') {
  ...
  runInNewContext(compiled, { exports, URL, AbortSignal, crypto: { randomUUID: () => 'request-1' }, require: (name: string) => {
    ...
    if (name.endsWith('/tenantRuntimeClient')) return { buildServiceCommandRuntimeHeaders: async (args: any) => { calls.push({ signed: args }); return { 'x-hzy-service-command-signature': 'signed' } } }
    if (name.endsWith('/serviceOidc')) return { requestWithServiceAccessToken: async (args: any) => { calls.push({ token: { audience: args.audience, scope: args.scope } }); return await args.request('aims-owned-token') } }
    if (name.endsWith('/appServiceBinding')) return { serviceAppFetch: async (_event: any, app: string, url: string, args: any) => { calls.push({ target: app, url, args }); return { code: 0, data: { accepted: true } } } }
    throw Error(name)
  } })
...
test('completion transport uses aims.runtime signed identity and exact Workflow token/binding without Enterprise credentials', async () => {
  ...
  await exports.sendWorkItemCompletion({}, operation, { serviceCommand: { command: { actorUid: 'U1' } } })
  ...
  assert.equal(calls[2].target, 'workflow')
  assert.equal(calls[2].args.headers.authorization, 'Bearer aims-owned-token')
  assert.equal(calls[2].args.headers['x-hzy-actor-uid'], 'U1')
  assert.equal(calls[1].signed.requestTarget, new URL(calls[2].url).pathname)
})
test('wrong source or missing target route fail before credentials or outbound call; event-less requires explicit target deployment', async () => {
  for (const mode of ['wrong-source', 'missing-route']) { const { exports, calls } = transport(mode); await assert.rejects(exports.sendWorkItemCompletion({}, operation, {})); assert.equal(calls.length, 0) }
  const { exports, calls } = transport()
...
test('cron uses the actual Gateway binding with workflow prefix and the same signed target; never public fallback', async () => {
  const { exports, calls } = transport()
  const sent: any[] = []
  const env = { HZY_TENANT_GATEWAY_SERVICE_URL: 'https://tenant.example', HZY_TENANT_GATEWAY_SERVICE: { async fetch(url: string, args: any) { sent.push({ url, args }); return { ok: true, json: async () => ({ code: 0, data: { accepted: true } }) } } } }
...
test('scheduled binding enters the real Gateway which creates Workflow trust and preserves signed command headers', async () => {
  ...
  const targetEnv = {
    ...
    HZY_WORKFLOW_SERVICE: { async fetch(url: string, init: any) { forwarded.push({ url, headers: new Headers(init.headers) }); return Response.json({ code: 0, data: { accepted: true } }) } }
  }
...
test('scheduled Gateway failures preserve authorization/conflict/unavailable status and timeout without direct fallback', async () => {
  for (const status of [403, 409, 503]) {
    const { exports, calls } = transport()
    const env = { HZY_TENANT_GATEWAY_SERVICE_URL: 'https://tenant.example', HZY_TENANT_GATEWAY_SERVICE: { async fetch() { return Response.json({ code: status, message: 'rejected' }, { status }) } } }
  ...
  const { exports, calls } = transport()
  const env = { HZY_TENANT_GATEWAY_SERVICE_URL: 'https://tenant.example', HZY_TENANT_GATEWAY_SERVICE: { async fetch(_url: string, init: any) { assert.ok(init.signal instanceof AbortSignal); throw new DOMException('timed out', 'TimeoutError') } } }
