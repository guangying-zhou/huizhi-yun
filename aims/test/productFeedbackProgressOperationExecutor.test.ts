...
const compiled = ts.transpileModule(readFileSync(new URL('../server/utils/productFeedbackProgressOperationExecutor.ts', import.meta.url), 'utf8'), { compilerOptions: { module: ts.ModuleKind.CommonJS } }).outputText
async function run(mode = '') {
  const exports: any = {}, calls: any[] = []
  const command = { productCode: 'P', ticketCode: 'ST-1', requestBizId: '00000000-0000-4000-8000-000000000001', canonicalRequestBizId: '00000000-0000-4000-8000-000000000001', decisionStatus: 'accepted', sourceRevision: 3, canonicalDecisionStatus: 'accepted', versions: [] }
  const operation = { operationId: '00000000-0000-4000-8000-000000000003', operationKey: 'key', idempotencyKey: 'key', tenantCode: 'TENANT', deploymentCode: 'AIMS', sourceApp: 'aims', targetApp: 'altoc', operationCode: 'aims.altoc.product-feedback.update-progress.v1', requiredCapability: 'altoc:product-feedback:update-progress', commandSchemaVersion: 'product-feedback-progress.v1', commandSha256: await hashServiceCommandPayload(command), fencingToken: 2, command }
  ...
  runInNewContext(dispatcherCode, { exports: dispatcher, require: (name: string) => {
    if (name === './productFeedbackProgressOperationExecutor') return exports
    return {}
  } })
  const result = await dispatcher.executeClaimedAimsOperation(operation, {
    callAltocProductFeedbackProgress: mode === 'missing-transport'
      ? undefined
      : async (body: any) => {
