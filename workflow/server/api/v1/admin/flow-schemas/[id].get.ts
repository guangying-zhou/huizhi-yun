/**
 * GET /api/v1/admin/flow-schemas/:id
 * 获取流程定义详情
 */
...
export default defineEventHandler(async (event) => {
  ...
  try {
    const row = await queryRow<FlowSchemaRow>(
      'SELECT * FROM flow_schemas WHERE id = ?',
      [id]
    )
    ...
    console.error('查询流程定义详情失败:', error)
    throw createError({
      statusCode: 500,
      message: error.message || '查询流程定义详情失败'
    })
  }
})
