...
export default defineEventHandler(async (event) => {
  ...
  if (!command || command.targetApp !== 'workflow' || command.operationCode !== 'aims.work-item.completion.workflow-submit.v1' || command.requiredCapability !== 'workflow:work-item-complete:create' || !(targetV1 || matterV2) || command.commandSha256 !== await hashServiceCommandPayload(command.command)) throw createError({ statusCode: 403, message: 'Completion signed command is invalid.' })
