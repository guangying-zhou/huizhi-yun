    ...
    <template #body>
      <div class="p-4 space-y-4">
        ...
        <div v-if="activeTab === 'pending'">
          ...
          >
            <template #empty>
              <CommonEmptyState icon="i-lucide-inbox" title="暂无待办任务" description="当前没有需要你处理的审批任务。" />
            </template>
        ...
        <div v-if="activeTab === 'done'">
          ...
          >
            <template #empty>
              <CommonEmptyState icon="i-lucide-circle-check-big" title="暂无已办任务" description="处理过的审批任务会显示在这里。" />
            </template>
            <template #instance_no-cell="{ row }">
              <span class="text-sm font-mono">{{ row.original.instance_no }}</span>
            </template>
            ...
            <template #created_at-cell="{ row }">
              <span class="text-sm text-(--ui-text-muted)">
                {{ formatDateTime(row.original.completed_at || row.original.created_at) }}
              </span>
            </template>
          </UTable>
