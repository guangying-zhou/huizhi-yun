  ...
  <UDashboardPanel grow>
    ...
    <div class="p-4">
      <UTable :data="list" :columns="columns" :loading="loading">
        <template #empty>
          <CommonEmptyState icon="i-lucide-zap" title="暂无动作定义" description="业务应用同步审批动作后会显示在这里。" />
        </template>
...
const loadList = async () => {
  loading.value = true
  try {
    const res = await $fetch<{ code: number, data: { items: ActionDef[], total: number } }>('/api/v1/admin/action-defs', {
      query: {
