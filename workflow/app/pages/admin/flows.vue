  ...
  <UDashboardPanel grow>
    ...
    <div class="p-4">
      <UTable :data="list" :columns="columns" :loading="loading">
        <template #empty>
          <CommonEmptyState icon="i-lucide-workflow" title="暂无流程定义" description="新建流程定义后可配置审批节点。" />
        </template>
...
const loadList = async () => {
  loading.value = true
  try {
    const res = await $fetch<{ code: number, data: { items: FlowSchema[], total: number } }>('/api/v1/admin/flow-schemas', {
      query: {
