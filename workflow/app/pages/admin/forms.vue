  ...
  <UDashboardPanel grow>
    ...
    <div class="p-4">
      <UTable :data="list" :columns="columns" :loading="loading">
        <template #empty>
          <CommonEmptyState icon="i-lucide-file-input" title="暂无表单定义" description="新建表单定义后可用于流程配置。" />
        </template>
...
const total = ref(0)
const { page, pageSize, resetPage } = useListPage({ pageSize: 20 })
const {
  search,
  debounced: debouncedSearch,
  flush: onSearchEnter
} = useDebouncedSearch({ onChange: resetPage })
...
const loadList = async () => {
  loading.value = true
  try {
    const res = await $fetch<{ code: number, data: { items: FormSchemaRaw[], total: number } }>('/api/v1/admin/form-schemas', {
      query: {
...
watch([page, debouncedSearch], loadList)
</script>
