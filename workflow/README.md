...
待办投影的 task generation、事务 outbox、Console CAS 与失败恢复契约见 [docs/actionable-lifecycle-effects.md](./docs/actionable-lifecycle-effects.md)。
...
- **认证**: Console OIDC 单点登录（企业微信 OAuth 仅在显式 `HZY_AUTH_MODE=legacy` 或 `HZY_LEGACY_AUTH_BRIDGE=true` 时作为兼容入口）
...
Workflow 通知 effects 是可操作合同：必须保留原业务 `targetAppCode + resourceCode + bizId`、Workflow instance/task identity 和稳定事件版本。BFF 使用 Console runtime 下发的签名 policy bundle application catalog，把 `biz_url` 绑定到 `actionTargetAppCode` 的 exact origin/home path 并生成 canonical absolute URL；不匹配时切换到 Workflow task/instance fallback，保持业务 target 不变。可信 catalog 或 Workflow 自身入口不可用时不得外发原始 URL，通知失败并保留 lifecycle/outbox 待重试；不得用标题、时间或随机数派生业务/幂等身份。完整字段见根目录 `docs/MODULE_CONTRACTS.md` 的 Workflow actionable notification 合同。
...
│   │   └── v1/
│   │       ├── action-defs/
│   │       │   └── sync.post.ts # 服务令牌校验后转发 tenant-runtime
│   │       └── authorization/
│   │           └── instance-conflict-explain.post.ts # 基于任务事实解释职责冲突
│   ├── middleware/
│   │   └── data-runtime.ts  # /api/v1 主路径转发 tenant-runtime
│   └── utils/
│       ├── db.ts            # 防误用桩，不得作为业务 DB fallback
│       ├── dataRuntime.ts   # tenant-runtime 调用与 effects 处理
│       ├── directoryRuntimeClient.ts # Console Directory Runtime 客户端
│       ├── initiatorContext.ts # 发起人目录上下文
│       └── workflowInstanceConflictExplanation.ts # Console 实例冲突解释编排
├── docs/
...
| POST | `/authorization/instance-conflict-explain` | 基于待办任务事实解释审批职责冲突 |

`POST /authorization/instance-conflict-explain` 请求体为 `{ taskId, action?, includeBaseline? }`，其中 `action` 支持 `approve`、`reject`、`delegate`，默认 `approve`。接口先校验当前用户具备 `workflow_tasks:view`，再从 tenant-runtime `/v1/workflow/tasks/{taskId}` 读取任务和实例事实，不读取 Workflow 本地 DB；解释输入中的 principals 由 `flow_instances.initiator_uid` 和 `flow_tasks.assignee_uid` 派生，并通过 Foundation `loadInstanceConflictExplanationFromConsoleRuntime()` 调 Console/Platform active 冲突规则返回结果。
