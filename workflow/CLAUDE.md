...
- 审批流转、条件路由匹配、回调日志和流程数据读写由 tenant-runtime/data-runtime 执行。
- `server/middleware/data-runtime.ts` — Workflow Nuxt BFF 的 `/api/v1/**` 主路径转发入口，负责 Console 登录、精确权限、委托人目录校验和发起人目录上下文注入。
- `server/utils/dataRuntime.ts` — 统一调用 tenant-runtime 并处理 runtime 返回的通知与业务回调 effects。
- `server/api/v1/action-defs/sync.post.ts` — 仅保留服务令牌校验入口，通过后转发 tenant-runtime 同步动作定义。

## 依赖的模块

- **Console**：SSO、目录运行时（用户/部门查询）；审批人解析通过 Foundation 受限共享投影，使用 `workflow.runtime` 的 `aud=console`、`console:directory-users:read` 服务授权
