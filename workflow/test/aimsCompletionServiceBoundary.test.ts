...
function boundary(mode = '') {
  ...
  runInNewContext(compiled, { exports, defineEventHandler: (handler: any) => handler, require: (name: string) => {
    ...
    if (name.endsWith('/tenantRuntimeClient')) return { hashServiceCommandPayload: async () => 'hash', verifyServiceCommandRuntimeHeaders: async (input: any) => { calls.push({ verify: input }); if (['missing-signature', 'wrong-signature', 'wrong-source-deployment'].includes(mode)) throw createError({ statusCode: 403, message: 'invalid signature context' }) } }
    if (name.endsWith('/initiatorContext')) return { collectWorkflowInitiatorContext: async () => { calls.push({ directory: true }); return {} } }
    if (name.endsWith('/dataRuntime')) return { maybeCallWorkflowDataRuntime: async (_event: any, path: string, options: any) => { calls.push({ runtime: { path, options } }); return { handled: true, data: { code: 0, data: { result: {} } } } }, runWorkflowRuntimeEffects: async () => [] }
    throw Error(name)
  } })
...
test('completion BFF invalid command, scope, binding or signature never dispatches directory or Runtime', async () => {
  for (const mode of ['no-cap', 'wrong-client', 'wrong-actor', 'wrong-operation', 'wrong-hash', 'wrong-tenant', 'missing-signature', 'wrong-signature', 'wrong-source-deployment']) {
