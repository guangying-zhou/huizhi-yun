...
> **当前事实边界（2026-07-11 核对）**：本文保留 Workflow 的早期流程模型与页面/表单设计，不是当前运行实现说明。当前用户认证走 Foundation 接入的 Console OIDC；用户、部门与审批人组织信息走 Console Directory；业务数据由 tenant-runtime/data-runtime 管理；跨应用调用及终态回调使用短期 Console `token_use=service` JWT、精确 capability 与受信 actor。本文的 Account、CAS、`Bearer {apiKey}:{apiSecret}`、本地 MySQL 或静态 webhook/key 描述仅属历史或显式兼容路径，不得作为新接入或部署依据。以根 `CLAUDE.md`、`docs/MODULE_CONTRACTS.md`、`workflow/CLAUDE.md` 与当前代码为准；未执行任何外部操作。
...
* **模块间调用**：调用方经 Foundation 获取 Console 签发的短期 service token；目标端校验 `token_use=service`、`aud`、精确 `scope`、来源应用、tenant 与 deployment。不得新增静态 API key/secret。

* **前端调用**：通过 Foundation `/api/workflow-proxy/**` 代理已验证的 Console OIDC 用户会话；代理以本应用 service token 调用 Workflow，并只传递服务端验证后的 actor。
...
| POST | `/authorization/instance-conflict-explain` | 解释待办审批职责冲突 | 前端 |
...
Workflow 任务详情页可在审批前解释当前任务是否命中 active 职责冲突规则。该接口只做诊断，不改变任务或实例状态。

**请求**

```
POST /api/v1/authorization/instance-conflict-explain
```
...
1. Nuxt BFF 先校验当前登录用户具备 `workflow_tasks:view`。
2. BFF 只从 tenant-runtime `/v1/workflow/tasks/{taskId}` 读取任务详情和实例事实；tenant-runtime 不可用时返回 503，不读取 Workflow 本地 DB。
3. 冲突解释 principals 由任务事实派生：`flow_instances.initiator_uid` 作为 `initiator`，`flow_tasks.assignee_uid` 作为 `assignee`。
4. BFF 通过 Foundation `loadInstanceConflictExplanationFromConsoleRuntime()` 调 Console/Platform active 规则，资源固定为 `workflow_tasks`，动作支持 `approve`、`reject`、`delegate`。
...
```json
{
  ...
  "data": {
    ...
    "taskId": "101",
    "instanceId": "42",
    "instanceNo": "WF202603180001",
    "action": "approve",
    "principals": [
      { "kind": "initiator", "uid": "zhangsan" },
      { "kind": "assignee", "uid": "lisi" }
    ],
    "explanation": {
      "allowed": true,
      "rules": []
