...
CREATE TABLE `flow_tasks` (
	...
	`generation_key` VARCHAR(191) NULL COMMENT '同一次节点进入创建的 task generation 身份',
	`actionable_key` VARCHAR(191) NULL COMMENT 'Console 待办投影的精确 task generation 身份',
	`actionable_version` VARCHAR(191) NULL COMMENT 'Console 待办投影 CAS 版本',
	...
	INDEX `idx_task_generation` (`instance_id`, `generation_key`),
	INDEX `idx_actionable_identity` (`actionable_key`, `actionable_version`),
    INDEX `idx_status_created` (`status`, `created_at`),
...
DROP TABLE IF EXISTS `flow_actionable_outbox`;
