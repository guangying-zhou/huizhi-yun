...
> **2026-09-05 实际部署覆盖说明：** 用户确认生产 Platform 位于 Cloudflare `https://huizhi.yun`；
> 已批准将旧 `hzy.wiztek.cn` PM2 入口改为开发 Platform（3011、`hzy_platform_dev`），
> `platform-dev.wiztek.cn` 以 308 跳转到该域名。Console / People 测试业务数据也已使用国内独立测试库，
> 不与日本生产库共用。当前操作依据为 [`deploy/test-env/README.md`](../deploy/test-env/README.md)，
> 下文历史共库方案、示例域名和通用路由命令不能直接覆盖这个环境。

> **Console 数据边界更新（2026-07-17）**
>
> 本文的 Platform prod/dev 隔离与路由内容仍有效；其中“Console 共用
> `hzy_console`、DB runtime cache、Hyperdrive、Console DB verifier、Console
> Vault/OIDC 私钥”等历史设计已被 ADR-017 和
> `console/docs/Console-Database-Split-and-Data-Runtime-Migration-Task-List.md` 取代。
> Console 现在只能通过 Tenant Runtime 访问租户数据，严格验收会拒绝所有
> `--console-db-*` 参数。生产切换使用
> `console/docs/Console-Tenant-Runtime-Wiztek-Cutover-Runbook.md`。
...
> **文档状态与证据边界**
>
> - **状态 / 参考范围：** 落地中参考方案；说明 Platform/Console prod、shared test 与本地开发的隔离要求，不替代各环境当前激活材料或运行配置。
> - **最后核验：** 2026-07-11。
> - **当前权威来源：** [`platform/.env.prod.example`](../platform/.env.prod.example)、[`platform/.env.dev.example`](../platform/.env.dev.example)、[`console/.env.prod.example`](../console/.env.prod.example)、[`console/.env.dev.example`](../console/.env.dev.example)、[`scripts/validate-platform-environment-isolation.mjs`](../scripts/validate-platform-environment-isolation.mjs) 与 [`docs/MODULE_CONTRACTS.md`](MODULE_CONTRACTS.md)。
> - **执行边界：** 本地或离线验证结果不构成数据库迁移、Cloudflare 部署、live probe 或真实租户操作的授权；这些操作须获得单独、明确的环境授权。
...
| Platform Cloudflare 配置漂移导致控制面不可用 | wiztek Platform 默认发布到 Cloudflare；`pnpm run deploy:cloudflare` 必须先完成 lint、typecheck、测试、build 和 dry-run；默认域名固定为 `huizhi.yun` / `www.huizhi.yun`，数据库使用 Hyperdrive；真实 `platform/.env.cloudflare` 不跟踪进 git。PM2/Nginx 仅作为明确固定 IP 场景的可选 profile |
...
2. `validate:runtime-acceptance-strict`，证明可选 PM2 隔离 profile 的 strict 验收不会接受缺失真实运行态、PM2 fixture、共享 release workdir、跳过公网 HTTP、废弃的 `--allow-platform-cloudflare` 参数或 legacy unscoped cache 等捷径。
...
9. `validate:platform-cloudflare-default`，证明 wiztek Platform 默认 Cloudflare 域名、Hyperdrive 和部署前置检查完整，且不再依赖 legacy override。
...
12. `probe:public-routing`，用于可选 PM2 profile 时证明 `platform.wiztek.cn` / `platform-dev.wiztek.cn` 指向自托管入口；默认 Cloudflare Platform 则以 `huizhi.yun` / `www.huizhi.yun` 和 Wrangler 部署结果为准。
...
9. Platform 的 `deploy:cloudflare` 是 wiztek 默认发布入口，并由根目录 `pnpm run validate:platform-cloudflare-default` 纳入统一验收；命令必须先通过 lint、typecheck、测试、Cloudflare build 和 dry-run，再发布绑定 `huizhi.yun` / `www.huizhi.yun` 且使用 Hyperdrive 的 Worker。PM2/Nginx 仅作为明确固定 IP 场景的可选 profile。
...
28. 根目录提供 `pnpm run accept:runtime-isolation` 统一验收入口，用于可选 PM2/Nginx profile 的严格 env、签名、进程、server upstream loopback probe、公共路由和运行态验收，同时执行 Platform Cloudflare 默认配置校验；默认 Cloudflare Platform 发布由模块 `deploy:cloudflare` 自带的 preflight、build 和 dry-run 门禁负责。
...
- [ ] wiztek Platform 已通过 `pnpm --dir platform run deploy:cloudflare` 发布到 Cloudflare，且无需 override；Worker 绑定 `huizhi.yun` / `www.huizhi.yun`、使用 Hyperdrive，并通过完整 preflight 与 dry-run。
...
- [ ] 可选 PM2 profile 的 `pnpm run validate:runtime-acceptance-strict` 通过，且 strict 验收仍拒绝 tracked `*.example` env 文件、PM2 fixture、跳过公网 HTTP、废弃的 `--allow-platform-cloudflare` 参数、共享 release workdir和 legacy unscoped Console cache。
- [ ] `pnpm run validate:runtime-probe-guardrails` 通过，且本地 fixture 证明运行态 probe 保留 prod/test/dev pairwise 串环境检查。
- [ ] `pnpm run validate:platform-cloudflare-default` 通过，且仓库不跟踪真实 `platform/.env.cloudflare`。
