...
源码记录覆盖 enterprise、aims、assets、foundation（含其 packages）、platform/packages 和 data-runtime；同时记录根 package.json、pnpm-lock.yaml、pnpm-workspace.yaml 的 Git blob。生成器检查这些实际依赖路径的未提交改动，避免共享层或依赖版本变化未进入发布记录。该记录仍不能单独证明二进制由对应源码构建，正式构建需保留固定提交与构件 hash 的关联证据。
