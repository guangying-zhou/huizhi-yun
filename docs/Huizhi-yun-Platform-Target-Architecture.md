...
- `subject_id`
- `subject_type`（`user / department / committee / job / project`；`job` 与 `project` 语义严格分离）
- `tenant_code`
- `subject_code`
- `external_ref`
...
目录服务定位：`console` 承接汇智云应用侧的企业目录投影与访问主数据能力。它可以直接维护目录，也可以从 LDAP / AD、企业微信、表格导入等技术目录源同步；钉钉作为 HR 事实源时，由 People 管理同步决策并将正式行政部门、人员主归属和在离职事实投影到 Console。业务应用只面向 `Console Directory API`，不直接依赖具体上游目录系统。

这意味着：

- 对没有 LDAP / AD 的团队，`console` 可以作为企业目录主数据源。
- 对已有 LDAP / AD、企业微信的团队，`console` 作为技术目录适配层；对钉钉 HR 事实源，People 负责业务入口与映射决策，Console 负责 canonical Directory 投影与访问生命周期。
...
- `console/docs/sql/Console-SQL-DDL-Draft-v1.sql`
- `console/docs/Console-API-Contract-v1.md`
- `console/docs/Console-Bootstrap-and-Rotation-Sequence-v1.md`
- `console/docs/Console-Vault-Credential-Management-Plan.md`
...
仅列出当前主线 schema 的关键对象簇，完整 DDL 以 `platform/docs/sql/HZY-Platform-SQL-DDL-Draft-v2.sql` 为准（已并入 v2.4 应用版本、订阅计划与 manifest action 快照约束）。
...
| 文档 | 状态 | 定位 |
|------|------|------|
| `platform/docs/HZY-Platform-Schema-Draft-v2.md` | Draft v2 | 当前三域 schema 主设计 |
| `platform/docs/sql/HZY-Platform-SQL-DDL-Draft-v2.sql` | Draft v2 + v2.1 | 当前主线 DDL |
| `console/docs/Console-Functional-Design-v1.md` | Draft | `console` 功能边界与子域设计 |
| `console/docs/sql/Console-SQL-DDL-Draft-v1.sql` | Draft | `hzy_console` 本地库 DDL 草案 |
| `console/docs/Console-API-Contract-v1.md` | Draft | `console` 本地管理面与服务间接口契约 |
| `console/docs/Console-Bootstrap-and-Rotation-Sequence-v1.md` | Draft | `console` 初始化、轮换与 reveal/resolve 时序 |
| `console/docs/Console-Vault-Credential-Management-Plan.md` | Draft | `console` vault 凭证分类、零配置运行链路与实施计划 |
| `console/docs/Console-Auth-Runtime-IdP-Implementation-Plan.md` | Draft | `console` 升级为 auth-runtime / OIDC IdP 的实施计划 |
| `console/docs/Console-Workflow-Runtime-Integration-Plan.md` | Draft | `workflow` 并入 `console.workflow-runtime` 实施方案 |
| `platform/docs/HZY-Platform-Schema-Addendum-v2.1.md` | Draft v2.1 | 当前增量模型补遗 |
...
| `platform/docs/HZY-Platform-ERD-v2.md` | Draft | 当前主线 DDL 对应 ER 图 |
