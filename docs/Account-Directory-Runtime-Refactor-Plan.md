...
状态：历史/目标迁移设计参考（非当前发布、切流或停机验收依据）
原始日期：2026-04-26
最后事实核对：2026-07-11
定位：迁移设计参考，配套 `Directory-Runtime-Contract.md`、`console/docs/Console-Directory-Runtime-Integration-Plan.md`、`Huizhi-yun-Platform-Target-Architecture.md`

当前事实源：[`Development-Execution-Checklist-2026-07.md`](./Development-Execution-Checklist-2026-07.md)、[`MODULE_CONTRACTS.md`](./MODULE_CONTRACTS.md)、[`console/docs/Console-Directory-Runtime-Integration-Plan.md`](../console/docs/Console-Directory-Runtime-Integration-Plan.md)、Console `CLAUDE.md` 与 Foundation `server/utils/directoryApi.ts`。本文件记录的 2026-04 阶段、mirror、shadow compare 和兼容设想均不是完成证据；真实 Account 导入/对账、业务切流、Account 停机演练、migration、Cloudflare 部署和真实租户验收必须分别获得授权与证据。

已核对的安全边界：新 Foundation directory adapter 仅使用 Console Directory API，授权或配置失败必须 fail-fast；`401/403` 是授权决定，绝不可回退到 Account 或更弱兼容路由。`HZY_CONSOLE_CLIENT_ID/SECRET` Basic 不是已实现的跨应用目录读取授权合同；无人值守读取须先建立 Console 签发的短期 service token 与细粒度 capability。Account 仅作为未迁移旧调用的 legacy facade/迁移源，不能成为新 adapter 的 fallback。
...
- **迁移完成后，`account` 只保留 legacy 兼容 facade 或直接下线；新目录 adapter 不以它作为 fallback**
...
以下是 2026-04 的迁移起点，不代表 2026-07 的当前主路径。当前新 Foundation directory adapter 已只读 Console；未迁移的旧 `/api/account/**` 兼容调用才可继续使用 Account。

- 当时的 Foundation store / composable 名称：
...
**`console.directory-runtime` 的迁移源与 legacy 兼容 facade。**

一句话定义：

- `console.directory-runtime` 负责“人、部门、岗位、项目注册表长什么样”
- `account` 在迁移期仅为未迁移旧路径继续提供兼容 API
- `platform` 负责“这些主体对哪些应用有什么权限”
- 新接入业务应用从 `platform` 取授权结果，并通过 Foundation directory adapter 从 `console.directory-runtime` 取目录信息；Console 授权/配置失败不得 fallback Account
...
- Foundation 可保留历史 `useAccount*` 名称作为兼容外观，但新 directory adapter 只调用 Console；旧 Account 调用与新 adapter 不可混合或自动切换
...
下列为迁移起点的历史消费面；当前 `useAccount*` 可作为兼容外观委托目录层，但新 directory adapter 不直接依赖 Account：
...
### Phase 3：Account 数据导入/对账与切流（未完成的历史目标）

- Account → Console 的受控导入、hash 对账和切流方案尚未获得真实数据验收，不得表述为已有定时 mirror。
- 当前主路径仅有 Console 启动时 `POST /api/v1/runtime/subjects/sync` 的最小 subject sync；它不是本阶段设想的 Account pull/push worker，也不证明 Account 数据已完成导入或对账。
- 如后续需要 shadow compare，必须单独设计为受控、脱敏、无浏览器回退的运维验证；不能把它接入业务读取请求。

### Phase 4：Console 成为目录主读入口（目标）

- Foundation 新 directory adapter 只读 Console，授权或配置失败 fail-fast
- Account 仅服务于未迁移 legacy 路径，不是新 adapter fallback
...
### 2026-04-26：Console Directory Runtime 只读 API 与兼容 facade（历史实施记录）
...
该历史记录不证明当前真实数据迁移、Account 导入/对账、切流或停机验收已完成；当前状态以前言事实源为准。当时记录不包含：
