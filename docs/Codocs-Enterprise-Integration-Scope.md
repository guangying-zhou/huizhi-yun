...
日期：2026-09-19。依据：用户确认“个人文档先行、部门文档随后；旧 Codocs 项目文档暂不迁移，后续在项目管理实现”，并进一步明确“把整个我的文档一并迁移”。第一批包含完整 `mydocs` 页面树及可达的既有操作，不再裁剪为基础文档子集。本文是范围与实施输入，不表示页面、接口或身份已经接通。
...
- 编辑器仍复用 `codocs/app/pages/documents/[uuid].vue` 对应的现有独立页面，通过受控 iframe 或明确兼容入口打开；不在首批搬入 Milkdown、重写 Y.js 或合并 Collab。保留短期协作 token、文档 ACL 与保存/断线恢复规则。
...
| 批次 | 源页面/能力 | 迁移边界 |
| --- | --- | --- |
| 第一批：全部我的文档 | `codocs/app/pages/mydocs/index.vue`、`favorites.vue`、`recently.vue`、`recycle.vue` | 私人文件夹与文档列表、创建、上传、预览/打开编辑器、重命名、移动、收藏、最近访问、下载、软删除及恢复；分享入口复用现有能力，包含权限授予和撤回验证 |
| 同属第一批 | `mydocs/cabinet.vue`、`shared.vue` | 文件柜上传/预览/下载/删除/转文档，共享发现、移交及页面实际可达的发文/收发/盖章操作；外部依赖必须按现有合同接通，不因依赖复杂排除 |
| 同属第一批 | `mydocs/journal.vue`、`worklogs.vue`、`weekly-reports.vue`、`slides.vue` | 日志/工作日志/个人周报的创建、提交、修订与关联入口，演示文稿创建/编辑/预览/导出/删除；包含未列入菜单但可达的兼容页面 |
...
| 功能 | 目标主入口 | 交付批次 |
| --- | --- | --- |
| 私人文档、文件柜、共享、最近/收藏/回收站、演示 | 文档 → 我的文档 | 第一批，保留全部既有动作 |
...
- 下一步先固定逐动作 API/capability 与主路由、兼容路由对照，再由 Luna 承接窄范围注册和测试；主代理负责权限与集成。首批内部先打通文档基础及编辑器，再完成工作汇报、演示和共享副作用，最后统一验收，不将内部顺序变成范围裁剪。
...
- 页面验收：整个 mydocs 页面树逐页逐动作验证，包含文件柜、最近/收藏/回收站、共享/移交/发文相关动作、日志/周报和演示文稿；原编辑器保存后回列表能读取最新内容；旧 URL 兼容；桌面 1440px、移动 390px；明确区分空列表、无权、未配置和依赖故障。
...
完成标准是整个“我的文档”的真实闭环，不是新增一个菜单、只迁基础列表、让旧个人页面出现在 iframe 中或让构建通过。第一批验收前不继续铺开部门/独立公司管理/项目页面；mydocs 已有动作所必需的外部依赖仍在第一批范围内。
