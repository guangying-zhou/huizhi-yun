...
> **文档状态与证据边界**
>
> - **状态 / 参考范围：** 已实现检查点的收敛参考；描述环境变量职责边界，不替代各模块当前 `.env` 示例、Cloudflare 配置生成器或运行时契约。
> - **最后核验：** 2026-07-11。
> - **当前权威来源：** [`console/.env.cloudflare.example`](../console/.env.cloudflare.example)、[`codocs/.env.cloudflare.example`](../codocs/.env.cloudflare.example)、[`platform/.env.cloudflare.example`](../platform/.env.cloudflare.example)、[`scripts/validate-env-tracking.mjs`](../scripts/validate-env-tracking.mjs)、[`scripts/validate-cloudflare-deploy-gates.mjs`](../scripts/validate-cloudflare-deploy-gates.mjs) 与 [`docs/MODULE_CONTRACTS.md`](MODULE_CONTRACTS.md)。
...
Console 自身也不再消费 `HZY_ACCOUNT_API_URL`、`HZY_ACCOUNT_API_KEY` 或 `HZY_ACCOUNT_API_SECRET`，两个 Console env 示例均不再声明这三项。legacy Account 仅保留为显式 directory-sync 导入源，不是 Console 运行时上游或兼容回退。
