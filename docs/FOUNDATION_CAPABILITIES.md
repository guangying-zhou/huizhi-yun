...
| 工具 | 用途 |
|------|------|
| `callEnterpriseRuntime('codocs.personal-document-update-plan', …)` | 固定 `POST /v1/enterprise/codocs/personal-documents:update-plan`，复用精确 `codocs:personal-documents:edit`；携带原 Idempotency-Key、签名 actor 及短期 edit permit。Runtime 只读核对当前写 ACL 与完整命令成功回执，Host 在 OSS 前短路已完成请求；不是预占锁，不保证未提交并发写安全。与原 update 共享命令摘要，不新增 grant；候选未部署。 |
| `callEnterpriseRuntime('codocs.personal-document-snapshot-{prepare,publish,read}', …)` | v2 快照受信路由：`POST /v1/enterprise/codocs/personal-documents:snapshot-prepare`/`:snapshot-publish`（精确 `codocs:personal-documents:edit`，须 Idempotency-Key）与 `:snapshot-read`（`:read`），不新增 grant。Runtime 仅在 `apps.codocs.snapshotV2Enabled=true`（默认 false）时登记；publish 先核验 bucket 对 `codocs/snapshots/` 的写一次保留（成功缓存 10 分钟），再经绑定租户/部署/命名空间的读取器逐字节核对精确版本后原子发布。尚无 Host 调用方，见 [写入协调合同](./Codocs-Document-Write-Coordination.md)。 |
