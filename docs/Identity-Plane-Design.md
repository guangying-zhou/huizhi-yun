...
日期：2026-04-29  
定位：目标设计，作为 `Huizhi-yun-Platform-Target-Architecture.md` 的配套文档；企业应用用户 IdP 详见 `console/docs/Console-Auth-Runtime-IdP-Implementation-Plan.md`

> 状态：目标设计 / 历史决策参考（非当前部署或生产验收依据）
> 最后事实核对：2026-07-11。当前事实源为根与模块 `CLAUDE.md`、`docs/MODULE_CONTRACTS.md`、`console/docs/Console-Auth-Runtime-IdP-Implementation-Plan.md`、Console OAuth/OIDC 实现与 Platform `/api/v1/**` 路由。代码或离线测试不等同于 migration、secret 配置、Cloudflare 部署、真实会话或租户验收；这些外部操作均须另行授权。
>
> Wiztek 当前生产上游 Keycloak、OIDC Client 与 LDAP User Federation 的脱敏配置及运维步骤见
> [`Keycloak-Wiztek-Production-Configuration-Runbook.md`](./Keycloak-Wiztek-Production-Configuration-Runbook.md)。
> 该 Runbook 只记录当前部署事实，不改变本文的目标架构边界。
...
Console 登录成功后，业务应用通过 token 中的 `hzy.subjectCode` / `hzy.uid` 与 Platform policy bundle 中的主体稳定标识对齐授权主体。当前主 bundle schema 为 `policy-bundle.v2`，应消费 `roleAssignments` 等 v2 字段；下文 `subjectRoles` 表述仅保留为历史 MVP 快照，不应被新实现复用。
...
sequenceDiagram
  ...
  A->>P: Pull signed policy bundle through trusted runtime path when needed
...
4. 历史 bundle 用 `subjects.subjectCode` 与 `subjectRoles.subjectCode` 下发授权；当前 `policy-bundle.v2` 使用 `roleAssignments`、`rolePermissionGrants`、`assignmentScopes`、`roleDefaultScopes` 与 `baselineGrants` 等字段，仍以稳定主体标识对齐。
...
本节是 Platform runtime/control token 的目标 JWT 模型；业务应用用户 OIDC token 由 Console 签发，详见 `console/docs/Console-Auth-Runtime-IdP-Implementation-Plan.md`。当前 Platform runtime 路由仍保留 opaque `hzy_rt_...` credential 的显式离线/兼容校验实现；这不构成在托管云、共享测试或生产环境使用静态 token 的授权。
