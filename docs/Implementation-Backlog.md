...
> **历史执行记录与证据边界**：最后事实核验：2026-07-11。当前事实源：[开发执行清单](Development-Execution-Checklist-2026-07.md)、[综合运营路线图](Huizhi-yun-Integrated-Operations-Roadmap.md)、[跨模块契约](MODULE_CONTRACTS.md)、根与模块 `CLAUDE.md` 及当前代码/契约。本文的任务状态和历史“完成”表述不等于当前发布、部署或真实租户验收证据；migration/seed、Cloudflare 发布、secret 配置、真实会话/请求和回滚操作均须按目标环境逐项、明确授权。
...
- `platform/docs/Platform-SDK-API-Draft.md`
...
- `platform/docs/Platform-Adapter-Nuxt-API-Draft.md`
...
- `foundation/docs/Foundation-Split-and-Transition-Plan.md`
- `foundation/docs/Foundation-Auth-Code-Split-Checklist.md`
- `foundation/docs/Foundation-File-Migration-Task-List.md`
...
- `aims/docs/AIMS-Integration-and-Authorization-Implementation-Plan.md`
- `aims/docs/AIMS-Initial-Integration-Change-Points.md`
...
- `codocs/docs/Codocs-Compatibility-Integration-Plan.md`
...
- 生成 bundle 后能看到 `roles / roleAssignments / rolePermissionGrants` 包含对应数据
...
- [ ] E8-T16：补 Console 权限消费验证
  ...
  - 验证未授权用户无法进入受控页面
  - 部分完成：新增 `console/test/consolePolicyConsumption.test.ts`，源码回归锁定菜单过滤、前端路由守卫、服务端页面守卫、`/api/auth/permissions` 与 `requirePermission` 共同消费本地 policy bundle 授权快照；本轮进一步抽出 `buildPolicyAuthorizationSnapshotFromPayload()`，让 `loadPolicyAuthorizationSnapshot()` 的生产路径和本地回归共享同一 policy payload → snapshot 构建器，并锁定 `org_profile:view` 同时驱动 `/org-profile`、`/admin/business-domains` 与服务端 `business-domains` API 守卫；仍待真实授权数据环境执行 runbook 第 4-6 节并留档
...
当前进度：E8-T12 至 E8-T15 已完成。Console 新增本地 policy bundle 授权解析器，`/api/auth/permissions` 与服务端 `checkPermission/requirePermission` 已切换为读取本地已验签 bundle，不再依赖 Account 权限 API；启动流程会在有缓存时主动刷新 bundle，并根据 heartbeat `download_bundle` action 拉取新版本。E8-T16 已补源码回归与验收脚本：`console/test/consolePolicyConsumption.test.ts` 锁定菜单过滤、前端路由守卫、服务端页面守卫和 API 守卫消费同一权限快照，本轮又锁定 `loadPolicyAuthorizationSnapshot()` 复用 `buildPolicyAuthorizationSnapshotFromPayload()` 并把 `org_profile:view` 串到菜单、路由和服务端 API 守卫；`console/docs/Console-Policy-Authorization-Acceptance-Runbook.md` 与 `accept:platform-policy-roles` / `accept:console-policy-consumption` 覆盖真实环境验收流程；剩余动作是在真实授权数据环境执行一次端到端验收并按手册第 6 节留档。
...
- [x] E8-T19：支持“复制全局角色为自定义角色”
  已实现 `POST /api/platform/tenant-admin/roles/{id}/copy`，仅允许复制平台继承/系统角色，生成 `tenant_roles(source='custom', role_type='custom', parent_id=<sourceRoleId>, source_role_code=<sourceRoleCode>)`，并复制源角色的 `tenant_role_app_role_maps`、额外 `tenant_role_permissions` 与 `tenant_role_scopes` 后刷新角色 policy hash；Dashboard `RolesManager` 在选中平台继承角色时提供“复制为自定义角色”入口，复制后自动选中新角色并加载权限配置。
...
当前进度：E8-T17 至 E8-T20 已完成（按 v2.15 企业角色 / 应用权限角色拆分后的模型落地，见 `platform/docs/sql/HZY-Platform-SQL-Migration-v2.15-enterprise-role-split.sql`）。自定义角色可从零创建，也可从平台继承角色复制后继续编辑应用权限角色、高级权限和 scope。
...
- [x] E8-T21：更新权限治理相关文档
  已完成：权限治理设计已收敛到 `platform/docs/Platform-Role-Permission-Design.md`（覆盖 v2.15 企业角色模型）；`docs/Control-Plane-API-Contract.md` 已同步 `policy-bundle.v2`、tenant-admin 角色治理、复制平台继承角色为自定义角色、app-role 映射和高级权限/scope 契约；`docs/Platform-Console-MVP-Integration-Plan.md` 已补充当前授权治理契约补记，明确 v2.15 企业角色拆分、`tenant_role_app_role_maps` 和 `policy-bundle.v2` 为当前主路径。
- [x] E8-T22：补充必要 SQL migration/seed
  已完成：`platform/docs/sql/HZY-Platform-SQL-Migration-v2.15-enterprise-role-split.sql`（含 resume 脚本）、`platform/docs/sql/HZY-Platform-SQL-Seed-v2.16-enterprise-roles.sql`（企业标准角色与 console 旧角色清理）、`platform/docs/sql/HZY-Platform-SQL-Migration-v2.9-role-policy-hashes.sql`。
- [x] E8-T23：补 Platform API 测试/验收脚本
  已完成：`scripts/accept-platform-policy-roles.mjs`（`pnpm run accept:platform-policy-roles`），覆盖全局角色存在性、物化到租户、可分配角色、subject 授权、bundle 生成与 payload 断言。用法见 `console/docs/Console-Policy-Authorization-Acceptance-Runbook.md`。
- [x] E8-T24：补 Console 权限消费测试/验收脚本
  已完成：`scripts/accept-console-policy-consumption.mjs`（`pnpm run accept:console-policy-consumption`），覆盖未登录 401、bundle resolver 权限快照、`requirePermission` 保护接口与未授权用户反向验证；`console/test/consolePolicyConsumption.test.ts` 覆盖菜单过滤、路由守卫、服务端页面守卫和 API 守卫共享同一权限快照的源码契约，并锁定 `org_profile:view` 经统一 snapshot builder 驱动 Console 受控页面与服务端 API 守卫；UI 行为仍按 runbook 第 5 节在真实环境核对。
