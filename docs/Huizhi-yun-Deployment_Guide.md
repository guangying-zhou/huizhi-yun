...
>
> - **最后事实核验：** 2026-07-11。
> - **当前事实源：** [`Huizhi-Yun-Cloudflare部署指南.md`](Huizhi-Yun-Cloudflare部署指南.md)、[`deploy/cloudflare/tenant-gateway/README.md`](../deploy/cloudflare/tenant-gateway/README.md)、[`data-runtime/README.md`](../data-runtime/README.md)、[`data-runtime/deploy/install.sh`](../data-runtime/deploy/install.sh)、[`MODULE_CONTRACTS.md`](MODULE_CONTRACTS.md) 与 [`Development-Execution-Checklist-2026-07.md`](Development-Execution-Checklist-2026-07.md)。
> - **执行边界：** 本文描述的 profile 或命令不证明任何环境已配置或上线；Cloudflare 部署/secret 或 DNS 变更、数据库 migration/seed、R2/runtime 发布、客户侧 Agent 安装、live probe 与真实租户切换均须按目标环境逐项、明确授权。
...
- Data Runtime static token 由 Platform 生成，企业可在 `/dashboard/deployments` 生成或轮换一键安装指令。指令会包含 `HZY_DATA_RUNTIME_TENANT`、`HZY_DATA_RUNTIME_DEPLOYMENT`、`HZY_DATA_RUNTIME_STATIC_TOKEN` 和当前环境 active deployment 对应的 `HZY_<APP>_AGENT_ENABLED` 标记。该 token 只用于 Agent 安装、enrollment 和明确的离线兼容；共享 Platform resolve 不读取、不返回 bootstrap static token。
- Agent 服务器侧变量名是 `HZY_DATA_RUNTIME_STATIC_TOKEN`。
- gateway 只在受信边界注入 tenant、deployment、environment、app 以及 Runtime endpoint/audience；共享托管路径不注入长期 Runtime token。业务 Worker 使用受信 runtime app identity 向 Console 申请短期 service token。
...
> 2026-07-10 安全更新：以下历史“一键 pipe 到 sudo bash”方式已经停用。
> 当前流程必须先下载 `install.sh` 与 `install.sh.sig`，使用独立于 R2 下发的
> Ed25519 public key 验签，再运行本地 installer；完整命令见
> `data-runtime/README.md` 的 Signed Install / Upgrade。自定义下载源还必须进入
> runtime HTTPS allowlist，不能由 HTTP 更新请求指定。

推荐从租户 Dashboard `/dashboard/deployments` 复制平台生成的一键安装指令完成首次安装。当前指令不再包含长期 static token，而是包含默认 15 分钟有效、只能兑换一次的 enrollment code，并固定 Platform 已批准的精确 Runtime 版本。命令先下载独立提供的 release public key，校验 SPKI SHA-256 和精确版本安装器签名，验签成功后才进入 `sudo`。数据库地址、端口、账号和密码仍由安装脚本在服务器本地交互输入并测试，不上传 Platform。

兑换成功后，Agent 持久化入站兼容 token、独立 control token、Runtime instance code 和各应用 deployment binding；Platform 只保存 token hash。Agent 每 5 分钟向 Platform 上报当前版本、release key id、数据库连通性和应用 Schema 状态。Tenant Gateway 仅在 Runtime 为 `ready` 且应用达到 `schema_ready` 后下发 endpoint。

只做升级时，先验证发布到 R2 的安装脚本：
...
openssl pkeyutl -verify -rawin -pubin \
  ...
  -in install.sh -sigfile install.sh.sig
...
- Agent 模式下的租户域名、自定义域名和 Agent endpoint 由企业在 Dashboard 维护；static token 由 Platform 生成但仅用于 Agent 安装/离线兼容。`hzy-tenant-gateway` 向业务 Worker 注入 endpoint/audience 和可验证的运行时上下文，不注入长期 token。
...
4. 企业在 `/dashboard/deployments` 生成或轮换一键安装指令。Platform 保存的 `deployment_bootstrap_secrets.secret_code='data-runtime.static_token'` 仅用于 Agent 安装/enrollment 和明确的离线兼容，共享 Gateway resolve 不读取它。
...
- `missing_bearer_token`：Worker 没有发送 Bearer token，通常是未解析到 Runtime endpoint，runtime app identity 不完整，或向 Console 申请短期 JWT/service token 失败。
- `invalid_static_token`：仅在显式启用离线/专属兼容时表示 Agent `.env` 中的 `HZY_DATA_RUNTIME_STATIC_TOKEN` 与安装或离线 registry 不一致；共享托管路径不应发送 static token。
