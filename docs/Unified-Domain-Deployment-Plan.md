...
> **文档状态与证据边界**
>
> - **状态 / 参考范围：** 规划参考；定义统一域名与 path 路由目标模型，不替代当前 Tenant Gateway 路由、Platform deployment 设置或发布手册。
> - **最后核验：** 2026-07-11。
> - **当前权威来源：** [`deploy/cloudflare/tenant-gateway/README.md`](../deploy/cloudflare/tenant-gateway/README.md)、[`docs/Huizhi-Yun-Cloudflare部署指南.md`](Huizhi-Yun-Cloudflare部署指南.md)、[`docs/MODULE_CONTRACTS.md`](MODULE_CONTRACTS.md)、`deploy/cloudflare/tenant-gateway/wrangler.jsonc` 与 Platform deployment 配置/迁移契约。
