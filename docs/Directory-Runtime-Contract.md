...
状态：Draft  
日期：2026-04-23  
定位：目标设计，作为 `Huizhi-yun-Platform-Target-Architecture.md`、`platform/docs/sql/HZY-Platform-SQL-DDL-Draft-v2.sql` 与 `Control-Plane-API-Contract.md` 的配套文档

> **当前事实边界（2026-07-11 核对）**：本文保留 2026-04 的目标设计与迁移设想，不得把其中 Account mirror/fallback、Basic/static credential 或独立 directory-runtime 进程描述当作当前主路径。当前新接入由 Foundation Console Directory adapter 读取 Console directory-runtime；适配器不提供 Account fallback，`401/403`、配置或目录不可用均失败关闭。用户认证为 Console OIDC，跨应用服务调用使用短期 Console `token_use=service` JWT；legacy Account 仅服务未迁移兼容调用或显式导入。以根 `CLAUDE.md`、`docs/MODULE_CONTRACTS.md`、`console/docs/account-migration-status.md` 和当前代码为准；文档核对不授权迁移、导入、部署或真实目录操作。

配套迁移清单：`console/docs/Console-Directory-Runtime-Integration-Plan.md`、`Account-Directory-Runtime-Refactor-Plan.md`
...
本文起草时，`account` 被规划为迁移阶段的目录事实源、兼容 API 和迁移源；当前主路径已是 `console` 内的 `directory-runtime` 逻辑域，`account` 只保留未迁移调用的兼容与显式导入语义。
...
- 当前新接入已通过 Foundation directory adapter 访问 Console，而不是直连或 fallback 到 Account
...
- 业务模块新服务调用使用 Console 签发的短期 `token_use=service` JWT 和精确 capability
- `Authorization: Bearer {api_key}:{api_secret}` 仅是历史兼容格式，不得为新目录调用配置或恢复
...
| `deptCode` | 否 | 按主部门筛选 |
| `page` | 否 | 管理列表页码，从 1 开始；与 `pageSize` 同时用于偏移分页 |
| `pageSize` | 否 | 管理列表每页数量，默认 20，最大 100 |
| `cursor` | 否 | 同步/兼容调用的游标分页（管理列表优先使用 `page/pageSize`） |
| `limit` | 否 | 同步/兼容调用的批量上限 |
...
- 管理列表支持 `page` / `pageSize`，响应返回 `items`、`flat`、`total`、`page`、`pageSize`
- 未传 `page/pageSize` 时保留全量注册表响应，供同步与兼容调用使用；新管理页面不得依赖该模式
...
- 返回用户可见或参与的项目注册表摘要
- `parentId` / `parent_id` 用于按父项目组精确筛选，并且必须在结果上限之前下推到数据库查询
- 用户对父项目组的负责人或 active 成员关系会继承到该项目组的直属子项目；直属项目负责人或 active 成员关系仍然有效
- 父项目组继承只扩大到该父组的直属子项目，不得退化为登录用户可读取全局项目注册表
...
以下是本文起草时的第一阶段迁移设想，不是当前运行策略：

- Account 可继续服务尚未迁移的 legacy 调用
- 新契约与新调用面已收敛到 Console `directory-runtime`
- 不得为新 Foundation adapter 增加 Console→Account mirror 或 fallback
- 新业务只认标准目录契约，不把 `account` 当成平台控制面的一部分
...
| `account`（legacy） | 未迁移调用的兼容 API、显式导入源 | 新增平台治理能力、新 Foundation adapter fallback、长期必选运行时职责 |
...
- 取姓名、部门、项目归属等目录字段时，调用 Foundation directory adapter；目标固定为 `console.directory-runtime`，不可 fallback Account
...
## 11. LDAP 托管读写与 Directory Connector

Cloudflare 上的共享 Console 不直接连接租户 LDAP，也不执行 LDAP 全量扫描、逐用户写库或
高频命令 lease。LDAP/AD 访问由客户网络内 `hzy-data-runtime directory-connector` 子进程完成：
它与业务 runtime 使用同一签名发布包，但使用独立 Linux 用户、独立 systemd unit、独立
Console service client 和本机 RSA 私钥。数据库访问统一收口到主 data-runtime 的 Directory
adapter，Connector 本身不持有数据库账号。

安全与可靠性约束：

- Connector 只发起到 tenant Console、LDAP 和本机 loopback data-runtime 的连接，不开放新的公网端口。
- 安装命令携带 Platform 签名、短期、单次 enrollment token；Console 将 token 绑定到可信
  tenant 和 Console deployment 后签发 Connector credential。
- LDAP bind password 使用 Console Vault `db_encrypted` 保存；Console 解析后只以 Connector
  公钥 RSA-OAEP-SHA256 密文下发。
- Connector 在企业服务器以 `0600` 缓存 connector id 和上述密文配置，不缓存 LDAP 明文密码；
  Console 配置接口暂时不可用时继续使用本地密文缓存，失败刷新至少退避 300 秒，不得按 3 秒
  命令轮询间隔重试。
- LDAP 全量读取只响应目录同步页显式提交的 `console.directory-connector.sync-now.v1` 命令；
  Connector 启动和周期 tick 均不得自动同步。周期任务只允许低频刷新加密配置及轮询本机命令。
- 新建用户初始密码以及 profile 修改密码的当前/新密码只以密文进入
  `integration_operation.command_json`，不得写日志或 API 响应。
- Connector 对 `/runtime/internal/directory-connector/**` 的每个请求使用 enrollment RSA 私钥做
  RSA-PSS-SHA256 签名；data-runtime 从 `directory_connectors` 读取公钥并校验 tenant、Console
  deployment、时间戳、body digest 与 nonce，拒绝跨租户、篡改和重放请求。
- 命令 lease、attempt、fencing token、重试和 dead-letter，以及 LDAP 用户写入、sync job 和
  subject export 重建均由本地 data-runtime 对 `hzy_console` 执行；重复创建与丢失的密码修改
  完成响应必须可幂等恢复。
- 本地同步提交后，data-runtime 使用 enrollment 返回的租户 Runtime Token 把最小 subject 与
  membership 结果按最多 10 条一块提交给 Platform。首块把 deployment 标成 `syncing` 并重置
  runtime membership，末块才更新 snapshot/cursor 和 `healthy`；任一块失败可从首块幂等重试，
  避免把大目录 JSON 与逐条投影塞进单个 Cloudflare 请求。Platform 仍不接收姓名、邮箱、手机
  号、LDAP DN 或密码。
- Console 原 service `commands/lease`、`commands/{id}/complete`、`sync` 路由暂保留一个版本用于
  旧 Connector 回滚；新 Connector 不调用这些高 CPU 路由。

Console 管理面支持 OpenLDAP 与 Active Directory、LDAPS/StartTLS、企业 CA、用户过滤器、
DN 模板、配置刷新间隔和分页。`managementMode=readonly` 只同步；`managed` 才允许目录用户页创建
LDAP 用户及 LDAP 身份用户在 Profile 修改自己的密码。
