...
状态：Draft  
日期：2026-06-15
定位：目标设计，作为 `Huizhi-yun-Platform-Target-Architecture.md`、`Control-Plane-API-Contract.md` 与 `foundation/docs/Foundation-SDK-Contract.md` 的配套文档
...
因此，manifest 应随 GitLab release 提交和发布；Platform 按 release/tag 导入 manifest 并物化资源、动作、角色建议。独立仓默认读取 `app.manifest.json`；monorepo 由应用台账配置 `<app_code>/app.manifest.json` 与 `<app_code>/` Tag 前缀。不要把客户域名、localhost 端口等环境地址写入通用 manifest。
...
- 应用版本（独立仓通常为 GitLab tag；monorepo 为移除 `<app_code>/` 前缀后的版本）
...
- monorepo 的实际 tag 保存在 `source_tag`（例如 `aims/v1.2.3`），`release_version` 保存应用内版本 `v1.2.3`
