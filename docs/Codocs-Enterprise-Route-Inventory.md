...
盘点范围：保留 `codocs/app/pages/mydocs` 10 页的源码清单；按用户最新决定，`mydocs/slides.vue` 演示文稿后续迁移，当前首批为其余 9 页。下文 slides 及其专用 API/组件只作为后续批次输入，不要求本轮接线或验收。文件柜既有 PPT/PPTX 预览仍属首批。只记录源码实际调用，不推断 Enterprise capability；项目文档页不在本批，编辑器、Collab、数据 Runtime、OSS 归属保持现状。
