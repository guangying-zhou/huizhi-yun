...
| Aims V2 状态能力计划 | [加法迁移计划](./Unified-Enterprise-Aims-Workflow-V2-Migration-Plan.md) | 本租户规则表列类型、旧行摘要与目标 V2 规则已独立 plan；未安装前不开放 V2 工作项状态操作 |
...
### 3.5. 目标库 Aims V2 工作项流程安装

在完整影子复制及其 `verified-shadow`、上述逐表零差异对账**之后**，如本租户将开放 Enterprise V2 工作项状态路由，须先依[统一库 Aims V2 流程加法迁移计划](./Unified-Enterprise-Aims-Workflow-V2-Migration-Plan.md)对**目标统一库**单独 plan、备份、复核 hash，再经批准 apply。迁移扩展 `aims_workflow_status_catalog` / `aims_workflow_transitions` 的 `entity_type`，安装 target/matter 全局规则，保留旧规则与业务行。逐项记录旧行摘要、前后状态/规则行数和外键；未通过不得激活/开放这些路由。**DDL 非事务性**：若在删除与重建两个复合外键之间中断，当前程序的 plan 会因外键不完整而拒绝重跑；保持写流量关闭，按已核验的加密备份受控恢复并重新 plan/审查，不能仅重复同一 apply。原影子复制计划与回执不可因本步骤回写；迁移后的两张规则表预期与源端 hash 不同，其余业务表继续按第 3 节对账。已激活租户的补装须使用独立维护窗口，不能伪作原切换步骤已完成。
