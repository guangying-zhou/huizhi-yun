...
## FE-2 / INT-503 当前任务输入（2026-09-20）

**状态**：已有产品主档、需求/版本计划和项目承接的局部代码/HTTP/Runtime 证据；未形成真实登录、多角色的完整任务验收。源码核对基于 `91790e7b` 及当前 Host 复审工作区，不代表本轮重新部署或执行环境写入。总进度只记在[实施台账](./Unified-Enterprise-Implementation-Plan.md)。以下旧表和增量按发生时间保留，“待接”须与当前 handler/页面复核，不能机械重做或仅凭 Runtime 测试宣布页面完成。

**冻结任务脚本**：普通产品负责人找到产品 → 查看当前定义与正式资料 → 选择有权访问的关联管理空间 → 处理需求并纳入版本、确认计划 → 关联研发项目执行并查看进展 → 返回原产品、空间及版本上下文。项目承接仍遵循项目评审、基线和创建任务规则，不将产品需求采纳直接视为项目已基线。不新增商业版型、自动授权或收益分析。

| 核对项 | 现有基础 | 待实现/待证明的差距 |
| --- | --- | --- |
| 产品与空间 | 产品目录和 Aims 管理空间区分，当前主档读取已有证据 | 核对正式多空间关联查询、空间标识、授权过滤和选择器；当前特定统一空间的来源唯一约束不等于通用多空间任务已通过，不能默认取第一条 |
| 需求与版本 | 需求动作、轻量版本计划、确认与承接已有局部合同 | 将任务必经页面动作逐一对照当前 handler；旧表的高级规划、版本修改/状态动作等只登记核实后的真实缺口，缺实现补代码、已实现补环境证据 |
| 执行与返回 | 候选项目/工作项独立授权、受控返回来源已有实现 | 真实登录完成项目承接、执行及返回，保留空间/版本；非法或撤权来源安全回退 |
| 当前与历史 | 当前目录代次读取与不可变发布/验收快照有独立语义 | 修改产品后正常查询看到新名称，历史发布、接纳证据和快照不被改写，不要求手动同步名称 |
| 不对称权限 | 主档、需求、版本、项目动作各自授权 | 普通角色可进入允许部分，不要求先持有全部模块权限；项目查看不隐含需求池、编辑、验收或发布权 |

**环境前置与证据**：固定同组 Host/Console/Gateway/Runtime/schema/权限目录；明确试点产品、多个关联空间、版本、项目及各岗位授权，准备无权关联和撤权样本。记录实际对象、动作、预期/实际结果、失败恢复、重复操作及测试清理边界。涉及发布时保持范围 revision、验收/发布分权与不可自审规则。当前阻塞是完整任务差距核对及真实岗位环境证据，不能把两个列表同侧栏当 FE-2 完成。浏览器验收暂缓时可准备脚本与环境，FE-2 / FE-A05 / INT-503 保持待验。
