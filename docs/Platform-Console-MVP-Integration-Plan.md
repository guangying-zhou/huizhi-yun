...
> 状态：历史实施复盘 / 目标设计参考（非当前发布或生产验收依据）
> 最后事实核对：2026-07-11。当前事实源为根与模块 `CLAUDE.md`、`docs/MODULE_CONTRACTS.md`、`console/docs/Console-Auth-Runtime-IdP-Implementation-Plan.md`、`console/server/utils/platformRuntime.ts`、`console/server/plugins/bootstrap.ts`、`platform/server/utils/runtimeAuth.ts` 与当前 `/api/v1/**` 路由。下文“已落地”仅表示仓库静态实现已被核对，不表示迁移已执行、secret 已配置、Cloudflare 已部署或真实租户已验收；这些外部操作均须另行授权。
>
> 口径收敛说明（2026-06-15）：本文仍可用于追溯 MVP 决策和已完成链路，但不再作为当前业务闭环排期主计划。当前模块状态、端口和跨模块契约以上述事实源为准。

配套文档：
- `docs/Huizhi-yun-Platform-Target-Architecture.md`
- `docs/Huizhi-yun-Architecture.md`
- `console/docs/Console-Functional-Design-v1.md`
- `console/docs/Console-Directory-Runtime-Integration-Plan.md`
...
| ID | 决策 | 理由 |
|----|------|------|
| **D-1** | runtime token 形态：**静态 long-lived token（历史 MVP / 显式离线兼容）** | 这是历史方案，不是托管云、共享测试或生产主路径授权；当前新增跨应用调用应使用 Console 签发的短期 `token_use=service` JWT 和细粒度 capability。是否将既有 runtime credential 改造为短期签名/mTLS，仍需单独设计、迁移与环境验收。 |
...
> D-1 ~ D-7 是当时 MVP 内的决策记录，并非对当前部署状态的断言。每条决策都允许在波次三结束后重新评估升级路径。
...
### 0.1 历史实现核对摘要（2026-04-28；2026-07-11 仅做源码事实复核）

当前代码已经跑通 MVP 主链路：

- Platform 源码具备 `/api/v1` runtime 合同鉴权、租户级 runtime token hash 校验、Ed25519 平台根签名、policy bundle 生成/下载、license 签发、onboarding 向导、console 内置应用 seed、heartbeat 接收与 deployment 摘要更新。v1 federation/refresh 路由当前明确返回“由 Console 处理”的未实现响应，不能据此宣称 Platform 承担企业应用用户 IdP。
...
### 0.2 授权治理契约补记（2026-07-01）

本文前置波次仍按 MVP 历史口径保留；当前授权治理主路径已推进到 v2.15 企业角色拆分与 `policy-bundle.v2`：

- 平台企业角色不再直接展开全部应用权限。`platform_system_roles` 表达企业级岗位/职责，默认应用权限通过 `platform_system_app_role_maps` 指向各应用的 `platform_app_roles`。
- 租户物化后的企业角色写入 `tenant_roles`；应用权限角色映射写入 `tenant_role_app_role_maps`。少量企业例外权限和默认 scope 仍分别写入 `tenant_role_permissions`、`tenant_role_scopes`。
- Dashboard 自定义角色可以从零创建，也可以通过 `POST /api/platform/tenant-admin/roles/:id/copy` 从平台继承/系统角色复制为 `source='custom'` 的租户角色。复制会保留 `parent_id` / `source_role_code` 作为来源线索，并复制 app-role 映射、高级权限和 scope；复制后的 custom role 不参与后续系统角色自动同步。
- `policy-bundle.v2` 是当前主 schema。Payload 使用 `roleAssignments`、`rolePermissionGrants`、`assignmentScopes`、`roleDefaultScopes`、`baselineGrants`、`actionImplications`、`conflictRules` 和单调 `policyRevision`，不再输出被 v2 替代的 `subjectRoles`、`subjectRoleScopes`、`rolePermissions`、`roleScopes`、`baselinePermissions` 冗余字段。
- Console 本地鉴权、Foundation 授权 helper 和业务应用 runtime 授权路径应优先读取 v2 字段；历史 bundle 兼容逻辑只用于迁移期回退。

同步契约见 `docs/Control-Plane-API-Contract.md` §6 / §9.4；角色设计见 `platform/docs/Platform-Role-Permission-Design.md`。
...
| Runtime API | ✅ 源码存在 `/api/v1/runtime/**`、`/api/v1/policy/**`、`/api/v1/revocations/**`、`/api/v1/registry/**` 合同路由；旧 `/api/platform/runtime/**` 在中间件上返回退役头。该结论不证明已部署，也不表示目前所有 ops/tenant-admin UI API 都已迁至 `/api/v1/**`。 |
...
| 静态 runtime token | ⚠️ 源码仍有 `tenant_runtime_credentials` 与 opaque token 校验兼容链路；按当前根契约，它只允许显式离线/兼容部署，不能作为生产、共享测试或托管云的默认安全边界。 |
...
| Deployment 概念 | ✅ 源码支持以 `HZY_PLATFORM_DEPLOYMENT_CODE`、`HZY_PLATFORM_RUNTIME_TOKEN`、`HZY_PLATFORM_SIGNING_PUBKEY` 等配置激活 legacy/self-hosted 路径；managed-cloud 分支另要求可信 tenant-gateway 与内部 service token。未核对真实环境配置。 |
...
| P-03 | 兼容退役机制 + 文档同步 | `platform/server/middleware/platform-access.ts` + `docs/Control-Plane-API-Contract.md` | 历史目标：public runtime/SDK 合同收敛到 `/api/v1/**`。当前源码只对 legacy runtime/admin 分支设置退役头；仍位于 `/api/platform/**` 的 ops/tenant-admin UI 路由不可据此视为已迁移或已设置退役窗口。 |

### 3.2 Platform：静态 runtime token 与 deployment 凭证（历史兼容链路）

> 本节记录 opaque `hzy_rt_...` 方案的实现历史。当前跨应用主路径应遵守 `MODULE_CONTRACTS.md` 的 Console 短期 service JWT、目标 audience 和细粒度 capability 规则；不得因本节存在而新增静态 token、共享 API key 或托管环境回退。

| ID | 内容 | 文件路径 | 备注 |
|----|------|----------|------|
| P-04 | 新建 `tenant_runtime_credentials`（`tenant_code` 主键）并落 `runtime_token_hash` / `runtime_token_last4` / `status` / `revoked_at` | `platform/docs/sql/HZY-Platform-SQL-Migration-v2.4-runtime-token.sql`（新建迁移） + `platform/docs/sql/HZY-Platform-SQL-DDL-Draft-v2.sql` 同步 | token 仅存 hash（sha256），明文一次性返回；不复用 `tenant_app_credentials` |
...
实现进度（2026-04-26，源码记录）：P-04 已补齐迁移文件；P-05/P-06/P-07 已落地。当前 `runtimeAuth.ts` 对 `/api/v1/runtime|policy|revocations|registry/**` 与旧 `/api/platform/runtime/**` 仍校验 `Authorization: Bearer hzy_rt_...` 并注入 `event.context.platformRuntime` / `event.context.deployment`。这不是生产配置、迁移执行或 secret 轮换的证明。
...
| P-10b | `deployments` 表增加治理摘要字段：`current_kid VARCHAR(64) / current_pubkey_fingerprint VARCHAR(64) / last_kid_reported_at DATETIME / last_key_rotated_at DATETIME` | DDL + 迁移 + `platform/docs/sql/HZY-Platform-SQL-DDL-Draft-v2.sql` 同步 | **不新增 platform_deployment_pubkeys 表**；平台只通过心跳收集 deployment 当前 kid + 公钥指纹做治理，永远不持有完整公钥；指纹突变可触发告警 |
...
| P-10d | 删除 `deployment_signing_keys` 模型与迁移残留（DDL + 代码引用） | `platform/docs/sql/HZY-Platform-SQL-Migration-v2.4-drop-deployment-signing-keys.sql`（新）+ `platform/docs/sql/HZY-Platform-SQL-DDL-Draft-v2.sql` + `docs/Huizhi-yun-Platform-Target-Architecture.md` | 旧表执行 `DROP TABLE IF EXISTS deployment_signing_keys`；所有逻辑统一改读 `deployments.current_kid/current_pubkey_fingerprint` |
...
实现进度（2026-04-26，源码记录）：P-15/P-16 已落地 `platform/server/utils/onboardingFlow.ts` 与 `/api/platform/ops/onboarding/{start,step,finalize,status}`。历史 flow 会创建/更新租户、确认内置 `console` 应用、创建租户主订阅与 per-app entitlement、创建 `console` deployment、签发带 Ed25519 签名的 `license.lic`、签发一次性 runtime token，并输出 `console.env`。该输出包含敏感部署材料；不得在文档、测试或未获授权的环境操作中生成、复制或下发。P-17/P-17b 已新增 `/admin/onboarding` 与 `/admin/onboarding/{tenantCode}`；bundle 生成默认受 subject gate 约束，未同步 active user subject 时将 `subject_sync` / `bundle` 步骤置为 blocked，可显式 `forceBundle` 跳过。P-18 已新增启动种子 `platform/server/plugins/ensureConsoleApp.ts`，注册 app_code=`console`、`service_role='supporting_service'` 并同步初始 manifest。
...
| P-20a | MVP 直接接收 Console 启动时推送的最小 subject 投影 | `platform/server/api/v1/runtime/subjects/sync.post.ts` | 已实现。按 runtime token 鉴权，允许 `user / department / committee / job / project`，过滤不支持类型，用户 `display_name=NULL`，写 `tenant_subjects` 与 deployment directory sync 摘要；`committee/project` 不进入通用主体目录 UI，项目不得映射为岗位 |
...
| D-02 | DDL 同步 | `platform/docs/sql/HZY-Platform-SQL-DDL-Draft-v2.sql` + `platform/docs/HZY-Platform-Schema-Addendum-v2.4.md` | 把 P-04、P-08、P-10b、P-10d 的结构变更并入 |
...
以下是历史波次的验收目标，不是本次源码核对的已完成声明。执行其中任一真实 ops、数据库、token、Console 启动或 Cloudflare 验收，须另行获得操作授权并留下可定位记录：
...
| console directory 投影 / account fallback 与 platform 字段不一致 | 当前 `POST /api/v1/runtime/subjects/sync` 在 Platform 侧白名单过滤 `user/department/committee/job/project` 并禁止用户 PII；`job` 仅表示岗位，`project` 保持项目语义；后续任务队列版需补 schema 校验、任务日志与可重试状态 |
