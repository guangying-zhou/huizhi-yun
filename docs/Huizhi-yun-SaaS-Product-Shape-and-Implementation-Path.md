...
> **历史/目标实现路径参考**：最后事实核验：2026-07-11。当前事实源：[开发执行清单](Development-Execution-Checklist-2026-07.md)、[综合运营路线图](Huizhi-yun-Integrated-Operations-Roadmap.md)、[跨模块契约](MODULE_CONTRACTS.md)、[托管云 Agent ADR](ADR-014-Managed-Cloud-Agent-and-Deployment-Profiles.md)、当前 Platform/Console/Foundation `CLAUDE.md` 与部署运行手册。本文件的目标域名、迁移阶段和产品形态不表示当前已部署状态；migration/seed、Cloudflare 部署、R2/runtime 发布、live probe 和真实租户切换须按目标环境单独、明确授权。
...
- `codocs/docs/Codocs-Cloudflare-Migration-Plan.md`
...
10. 按 `codocs/docs/Codocs-Cloudflare-Migration-Plan.md` 推进 Codocs / Collab Durable Objects POC 与对象存储抽象。
